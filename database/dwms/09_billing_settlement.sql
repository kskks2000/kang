-- DWMS enterprise WMS schema - service contracts, rating, charges, invoicing, settlement, payment, and accounting.

CREATE TABLE IF NOT EXISTS dwms.service_contracts (
    ...
    counterparty_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    ...
    CONSTRAINT ck_dwms_contract_type CHECK (contract_type IN ('CUSTOMER','SUPPLIER','CARRIER','CUSTOMS_BROKER','LABOR','OTHER')),
    CONSTRAINT ck_dwms_contract_dates CHECK (valid_to IS NULL OR valid_to >= valid_from),
    CONSTRAINT ck_dwms_contract_notice CHECK (notice_days IS NULL OR notice_days >= 0),
    CONSTRAINT ck_dwms_contract_status CHECK (status IN ('DRAFT','APPROVAL_PENDING','ACTIVE','SUSPENDED','EXPIRED','TERMINATED'))
);

CREATE TABLE IF NOT EXISTS dwms.service_contract_versions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    contract_id uuid NOT NULL REFERENCES dwms.service_contracts(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_dwms_contract_version_no CHECK (version_no > 0),
    CONSTRAINT ck_dwms_contract_version_dates CHECK (effective_to IS NULL OR effective_to >= effective_from),
    CONSTRAINT ck_dwms_contract_version_status CHECK (status IN ('DRAFT','APPROVAL_PENDING','ACTIVE','SUPERSEDED','REJECTED'))
);

CREATE TABLE IF NOT EXISTS dwms.contract_services (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    contract_version_id uuid NOT NULL REFERENCES dwms.service_contract_versions(id) ON DELETE CASCADE,
    service_code varchar(80) NOT NULL,
    service_name varchar(200) NOT NULL,
    charge_code_id uuid NOT NULL REFERENCES dwms.charge_codes(id),
    ...
    included_uom_code varchar(20) REFERENCES dwms.units_of_measure(uom_code),
    taxable boolean NOT NULL DEFAULT true,
    is_active boolean NOT NULL DEFAULT true,
    UNIQUE (contract_version_id, service_code),
    CONSTRAINT ck_dwms_contract_service_frequency CHECK (billing_frequency IN ('EVENT','DAILY','WEEKLY','MONTHLY','QUARTERLY','MANUAL')),
    CONSTRAINT ck_dwms_contract_service_amounts CHECK (
...
CREATE TABLE IF NOT EXISTS dwms.rate_rules (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    contract_service_id uuid NOT NULL REFERENCES dwms.contract_services(id) ON DELETE CASCADE,
    rule_no integer NOT NULL,
    rule_name varchar(200) NOT NULL,
    charge_basis varchar(30) NOT NULL,
    basis_uom_code varchar(20) REFERENCES dwms.units_of_measure(uom_code),
    ...
    CONSTRAINT ck_dwms_rate_rule_no CHECK (rule_no > 0),
    CONSTRAINT ck_dwms_rate_basis CHECK (charge_basis IN (
        'FLAT','QUANTITY','WEIGHT','VOLUME','PALLET','LPN','LOCATION','AREA','DAY','HOUR','EVENT','PERCENT','TIERED','ACTUAL'
    )),
    CONSTRAINT ck_dwms_rate_value CHECK (num_nonnulls(rate_amount,rate_percent,flat_amount) >= 1),
    CONSTRAINT ck_dwms_rate_percent CHECK (rate_percent IS NULL OR rate_percent >= 0),
    CONSTRAINT ck_dwms_rate_limits CHECK (
    ...
    CONSTRAINT ck_dwms_rate_rounding CHECK (rounding_mode IN ('HALF_UP','HALF_EVEN','UP','DOWN','CEILING','FLOOR') AND rounding_scale BETWEEN 0 AND 8),
    CONSTRAINT ck_dwms_rate_dates CHECK (valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from)
);

CREATE TABLE IF NOT EXISTS dwms.rate_rule_tiers (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    rate_rule_id uuid NOT NULL REFERENCES dwms.rate_rules(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_dwms_rate_tier_no CHECK (tier_no > 0),
    CONSTRAINT ck_dwms_rate_tier_range CHECK (from_quantity >= 0 AND (to_quantity IS NULL OR to_quantity > from_quantity)),
    CONSTRAINT ck_dwms_rate_tier_amount CHECK (rate_amount >= 0 AND flat_amount >= 0)
);

CREATE TABLE IF NOT EXISTS dwms.charge_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    CONSTRAINT ck_dwms_charge_event_measures CHECK (
    ...
    CONSTRAINT ck_dwms_charge_event_status CHECK (status IN ('NEW','RATING','RATED','FAILED','WAIVED','REVERSED'))
);

CREATE TABLE IF NOT EXISTS dwms.rating_runs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    CONSTRAINT ck_dwms_rating_run_type CHECK (run_type IN ('EVENT','PERIODIC','RECALCULATION','SIMULATION','MANUAL')),
    CONSTRAINT ck_dwms_rating_run_period CHECK (service_period_to IS NULL OR service_period_from IS NULL OR service_period_to >= service_period_from),
    CONSTRAINT ck_dwms_rating_run_counts CHECK (
        total_events >= 0 AND success_events >= 0 AND failed_events >= 0 AND success_events + failed_events <= total_events
    ),
    CONSTRAINT ck_dwms_rating_run_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS dwms.rating_run_details (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    rating_run_id uuid NOT NULL REFERENCES dwms.rating_runs(id) ON DELETE CASCADE,
    charge_event_id uuid NOT NULL REFERENCES dwms.charge_events(id),
    contract_version_id uuid REFERENCES dwms.service_contract_versions(id),
    contract_service_id uuid REFERENCES dwms.contract_services(id),
    rate_rule_id uuid REFERENCES dwms.rate_rules(id),
    ...
    CONSTRAINT ck_dwms_rating_detail_basis CHECK (basis_quantity IS NULL OR basis_quantity >= 0),
    CONSTRAINT ck_dwms_rating_detail_amount CHECK (calculated_net_amount IS NULL OR calculated_net_amount >= 0)
);

CREATE TABLE IF NOT EXISTS dwms.charges (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    charge_code_id uuid NOT NULL REFERENCES dwms.charge_codes(id),
    charge_event_id uuid REFERENCES dwms.charge_events(id),
    rating_detail_id uuid REFERENCES dwms.rating_run_details(id),
    contract_version_id uuid REFERENCES dwms.service_contract_versions(id),
    warehouse_id uuid NOT NULL REFERENCES dwms.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    payer_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    payee_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    service_date date NOT NULL,
    service_period_from date,
    service_period_to date,
    ...
    tax_code_id uuid REFERENCES dwms.tax_codes(id),
    tax_rate_percent numeric(9,6),
    tax_amount numeric(20,4) NOT NULL DEFAULT 0,
    gross_amount numeric(20,4) NOT NULL,
    base_currency_code char(3) REFERENCES dwms.currencies(currency_code),
    ...
    reversal_of_charge_id uuid REFERENCES dwms.charges(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, charge_no),
    CONSTRAINT ck_dwms_charge_direction CHECK (direction IN ('RECEIVABLE','PAYABLE')),
    CONSTRAINT ck_dwms_charge_stage CHECK (stage IN ('ESTIMATE','ACCRUAL','ACTUAL','INVOICE','SETTLEMENT')),
    CONSTRAINT ck_dwms_charge_parties CHECK (payer_partner_id <> payee_partner_id),
    CONSTRAINT ck_dwms_charge_period CHECK (service_period_to IS NULL OR service_period_from IS NULL OR service_period_to >= service_period_from),
    CONSTRAINT ck_dwms_charge_qty CHECK (quantity IS NULL OR quantity >= 0),
    CONSTRAINT ck_dwms_charge_tax_rate CHECK (tax_rate_percent IS NULL OR tax_rate_percent BETWEEN 0 AND 100),
    CONSTRAINT ck_dwms_charge_amounts CHECK (
    ...
    CONSTRAINT ck_dwms_charge_reversal CHECK (reversal_of_charge_id IS NULL OR reversal_of_charge_id <> id),
    CONSTRAINT ck_dwms_charge_status CHECK (status IN ('OPEN','ON_HOLD','APPROVED','INVOICED','SETTLED','WAIVED','REVERSED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS dwms.charge_allocations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    charge_id uuid NOT NULL REFERENCES dwms.charges(id) ON DELETE CASCADE,
    allocation_no integer NOT NULL,
    warehouse_id uuid REFERENCES dwms.warehouses(id),
    owner_partner_id uuid REFERENCES dwms.business_partners(id),
    outbound_order_id uuid REFERENCES dwms.outbound_orders(id),
    inbound_order_id uuid REFERENCES dwms.inbound_orders(id),
    ...
    reversed_by uuid REFERENCES dwms.users(id),
    ...
    CONSTRAINT ck_dwms_charge_alloc_no CHECK (allocation_no > 0),
    CONSTRAINT ck_dwms_charge_alloc_target CHECK (
    ...
    CONSTRAINT ck_dwms_charge_alloc_percent CHECK (allocation_percent IS NULL OR allocation_percent BETWEEN 0 AND 100),
    CONSTRAINT ck_dwms_charge_alloc_amount CHECK (
    ...
    CONSTRAINT ck_dwms_charge_alloc_reversal CHECK (
...
CREATE TABLE IF NOT EXISTS dwms.charge_adjustments (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    charge_id uuid NOT NULL REFERENCES dwms.charges(id),
    ...
    CONSTRAINT ck_dwms_charge_adjustment_no CHECK (adjustment_no > 0),
    CONSTRAINT ck_dwms_charge_adjustment_amount CHECK (gross_amount_delta = net_amount_delta + tax_amount_delta),
    CONSTRAINT ck_dwms_charge_adjustment_approval CHECK (approval_status IN ('PENDING','APPROVED','REJECTED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS dwms.accruals (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    currency_code char(3) NOT NULL REFERENCES dwms.currencies(currency_code),
    accrued_amount numeric(20,4) NOT NULL,
    base_currency_code char(3) NOT NULL REFERENCES dwms.currencies(currency_code),
    base_amount numeric(20,4) NOT NULL,
    status varchar(20) NOT NULL DEFAULT 'OPEN',
    reversed_at timestamptz,
    reversal_accrual_id uuid REFERENCES dwms.accruals(id),
    invoice_line_id uuid,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, accrual_no),
    CONSTRAINT ck_dwms_accrual_amount CHECK (accrued_amount >= 0 AND base_amount >= 0),
    CONSTRAINT ck_dwms_accrual_reversal CHECK (reversal_accrual_id IS NULL OR reversal_accrual_id <> id)
);

CREATE TABLE IF NOT EXISTS dwms.landed_costs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    CONSTRAINT ck_dwms_landed_cost_source CHECK (num_nonnulls(receipt_id,trade_shipment_id,customs_declaration_id) >= 1),
    CONSTRAINT ck_dwms_landed_cost_amount CHECK (
    ...
    CONSTRAINT ck_dwms_landed_cost_method CHECK (allocation_method IN ('QUANTITY','WEIGHT','VOLUME','VALUE','MANUAL'))
);

CREATE TABLE IF NOT EXISTS dwms.landed_cost_allocations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    landed_cost_id uuid NOT NULL REFERENCES dwms.landed_costs(id) ON DELETE CASCADE,
    receipt_line_id uuid REFERENCES dwms.receipt_lines(id),
    ...
    CONSTRAINT ck_dwms_landed_alloc_target CHECK (receipt_line_id IS NOT NULL OR inventory_lot_id IS NOT NULL),
    CONSTRAINT ck_dwms_landed_alloc_values CHECK (
...
CREATE TABLE IF NOT EXISTS dwms.invoices (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    issuer_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    recipient_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    owner_partner_id uuid REFERENCES dwms.business_partners(id),
    ...
    original_invoice_id uuid REFERENCES dwms.invoices(id),
    ...
    CONSTRAINT ck_dwms_invoice_type CHECK (invoice_type IN ('RECEIVABLE','PAYABLE','CREDIT_NOTE','DEBIT_NOTE')),
    CONSTRAINT ck_dwms_invoice_parties CHECK (issuer_partner_id <> recipient_partner_id),
    CONSTRAINT ck_dwms_invoice_period CHECK (service_period_to IS NULL OR service_period_from IS NULL OR service_period_to >= service_period_from),
    CONSTRAINT ck_dwms_invoice_due CHECK (due_date IS NULL OR due_date >= invoice_date),
    CONSTRAINT ck_dwms_invoice_amounts CHECK (
        net_amount >= 0 AND tax_amount >= 0 AND gross_amount = net_amount + tax_amount AND
        paid_amount >= 0 AND paid_amount <= gross_amount AND outstanding_amount = gross_amount - paid_amount
    ),
    CONSTRAINT ck_dwms_invoice_original CHECK (original_invoice_id IS NULL OR original_invoice_id <> id),
    CONSTRAINT ck_dwms_invoice_status CHECK (status IN (
        'DRAFT','RECEIVED','VALIDATING','REJECTED','APPROVAL_PENDING','APPROVED','POSTED',
        'PARTIALLY_PAID','PAID','CANCELLED','REVERSED'
    )),
    CONSTRAINT ck_dwms_invoice_approval_status CHECK (
        approval_status IN ('NOT_REQUIRED','PENDING','APPROVED','REJECTED','CANCELLED')
    ),
    CONSTRAINT ck_dwms_invoice_match_status CHECK (match_status IN ('NOT_MATCHED','MATCHING','PARTIAL','MATCHED','EXCEPTION'))
);

CREATE TABLE IF NOT EXISTS dwms.invoice_lines (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    invoice_id uuid NOT NULL REFERENCES dwms.invoices(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    charge_code_id uuid REFERENCES dwms.charge_codes(id),
    ...
    CONSTRAINT ck_dwms_invoice_line_no CHECK (line_no > 0),
    CONSTRAINT ck_dwms_invoice_line_qty CHECK (quantity >= 0),
    CONSTRAINT ck_dwms_invoice_line_amounts CHECK (
...
ALTER TABLE dwms.accruals
    DROP CONSTRAINT IF EXISTS fk_dwms_accrual_invoice_line;
ALTER TABLE dwms.accruals
    ADD CONSTRAINT fk_dwms_accrual_invoice_line
    FOREIGN KEY (invoice_line_id) REFERENCES dwms.invoice_lines(id) ON DELETE SET NULL;

CREATE TABLE IF NOT EXISTS dwms.invoice_line_charges (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    invoice_id uuid NOT NULL REFERENCES dwms.invoices(id) ON DELETE RESTRICT,
    ...
    FOREIGN KEY (invoice_id, invoice_line_id)
        REFERENCES dwms.invoice_lines(invoice_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_dwms_invoice_line_charge_amount CHECK (
        allocated_net_amount >= 0 AND allocated_tax_amount >= 0 AND
        allocated_gross_amount > 0 AND
        allocated_gross_amount = allocated_net_amount + allocated_tax_amount
    ),
    CONSTRAINT ck_dwms_invoice_line_charge_reversal CHECK (
        (reversed_at IS NULL AND reversed_by IS NULL AND reversal_reason IS NULL) OR
        (reversed_at IS NOT NULL AND reversed_by IS NOT NULL AND
         reversal_reason IS NOT NULL AND btrim(reversal_reason) <> '' AND
         reversed_at >= created_at)
...
CREATE TABLE IF NOT EXISTS dwms.invoice_taxes (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    invoice_id uuid NOT NULL REFERENCES dwms.invoices(id) ON DELETE CASCADE,
    tax_code_id uuid NOT NULL REFERENCES dwms.tax_codes(id),
    ...
    CONSTRAINT ck_dwms_invoice_tax_values CHECK (
        taxable_amount >= 0 AND tax_rate_percent BETWEEN 0 AND 100 AND tax_amount >= 0 AND exemption_amount >= 0
    )
);

CREATE TABLE IF NOT EXISTS dwms.invoice_match_results (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    invoice_line_id uuid NOT NULL REFERENCES dwms.invoice_lines(id) ON DELETE CASCADE,
    ...
    matched_by uuid REFERENCES dwms.users(id),
    CONSTRAINT ck_dwms_invoice_match_result CHECK (result IN ('MATCHED','WITHIN_TOLERANCE','VARIANCE','MISSING_REFERENCE','DUPLICATE'))
);

CREATE TABLE IF NOT EXISTS dwms.invoice_disputes (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    dispute_no varchar(120) NOT NULL,
    invoice_id uuid NOT NULL REFERENCES dwms.invoices(id),
    invoice_line_id uuid REFERENCES dwms.invoice_lines(id),
    disputed_amount numeric(20,4) NOT NULL,
    currency_code char(3) NOT NULL REFERENCES dwms.currencies(currency_code),
    reason_code_id uuid REFERENCES dwms.reason_codes(id),
    reason_text text NOT NULL,
    opened_by uuid REFERENCES dwms.users(id),
    ...
    CONSTRAINT ck_dwms_invoice_dispute_amount CHECK (
        disputed_amount > 0 AND (resolved_amount IS NULL OR resolved_amount BETWEEN 0 AND disputed_amount)
    ),
    CONSTRAINT ck_dwms_invoice_dispute_dates CHECK (resolved_at IS NULL OR resolved_at >= opened_at),
    CONSTRAINT ck_dwms_invoice_dispute_status CHECK (status IN ('OPEN','INVESTIGATING','ACCEPTED','PARTIALLY_ACCEPTED','REJECTED','CLOSED'))
);

CREATE TABLE IF NOT EXISTS dwms.invoice_dispute_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    dispute_id uuid NOT NULL REFERENCES dwms.invoice_disputes(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.invoice_status_history (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    invoice_id uuid NOT NULL REFERENCES dwms.invoices(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
    ...
    CONSTRAINT ck_dwms_invoice_status_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS dwms.tax_invoices (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    tax_invoice_no varchar(150) NOT NULL,
    invoice_id uuid REFERENCES dwms.invoices(id),
    issue_type varchar(20) NOT NULL DEFAULT 'NORMAL',
    issue_date date NOT NULL,
    supplier_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    buyer_partner_id uuid NOT NULL REFERENCES dwms.business_partners(id),
    ...
    error_detail_masked text,
    original_tax_invoice_id uuid REFERENCES dwms.tax_invoices(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, tax_invoice_no),
    CONSTRAINT ck_dwms_tax_invoice_parties CHECK (supplier_partner_id <> buyer_partner_id),
    CONSTRAINT ck_dwms_tax_invoice_amount CHECK (supply_amount >= 0 AND tax_amount >= 0 AND total_amount = supply_amount + tax_amount),
    CONSTRAINT ck_dwms_tax_invoice_original CHECK (original_tax_invoice_id IS NULL OR original_tax_invoice_id <> id),
    CONSTRAINT ck_dwms_tax_invoice_status CHECK (nts_status IN ('DRAFT','QUEUED','TRANSMITTED','ACCEPTED','REJECTED','CANCELLED','CORRECTED'))
);

CREATE TABLE IF NOT EXISTS dwms.tax_invoice_lines (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    tax_invoice_id uuid NOT NULL REFERENCES dwms.tax_invoices(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    ...
    CONSTRAINT ck_dwms_tax_invoice_line_no CHECK (line_no > 0),
    CONSTRAINT ck_dwms_tax_invoice_line_values CHECK (
        (quantity IS NULL OR quantity >= 0) AND (unit_price IS NULL OR unit_price >= 0) AND supply_amount >= 0 AND tax_amount >= 0
    )
);

CREATE TABLE IF NOT EXISTS dwms.tax_invoice_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    tax_invoice_id uuid NOT NULL REFERENCES dwms.tax_invoices(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.settlement_batches (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    CONSTRAINT ck_dwms_settlement_batch_type CHECK (settlement_type IN ('RECEIVABLE','PAYABLE','NETTING')),
    CONSTRAINT ck_dwms_settlement_batch_period CHECK (period_to >= period_from),
    CONSTRAINT ck_dwms_settlement_batch_status CHECK (status IN ('DRAFT','CALCULATING','REVIEW','APPROVAL_PENDING','APPROVED','POSTED','CLOSED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS dwms.settlements (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    settlement_batch_id uuid REFERENCES dwms.settlement_batches(id),
    ...
    CONSTRAINT ck_dwms_settlement_parties CHECK (payer_partner_id <> payee_partner_id),
    CONSTRAINT ck_dwms_settlement_amounts CHECK (
    ...
    CONSTRAINT ck_dwms_settlement_status CHECK (status IN (
        'DRAFT','REVIEW','APPROVAL_PENDING','APPROVED','INVOICED',
        'PARTIALLY_PAID','PAID','CLOSED','CANCELLED','REVERSED'
    ))
);

CREATE TABLE IF NOT EXISTS dwms.settlement_lines (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    settlement_id uuid NOT NULL REFERENCES dwms.settlements(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    charge_id uuid NOT NULL REFERENCES dwms.charges(id),
    ...
    CONSTRAINT ck_dwms_settlement_line_no CHECK (line_no > 0),
    CONSTRAINT ck_dwms_settlement_line_amount CHECK (
        charge_amount >= 0 AND tax_amount >= 0 AND settled_amount >= 0 AND
        settled_amount = charge_amount + adjustment_amount + tax_amount
    )
);

CREATE TABLE IF NOT EXISTS dwms.payments (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    bank_account_id uuid REFERENCES dwms.partner_bank_accounts(id),
    status varchar(20) NOT NULL DEFAULT 'RECEIVED',
    reversed_payment_id uuid REFERENCES dwms.payments(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, payment_no),
    CONSTRAINT ck_dwms_payment_direction CHECK (payment_direction IN ('IN','OUT')),
    CONSTRAINT ck_dwms_payment_parties CHECK (payer_partner_id <> payee_partner_id),
    CONSTRAINT ck_dwms_payment_amount CHECK (payment_amount > 0 AND unapplied_amount BETWEEN 0 AND payment_amount),
    CONSTRAINT ck_dwms_payment_reversal CHECK (reversed_payment_id IS NULL OR reversed_payment_id <> id),
    CONSTRAINT ck_dwms_payment_status CHECK (status IN ('PENDING','RECEIVED','CLEARED','PARTIALLY_APPLIED','APPLIED','FAILED','REVERSED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS dwms.payment_applications (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    payment_id uuid NOT NULL REFERENCES dwms.payments(id) ON DELETE CASCADE,
    invoice_id uuid REFERENCES dwms.invoices(id),
    settlement_id uuid REFERENCES dwms.settlements(id),
    applied_amount numeric(20,4) NOT NULL,
    applied_at timestamptz NOT NULL DEFAULT now(),
    applied_by uuid NOT NULL REFERENCES dwms.users(id),
    reversed_at timestamptz,
    reversed_by uuid REFERENCES dwms.users(id),
    reversal_reason text,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_dwms_payment_application_target CHECK (num_nonnulls(invoice_id, settlement_id) = 1),
    CONSTRAINT ck_dwms_payment_application_amount CHECK (applied_amount > 0),
    CONSTRAINT ck_dwms_payment_application_dates CHECK (reversed_at IS NULL OR reversed_at >= applied_at),
    CONSTRAINT ck_dwms_payment_application_reversal CHECK (
        (reversed_at IS NULL AND reversed_by IS NULL AND reversal_reason IS NULL) OR
        (reversed_at IS NOT NULL AND reversed_by IS NOT NULL AND
         reversal_reason IS NOT NULL AND btrim(reversal_reason) <> '' AND
         reversed_at >= created_at)
...
CREATE TABLE IF NOT EXISTS dwms.finance_projection_authorizations (
    ...
    CONSTRAINT ck_dwms_finance_auth_scope CHECK (
        authorization_scope IN ('PAYMENT_APPLICATION')
    ),
    CONSTRAINT ck_dwms_finance_auth_expiry CHECK (expires_at > created_at)
);

REVOKE ALL ON TABLE dwms.finance_projection_authorizations FROM PUBLIC;

CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_charge_alloc_outbound_active
    ON dwms.charge_allocations (charge_id, outbound_order_id)
    WHERE reversed_at IS NULL AND outbound_order_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_charge_alloc_inbound_active
    ON dwms.charge_allocations (charge_id, inbound_order_id)
    WHERE reversed_at IS NULL AND inbound_order_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_charge_alloc_shipment_active
    ON dwms.charge_allocations (charge_id, shipment_id)
    WHERE reversed_at IS NULL AND shipment_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_charge_alloc_receipt_active
    ON dwms.charge_allocations (charge_id, receipt_id)
    WHERE reversed_at IS NULL AND receipt_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_invoice_line_charge_active
    ON dwms.invoice_line_charges (invoice_line_id, charge_id)
    WHERE reversed_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_invoice_charge_per_invoice_active
    ON dwms.invoice_line_charges (invoice_id, charge_id)
    WHERE reversed_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_dwms_invoice_charge_active_sum
    ON dwms.invoice_line_charges (charge_id, invoice_id, invoice_line_id)
    WHERE reversed_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_payment_invoice_application_active
    ON dwms.payment_applications (payment_id, invoice_id)
    WHERE reversed_at IS NULL AND invoice_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_payment_settlement_application_active
    ON dwms.payment_applications (payment_id, settlement_id)
    WHERE reversed_at IS NULL AND settlement_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_dwms_payment_application_invoice_active
    ON dwms.payment_applications (invoice_id, payment_id)
    WHERE reversed_at IS NULL AND invoice_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_dwms_payment_application_settlement_active
    ON dwms.payment_applications (settlement_id, payment_id)
    WHERE reversed_at IS NULL AND settlement_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_dwms_settlement_canonical_invoice
    ON dwms.settlements (invoice_id)
    WHERE invoice_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_dwms_settlement_line_charge
    ON dwms.settlement_lines (charge_id, settlement_id);

CREATE TABLE IF NOT EXISTS dwms.accounting_periods (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    ...
    CONSTRAINT ck_dwms_accounting_year CHECK (fiscal_year BETWEEN 1900 AND 9999),
    CONSTRAINT ck_dwms_accounting_period_no CHECK (period_no BETWEEN 1 AND 53),
    CONSTRAINT ck_dwms_accounting_period_dates CHECK (end_date >= start_date),
    CONSTRAINT ck_dwms_accounting_period_status CHECK (status IN ('FUTURE','OPEN','SOFT_CLOSED','CLOSED','REOPENED'))
);

CREATE TABLE IF NOT EXISTS dwms.gl_accounts (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    ...
    parent_account_id uuid REFERENCES dwms.gl_accounts(id),
    ...
    CONSTRAINT ck_dwms_gl_account_type CHECK (account_type IN ('ASSET','LIABILITY','EQUITY','REVENUE','EXPENSE','CONTRA')),
    CONSTRAINT ck_dwms_gl_normal_balance CHECK (normal_balance IN ('DEBIT','CREDIT')),
    CONSTRAINT ck_dwms_gl_parent CHECK (parent_account_id IS NULL OR parent_account_id <> id)
);

CREATE TABLE IF NOT EXISTS dwms.journal_batches (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    batch_no varchar(120) NOT NULL,
    ...
    accounting_date date NOT NULL,
    accounting_period_id uuid NOT NULL REFERENCES dwms.accounting_periods(id),
    currency_code char(3) NOT NULL REFERENCES dwms.currencies(currency_code),
    ...
    posted_at timestamptz,
    reversal_batch_id uuid REFERENCES dwms.journal_batches(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, batch_no),
    CONSTRAINT ck_dwms_journal_batch_balance CHECK (total_debit >= 0 AND total_credit >= 0 AND total_debit = total_credit),
    CONSTRAINT ck_dwms_journal_batch_reversal CHECK (reversal_batch_id IS NULL OR reversal_batch_id <> id),
    CONSTRAINT ck_dwms_journal_batch_status CHECK (status IN ('DRAFT','VALIDATING','APPROVAL_PENDING','APPROVED','POSTED','REVERSED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS dwms.journal_entries (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    journal_batch_id uuid NOT NULL REFERENCES dwms.journal_batches(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_dwms_journal_entry_no CHECK (entry_no > 0)
);

CREATE TABLE IF NOT EXISTS dwms.journal_lines (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    journal_entry_id uuid NOT NULL REFERENCES dwms.journal_entries(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    gl_account_id uuid NOT NULL REFERENCES dwms.gl_accounts(id),
    owner_partner_id uuid REFERENCES dwms.business_partners(id),
    warehouse_id uuid REFERENCES dwms.warehouses(id),
    ...
    CONSTRAINT ck_dwms_journal_line_no CHECK (line_no > 0),
    CONSTRAINT ck_dwms_journal_line_side CHECK (
        debit_amount >= 0 AND credit_amount >= 0 AND num_nonnulls(NULLIF(debit_amount,0), NULLIF(credit_amount,0)) = 1
    ),
    CONSTRAINT ck_dwms_journal_line_rate CHECK (exchange_rate IS NULL OR exchange_rate > 0)
);

CREATE TABLE IF NOT EXISTS dwms.accounting_exports (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    export_no varchar(120) NOT NULL,
    external_system_id uuid NOT NULL REFERENCES dwms.external_systems(id),
    accounting_period_id uuid REFERENCES dwms.accounting_periods(id),
    ...
    CONSTRAINT ck_dwms_accounting_export_count CHECK (record_count >= 0),
    CONSTRAINT ck_dwms_accounting_export_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

COMMENT ON TABLE dwms.rating_run_details IS
    'Immutable explanation of the contract version, rule, input basis and calculation trace used for each charge event.';
COMMENT ON TABLE dwms.charges IS
...
COMMENT ON TABLE dwms.partner_bank_accounts IS
...
CREATE OR REPLACE FUNCTION dwms.open_finance_projection_authorization(p_scope text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    DELETE FROM dwms.finance_projection_authorizations
     WHERE expires_at <= clock_timestamp();

    authorization_token := dwms.generate_uuid();
    INSERT INTO dwms.finance_projection_authorizations (
    ...
    PERFORM set_config('dwms.finance_projection_token', authorization_token::text, true);
    RETURN authorization_token;
END;
$function$;

CREATE OR REPLACE FUNCTION dwms.close_finance_projection_authorization(p_token uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    PERFORM set_config('dwms.finance_projection_token', '', true);
END;
$function$;

CREATE OR REPLACE FUNCTION dwms.finance_projection_authorized(p_scope text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
DECLARE
    ...
    token_value uuid;
BEGIN
    token_text := current_setting('dwms.finance_projection_token', true);
    ...
    RETURN EXISTS (
        SELECT 1
          FROM dwms.finance_projection_authorizations finance_auth
...
REVOKE ALL ON FUNCTION dwms.open_finance_projection_authorization(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.close_finance_projection_authorization(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.finance_projection_authorized(text) FROM PUBLIC;

CREATE OR REPLACE FUNCTION dwms.validate_charge_allocation_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(hashtext('dwms.finance'), hashtext(NEW.tenant_id::text));
    ...
    IF actor_id IS NOT NULL AND NOT EXISTS (
        SELECT 1
          FROM dwms.tenant_memberships membership
    ...
    IF NEW.outbound_order_id IS NOT NULL THEN
        SELECT tenant_id, warehouse_id, owner_partner_id INTO target_row
          FROM dwms.outbound_orders WHERE id = NEW.outbound_order_id FOR SHARE;
    ELSIF NEW.inbound_order_id IS NOT NULL THEN
        SELECT tenant_id, warehouse_id, owner_partner_id INTO target_row
          FROM dwms.inbound_orders WHERE id = NEW.inbound_order_id FOR SHARE;
    ELSIF NEW.shipment_id IS NOT NULL THEN
        SELECT tenant_id, warehouse_id, owner_partner_id INTO target_row
          FROM dwms.shipments WHERE id = NEW.shipment_id FOR SHARE;
    ELSIF NEW.receipt_id IS NOT NULL THEN
        SELECT tenant_id, warehouse_id, owner_partner_id INTO target_row
          FROM dwms.receipts WHERE id = NEW.receipt_id FOR SHARE;
    END IF;
    IF NOT FOUND THEN
    ...
    SELECT COALESCE(sum(allocated_net_amount), 0),
      ...
      FROM dwms.charge_allocations allocation
...
CREATE OR REPLACE FUNCTION dwms.validate_invoice_line_charge_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    ELSE
        ...
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM dwms.tenant_memberships membership
             WHERE membership.tenant_id = NEW.tenant_id
    ...
    SELECT COALESCE(sum(allocated_net_amount), 0),
      ...
      FROM dwms.invoice_line_charges allocation
     WHERE allocation.charge_id = NEW.charge_id
       AND allocation.reversed_at IS NULL
       AND allocation.id <> NEW.id;
    SELECT COALESCE(sum(allocated_net_amount), 0),
...
CREATE OR REPLACE FUNCTION dwms.guard_charge_allocation_parent()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    SELECT EXISTS (
               SELECT 1 FROM dwms.charge_allocations WHERE charge_id = OLD.id
               UNION ALL
               SELECT 1 FROM dwms.invoice_line_charges WHERE charge_id = OLD.id
               UNION ALL
               SELECT 1 FROM dwms.settlement_lines WHERE charge_id = OLD.id
           ),
           EXISTS (SELECT 1 FROM dwms.charge_allocations WHERE charge_id = OLD.id AND reversed_at IS NULL),
           EXISTS (SELECT 1 FROM dwms.invoice_line_charges WHERE charge_id = OLD.id AND reversed_at IS NULL),
           EXISTS (
               SELECT 1
                 FROM dwms.settlement_lines line
                 JOIN dwms.settlements settlement ON settlement.id = line.settlement_id
...
CREATE OR REPLACE FUNCTION dwms.guard_invoice_line_charge_parent()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    SELECT EXISTS (
               SELECT 1 FROM dwms.invoice_line_charges WHERE invoice_line_id = OLD.id
           ),
...
CREATE OR REPLACE FUNCTION dwms.assert_charge_allocation_integrity(
    p_charge_id uuid,
...
BEGIN
    SELECT charge.* INTO charge_row
      FROM dwms.charges charge WHERE charge.id = p_charge_id FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF p_invoice_line_id IS NOT NULL THEN
        SELECT line.* INTO line_row
          FROM dwms.invoice_lines line
          JOIN dwms.invoices invoice ON invoice.id = line.invoice_id
...
CREATE OR REPLACE FUNCTION dwms.deferred_check_charge_allocation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
AS $function$
BEGIN
    PERFORM dwms.assert_charge_allocation_integrity(
        NEW.charge_id,
...
DROP TRIGGER IF EXISTS trg_finance_validate_charge_allocation ON dwms.charge_allocations;
CREATE TRIGGER trg_finance_validate_charge_allocation
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.charge_allocations
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_charge_allocation_mutation();

DROP TRIGGER IF EXISTS trg_finance_validate_invoice_line_charge ON dwms.invoice_line_charges;
CREATE TRIGGER trg_finance_validate_invoice_line_charge
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.invoice_line_charges
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_invoice_line_charge_mutation();

DROP TRIGGER IF EXISTS trg_finance_guard_charge_allocation_parent ON dwms.charges;
CREATE TRIGGER trg_finance_guard_charge_allocation_parent
    BEFORE UPDATE OR DELETE ON dwms.charges
    FOR EACH ROW EXECUTE PROCEDURE dwms.guard_charge_allocation_parent();

DROP TRIGGER IF EXISTS trg_finance_guard_invoice_line_parent ON dwms.invoice_lines;
CREATE TRIGGER trg_finance_guard_invoice_line_parent
    BEFORE UPDATE OR DELETE ON dwms.invoice_lines
    FOR EACH ROW EXECUTE PROCEDURE dwms.guard_invoice_line_charge_parent();

DROP TRIGGER IF EXISTS trg_finance_deferred_charge_allocation ON dwms.charge_allocations;
CREATE CONSTRAINT TRIGGER trg_finance_deferred_charge_allocation
    AFTER INSERT OR UPDATE ON dwms.charge_allocations
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE dwms.deferred_check_charge_allocation();

DROP TRIGGER IF EXISTS trg_finance_deferred_invoice_line_charge ON dwms.invoice_line_charges;
CREATE CONSTRAINT TRIGGER trg_finance_deferred_invoice_line_charge
    AFTER INSERT OR UPDATE ON dwms.invoice_line_charges
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE dwms.deferred_check_charge_allocation();

CREATE OR REPLACE FUNCTION dwms.validate_payment_application_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    SELECT COALESCE(sum(applied_amount), 0)
      ...
      FROM dwms.payment_applications application
    ...
    IF NEW.invoice_id IS NOT NULL THEN
        ...
        SELECT settlement.* INTO linked_settlement_row
          FROM dwms.settlements settlement
        ...
        IF FOUND THEN
            ...
            END IF;
            IF EXISTS (
                SELECT 1 FROM dwms.payment_applications application
    ...
    ELSE
        SELECT settlement.tenant_id, settlement.payer_partner_id, settlement.payee_partner_id,
          ...
          INTO settlement_row
          FROM dwms.settlements settlement
          LEFT JOIN dwms.settlement_batches batch ON batch.id = settlement.settlement_batch_id
...
CREATE OR REPLACE FUNCTION dwms.guard_payment_projection()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    SELECT COALESCE(sum(applied_amount) FILTER (WHERE reversed_at IS NULL), 0), count(*) > 0
      ...
      FROM dwms.payment_applications
     WHERE payment_id = OLD.id;
    authorized := dwms.finance_projection_authorized('PAYMENT_APPLICATION');
...
CREATE OR REPLACE FUNCTION dwms.guard_settlement_payment_projection()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        IF NEW.invoice_id IS NOT NULL THEN
            PERFORM pg_advisory_xact_lock(hashtext('dwms.finance'), hashtext(NEW.tenant_id::text));
            SELECT invoice.* INTO invoice_row
              FROM dwms.invoices invoice WHERE invoice.id = NEW.invoice_id FOR SHARE;
            IF NOT FOUND THEN
    ...
    IF TG_OP = 'DELETE' THEN
        IF has_history THEN
            RAISE EXCEPTION 'Settlement % with payment history cannot be deleted', OLD.id
                USING ERRCODE = '55000';
        END IF;
        IF EXISTS (SELECT 1 FROM dwms.settlement_lines WHERE settlement_id = OLD.id) THEN
    ...
    IF OLD.invoice_id IS DISTINCT FROM NEW.invoice_id AND NEW.invoice_id IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM dwms.payment_applications WHERE settlement_id = OLD.id) THEN
...
CREATE OR REPLACE FUNCTION dwms.project_payment_application()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
DECLARE
    ...
    target_status text;
BEGIN
    authorization_token := dwms.open_finance_projection_authorization('PAYMENT_APPLICATION');
    BEGIN
        SELECT payment.* INTO payment_row
          FROM dwms.payments payment
         WHERE payment.id = NEW.payment_id
        ...
        ELSE
            ...
            target_status := CASE
                ...
                ELSE 'PAID'
            END;
            UPDATE dwms.settlements SET status = target_status WHERE id = NEW.settlement_id;
        END IF;
        PERFORM dwms.close_finance_projection_authorization(authorization_token);
...
CREATE OR REPLACE FUNCTION dwms.assert_payment_application_projection()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    SELECT payment.* INTO payment_row
      FROM dwms.payments payment WHERE payment.id = NEW.payment_id FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF NEW.invoice_id IS NOT NULL THEN
        SELECT invoice.* INTO invoice_row
          FROM dwms.invoices invoice WHERE invoice.id = NEW.invoice_id FOR UPDATE;
        IF NOT FOUND THEN
    ...
    ELSE
        SELECT settlement.* INTO settlement_row
          FROM dwms.settlements settlement WHERE settlement.id = NEW.settlement_id FOR UPDATE;
        IF NOT FOUND THEN
...
CREATE OR REPLACE FUNCTION dwms.prevent_finance_history_truncate()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
REVOKE ALL ON FUNCTION dwms.validate_charge_allocation_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.validate_invoice_line_charge_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.guard_charge_allocation_parent() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.guard_invoice_line_charge_parent() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.assert_charge_allocation_integrity(uuid,uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.deferred_check_charge_allocation() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.validate_payment_application_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.guard_payment_projection() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.guard_settlement_payment_projection() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.project_payment_application() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.assert_payment_application_projection() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_finance_validate_payment_application ON dwms.payment_applications;
CREATE TRIGGER trg_finance_validate_payment_application
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.payment_applications
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_payment_application_mutation();

DROP TRIGGER IF EXISTS trg_finance_project_payment_application ON dwms.payment_applications;
CREATE TRIGGER trg_finance_project_payment_application
    AFTER INSERT OR UPDATE ON dwms.payment_applications
    FOR EACH ROW EXECUTE PROCEDURE dwms.project_payment_application();

DROP TRIGGER IF EXISTS trg_finance_deferred_payment_projection ON dwms.payment_applications;
CREATE CONSTRAINT TRIGGER trg_finance_deferred_payment_projection
    AFTER INSERT OR UPDATE ON dwms.payment_applications
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE dwms.assert_payment_application_projection();

DROP TRIGGER IF EXISTS trg_finance_guard_payment_projection ON dwms.payments;
CREATE TRIGGER trg_finance_guard_payment_projection
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.payments
    FOR EACH ROW EXECUTE PROCEDURE dwms.guard_payment_projection();

DROP TRIGGER IF EXISTS trg_finance_guard_settlement_projection ON dwms.settlements;
CREATE TRIGGER trg_finance_guard_settlement_projection
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.settlements
    FOR EACH ROW EXECUTE PROCEDURE dwms.guard_settlement_payment_projection();

DROP TRIGGER IF EXISTS trg_finance_prevent_truncate ON dwms.charge_allocations;
CREATE TRIGGER trg_finance_prevent_truncate
    BEFORE TRUNCATE ON dwms.charge_allocations
    FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_finance_history_truncate();
DROP TRIGGER IF EXISTS trg_finance_prevent_truncate ON dwms.invoice_line_charges;
CREATE TRIGGER trg_finance_prevent_truncate
    BEFORE TRUNCATE ON dwms.invoice_line_charges
    FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_finance_history_truncate();
DROP TRIGGER IF EXISTS trg_finance_prevent_truncate ON dwms.payment_applications;
CREATE TRIGGER trg_finance_prevent_truncate
    BEFORE TRUNCATE ON dwms.payment_applications
    FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_finance_history_truncate();

CREATE OR REPLACE FUNCTION dwms.validate_settlement_line_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(hashtext('dwms.finance'), hashtext(target_tenant_id::text));

    -- Charge is always locked before settlement, matching the other finance allocation paths.
    SELECT charge.* INTO charge_row
      FROM dwms.charges charge WHERE charge.id = target_charge_id FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.settlement_lines other_line
          JOIN dwms.settlements other_settlement ON other_settlement.id = other_line.settlement_id
...
CREATE OR REPLACE FUNCTION dwms.validate_settlement_commitment()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    IF NEW.status NOT IN ('APPROVED','INVOICED','PARTIALLY_PAID','PAID','CLOSED') THEN
        RETURN NEW;
    END IF;
    IF TG_OP = 'UPDATE' AND dwms.finance_projection_authorized('PAYMENT_APPLICATION') THEN
        RETURN NEW;
    END IF;
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.settlement_lines line
          JOIN dwms.charges charge ON charge.id = line.charge_id
          LEFT JOIN dwms.settlement_batches batch ON batch.id = NEW.settlement_batch_id
         WHERE line.settlement_id = NEW.id
           AND (
               ...
               OR EXISTS (
                   SELECT 1
                     FROM dwms.invoice_line_charges invoice_allocation
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.settlement_lines line
          JOIN dwms.settlement_lines other_line
    ...
    IF NEW.invoice_id IS NOT NULL
       AND NEW.status IN ('APPROVED','INVOICED','PARTIALLY_PAID','PAID','CLOSED') THEN
        ...
        IF EXISTS (
            SELECT 1
              FROM dwms.settlement_lines line
              LEFT JOIN dwms.invoice_line_charges allocation
            ...
            SELECT 1
              FROM dwms.invoice_line_charges allocation
              LEFT JOIN dwms.settlement_lines line
            ...
            SELECT 1
              FROM dwms.invoice_lines invoice_line
              LEFT JOIN dwms.invoice_line_charges allocation
...
CREATE OR REPLACE FUNCTION dwms.deferred_check_settlement_line()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    SELECT line.* INTO line_row
      FROM dwms.settlement_lines line WHERE line.id = NEW.id;
    IF NOT FOUND THEN
        RETURN NULL;
    END IF;
    PERFORM 1 FROM dwms.charges charge WHERE charge.id = line_row.charge_id FOR UPDATE;
    IF EXISTS (
        SELECT 1
          FROM dwms.settlement_lines other_line
          JOIN dwms.settlements other_settlement ON other_settlement.id = other_line.settlement_id
...
REVOKE ALL ON FUNCTION dwms.validate_settlement_line_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.validate_settlement_commitment() FROM PUBLIC;
REVOKE ALL ON FUNCTION dwms.deferred_check_settlement_line() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_finance_validate_settlement_line ON dwms.settlement_lines;
CREATE TRIGGER trg_finance_validate_settlement_line
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.settlement_lines
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_settlement_line_mutation();

DROP TRIGGER IF EXISTS trg_finance_validate_settlement_commitment ON dwms.settlements;
CREATE TRIGGER trg_finance_validate_settlement_commitment
    BEFORE INSERT OR UPDATE OF status ON dwms.settlements
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_settlement_commitment();

DROP TRIGGER IF EXISTS trg_finance_deferred_settlement_line ON dwms.settlement_lines;
CREATE CONSTRAINT TRIGGER trg_finance_deferred_settlement_line
    AFTER INSERT OR UPDATE ON dwms.settlement_lines
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE dwms.deferred_check_settlement_line();

DROP TRIGGER IF EXISTS trg_finance_prevent_truncate ON dwms.settlement_lines;
CREATE TRIGGER trg_finance_prevent_truncate
    BEFORE TRUNCATE ON dwms.settlement_lines
    FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_finance_history_truncate();
