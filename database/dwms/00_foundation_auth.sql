-- DWMS enterprise WMS schema - foundation, tenancy, Firebase IAM, and platform services.
-- PostgreSQL 11 compatible. Executed by scripts/apply_dwms_schema.py in one transaction.

CREATE SCHEMA IF NOT EXISTS dwms;

CREATE OR REPLACE FUNCTION dwms.generate_uuid()
RETURNS uuid
LANGUAGE sql
VOLATILE
AS $function$
...
CREATE OR REPLACE FUNCTION dwms.jsonb_contains_forbidden_secret_key(p_value jsonb)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
STRICT
    ...
    IF jsonb_typeof(p_value) = 'object' THEN
        ...
        LOOP
            ...
            IF jsonb_typeof(object_entry.value) IN ('object','array')
               AND dwms.jsonb_contains_forbidden_secret_key(object_entry.value) THEN
                RETURN true;
            END IF;
        END LOOP;
    ELSIF jsonb_typeof(p_value) = 'array' THEN
        ...
        LOOP
            IF jsonb_typeof(array_value) IN ('object','array')
               AND dwms.jsonb_contains_forbidden_secret_key(array_value) THEN
                RETURN true;
            END IF;
        END LOOP;
    END IF;
...
CREATE OR REPLACE FUNCTION dwms.touch_row()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE TABLE IF NOT EXISTS dwms.schema_migrations (
...
CREATE TABLE IF NOT EXISTS dwms.tenants (
...
CREATE TABLE IF NOT EXISTS dwms.tenant_settings (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_tenant_settings_no_inline_secrets CHECK (
        setting_value IS NULL OR NOT dwms.jsonb_contains_forbidden_secret_key(setting_value)
    )
);

COMMENT ON TABLE dwms.tenant_settings IS
    'Tenant configuration. Secret values stay in an external secret manager; this table stores only an opaque secret_reference.';

CREATE TABLE IF NOT EXISTS dwms.organizations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS dwms.organization_units (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    organization_id uuid NOT NULL REFERENCES dwms.organizations(id),
    parent_unit_id uuid REFERENCES dwms.organization_units(id),
...
CREATE TABLE IF NOT EXISTS dwms.users (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'dwms.users'::regclass
          AND conname = 'fk_dwms_users_platform_user'
    ) THEN
        ALTER TABLE dwms.users
            ADD CONSTRAINT fk_dwms_users_platform_user
...
CREATE INDEX IF NOT EXISTS ix_users_email_search
    ON dwms.users (lower(primary_email))
    WHERE primary_email IS NOT NULL AND deleted_at IS NULL;

COMMENT ON TABLE dwms.users IS
    'DWMS principals mapped to canonical kang.users and Firebase token subjects. Tenant access is granted only through tenant_memberships.';
COMMENT ON COLUMN dwms.users.firebase_uid IS
    'Canonical Firebase token subject. The unique identity key is firebase_project_id, COALESCE(firebase_tenant_id, ''''), firebase_uid.';
COMMENT ON COLUMN dwms.users.primary_email IS
...
COMMENT ON COLUMN dwms.users.platform_user_id IS
    'Required canonical platform principal in kang.users; DWMS must not maintain an independent password database.';

CREATE OR REPLACE FUNCTION dwms.validate_user_platform_subject()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'DWMS platform_user_id and Firebase uid must identify the same canonical kang.users row'
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
...
REVOKE ALL ON FUNCTION dwms.validate_user_platform_subject() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_validate_user_platform_subject ON dwms.users;
CREATE TRIGGER trg_validate_user_platform_subject
    BEFORE INSERT OR UPDATE OF platform_user_id, firebase_uid ON dwms.users
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_user_platform_subject();

CREATE OR REPLACE FUNCTION dwms.prevent_user_identity_rekey()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    IF OLD.firebase_project_id IS DISTINCT FROM NEW.firebase_project_id
       ...
       OR OLD.platform_user_id IS DISTINCT FROM NEW.platform_user_id THEN
        RAISE EXCEPTION
            'DWMS canonical user identity keys are immutable; use an explicit audited account-link/migration workflow';
    END IF;
    RETURN NEW;
END;
$function$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'dwms.users'::regclass
        ...
        ON dwms.users
        FOR EACH ROW EXECUTE PROCEDURE dwms.prevent_user_identity_rekey();
    END IF;
END;
$block$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'dwms.organization_units'::regclass
          AND conname = 'fk_org_units_manager'
    ) THEN
        ALTER TABLE dwms.organization_units
            ADD CONSTRAINT fk_org_units_manager FOREIGN KEY (manager_user_id)
            REFERENCES dwms.users(id) ON DELETE SET NULL;
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS dwms.user_profiles (
    user_id uuid PRIMARY KEY REFERENCES dwms.users(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN dwms.user_profiles.phone_encrypted IS
    'Encrypted personal data only; the encryption key must remain in an external KMS/secret manager.';

CREATE TABLE IF NOT EXISTS dwms.auth_identities (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    ...
    CONSTRAINT ck_auth_identities_no_secret_metadata CHECK (
        NOT dwms.jsonb_contains_forbidden_secret_key(metadata)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_auth_identities_provider_subject
    ON dwms.auth_identities (
        firebase_project_id,
        COALESCE(firebase_tenant_id, ''),
        provider_code,
        provider_subject
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_auth_identities_active_user_provider
    ON dwms.auth_identities (user_id, provider_code)
    WHERE disabled_at IS NULL;

COMMENT ON TABLE dwms.auth_identities IS
...
COMMENT ON COLUMN dwms.auth_identities.firebase_uid IS
    'Firebase account subject from the verified ID token; deliberately separate from the provider-specific subject.';
COMMENT ON COLUMN dwms.auth_identities.provider_subject IS
...
COMMENT ON COLUMN dwms.auth_identities.provider_email IS
...
COMMENT ON COLUMN dwms.auth_identities.metadata IS
    'Non-secret provider metadata only. Password material and raw Firebase/Google tokens are prohibited.';

CREATE OR REPLACE FUNCTION dwms.validate_auth_identity_owner()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    owner_row dwms.users%ROWTYPE;
BEGIN
    IF TG_OP = 'UPDATE' AND TG_TABLE_NAME = 'auth_identities' AND (
        ...
        RAISE EXCEPTION
            'DWMS authentication identity keys are immutable; disable the identity and complete a new explicit link request';
    END IF;
    ...
    IF TG_OP = 'UPDATE' AND TG_TABLE_NAME = 'account_link_requests' AND (
        ...
        RAISE EXCEPTION
            'DWMS account-link identity keys are immutable; cancel the request and create a new one';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'DWMS auth identity user does not exist';
    END IF;
    IF owner_row.firebase_project_id IS DISTINCT FROM NEW.firebase_project_id
       ...
       OR owner_row.firebase_uid IS DISTINCT FROM NEW.firebase_uid THEN
        RAISE EXCEPTION 'DWMS auth identity Firebase subject must match its user';
    END IF;
    RETURN NEW;
END;
$function$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'dwms.auth_identities'::regclass
        ...
        ON dwms.auth_identities
        FOR EACH ROW EXECUTE PROCEDURE dwms.validate_auth_identity_owner();
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS dwms.account_link_requests (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES dwms.users(id) ON DELETE CASCADE,
    ...
    decided_by uuid REFERENCES dwms.users(id),
    decided_at timestamptz,
    decision_reason text,
    completed_identity_id uuid REFERENCES dwms.auth_identities(id) ON DELETE SET NULL,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS ix_account_link_requests_user_status
    ON dwms.account_link_requests (user_id, status, requested_at DESC);

COMMENT ON TABLE dwms.account_link_requests IS
...
COMMENT ON COLUMN dwms.account_link_requests.provider_email IS
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'dwms.account_link_requests'::regclass
          AND tgname = 'trg_validate_account_link_owner'
          AND NOT tgisinternal
    ) THEN
CREATE TRIGGER trg_validate_account_link_owner
        BEFORE INSERT OR UPDATE OF user_id, firebase_project_id, firebase_tenant_id, firebase_uid, provider_code, provider_subject
        ON dwms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE dwms.validate_auth_identity_owner();
    END IF;
END;
$block$;

CREATE OR REPLACE FUNCTION dwms.validate_account_link_completion()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    identity_row dwms.auth_identities%ROWTYPE;
BEGIN
...
CREATE OR REPLACE FUNCTION dwms.validate_account_link_transition()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    IF NOT (
        ...
        RAISE EXCEPTION 'Invalid DWMS account-link status transition: % -> %',
...
BEGIN
    IF NOT EXISTS (
        ...
        ON dwms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE dwms.validate_account_link_completion();
    END IF;
END;
$block$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ON dwms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE dwms.validate_account_link_transition();
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS dwms.account_link_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    request_id uuid NOT NULL REFERENCES dwms.account_link_requests(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_account_link_event_no_secrets CHECK (
        NOT dwms.jsonb_contains_forbidden_secret_key(evidence_masked)
    )
);

COMMENT ON TABLE dwms.account_link_events IS
    'Append-oriented audit trail for explicit account-link decisions; raw credentials, tokens, and nonces are prohibited.';

CREATE TABLE IF NOT EXISTS dwms.user_invitations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    email varchar(320) NOT NULL,
    invitation_token_hash char(64) NOT NULL UNIQUE,
    invited_by uuid REFERENCES dwms.users(id),
    expires_at timestamptz NOT NULL,
    accepted_by uuid REFERENCES dwms.users(id),
...
COMMENT ON TABLE dwms.user_invitations IS
...
CREATE TABLE IF NOT EXISTS dwms.tenant_memberships (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    user_id uuid NOT NULL REFERENCES dwms.users(id) ON DELETE CASCADE,
    organization_id uuid REFERENCES dwms.organizations(id),
    default_org_unit_id uuid REFERENCES dwms.organization_units(id),
    employee_no varchar(50),
...
CREATE TABLE IF NOT EXISTS dwms.user_groups (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.user_group_members (
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    group_id uuid NOT NULL REFERENCES dwms.user_groups(id) ON DELETE CASCADE,
    membership_id uuid NOT NULL REFERENCES dwms.tenant_memberships(id) ON DELETE CASCADE,
    joined_by uuid REFERENCES dwms.users(id),
    joined_at timestamptz NOT NULL DEFAULT now(),
    removed_by uuid REFERENCES dwms.users(id),
...
CREATE TABLE IF NOT EXISTS dwms.permissions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS dwms.roles (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_roles_tenant_code
    ON dwms.roles (COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), role_code);

CREATE TABLE IF NOT EXISTS dwms.role_permissions (
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    role_id uuid NOT NULL REFERENCES dwms.roles(id) ON DELETE CASCADE,
    permission_id uuid NOT NULL REFERENCES dwms.permissions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.role_assignments (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    membership_id uuid REFERENCES dwms.tenant_memberships(id) ON DELETE CASCADE,
    group_id uuid REFERENCES dwms.user_groups(id) ON DELETE CASCADE,
    role_id uuid NOT NULL REFERENCES dwms.roles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.delegations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    delegator_membership_id uuid NOT NULL REFERENCES dwms.tenant_memberships(id),
    delegate_membership_id uuid NOT NULL REFERENCES dwms.tenant_memberships(id),
...
CREATE TABLE IF NOT EXISTS dwms.user_sessions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES dwms.users(id) ON DELETE CASCADE,
    ...
    FOREIGN KEY (active_membership_id, user_id)
        REFERENCES dwms.tenant_memberships(id, user_id) ON DELETE CASCADE,
...
COMMENT ON TABLE dwms.user_sessions IS
    'Server-side session metadata only. Raw session cookies and Firebase/Google ID, access, or refresh tokens are prohibited.';
COMMENT ON COLUMN dwms.user_sessions.session_handle_hash IS
    'One-way SHA-256 hash of a high-entropy server-issued session handle; never store the raw handle or Firebase token.';

CREATE TABLE IF NOT EXISTS dwms.login_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id),
    user_id uuid REFERENCES dwms.users(id) ON DELETE SET NULL,
...
COMMENT ON TABLE dwms.login_events IS
...
CREATE TABLE IF NOT EXISTS dwms.user_devices (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES dwms.users(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN dwms.user_devices.push_token_encrypted IS
    'Push token ciphertext only; encryption keys must remain in the external secret manager.';

CREATE TABLE IF NOT EXISTS dwms.terms_versions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS dwms.user_terms_agreements (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES dwms.users(id) ON DELETE CASCADE,
    terms_version_id uuid NOT NULL REFERENCES dwms.terms_versions(id),
...
CREATE TABLE IF NOT EXISTS dwms.api_clients (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.api_client_permissions (
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    api_client_id uuid NOT NULL REFERENCES dwms.api_clients(id) ON DELETE CASCADE,
    permission_id uuid NOT NULL REFERENCES dwms.permissions(id) ON DELETE CASCADE,
    granted_by uuid REFERENCES dwms.users(id),
    granted_at timestamptz NOT NULL DEFAULT now(),
    revoked_by uuid REFERENCES dwms.users(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_code_sets_scope
    ON dwms.code_sets (COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), code_set);

CREATE TABLE IF NOT EXISTS dwms.code_values (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    code_set_id uuid NOT NULL REFERENCES dwms.code_sets(id) ON DELETE CASCADE,
    code varchar(80) NOT NULL,
    code_name varchar(200) NOT NULL,
    description text,
    sort_order integer NOT NULL DEFAULT 0,
    parent_code_value_id uuid REFERENCES dwms.code_values(id),
...
CREATE TABLE IF NOT EXISTS dwms.number_sequences (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.business_calendars (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.calendar_dates (
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    calendar_id uuid NOT NULL REFERENCES dwms.business_calendars(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.files (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    uploaded_by uuid REFERENCES dwms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    deleted_at timestamptz,
...
COMMENT ON TABLE dwms.files IS
    'Object-storage metadata only. File bytes, credentials, and encryption keys are never stored in PostgreSQL.';

CREATE TABLE IF NOT EXISTS dwms.entity_files (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    file_id uuid NOT NULL REFERENCES dwms.files(id),
...
CREATE TABLE IF NOT EXISTS dwms.entity_notes (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.custom_field_definitions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    entity_type varchar(80) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.custom_field_values (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    definition_id uuid NOT NULL REFERENCES dwms.custom_field_definitions(id) ON DELETE CASCADE,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.idempotency_keys (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
COMMENT ON COLUMN dwms.outbox_events.payload IS
    'Business event payload with secrets and authentication tokens removed.';

CREATE TABLE IF NOT EXISTS dwms.external_systems (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    ...
    CONSTRAINT ck_external_systems_no_inline_secrets CHECK (
        NOT dwms.jsonb_contains_forbidden_secret_key(config)
    )
);

COMMENT ON COLUMN dwms.external_systems.credential_secret_ref IS
    'Opaque external secret-manager reference; never store a credential value here or in config.';

CREATE TABLE IF NOT EXISTS dwms.external_id_mappings (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.integration_messages (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
COMMENT ON TABLE dwms.integration_messages IS
...
CREATE TABLE IF NOT EXISTS dwms.audit_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id),
    actor_user_id uuid REFERENCES dwms.users(id) ON DELETE SET NULL,
    actor_api_client_id uuid REFERENCES dwms.api_clients(id) ON DELETE SET NULL,
...
COMMENT ON TABLE dwms.audit_events IS
...
CREATE TABLE IF NOT EXISTS dwms.data_retention_policies (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    entity_type varchar(80) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.legal_holds (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.legal_hold_entities (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    legal_hold_id uuid NOT NULL REFERENCES dwms.legal_holds(id) ON DELETE RESTRICT,
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
COMMENT ON TABLE dwms.legal_hold_entities IS
