-- DWMS workflow, notifications, EDI/webhooks/jobs, audit extensions, and analytics.

CREATE TABLE IF NOT EXISTS dwms.status_definitions (
...
CREATE TABLE IF NOT EXISTS dwms.entity_tags (
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    tag_id uuid NOT NULL REFERENCES dwms.tags(id) ON DELETE CASCADE,
    assigned_by uuid REFERENCES dwms.users(id),
    assigned_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, entity_type, entity_id, tag_id)
);

CREATE TABLE IF NOT EXISTS dwms.notification_templates (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.notifications (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    event_type varchar(100) NOT NULL,
    template_id uuid REFERENCES dwms.notification_templates(id),
    ...
    CONSTRAINT ck_notification_no_secret_variables CHECK (
        NOT dwms.jsonb_contains_forbidden_secret_key(variables_masked)
    )
);

CREATE TABLE IF NOT EXISTS dwms.notification_deliveries (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    notification_id uuid NOT NULL REFERENCES dwms.notifications(id) ON DELETE CASCADE,
    user_id uuid REFERENCES dwms.users(id),
    contact_id uuid REFERENCES dwms.contacts(id),
    channel varchar(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.edi_documents (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.edi_validation_errors (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    edi_document_id uuid NOT NULL REFERENCES dwms.edi_documents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.webhook_subscriptions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
COMMENT ON TABLE dwms.webhook_subscriptions IS
...
CREATE TABLE IF NOT EXISTS dwms.webhook_deliveries (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    subscription_id uuid NOT NULL REFERENCES dwms.webhook_subscriptions(id) ON DELETE CASCADE,
    outbox_event_id uuid REFERENCES dwms.outbox_events(id),
...
CREATE TABLE IF NOT EXISTS dwms.inbox_deduplication (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    source_system_id uuid NOT NULL REFERENCES dwms.external_systems(id),
...
CREATE TABLE IF NOT EXISTS dwms.import_jobs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    job_no varchar(100) NOT NULL,
    import_type varchar(80) NOT NULL,
    source_file_id uuid NOT NULL REFERENCES dwms.files(id),
    ...
    result_file_id uuid REFERENCES dwms.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS dwms.import_job_rows (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    import_job_id uuid NOT NULL REFERENCES dwms.import_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.export_jobs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    job_no varchar(100) NOT NULL,
    ...
    output_file_id uuid REFERENCES dwms.files(id),
    requested_by uuid REFERENCES dwms.users(id),
    requested_at timestamptz NOT NULL DEFAULT now(),
    completed_at timestamptz,
    expires_at timestamptz,
...
CREATE TABLE IF NOT EXISTS dwms.export_job_rows (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    export_job_id uuid NOT NULL REFERENCES dwms.export_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.scheduled_jobs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_scheduled_job_no_inline_secrets CHECK (
        NOT dwms.jsonb_contains_forbidden_secret_key(parameters)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_scheduled_jobs_scope
    ON dwms.scheduled_jobs (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), job_code
    );

CREATE TABLE IF NOT EXISTS dwms.job_runs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    scheduled_job_id uuid NOT NULL REFERENCES dwms.scheduled_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.dead_letter_messages (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.impersonation_sessions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    administrator_user_id uuid NOT NULL REFERENCES dwms.users(id),
    impersonated_user_id uuid NOT NULL REFERENCES dwms.users(id),
...
CREATE TABLE IF NOT EXISTS dwms.impersonation_events (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    impersonation_session_id uuid NOT NULL REFERENCES dwms.impersonation_sessions(id) ON DELETE RESTRICT,
    sequence_no integer NOT NULL,
    event_type varchar(20) NOT NULL,
    actor_user_id uuid NOT NULL REFERENCES dwms.users(id),
    reason text,
...
CREATE TABLE IF NOT EXISTS dwms.entity_change_logs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.data_access_logs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id),
    actor_user_id uuid REFERENCES dwms.users(id),
    actor_api_client_id uuid REFERENCES dwms.api_clients(id),
...
CREATE TABLE IF NOT EXISTS dwms.reconciliation_runs (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.reconciliation_details (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    reconciliation_run_id uuid NOT NULL REFERENCES dwms.reconciliation_runs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.workflow_definitions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.workflow_versions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    workflow_definition_id uuid NOT NULL REFERENCES dwms.workflow_definitions(id) ON DELETE CASCADE,
    version_no integer NOT NULL,
    effective_from timestamptz NOT NULL,
    effective_to timestamptz,
    trigger_conditions jsonb NOT NULL DEFAULT '{}'::jsonb,
    status varchar(20) NOT NULL DEFAULT 'DRAFT',
    published_by uuid REFERENCES dwms.users(id),
...
CREATE TABLE IF NOT EXISTS dwms.workflow_steps (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES dwms.workflow_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.workflow_instances (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES dwms.workflow_versions(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    status varchar(20) NOT NULL DEFAULT 'RUNNING',
    current_step_id uuid REFERENCES dwms.workflow_steps(id),
...
CREATE TABLE IF NOT EXISTS dwms.workflow_tasks (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    workflow_instance_id uuid NOT NULL REFERENCES dwms.workflow_instances(id) ON DELETE CASCADE,
    workflow_step_id uuid NOT NULL REFERENCES dwms.workflow_steps(id),
    task_no integer NOT NULL,
    assignee_user_id uuid REFERENCES dwms.users(id),
    assignee_group_id uuid REFERENCES dwms.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    completed_at timestamptz,
...
CREATE TABLE IF NOT EXISTS dwms.approval_policies (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dwms.approval_requests (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    request_no varchar(100) NOT NULL,
    policy_id uuid REFERENCES dwms.approval_policies(id),
    workflow_instance_id uuid REFERENCES dwms.workflow_instances(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    amount numeric(20,4),
    currency_code char(3) REFERENCES dwms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dwms.approval_steps (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    approval_request_id uuid NOT NULL REFERENCES dwms.approval_requests(id) ON DELETE CASCADE,
    level_no integer NOT NULL,
    approver_user_id uuid REFERENCES dwms.users(id),
    approver_group_id uuid REFERENCES dwms.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    ...
    decision varchar(20),
    comments text,
    delegated_from_user_id uuid REFERENCES dwms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_user
    ON dwms.approval_steps (approval_request_id, level_no, approver_user_id)
    WHERE approver_user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_group
    ON dwms.approval_steps (approval_request_id, level_no, approver_group_id)
    WHERE approver_group_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS dwms.approval_actions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    approval_step_id uuid NOT NULL REFERENCES dwms.approval_steps(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.sod_rules (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dwms.sod_rule_conflicts (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    sod_rule_id uuid NOT NULL REFERENCES dwms.sod_rules(id) ON DELETE CASCADE,
    left_permission_id uuid NOT NULL REFERENCES dwms.permissions(id),
    right_permission_id uuid NOT NULL REFERENCES dwms.permissions(id),
...
CREATE TABLE IF NOT EXISTS dwms.sod_violations (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    sod_rule_id uuid NOT NULL REFERENCES dwms.sod_rules(id),
    user_id uuid NOT NULL REFERENCES dwms.users(id),
    left_role_assignment_id uuid REFERENCES dwms.role_assignments(id),
    right_role_assignment_id uuid REFERENCES dwms.role_assignments(id),
    ...
    waived_by uuid REFERENCES dwms.users(id),
...
COMMENT ON COLUMN dwms.workflow_versions.trigger_conditions IS
    'Declarative, allow-listed workflow DSL only; never execute this JSON as SQL, PL/pgSQL, shell, or template code.';
COMMENT ON COLUMN dwms.workflow_steps.assignee_rule IS
...
CREATE TABLE IF NOT EXISTS dwms.kpi_definitions (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    ...
    owner_org_unit_id uuid REFERENCES dwms.organization_units(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_definitions_scope
    ON dwms.kpi_definitions (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        kpi_code, calculation_version
    );

COMMENT ON COLUMN dwms.kpi_definitions.calculation_expression IS
    'Declarative, versioned KPI DSL only; never execute as SQL or application code.';

CREATE TABLE IF NOT EXISTS dwms.kpi_targets (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES dwms.kpi_definitions(id),
...
CREATE TABLE IF NOT EXISTS dwms.kpi_values (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES dwms.kpi_definitions(id),
    scope_type varchar(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwms.warehouse_daily_facts (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    fact_date date NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES dwms.warehouses(id),
    warehouse_client_id uuid REFERENCES dwms.warehouse_clients(id),
    owner_partner_id uuid REFERENCES dwms.business_partners(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_inventory_daily_facts_grain
    ON dwms.inventory_daily_facts (
        tenant_id, fact_date, warehouse_id,
        COALESCE(warehouse_client_id, '00000000-0000-0000-0000-000000000000'::uuid),
        owner_partner_id, item_id, inventory_status_id
    );

CREATE TABLE IF NOT EXISTS dwms.operation_daily_facts (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    fact_date date NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES dwms.warehouses(id),
...
CREATE TABLE IF NOT EXISTS dwms.data_quality_rules (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid REFERENCES dwms.tenants(id) ON DELETE CASCADE,
    rule_code varchar(100) NOT NULL,
    rule_name varchar(250) NOT NULL,
    ...
    is_active boolean NOT NULL DEFAULT true,
    owner_user_id uuid REFERENCES dwms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_data_quality_rules_scope
    ON dwms.data_quality_rules (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        rule_code
    );

COMMENT ON COLUMN dwms.data_quality_rules.rule_expression IS
    'Declarative, allow-listed validation DSL only; never execute this JSON as SQL or code.';

CREATE TABLE IF NOT EXISTS dwms.data_quality_results (
    id uuid PRIMARY KEY DEFAULT dwms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
    evaluation_id uuid NOT NULL,
    rule_id uuid NOT NULL REFERENCES dwms.data_quality_rules(id),
...
CREATE INDEX IF NOT EXISTS ix_data_quality_results_evaluation
    ON dwms.data_quality_results (tenant_id, evaluation_id, rule_id, status);

CREATE TABLE IF NOT EXISTS dwms.etl_watermarks (
    tenant_id uuid NOT NULL REFERENCES dwms.tenants(id),
