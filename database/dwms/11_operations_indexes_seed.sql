-- DWMS enterprise WMS schema - operational integrity, RLS, indexes, and baseline seeds.
-- PostgreSQL 11 compatible. Applied last, after all 00-10 modules, in one transaction.

CREATE OR REPLACE FUNCTION dwms.prevent_tenant_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION dwms.enforce_same_tenant_fk()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION dwms.prevent_append_only_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION dwms.protect_message_core()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION dwms.protect_finalized_row()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'settlements' AND TG_OP = 'UPDATE' THEN
        IF dwms.finance_projection_authorized('PAYMENT_APPLICATION') THEN
            RETURN NEW;
        END IF;
...
CREATE OR REPLACE FUNCTION dwms.protect_committed_row()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'settlements' AND TG_OP = 'UPDATE'
       AND dwms.finance_projection_authorized('PAYMENT_APPLICATION') THEN
...
CREATE OR REPLACE FUNCTION dwms.protect_versioned_filing_aggregate()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
    ...
    IF entering_protected_state
       ...
       OR new_version_no IS DISTINCT FROM OLD.current_version_no THEN
        EXECUTE format(
            ...
            'FROM dwms.%I WHERE id = $1 FOR SHARE',
            TG_ARGV[3], TG_ARGV[2]
...
CREATE OR REPLACE FUNCTION dwms.protect_child_when_parent_final()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF old_parent_id IS NOT NULL THEN
        EXECUTE format('SELECT %I::text FROM dwms.%I WHERE id = $1 FOR SHARE', TG_ARGV[2], TG_ARGV[1])
           INTO parent_status
           USING old_parent_id;
...
CREATE OR REPLACE FUNCTION dwms.validate_inventory_transaction_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
CREATE OR REPLACE FUNCTION dwms.protect_journal_line_when_posted()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF old_entry_id IS NOT NULL THEN
        SELECT b.status INTO batch_status
          FROM dwms.journal_entries e
          JOIN dwms.journal_batches b ON b.id = e.journal_batch_id
...
CREATE OR REPLACE FUNCTION dwms.protect_invoice_line_child_when_posted()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
CREATE OR REPLACE FUNCTION dwms.validate_journal_batch_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.journal_entries entry
          JOIN dwms.journal_lines line ON line.journal_entry_id = entry.id
          JOIN dwms.gl_accounts account ON account.id = line.gl_account_id
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.journal_entries entry
          LEFT JOIN dwms.journal_lines line ON line.journal_entry_id = entry.id
         WHERE entry.journal_batch_id = NEW.id
         GROUP BY entry.id
...
CREATE OR REPLACE FUNCTION dwms.validate_invoice_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE line.invoice_id = NEW.id
           AND EXISTS (
               SELECT 1
                 FROM dwms.invoice_line_charges allocation_history
    ...
    IF EXISTS (
        SELECT 1
          ...
          JOIN dwms.invoice_lines line ON line.id = allocation.invoice_line_id
          JOIN dwms.charges charge ON charge.id = allocation.charge_id
    ...
    IF EXISTS (
        SELECT 1
          FROM dwms.invoice_line_charges allocation
          JOIN dwms.settlement_lines settlement_line
            ON settlement_line.charge_id = allocation.charge_id
          JOIN dwms.settlements settlement
...
CREATE OR REPLACE FUNCTION dwms.protect_posted_invoice_core()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        IF OLD.status IN ('POSTED','PARTIALLY_PAID','PAID','CANCELLED','REVERSED')
           OR EXISTS (SELECT 1 FROM dwms.payment_applications WHERE invoice_id = OLD.id) THEN
...
CREATE OR REPLACE FUNCTION dwms.protect_sealed_version()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION dwms.protect_child_when_parent_sealed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    IF old_parent_id IS NOT NULL THEN
        EXECUTE format('SELECT sealed_at FROM dwms.%I WHERE id = $1 FOR SHARE', TG_ARGV[1])
           INTO parent_sealed_at
           USING old_parent_id;
        IF parent_sealed_at IS NOT NULL THEN
            RAISE EXCEPTION 'Child cannot move from or change under sealed dwms.% version %',
    ...
    IF new_parent_id IS NOT NULL AND new_parent_id IS DISTINCT FROM old_parent_id THEN
        EXECUTE format('SELECT sealed_at FROM dwms.%I WHERE id = $1 FOR SHARE', TG_ARGV[1])
           ...
           USING new_parent_id;
        IF parent_sealed_at IS NOT NULL THEN
            RAISE EXCEPTION 'Child cannot be added or moved under sealed dwms.% version %',
...
CREATE OR REPLACE FUNCTION dwms.validate_bonded_movement_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dwms
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM dwms.bonded_inventory_movements
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('dwms.%I', target.table_name)) IS NULL THEN
            CONTINUE;
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_row ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_row BEFORE UPDATE OR DELETE ON dwms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE dwms.protect_finalized_row(%L,%L)',
            target.table_name, target.status_column, target.terminal_statuses
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_truncate ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_truncate BEFORE TRUNCATE ON dwms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_append_only_change()',
            target.table_name
        );
    END LOOP;
END;
...
DROP TRIGGER IF EXISTS trg_protect_customs_filing_aggregate ON dwms.customs_declarations;
CREATE TRIGGER trg_protect_customs_filing_aggregate
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.customs_declarations
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_versioned_filing_aggregate(
        'workflow_status',
...
DROP TRIGGER IF EXISTS trg_protect_bonded_filing_aggregate ON dwms.bonded_inout_reports;
CREATE TRIGGER trg_protect_bonded_filing_aggregate
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.bonded_inout_reports
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_versioned_filing_aggregate(
        'report_status',
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_committed_row ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_committed_row BEFORE INSERT OR UPDATE OR DELETE ON dwms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE dwms.protect_committed_row(%L,%L,%L)',
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('dwms.%I', target.table_name)) IS NULL THEN
            CONTINUE;
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_parent ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_parent BEFORE INSERT OR UPDATE OR DELETE ON dwms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE dwms.protect_child_when_parent_final(%L,%L,%L,%L)',
            target.table_name, target.parent_fk, target.parent_table,
            target.parent_status_column, target.terminal_statuses
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_child_truncate ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_child_truncate BEFORE TRUNCATE ON dwms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_append_only_change()',
            target.table_name
        );
    END LOOP;
...
CREATE TRIGGER trg_guard_inventory_entry_mutation
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.inventory_transaction_entries
...
CREATE TRIGGER trg_validate_inventory_transaction_posting
    BEFORE INSERT OR UPDATE OF posting_status ON dwms.inventory_transaction_headers
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_inventory_transaction_posting();

DROP TRIGGER IF EXISTS trg_validate_bonded_movement_posting ON dwms.bonded_movement_groups;
CREATE TRIGGER trg_validate_bonded_movement_posting
    BEFORE INSERT OR UPDATE OF status ON dwms.bonded_movement_groups
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_bonded_movement_posting();

DROP TRIGGER IF EXISTS trg_protect_bonded_report_version
    ON dwms.bonded_inout_report_versions;
CREATE TRIGGER trg_protect_bonded_report_version
    BEFORE UPDATE OR DELETE ON dwms.bonded_inout_report_versions
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_sealed_version();

DROP TRIGGER IF EXISTS trg_protect_bonded_report_line
    ON dwms.bonded_inout_report_lines;
CREATE TRIGGER trg_protect_bonded_report_line
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.bonded_inout_report_lines
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_child_when_parent_sealed(
        'bonded_inout_report_version_id','bonded_inout_report_versions'
    );

DROP TRIGGER IF EXISTS trg_protect_posted_journal_line ON dwms.journal_lines;
CREATE TRIGGER trg_protect_posted_journal_line
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.journal_lines
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_journal_line_when_posted();

DROP TRIGGER IF EXISTS trg_validate_journal_batch_posting ON dwms.journal_batches;
CREATE TRIGGER trg_validate_journal_batch_posting
    BEFORE INSERT OR UPDATE OF status ON dwms.journal_batches
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_journal_batch_posting();

DROP TRIGGER IF EXISTS trg_validate_invoice_posting ON dwms.invoices;
CREATE TRIGGER trg_validate_invoice_posting
    BEFORE INSERT OR UPDATE OF status ON dwms.invoices
    FOR EACH ROW EXECUTE PROCEDURE dwms.validate_invoice_posting();

DROP TRIGGER IF EXISTS trg_protect_posted_invoice_core ON dwms.invoices;
CREATE TRIGGER trg_protect_posted_invoice_core
    BEFORE UPDATE OR DELETE ON dwms.invoices
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_posted_invoice_core();

DROP TRIGGER IF EXISTS trg_protect_invoice_line_charge ON dwms.invoice_line_charges;
CREATE TRIGGER trg_protect_invoice_line_charge
    BEFORE INSERT OR UPDATE OR DELETE ON dwms.invoice_line_charges
    FOR EACH ROW EXECUTE PROCEDURE dwms.protect_invoice_line_child_when_posted();
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_integrity_truncate ON dwms.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_integrity_truncate BEFORE TRUNCATE ON dwms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_append_only_change()',
            table_name
        );
    END LOOP;
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('dwms.%I', table_name)) IS NULL THEN
            CONTINUE;
        END IF;
        IF NOT EXISTS (
            SELECT 1
            FROM pg_trigger t
            WHERE t.tgrelid = format('dwms.%I', table_name)::regclass
              AND t.tgname IN ('trg_append_only','trg_block_immutable_change','trg_dwms_append_only')
              AND NOT t.tgisinternal
        ) THEN
            EXECUTE format(
                'CREATE TRIGGER trg_dwms_append_only BEFORE UPDATE OR DELETE ON dwms.%I '
                'FOR EACH ROW EXECUTE PROCEDURE dwms.prevent_append_only_change()',
                table_name
            );
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_dwms_append_only_truncate ON dwms.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_dwms_append_only_truncate BEFORE TRUNCATE ON dwms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_append_only_change()',
            table_name
        );
    END LOOP;
...
BEGIN
    ...
    LOOP
        ...
        IF target.table_name = 'unipass_messages' THEN
            EXECUTE 'DROP TRIGGER IF EXISTS trg_block_immutable_change ON dwms.unipass_messages';
            EXECUTE 'DROP TRIGGER IF EXISTS trg_dwms_append_only ON dwms.unipass_messages';
            EXECUTE 'DROP TRIGGER IF EXISTS trg_append_only ON dwms.unipass_messages';
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_message_core ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_message_core BEFORE UPDATE OR DELETE ON dwms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE dwms.protect_message_core(%s)',
            target.table_name, target.trigger_arguments
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_message_truncate ON dwms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_message_truncate BEFORE TRUNCATE ON dwms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE dwms.prevent_append_only_change()',
            target.table_name
        );
    END LOOP;
...
CREATE OR REPLACE FUNCTION dwms.current_tenant_id()
RETURNS uuid
LANGUAGE sql
STABLE
AS $function$
    SELECT NULLIF(current_setting('dwms.tenant_id', true), '')::uuid;
$function$;

COMMENT ON FUNCTION dwms.current_tenant_id() IS
...
CREATE OR REPLACE FUNCTION dwms.post_inventory_transaction_for_tenant(
...
BEGIN
    context_tenant_id := dwms.current_tenant_id();
    ...
    SELECT header.tenant_id
      ...
      FROM dwms.inventory_transaction_headers header
    ...
    IF NOT EXISTS (
        SELECT 1
          ...
          JOIN dwms.users principal
            ON principal.id = membership.user_id
           AND principal.status = 'ACTIVE'
          JOIN dwms.role_assignments assignment
           ...
           AND (
               ...
               OR EXISTS (
                   SELECT 1
                     FROM dwms.user_group_members group_member
          ...
          JOIN dwms.role_permissions role_permission
          ...
          JOIN dwms.permissions permission
    ...
    PERFORM dwms.post_inventory_transaction(p_inventory_transaction_id, p_posted_by);
END;
$function$;

REVOKE ALL ON FUNCTION dwms.post_inventory_transaction_for_tenant(uuid, uuid) FROM PUBLIC;

COMMENT ON FUNCTION dwms.post_inventory_transaction_for_tenant(uuid, uuid) IS
...
BEGIN
    FOR target IN
        ...
        JOIN information_schema.tables table_meta
         ...
         AND table_meta.table_type = 'BASE TABLE'
        WHERE column_meta.table_schema = 'dwms'
    ...
    LOOP
        ...
        IF target.table_name = ANY (global_reference_tables) THEN
            select_expression := 'tenant_id IS NULL OR tenant_id = dwms.current_tenant_id()';
            modify_expression := 'tenant_id = dwms.current_tenant_id()';
        ELSIF target.table_name = ANY (platform_event_tables) THEN
            select_expression := 'tenant_id = dwms.current_tenant_id() OR '
                || '(tenant_id IS NULL AND current_user = ''dwms_platform_logger'')';
            modify_expression := select_expression;
        ELSIF target.table_name = ANY (global_job_tables) THEN
            select_expression := 'tenant_id = dwms.current_tenant_id() OR '
                || '(tenant_id IS NULL AND current_user = ''dwms_global_job_runner'')';
            modify_expression := select_expression;
        ELSE
            select_expression := 'tenant_id = dwms.current_tenant_id()';
            modify_expression := select_expression;
        END IF;
...
BEGIN
    FOR relation IN
        ...
        WHERE con.contype = 'f' AND ns.nspname = 'dwms'
    ...
    LOOP
        IF NOT EXISTS (
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE dwms.prevent_tenant_change()',
                target.table_schema, target.table_name
            );
        END IF;
    END LOOP;
...
BEGIN
    FOR relation IN
        ...
        WHERE con.contype = 'f'
          AND child_ns.nspname = 'dwms'
          AND parent_ns.nspname = 'dwms'
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE dwms.enforce_same_tenant_fk(%L,%L,%L)',
...
CREATE INDEX IF NOT EXISTS ix_ops_inventory_tx_occurred_brin
    ON dwms.inventory_transaction_headers USING brin (occurred_at);
...
CREATE INDEX IF NOT EXISTS ix_ops_inventory_balance_last_tx
    ON dwms.inventory_balances (tenant_id, last_transaction_at DESC, stock_bucket_id);

CREATE INDEX IF NOT EXISTS ix_ops_receipts_work_queue
    ON dwms.receipts (tenant_id, warehouse_id, owner_partner_id, status, arrived_at, id)
...
CREATE INDEX IF NOT EXISTS ix_ops_pick_task_queue
    ON dwms.pick_tasks (tenant_id, warehouse_id, status, priority, scheduled_at, id)
    WHERE status NOT IN ('COMPLETED','CANCELLED');
CREATE INDEX IF NOT EXISTS ix_ops_shipments_status_schedule
    ON dwms.shipments (tenant_id, warehouse_id, status, planned_ship_at, id);

CREATE INDEX IF NOT EXISTS ix_ops_customs_declaration_queue
    ON dwms.customs_declarations (
...
INSERT INTO dwms.units_of_measure (
...
INSERT INTO dwms.transport_modes (mode_code, mode_name, mode_group, is_active) VALUES
    ('ROAD','Road','LAND',true),
    ('SEA','Sea','OCEAN',true),
    ('AIR','Air','AIR',true),
    ('RAIL','Rail','LAND',true),
...
INSERT INTO dwms.incoterms (incoterm_code, incoterm_name, version_year, is_active) VALUES
...
INSERT INTO dwms.inventory_statuses (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM dwms.inventory_statuses existing
...
INSERT INTO dwms.location_types (
...
WITH seed(role_code, role_name, description) AS (
    VALUES
        ('DWMS_VIEWER','DWMS Viewer','Read-only operational visibility; excludes privileged audit/data-access views.'),
        ('DWMS_OPERATOR','DWMS Operator','Inbound/outbound execution without ledger posting or customs/finance approval.'),
        ('DWMS_SUPERVISOR','DWMS Supervisor','Operational supervision and assigned workflow approvals.'),
        ('DWMS_INVENTORY_CONTROLLER','DWMS Inventory Controller','Inventory posting, adjustment and hold authority.'),
        ('DWMS_CUSTOMS_SPECIALIST','DWMS Customs Specialist','Customs and bonded filing authority.'),
        ('DWMS_BILLING_SPECIALIST','DWMS Billing Specialist','Billing and settlement approval authority.'),
        ('DWMS_AUDITOR','DWMS Auditor','Read-only audit, regulated access and control evidence.'),
        ('DWMS_TENANT_ADMIN','DWMS Tenant Administrator','IAM/configuration administration without inventory, customs or finance posting authority.')
)
INSERT INTO dwms.roles (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM dwms.roles existing
    WHERE existing.tenant_id IS NULL AND existing.role_code = s.role_code
);

WITH grants(role_code, permission_code) AS (
    VALUES
        ('DWMS_VIEWER','master.read'),
        ('DWMS_VIEWER','inbound.read'),
        ('DWMS_VIEWER','inventory.read'),
        ('DWMS_VIEWER','outbound.read'),
        ('DWMS_OPERATOR','master.read'),
        ('DWMS_OPERATOR','inbound.read'),
        ('DWMS_OPERATOR','inbound.execute'),
        ('DWMS_OPERATOR','inventory.read'),
        ('DWMS_OPERATOR','outbound.read'),
        ('DWMS_OPERATOR','outbound.execute'),
        ('DWMS_SUPERVISOR','master.read'),
        ('DWMS_SUPERVISOR','inbound.read'),
        ('DWMS_SUPERVISOR','inbound.execute'),
        ('DWMS_SUPERVISOR','inventory.read'),
        ('DWMS_SUPERVISOR','outbound.read'),
        ('DWMS_SUPERVISOR','outbound.execute'),
        ('DWMS_SUPERVISOR','workflow.approve'),
        ('DWMS_INVENTORY_CONTROLLER','master.read'),
        ('DWMS_INVENTORY_CONTROLLER','inbound.read'),
        ('DWMS_INVENTORY_CONTROLLER','inventory.read'),
        ('DWMS_INVENTORY_CONTROLLER','inventory.post'),
        ('DWMS_INVENTORY_CONTROLLER','inventory.adjust'),
        ('DWMS_INVENTORY_CONTROLLER','inventory.hold'),
        ('DWMS_INVENTORY_CONTROLLER','workflow.approve'),
        ('DWMS_CUSTOMS_SPECIALIST','master.read'),
        ('DWMS_CUSTOMS_SPECIALIST','inventory.read'),
        ('DWMS_CUSTOMS_SPECIALIST','customs.read'),
        ('DWMS_CUSTOMS_SPECIALIST','customs.submit'),
        ('DWMS_CUSTOMS_SPECIALIST','bonded.read'),
        ('DWMS_CUSTOMS_SPECIALIST','bonded.report'),
        ('DWMS_CUSTOMS_SPECIALIST','workflow.approve'),
        ('DWMS_BILLING_SPECIALIST','master.read'),
        ('DWMS_BILLING_SPECIALIST','billing.read'),
        ('DWMS_BILLING_SPECIALIST','billing.approve'),
        ('DWMS_BILLING_SPECIALIST','workflow.approve'),
        ('DWMS_AUDITOR','master.read'),
        ('DWMS_AUDITOR','inbound.read'),
        ('DWMS_AUDITOR','inventory.read'),
        ('DWMS_AUDITOR','outbound.read'),
        ('DWMS_AUDITOR','customs.read'),
        ('DWMS_AUDITOR','bonded.read'),
        ('DWMS_AUDITOR','billing.read'),
        ('DWMS_AUDITOR','audit.read'),
        ('DWMS_AUDITOR','data_access.read'),
        ('DWMS_AUDITOR','integration.monitor'),
        ('DWMS_TENANT_ADMIN','iam.user.read'),
        ('DWMS_TENANT_ADMIN','iam.role.manage'),
        ('DWMS_TENANT_ADMIN','config.manage'),
        ('DWMS_TENANT_ADMIN','master.read'),
        ('DWMS_TENANT_ADMIN','master.manage'),
        ('DWMS_TENANT_ADMIN','integration.monitor')
)
INSERT INTO dwms.role_permissions (
    tenant_id, role_id, permission_id, granted_at,
    row_version, created_at, updated_at
)
SELECT NULL, role_row.id, permission_row.id, now(), 1, now(), now()
FROM grants g
JOIN dwms.roles role_row
...
WHERE NOT EXISTS (
    SELECT 1 FROM dwms.role_permissions existing
...
INSERT INTO dwms.data_retention_policies (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM dwms.data_retention_policies existing
...
REVOKE ALL ON ALL TABLES IN SCHEMA dwms FROM PUBLIC;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA dwms FROM PUBLIC;
