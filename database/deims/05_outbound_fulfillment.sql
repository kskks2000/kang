-- DEIMS enterprise WMS schema - outbound orders, allocation, waves, picking, packing, shipping, and customer returns.

CREATE TABLE IF NOT EXISTS deims.carrier_services (
...
CREATE TABLE IF NOT EXISTS deims.outbound_orders (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    ship_to_address_id uuid NOT NULL REFERENCES deims.addresses(id),
    bill_to_partner_id uuid REFERENCES deims.business_partners(id),
    carrier_service_id uuid REFERENCES deims.carrier_services(id),
    allocation_strategy_id uuid REFERENCES deims.allocation_strategies(id),
    ...
    currency_code char(3) NOT NULL DEFAULT 'KRW' REFERENCES deims.currencies(currency_code),
    ...
    CONSTRAINT ck_deims_out_order_type CHECK (order_type IN (
        'SALES','TRANSFER','CUSTOMER_RETURN_REPLACEMENT','SUPPLIER_RETURN','SAMPLE','DISPOSAL','EXPORT','OTHER'
    )),
    CONSTRAINT ck_deims_out_order_priority CHECK (priority >= 0),
    CONSTRAINT ck_deims_out_order_ship_window CHECK (ship_not_after IS NULL OR ship_not_before IS NULL OR ship_not_after >= ship_not_before),
    CONSTRAINT ck_deims_out_order_delivery_window CHECK (requested_delivery_to IS NULL OR requested_delivery_from IS NULL OR requested_delivery_to >= requested_delivery_from),
    CONSTRAINT ck_deims_out_order_value CHECK (declared_value IS NULL OR declared_value >= 0),
    CONSTRAINT ck_deims_out_order_freight CHECK (freight_terms IN ('PREPAID','COLLECT','THIRD_PARTY','OTHER')),
    CONSTRAINT ck_deims_out_order_status CHECK (status IN (
    ...
    CONSTRAINT ck_deims_out_allocation_status CHECK (allocation_status IN ('NOT_ALLOCATED','PARTIAL','ALLOCATED','FAILED','RELEASED')),
    CONSTRAINT ck_deims_out_wave_status CHECK (wave_status IN ('NOT_WAVED','PARTIAL','WAVED','RELEASED')),
    CONSTRAINT ck_deims_out_shipping_status CHECK (shipping_status IN ('NOT_SHIPPED','PARTIAL','SHIPPED','REVERSED')),
    CONSTRAINT ck_deims_out_hold_status CHECK (hold_status IN ('CLEAR','ON_HOLD','PARTIAL_HOLD'))
);

CREATE TABLE IF NOT EXISTS deims.outbound_order_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id) ON DELETE CASCADE,
    ...
    requested_country_of_origin char(2) REFERENCES deims.countries(country_code),
    ...
    CONSTRAINT ck_deims_out_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_out_line_qty CHECK (
    ...
    CONSTRAINT ck_deims_out_line_shelf CHECK (minimum_shelf_life_days IS NULL OR minimum_shelf_life_days >= 0),
    CONSTRAINT ck_deims_out_line_value CHECK ((unit_price IS NULL OR unit_price >= 0) AND (line_value IS NULL OR line_value >= 0)),
    CONSTRAINT ck_deims_out_line_status CHECK (status IN (
        'OPEN','ALLOCATED','PARTIALLY_ALLOCATED','PICKING','PICKED','PACKED','PARTIALLY_SHIPPED','SHIPPED','BACKORDERED','CANCELLED','CLOSED'
    ))
);

CREATE TABLE IF NOT EXISTS deims.outbound_order_parties (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_deims_out_party_role CHECK (party_role IN (
        'OWNER','CUSTOMER','SHIPPER','CONSIGNEE','BILL_TO','CARRIER','FORWARDER','CUSTOMS_BROKER','NOTIFY'
    ))
);

CREATE TABLE IF NOT EXISTS deims.outbound_order_references (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id) ON DELETE CASCADE,
    reference_type varchar(50) NOT NULL,
    reference_value varchar(300) NOT NULL,
    line_id uuid REFERENCES deims.outbound_order_lines(id) ON DELETE CASCADE,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (outbound_order_id, reference_type, reference_value)
);

CREATE TABLE IF NOT EXISTS deims.outbound_order_holds (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id) ON DELETE CASCADE,
    outbound_order_line_id uuid REFERENCES deims.outbound_order_lines(id) ON DELETE CASCADE,
    ...
    release_reason text,
    CONSTRAINT ck_deims_out_hold_dates CHECK (
        (expires_at IS NULL OR expires_at > placed_at) AND (released_at IS NULL OR released_at >= placed_at)
    )
);

CREATE TABLE IF NOT EXISTS deims.outbound_order_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_deims_out_status_sequence CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.allocation_runs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    allocation_run_no varchar(120) NOT NULL,
    strategy_id uuid REFERENCES deims.allocation_strategies(id),
    ...
    CONSTRAINT ck_deims_alloc_run_type CHECK (run_type IN ('AUTO','MANUAL','RETRY','REBALANCE')),
    CONSTRAINT ck_deims_alloc_run_counts CHECK (
    ...
    CONSTRAINT ck_deims_alloc_run_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS deims.allocation_run_orders (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    allocation_run_id uuid NOT NULL REFERENCES deims.allocation_runs(id) ON DELETE CASCADE,
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    allocation_run_id uuid REFERENCES deims.allocation_runs(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id),
    outbound_order_line_id uuid NOT NULL REFERENCES deims.outbound_order_lines(id),
    stock_bucket_id uuid NOT NULL REFERENCES deims.stock_buckets(id),
    reservation_id uuid REFERENCES deims.inventory_reservations(id),
    ...
    CONSTRAINT ck_deims_inventory_alloc_qty CHECK (allocated_quantity > 0),
    CONSTRAINT ck_deims_inventory_alloc_seq CHECK (allocation_sequence > 0),
    CONSTRAINT ck_deims_inventory_alloc_status CHECK (status IN ('ALLOCATED','PICKING','PICKED','RELEASED','CANCELLED','SHORT')),
    CONSTRAINT ck_deims_inventory_alloc_release CHECK (released_at IS NULL OR released_at >= allocated_at)
);

CREATE TABLE IF NOT EXISTS deims.waves (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    ...
    CONSTRAINT ck_deims_wave_type CHECK (wave_type IN ('STANDARD','BATCH','ZONE','CLUSTER','ORDER','CARRIER','EXPEDITE','REPLENISHMENT')),
    CONSTRAINT ck_deims_wave_priority CHECK (priority >= 0),
    CONSTRAINT ck_deims_wave_status CHECK (status IN ('DRAFT','PLANNED','RELEASED','IN_PROGRESS','PAUSED','COMPLETED','CANCELLED')),
    CONSTRAINT ck_deims_wave_dates CHECK (planned_complete_at IS NULL OR planned_start_at IS NULL OR planned_complete_at >= planned_start_at)
);

CREATE TABLE IF NOT EXISTS deims.wave_orders (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    wave_id uuid NOT NULL REFERENCES deims.waves(id) ON DELETE CASCADE,
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id),
    sequence_no integer NOT NULL DEFAULT 1,
    added_at timestamptz NOT NULL DEFAULT now(),
    removed_at timestamptz,
    PRIMARY KEY (wave_id, outbound_order_id),
    CONSTRAINT ck_deims_wave_order_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.wave_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    wave_id uuid NOT NULL REFERENCES deims.waves(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
    ...
    CONSTRAINT ck_deims_wave_status_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.pick_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    task_no varchar(120) NOT NULL,
    wave_id uuid REFERENCES deims.waves(id),
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id),
    pick_method varchar(30) NOT NULL DEFAULT 'DISCRETE',
    source_zone_id uuid REFERENCES deims.warehouse_zones(id),
    ...
    CONSTRAINT ck_deims_pick_method CHECK (pick_method IN ('DISCRETE','BATCH','ZONE','CLUSTER','WAVE','CARTON','PALLET','VOICE','ROBOT')),
    CONSTRAINT ck_deims_pick_task_priority CHECK (priority >= 0),
    CONSTRAINT ck_deims_pick_task_status CHECK (status IN ('OPEN','ASSIGNED','IN_PROGRESS','PAUSED','COMPLETED','SHORT','CANCELLED')),
    CONSTRAINT ck_deims_pick_task_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS deims.pick_task_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    pick_task_id uuid NOT NULL REFERENCES deims.pick_tasks(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    allocation_id uuid NOT NULL REFERENCES deims.inventory_allocations(id),
    outbound_order_line_id uuid NOT NULL REFERENCES deims.outbound_order_lines(id),
    stock_bucket_id uuid NOT NULL REFERENCES deims.stock_buckets(id),
    source_location_id uuid NOT NULL REFERENCES deims.warehouse_locations(id),
    destination_location_id uuid REFERENCES deims.warehouse_locations(id),
    ...
    CONSTRAINT ck_deims_pick_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_pick_line_qty CHECK (
        requested_quantity > 0 AND picked_quantity >= 0 AND short_quantity >= 0 AND
        picked_quantity + short_quantity <= requested_quantity
    ),
    CONSTRAINT ck_deims_pick_line_status CHECK (status IN ('OPEN','IN_PROGRESS','PICKED','SHORT','SKIPPED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS deims.pick_task_assignments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    pick_task_id uuid NOT NULL REFERENCES deims.pick_tasks(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_deims_pick_assignment_target CHECK (user_id IS NOT NULL OR equipment_id IS NOT NULL),
    CONSTRAINT ck_deims_pick_assignment_dates CHECK (
        (accepted_at IS NULL OR accepted_at >= assigned_at) AND (released_at IS NULL OR released_at >= assigned_at)
    )
);

CREATE TABLE IF NOT EXISTS deims.pick_confirmations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    pick_task_line_id uuid NOT NULL REFERENCES deims.pick_task_lines(id),
    confirmation_no integer NOT NULL,
    source_stock_bucket_id uuid NOT NULL REFERENCES deims.stock_buckets(id),
    destination_stock_bucket_id uuid REFERENCES deims.stock_buckets(id),
    ...
    reversed_confirmation_id uuid REFERENCES deims.pick_confirmations(id),
    UNIQUE (pick_task_line_id, confirmation_no),
    UNIQUE (tenant_id, idempotency_key),
    CONSTRAINT ck_deims_pick_confirmation_no CHECK (confirmation_no > 0),
    CONSTRAINT ck_deims_pick_confirmation_qty CHECK (picked_quantity > 0),
    CONSTRAINT ck_deims_pick_confirmation_reversal CHECK (reversed_confirmation_id IS NULL OR reversed_confirmation_id <> id)
);

CREATE TABLE IF NOT EXISTS deims.pick_exceptions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    pick_task_line_id uuid NOT NULL REFERENCES deims.pick_task_lines(id),
    ...
    CONSTRAINT ck_deims_pick_exception_qty CHECK (exception_quantity IS NULL OR exception_quantity > 0),
    CONSTRAINT ck_deims_pick_exception_status CHECK (resolution_status IN ('OPEN','INVESTIGATING','RESOLVED','WAIVED')),
    CONSTRAINT ck_deims_pick_exception_dates CHECK (resolved_at IS NULL OR resolved_at >= reported_at)
);

CREATE TABLE IF NOT EXISTS deims.packing_sessions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    session_no varchar(120) NOT NULL,
    workstation_id uuid REFERENCES deims.workstations(id),
    outbound_order_id uuid REFERENCES deims.outbound_orders(id),
    wave_id uuid REFERENCES deims.waves(id),
    operator_user_id uuid REFERENCES deims.users(id),
    status varchar(20) NOT NULL DEFAULT 'OPEN',
    ...
    CONSTRAINT ck_deims_packing_session_scope CHECK (outbound_order_id IS NOT NULL OR wave_id IS NOT NULL),
    CONSTRAINT ck_deims_packing_session_status CHECK (status IN ('OPEN','IN_PROGRESS','PAUSED','COMPLETED','CANCELLED')),
    CONSTRAINT ck_deims_packing_session_dates CHECK (completed_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS deims.packages (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    packing_session_id uuid REFERENCES deims.packing_sessions(id),
    outbound_order_id uuid REFERENCES deims.outbound_orders(id),
    lpn_id uuid REFERENCES deims.lpns(id),
    parent_package_id uuid REFERENCES deims.packages(id),
    package_no varchar(150) NOT NULL,
    package_type varchar(30) NOT NULL,
    packaging_material_id uuid REFERENCES deims.packaging_materials(id),
    sscc varchar(30),
    ...
    dimension_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    declared_value numeric(24,4),
    ...
    CONSTRAINT ck_deims_package_parent CHECK (parent_package_id IS NULL OR parent_package_id <> id),
    CONSTRAINT ck_deims_package_type CHECK (package_type IN ('CARTON','PALLET','TOTE','BAG','CRATE','DRUM','ENVELOPE','OTHER')),
    CONSTRAINT ck_deims_package_weight CHECK (
    ...
    CONSTRAINT ck_deims_package_dimensions CHECK (
    ...
    CONSTRAINT ck_deims_package_value CHECK (declared_value IS NULL OR declared_value >= 0),
    CONSTRAINT ck_deims_package_seal CHECK (NOT sealed OR sealed_at IS NOT NULL),
    CONSTRAINT ck_deims_package_status CHECK (status IN ('OPEN','PACKED','SEALED','STAGED','LOADED','SHIPPED','VOID'))
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_package_sscc
    ON deims.packages (tenant_id, sscc) WHERE sscc IS NOT NULL;

CREATE TABLE IF NOT EXISTS deims.package_items (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    package_id uuid NOT NULL REFERENCES deims.packages(id) ON DELETE CASCADE,
    outbound_order_line_id uuid NOT NULL REFERENCES deims.outbound_order_lines(id),
    pick_confirmation_id uuid REFERENCES deims.pick_confirmations(id),
    stock_bucket_id uuid REFERENCES deims.stock_buckets(id),
    item_id uuid NOT NULL REFERENCES deims.items(id),
    packed_quantity numeric(24,8) NOT NULL,
    uom_code varchar(20) NOT NULL REFERENCES deims.units_of_measure(uom_code),
    serial_number_id uuid REFERENCES deims.serial_numbers(id),
    created_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_deims_package_item_qty CHECK (packed_quantity > 0)
);

CREATE TABLE IF NOT EXISTS deims.shipping_labels (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    package_id uuid NOT NULL REFERENCES deims.packages(id) ON DELETE CASCADE,
    carrier_service_id uuid REFERENCES deims.carrier_services(id),
    ...
    label_format varchar(30),
    label_file_id uuid REFERENCES deims.files(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_shipping_tracking
    ON deims.shipping_labels (tenant_id, carrier_service_id, tracking_no)
    WHERE tracking_no IS NOT NULL AND voided_at IS NULL;

CREATE TABLE IF NOT EXISTS deims.shipments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    ...
    ship_to_address_id uuid REFERENCES deims.addresses(id),
    ...
    CONSTRAINT ck_deims_shipment_type CHECK (shipment_type IN ('OUTBOUND','TRANSFER','RETURN','EXPORT','PARCEL','LTL','FTL','OTHER')),
    CONSTRAINT ck_deims_shipment_status CHECK (status IN (
    ...
    CONSTRAINT ck_deims_shipment_totals CHECK (
        total_packages >= 0 AND (total_weight IS NULL OR total_weight >= 0) AND (total_volume IS NULL OR total_volume >= 0)
    ),
    CONSTRAINT ck_deims_shipment_customs_release CHECK (customs_released_at IS NULL OR customs_release_required),
    CONSTRAINT ck_deims_shipment_dates CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_shipments_tenant_id
    ON deims.shipments (tenant_id, id);

CREATE TABLE IF NOT EXISTS deims.shipment_orders (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id) ON DELETE CASCADE,
    outbound_order_id uuid NOT NULL REFERENCES deims.outbound_orders(id),
    sequence_no integer NOT NULL DEFAULT 1,
    PRIMARY KEY (shipment_id, outbound_order_id),
    CONSTRAINT ck_deims_shipment_order_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.shipment_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    ...
    CONSTRAINT ck_deims_shipment_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_shipment_line_qty CHECK (shipped_quantity > 0 AND base_quantity > 0),
    CONSTRAINT ck_deims_shipment_line_hs CHECK (
        num_nonnulls(hs_country_code, hs_nomenclature_version, hs_code) IN (0, 3)
    ),
    CONSTRAINT ck_deims_shipment_line_value CHECK (declared_value IS NULL OR declared_value >= 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_shipment_lines_tenant_id
    ON deims.shipment_lines (tenant_id, id);

CREATE TABLE IF NOT EXISTS deims.shipment_packages (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id) ON DELETE CASCADE,
    package_id uuid NOT NULL REFERENCES deims.packages(id),
    loaded_at timestamptz,
    unloaded_at timestamptz,
    PRIMARY KEY (shipment_id, package_id),
    CONSTRAINT ck_deims_shipment_package_dates CHECK (unloaded_at IS NULL OR loaded_at IS NULL OR unloaded_at >= loaded_at)
);

CREATE TABLE IF NOT EXISTS deims.shipment_stops (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id) ON DELETE CASCADE,
    ...
    partner_id uuid REFERENCES deims.business_partners(id),
    address_id uuid NOT NULL REFERENCES deims.addresses(id),
    ...
    CONSTRAINT ck_deims_shipment_stop_seq CHECK (stop_sequence > 0),
    CONSTRAINT ck_deims_shipment_stop_type CHECK (stop_type IN ('PICKUP','DELIVERY','CROSS_DOCK','CUSTOMS','OTHER')),
    CONSTRAINT ck_deims_shipment_stop_plan_dates CHECK (planned_departure_at IS NULL OR planned_arrival_at IS NULL OR planned_departure_at >= planned_arrival_at),
    CONSTRAINT ck_deims_shipment_stop_actual_dates CHECK (actual_departure_at IS NULL OR actual_arrival_at IS NULL OR actual_departure_at >= actual_arrival_at)
);

CREATE TABLE IF NOT EXISTS deims.shipment_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
    ...
    CONSTRAINT ck_deims_shipment_status_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.loads (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    ...
    CONSTRAINT ck_deims_load_status CHECK (status IN ('PLANNED','ASSIGNED','AT_DOCK','LOADING','LOADED','SEALED','DISPATCHED','CANCELLED','CLOSED')),
    CONSTRAINT ck_deims_load_weight CHECK (total_weight IS NULL OR total_weight >= 0)
);

CREATE TABLE IF NOT EXISTS deims.load_shipments (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    load_id uuid NOT NULL REFERENCES deims.loads(id) ON DELETE CASCADE,
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    sequence_no integer NOT NULL DEFAULT 1,
    loaded_at timestamptz,
    PRIMARY KEY (load_id, shipment_id),
    UNIQUE (shipment_id),
    CONSTRAINT ck_deims_load_shipment_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.manifests (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    manifest_no varchar(120) NOT NULL,
    manifest_type varchar(30) NOT NULL,
    load_id uuid REFERENCES deims.loads(id),
    carrier_partner_id uuid REFERENCES deims.business_partners(id),
    ...
    CONSTRAINT ck_deims_manifest_type CHECK (manifest_type IN ('PARCEL','LTL','FTL','EXPORT','CUSTOMS','OTHER')),
    CONSTRAINT ck_deims_manifest_status CHECK (status IN ('DRAFT','FINAL','TRANSMITTED','ACCEPTED','REJECTED','VOID'))
);

CREATE TABLE IF NOT EXISTS deims.manifest_items (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    manifest_id uuid NOT NULL REFERENCES deims.manifests(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    shipment_id uuid REFERENCES deims.shipments(id),
    package_id uuid REFERENCES deims.packages(id),
    tracking_no varchar(200),
    status varchar(20) NOT NULL DEFAULT 'INCLUDED',
    UNIQUE (manifest_id, line_no),
    CONSTRAINT ck_deims_manifest_item_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_manifest_item_target CHECK (shipment_id IS NOT NULL OR package_id IS NOT NULL)
);

CREATE TABLE IF NOT EXISTS deims.shipping_confirmations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    confirmation_no integer NOT NULL,
    ...
    confirmed_at timestamptz NOT NULL DEFAULT now(),
    idempotency_key varchar(300) NOT NULL,
    reversal_confirmation_id uuid REFERENCES deims.shipping_confirmations(id),
    note text,
    UNIQUE (shipment_id, confirmation_no),
    UNIQUE (tenant_id, idempotency_key),
    FOREIGN KEY (tenant_id, shipment_id)
        REFERENCES deims.shipments(tenant_id, id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_shipping_confirmation_no CHECK (confirmation_no > 0),
    CONSTRAINT ck_deims_shipping_confirmation_reversal CHECK (reversal_confirmation_id IS NULL OR reversal_confirmation_id <> id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_shipping_confirmations_tenant_id
    ON deims.shipping_confirmations (tenant_id, id);
CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_shipping_confirmation_transaction
    ON deims.shipping_confirmations (inventory_transaction_id);

CREATE TABLE IF NOT EXISTS deims.shipping_inventory_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, shipping_confirmation_id)
        REFERENCES deims.shipping_confirmations(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, shipment_line_id)
        REFERENCES deims.shipment_lines(tenant_id, id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_shipping_inventory_alloc_qty CHECK (allocated_quantity > 0)
);

CREATE INDEX IF NOT EXISTS ix_deims_shipping_inventory_alloc_entry
    ON deims.shipping_inventory_allocations (
        inventory_transaction_entry_id, shipping_confirmation_id, shipment_line_id
    );
CREATE INDEX IF NOT EXISTS ix_deims_shipping_inventory_alloc_line
    ON deims.shipping_inventory_allocations (shipment_line_id, shipping_confirmation_id);

CREATE TABLE IF NOT EXISTS deims.delivery_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    stop_id uuid REFERENCES deims.shipment_stops(id),
    ...
    CONSTRAINT ck_deims_delivery_lat CHECK (latitude IS NULL OR latitude BETWEEN -90 AND 90),
    CONSTRAINT ck_deims_delivery_lon CHECK (longitude IS NULL OR longitude BETWEEN -180 AND 180)
);

CREATE TABLE IF NOT EXISTS deims.proof_of_deliveries (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    delivery_stop_id uuid REFERENCES deims.shipment_stops(id),
    delivered_at timestamptz NOT NULL,
    receiver_name_encrypted text,
    receiver_name_masked varchar(150),
    receiver_relationship varchar(80),
    signature_file_id uuid REFERENCES deims.files(id),
    photo_file_id uuid REFERENCES deims.files(id),
    ...
    CONSTRAINT ck_deims_pod_evidence CHECK (
        signature_file_id IS NOT NULL OR photo_file_id IS NOT NULL OR receiver_name_encrypted IS NOT NULL
    ),
    CONSTRAINT ck_deims_pod_status CHECK (status IN ('CAPTURED','VERIFIED','DISPUTED','VOID'))
);

CREATE TABLE IF NOT EXISTS deims.delivery_discrepancies (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    shipment_line_id uuid REFERENCES deims.shipment_lines(id),
    package_id uuid REFERENCES deims.packages(id),
    ...
    reported_at timestamptz NOT NULL DEFAULT now(),
    resolved_at timestamptz,
    CONSTRAINT ck_deims_delivery_discrepancy_qty CHECK (
        (expected_quantity IS NULL OR expected_quantity >= 0) AND (actual_quantity IS NULL OR actual_quantity >= 0)
    )
);

CREATE TABLE IF NOT EXISTS deims.customer_return_orders (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    ...
    customer_partner_id uuid REFERENCES deims.business_partners(id),
    original_outbound_order_id uuid REFERENCES deims.outbound_orders(id),
    original_shipment_id uuid REFERENCES deims.shipments(id),
    ...
    CONSTRAINT ck_deims_customer_return_type CHECK (return_type IN ('CUSTOMER_RETURN','RECALL','REFUSED_DELIVERY','WARRANTY','EXCHANGE')),
    CONSTRAINT ck_deims_customer_return_status CHECK (status IN (
        'REQUESTED','AUTHORIZED','IN_TRANSIT','RECEIVING','RECEIVED','INSPECTING','DISPOSITIONED','CLOSED','REJECTED','CANCELLED'
    ))
);

CREATE TABLE IF NOT EXISTS deims.customer_return_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    customer_return_order_id uuid NOT NULL REFERENCES deims.customer_return_orders(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    original_outbound_line_id uuid REFERENCES deims.outbound_order_lines(id),
    item_id uuid NOT NULL REFERENCES deims.items(id),
    ...
    CONSTRAINT ck_deims_customer_return_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_customer_return_line_qty CHECK (
...
COMMENT ON TABLE deims.inventory_allocations IS
...
CREATE OR REPLACE FUNCTION deims.assert_shipping_inventory_transaction(
...
BEGIN
    ...
    IF transaction_row.transaction_type = 'SHIPMENT' THEN
        SELECT count(*), min(confirmation.id::text)::uuid
          ...
          FROM deims.shipping_confirmations confirmation
        ...
        END IF;
        PERFORM deims.assert_shipping_confirmation_inventory(confirmation_id);
...
CREATE OR REPLACE FUNCTION deims.assert_shipment_inventory_coverage(
    p_shipment_id uuid
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
    ...
    FOR confirmation_row IN
        SELECT id FROM deims.shipping_confirmations
         WHERE shipment_id = shipment_row.id
         ORDER BY id
    LOOP
        PERFORM deims.assert_shipping_confirmation_inventory(confirmation_row.id);
    END LOOP;
    ...
    LOOP
        ...
        SELECT COALESCE(sum(allocation.allocated_quantity), 0)
          ...
          FROM deims.shipping_inventory_allocations allocation
          JOIN deims.shipping_confirmations confirmation
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.outbound_order_lines outbound_line
         WHERE EXISTS (
             SELECT 1 FROM deims.shipment_lines shipment_line
           ...
           AND (SELECT COALESCE(sum(allocation.allocated_quantity), 0)
                  ...
                  JOIN deims.shipping_confirmations confirmation
                    ON confirmation.id = allocation.shipping_confirmation_id
                  JOIN deims.shipment_lines allocated_shipment_line
...
CREATE OR REPLACE FUNCTION deims.guard_shipping_confirmation_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
CREATE OR REPLACE FUNCTION deims.guard_shipping_inventory_allocation_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
CREATE OR REPLACE FUNCTION deims.deferred_check_shipping_inventory_integrity()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    PERFORM deims.assert_shipping_confirmation_inventory(old_confirmation_id);
    IF new_confirmation_id IS DISTINCT FROM old_confirmation_id THEN
        PERFORM deims.assert_shipping_confirmation_inventory(new_confirmation_id);
    END IF;
    PERFORM deims.assert_shipping_inventory_transaction(old_transaction_id);
    IF new_transaction_id IS DISTINCT FROM old_transaction_id THEN
        PERFORM deims.assert_shipping_inventory_transaction(new_transaction_id);
    END IF;
        ...
        SELECT DISTINCT candidate.shipment_id
          FROM (
              ...
              SELECT shipment_line.shipment_id
                FROM deims.shipment_lines shipment_line
              ...
              SELECT shipment_order.shipment_id
                FROM deims.shipment_orders shipment_order
    ...
    LOOP
        PERFORM deims.assert_shipment_inventory_coverage(target_shipment_id);
    END LOOP;
    RETURN NULL;
END;
$function$;

DROP TRIGGER IF EXISTS trg_validate_shipment_line_dimensions ON deims.shipment_lines;
CREATE TRIGGER trg_validate_shipment_line_dimensions
    BEFORE INSERT OR UPDATE OF tenant_id, item_id, base_uom_code
    ON deims.shipment_lines
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_shipment_line_dimensions();

CREATE OR REPLACE FUNCTION deims.protect_outbound_document_header()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
DROP TRIGGER IF EXISTS trg_protect_outbound_order_header ON deims.outbound_orders;
CREATE TRIGGER trg_protect_outbound_order_header
    BEFORE UPDATE OR DELETE ON deims.outbound_orders
    FOR EACH ROW EXECUTE PROCEDURE deims.protect_outbound_document_header();

DROP TRIGGER IF EXISTS trg_protect_shipment_header ON deims.shipments;
CREATE TRIGGER trg_protect_shipment_header
    BEFORE UPDATE OR DELETE ON deims.shipments
    FOR EACH ROW EXECUTE PROCEDURE deims.protect_outbound_document_header();
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('DROP TRIGGER IF EXISTS %I ON deims.%I',
                       'trg_guard_finalized_' || trigger_suffix, target.table_name);
        EXECUTE format(
            'CREATE TRIGGER %I BEFORE INSERT OR UPDATE OR DELETE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.guard_finalized_document_child(%L,%L,%L,%L)',
            'trg_guard_finalized_' || trigger_suffix, target.table_name,
            target.parent_table, target.parent_column, 'status', target.frozen_statuses
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_prevent_core_truncate ON deims.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_prevent_core_truncate BEFORE TRUNCATE ON deims.%I '
...
CREATE OR REPLACE FUNCTION deims.validate_shipping_inventory_allocation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    confirmation_row deims.shipping_confirmations%ROWTYPE;
    transaction_row deims.inventory_transaction_headers%ROWTYPE;
    shipment_row deims.shipments%ROWTYPE;
    shipment_line_row deims.shipment_lines%ROWTYPE;
    outbound_line_row deims.outbound_order_lines%ROWTYPE;
    outbound_order_row deims.outbound_orders%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO confirmation_row
      FROM deims.shipping_confirmations
    ...
    IF shipment_line_row.shipment_id <> shipment_row.id
       ...
       OR NOT EXISTS (
           SELECT 1 FROM deims.shipment_orders shipment_order
    ...
    SELECT COALESCE(sum(allocation.allocated_quantity), 0)
      ...
      FROM deims.shipping_inventory_allocations allocation
      JOIN deims.shipping_confirmations allocated_confirmation
...
CREATE OR REPLACE FUNCTION deims.assert_shipping_confirmation_inventory(
...
BEGIN
    ...
    IF transaction_row.transaction_type = 'REVERSAL' THEN
        IF confirmation_row.reversal_confirmation_id IS NOT NULL OR EXISTS (
            SELECT 1 FROM deims.shipping_inventory_allocations allocation
        ...
        PERFORM 1
          ...
          JOIN deims.shipping_confirmations original_confirmation
    ...
    IF confirmation_row.reversal_confirmation_id IS NOT NULL THEN
        PERFORM 1
          FROM deims.shipping_confirmations reversal_confirmation
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.shipping_inventory_allocations allocation
          JOIN deims.shipment_lines shipment_line ON shipment_line.id = allocation.shipment_line_id
          JOIN deims.outbound_order_lines outbound_line
            ON outbound_line.id = shipment_line.outbound_order_line_id
          JOIN deims.outbound_orders outbound_order ON outbound_order.id = outbound_line.outbound_order_id
          JOIN deims.inventory_transaction_entries entry
            ON entry.id = allocation.inventory_transaction_entry_id
          JOIN deims.stock_buckets bucket ON bucket.id = allocation.source_stock_bucket_id
          JOIN deims.items item ON item.id = shipment_line.item_id
         WHERE allocation.shipping_confirmation_id = confirmation_row.id
           AND (
...
REVOKE ALL ON FUNCTION deims.validate_shipping_inventory_allocation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_shipping_confirmation_inventory(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_shipping_inventory_transaction(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_shipment_inventory_coverage(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_shipping_confirmation_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_shipping_inventory_allocation_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.deferred_check_shipping_inventory_integrity() FROM PUBLIC;
...
BEFORE INSERT OR UPDATE ON deims.shipping_inventory_allocations
FOR EACH ROW EXECUTE PROCEDURE deims.validate_shipping_inventory_allocation();

DROP TRIGGER IF EXISTS trg_guard_shipping_inventory_allocation
ON deims.shipping_inventory_allocations;
CREATE TRIGGER trg_guard_shipping_inventory_allocation
BEFORE UPDATE OR DELETE ON deims.shipping_inventory_allocations
FOR EACH ROW EXECUTE PROCEDURE deims.guard_shipping_inventory_allocation_mutation();

DROP TRIGGER IF EXISTS trg_guard_shipping_confirmation ON deims.shipping_confirmations;
CREATE TRIGGER trg_guard_shipping_confirmation
BEFORE UPDATE OR DELETE ON deims.shipping_confirmations
FOR EACH ROW EXECUTE PROCEDURE deims.guard_shipping_confirmation_mutation();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.shipping_confirmations
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.shipping_inventory_allocations
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();
...
DROP TRIGGER IF EXISTS trg_shipment_document_inventory_integrity ON deims.shipments;
CREATE CONSTRAINT TRIGGER trg_shipment_document_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.shipments
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();

DROP TRIGGER IF EXISTS trg_shipment_line_inventory_integrity ON deims.shipment_lines;
CREATE CONSTRAINT TRIGGER trg_shipment_line_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.shipment_lines
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();

DROP TRIGGER IF EXISTS trg_shipment_order_inventory_integrity ON deims.shipment_orders;
CREATE CONSTRAINT TRIGGER trg_shipment_order_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.shipment_orders
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();

DROP TRIGGER IF EXISTS trg_outbound_line_shipping_inventory_integrity
ON deims.outbound_order_lines;
CREATE CONSTRAINT TRIGGER trg_outbound_line_shipping_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.outbound_order_lines
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.outbound_orders
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_shipping_inventory_integrity();

DROP TRIGGER IF EXISTS trg_prevent_core_truncate ON deims.shipping_inventory_allocations;
CREATE TRIGGER trg_prevent_core_truncate
BEFORE TRUNCATE ON deims.shipping_inventory_allocations
FOR EACH STATEMENT EXECUTE PROCEDURE deims.prevent_core_document_truncate();

COMMENT ON TABLE deims.shipping_inventory_allocations IS
