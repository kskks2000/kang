-- DEIMS enterprise WMS schema - global trade, customs declarations, and Korea UNI-PASS integration.
...
AS $function$
BEGIN
    IF current_setting('deims.immutable_maintenance', true) = 'on' THEN
...
CREATE OR REPLACE FUNCTION deims.validate_tenant_owner_scope()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE TABLE IF NOT EXISTS deims.customs_code_sets (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    jurisdiction_country_code char(2) NOT NULL DEFAULT 'KR' REFERENCES deims.countries(country_code),
    ...
    CONSTRAINT ck_deims_customs_code_set_dates CHECK (valid_to IS NULL OR valid_to >= valid_from)
);

CREATE TABLE IF NOT EXISTS deims.customs_codes (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    code_set_id uuid NOT NULL REFERENCES deims.customs_code_sets(id) ON DELETE CASCADE,
    code_value varchar(150) NOT NULL,
    code_name_ko varchar(300),
    code_name_en varchar(300),
    parent_code_id uuid REFERENCES deims.customs_codes(id),
    ...
    CONSTRAINT ck_deims_customs_code_parent CHECK (parent_code_id IS NULL OR parent_code_id <> id),
    CONSTRAINT ck_deims_customs_code_dates CHECK (valid_to IS NULL OR valid_to >= valid_from)
);

CREATE INDEX IF NOT EXISTS ix_deims_customs_codes_lookup
    ON deims.customs_codes (code_set_id, code_value, valid_from DESC);

CREATE TABLE IF NOT EXISTS deims.vessels (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    ...
    mmsi_no varchar(50),
    flag_country_code char(2) REFERENCES deims.countries(country_code),
    ...
    CONSTRAINT ck_deims_vessel_tonnage CHECK (gross_tonnage IS NULL OR gross_tonnage >= 0),
    CONSTRAINT ck_deims_vessel_status CHECK (status IN ('ACTIVE','INACTIVE','RETIRED','BLOCKED'))
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_vessel_imo
    ON deims.vessels (tenant_id, imo_no)
    WHERE imo_no IS NOT NULL AND deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS deims.vessel_voyages (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    vessel_id uuid NOT NULL REFERENCES deims.vessels(id),
    ...
    CONSTRAINT ck_deims_voyage_status CHECK (status IN ('PLANNED','SAILED','ARRIVED','COMPLETED','CANCELLED')),
    CONSTRAINT ck_deims_voyage_schedule CHECK (
        scheduled_arrival_at IS NULL OR scheduled_departure_at IS NULL OR scheduled_arrival_at >= scheduled_departure_at
    ),
    CONSTRAINT ck_deims_voyage_actual CHECK (
        actual_arrival_at IS NULL OR actual_departure_at IS NULL OR actual_arrival_at >= actual_departure_at
    )
);

CREATE TABLE IF NOT EXISTS deims.flights (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    CONSTRAINT ck_deims_flight_status CHECK (status IN ('PLANNED','DEPARTED','ARRIVED','COMPLETED','CANCELLED','DIVERTED')),
    CONSTRAINT ck_deims_flight_schedule CHECK (
        scheduled_arrival_at IS NULL OR scheduled_arrival_at >= scheduled_departure_at
    ),
    CONSTRAINT ck_deims_flight_actual CHECK (
        actual_arrival_at IS NULL OR actual_departure_at IS NULL OR actual_arrival_at >= actual_departure_at
    )
);

CREATE TABLE IF NOT EXISTS deims.trade_shipments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    CONSTRAINT ck_deims_trade_shipment_direction CHECK (direction IN ('IMPORT','EXPORT','TRANSIT','CROSS_TRADE')),
    CONSTRAINT ck_deims_trade_shipment_scope CHECK (shipment_scope IN ('INTERNATIONAL','BONDED_TRANSFER','COASTAL','OTHER')),
    CONSTRAINT ck_deims_trade_shipment_status CHECK (status IN (
        'PLANNED','BOOKED','IN_TRANSIT','ARRIVED','CUSTOMS_HOLD','CUSTOMS_RELEASED','DELIVERED','CANCELLED','CLOSED'
    )),
    CONSTRAINT ck_deims_trade_shipment_quantities CHECK (
    ...
    CONSTRAINT ck_deims_trade_shipment_dates CHECK (
        (planned_arrival_at IS NULL OR planned_departure_at IS NULL OR planned_arrival_at >= planned_departure_at) AND
        (actual_arrival_at IS NULL OR actual_departure_at IS NULL OR actual_arrival_at >= actual_departure_at)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_trade_cargo_management_no
    ON deims.trade_shipments (tenant_id, cargo_management_no_normalized)
    WHERE cargo_management_no_normalized IS NOT NULL;

CREATE TABLE IF NOT EXISTS deims.trade_shipment_parties (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_trade_party_snapshot_no CHECK (current_snapshot_no > 0),
    CONSTRAINT ck_deims_trade_party_role CHECK (party_role IN (
        'OWNER','SELLER','BUYER','SHIPPER','CONSIGNEE','NOTIFY','CARRIER','FORWARDER',
        'CUSTOMS_BROKER','DECLARANT','MANUFACTURER','INSURER','BANK','OTHER'
    ))
);

CREATE TABLE IF NOT EXISTS deims.trade_party_snapshots (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    trade_shipment_party_id uuid NOT NULL REFERENCES deims.trade_shipment_parties(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_trade_party_snapshot_no_positive CHECK (snapshot_no > 0),
    CONSTRAINT ck_deims_trade_party_person_hash CHECK (
        personal_identifier_hash IS NULL OR personal_identifier_hash ~ '^[0-9A-Fa-f]{64}$'
    )
);

CREATE TABLE IF NOT EXISTS deims.trade_shipment_references (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_shipment_id uuid NOT NULL,
    ...
    issuing_party_id uuid REFERENCES deims.business_partners(id),
    created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS deims.trade_shipment_legs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_shipment_id uuid NOT NULL,
    ...
    carrier_partner_id uuid REFERENCES deims.business_partners(id),
    vessel_voyage_id uuid REFERENCES deims.vessel_voyages(id),
    flight_id uuid REFERENCES deims.flights(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_trade_leg_no CHECK (leg_no > 0),
    CONSTRAINT ck_deims_trade_leg_asset CHECK (num_nonnulls(vessel_voyage_id, flight_id, vehicle_no) <= 1),
    CONSTRAINT ck_deims_trade_leg_status CHECK (status IN ('PLANNED','BOOKED','DEPARTED','ARRIVED','COMPLETED','CANCELLED','EXCEPTION')),
    CONSTRAINT ck_deims_trade_leg_plan_dates CHECK (
        planned_arrival_at IS NULL OR planned_departure_at IS NULL OR planned_arrival_at >= planned_departure_at
    ),
    CONSTRAINT ck_deims_trade_leg_actual_dates CHECK (
        actual_arrival_at IS NULL OR actual_departure_at IS NULL OR actual_arrival_at >= actual_departure_at
    )
);

CREATE TABLE IF NOT EXISTS deims.trade_shipment_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_shipment_id uuid NOT NULL,
    trade_shipment_leg_id uuid REFERENCES deims.trade_shipment_legs(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_trade_event_sequence CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.trade_containers (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    CONSTRAINT ck_deims_trade_container_weight CHECK (
    ...
    CONSTRAINT ck_deims_trade_container_status CHECK (status IN ('AVAILABLE','BOOKED','STUFFED','SEALED','IN_TRANSIT','EMPTY','DAMAGED','RETIRED'))
);

CREATE TABLE IF NOT EXISTS deims.trade_container_shipments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_container_id uuid NOT NULL REFERENCES deims.trade_containers(id),
    trade_shipment_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_container_shipment_load CHECK (load_type IN ('FCL','LCL','BULK','AIR_ULD','OTHER')),
    CONSTRAINT ck_deims_container_shipment_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (gross_weight IS NULL OR gross_weight >= 0)
    ),
    CONSTRAINT ck_deims_container_shipment_dates CHECK (unloaded_at IS NULL OR loaded_at IS NULL OR unloaded_at >= loaded_at)
);

CREATE TABLE IF NOT EXISTS deims.trade_container_seals (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    trade_container_id uuid NOT NULL REFERENCES deims.trade_containers(id),
    seal_no varchar(150) NOT NULL,
    seal_type varchar(40) NOT NULL DEFAULT 'CARRIER',
    issuer_partner_id uuid REFERENCES deims.business_partners(id),
    ...
    CONSTRAINT ck_deims_trade_seal_status CHECK (status IN ('APPLIED','VERIFIED','BROKEN','REMOVED','REPLACED','VOID')),
    CONSTRAINT ck_deims_trade_seal_dates CHECK (removed_at IS NULL OR removed_at >= applied_at)
);

CREATE TABLE IF NOT EXISTS deims.trade_container_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    trade_container_id uuid NOT NULL REFERENCES deims.trade_containers(id),
    ...
    sequence_no bigint NOT NULL,
    event_type varchar(80) NOT NULL,
    seal_id uuid REFERENCES deims.trade_container_seals(id),
    ...
    UNIQUE (source_system_id, external_event_id),
    CONSTRAINT ck_deims_trade_container_event_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.trade_documents (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_shipment_id uuid REFERENCES deims.trade_shipments(id),
    ...
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_trade_document_revision CHECK (current_revision_no > 0),
    CONSTRAINT ck_deims_trade_document_dates CHECK (expiry_on IS NULL OR issued_on IS NULL OR expiry_on >= issued_on),
    CONSTRAINT ck_deims_trade_document_status CHECK (status IN ('DRAFT','ISSUED','PRESENTED','ACCEPTED','REJECTED','SUPERSEDED','VOID'))
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_trade_document_number
    ON deims.trade_documents (tenant_id, owner_partner_id, document_kind, document_no_normalized)
    WHERE document_no_normalized IS NOT NULL AND status <> 'VOID';

CREATE TABLE IF NOT EXISTS deims.trade_document_versions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    supersedes_version_id uuid REFERENCES deims.trade_document_versions(id),
    created_by uuid REFERENCES deims.users(id),
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (trade_document_id, revision_no),
    FOREIGN KEY (tenant_id, owner_partner_id, trade_document_id)
        REFERENCES deims.trade_documents(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_trade_doc_version_no CHECK (revision_no > 0),
    CONSTRAINT ck_deims_trade_doc_version_source CHECK (file_id IS NOT NULL OR object_uri IS NOT NULL),
    CONSTRAINT ck_deims_trade_doc_version_hash CHECK (object_sha256 IS NULL OR object_sha256 ~ '^[0-9A-Fa-f]{64}$'),
    CONSTRAINT ck_deims_trade_doc_version_size CHECK (byte_size IS NULL OR byte_size >= 0),
    CONSTRAINT ck_deims_trade_doc_version_supersede CHECK (supersedes_version_id IS NULL OR supersedes_version_id <> id)
);

CREATE TABLE IF NOT EXISTS deims.trade_document_parties (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    CONSTRAINT ck_deims_trade_doc_party_hash CHECK (
        personal_identifier_hash IS NULL OR personal_identifier_hash ~ '^[0-9A-Fa-f]{64}$'
    )
);

CREATE TABLE IF NOT EXISTS deims.trade_document_links (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    source_document_id uuid NOT NULL REFERENCES deims.trade_documents(id) ON DELETE RESTRICT,
    target_document_id uuid NOT NULL REFERENCES deims.trade_documents(id) ON DELETE RESTRICT,
    relation_type varchar(50) NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (source_document_id, target_document_id, relation_type),
    CONSTRAINT ck_deims_trade_doc_link_self CHECK (source_document_id <> target_document_id)
);

CREATE TABLE IF NOT EXISTS deims.commercial_invoices (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_document_id uuid NOT NULL,
    ...
    ship_to_partner_id uuid REFERENCES deims.business_partners(id),
    ...
    country_of_export char(2) REFERENCES deims.countries(country_code),
    destination_country_code char(2) REFERENCES deims.countries(country_code),
    ...
    UNIQUE (tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, trade_document_id)
        REFERENCES deims.trade_documents(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_commercial_invoice_amount CHECK (
    ...
    CONSTRAINT ck_deims_commercial_invoice_total CHECK (
        total_amount = goods_amount + freight_amount + insurance_amount + other_charge_amount - discount_amount
    ),
    CONSTRAINT ck_deims_commercial_invoice_status CHECK (status IN ('DRAFT','ISSUED','REVISED','CANCELLED','ACCEPTED'))
);

CREATE TABLE IF NOT EXISTS deims.commercial_invoice_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, commercial_invoice_id)
        REFERENCES deims.commercial_invoices(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_deims_commercial_invoice_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_commercial_invoice_line_qty CHECK (quantity > 0),
    CONSTRAINT ck_deims_commercial_invoice_line_amount CHECK (unit_price >= 0 AND line_amount >= 0),
    CONSTRAINT ck_deims_commercial_invoice_line_weight CHECK (
...
CREATE TABLE IF NOT EXISTS deims.packing_lists (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_document_id uuid NOT NULL,
    ...
    commercial_invoice_id uuid REFERENCES deims.commercial_invoices(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_document_id)
        REFERENCES deims.trade_documents(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_packing_list_qty CHECK (
        package_count >= 0 AND (gross_weight IS NULL OR gross_weight >= 0) AND
        (net_weight IS NULL OR net_weight >= 0) AND
        (gross_weight IS NULL OR net_weight IS NULL OR gross_weight >= net_weight) AND
        (volume_value IS NULL OR volume_value >= 0)
    ),
    CONSTRAINT ck_deims_packing_list_status CHECK (status IN ('DRAFT','ISSUED','REVISED','CANCELLED','ACCEPTED'))
);

CREATE TABLE IF NOT EXISTS deims.packing_list_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    packing_list_id uuid NOT NULL,
    line_no integer NOT NULL,
    commercial_invoice_line_id uuid REFERENCES deims.commercial_invoice_lines(id),
    trade_container_id uuid REFERENCES deims.trade_containers(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, packing_list_id)
        REFERENCES deims.packing_lists(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_packing_list_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_packing_list_line_qty CHECK (
...
CREATE TABLE IF NOT EXISTS deims.bills_of_lading (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_document_id uuid NOT NULL,
    ...
    parent_master_bill_id uuid REFERENCES deims.bills_of_lading(id),
    ...
    carrier_partner_id uuid REFERENCES deims.business_partners(id),
    forwarder_partner_id uuid REFERENCES deims.business_partners(id),
    vessel_voyage_id uuid REFERENCES deims.vessel_voyages(id),
    port_of_loading_code varchar(10) REFERENCES deims.ports(port_code),
    port_of_discharge_code varchar(10) REFERENCES deims.ports(port_code),
    place_of_receipt varchar(300),
    ...
    measurement_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_document_id)
        REFERENCES deims.trade_documents(tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_bl_type CHECK (bill_type IN ('MASTER','HOUSE','DIRECT')),
    CONSTRAINT ck_deims_bl_parent CHECK (
        parent_master_bill_id IS NULL OR (parent_master_bill_id <> id AND bill_type = 'HOUSE')
    ),
    CONSTRAINT ck_deims_bl_qty CHECK (
    ...
    CONSTRAINT ck_deims_bl_status CHECK (status IN ('DRAFT','ISSUED','SURRENDERED','RELEASED','AMENDED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS deims.bill_of_lading_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    line_no integer NOT NULL,
    commercial_invoice_line_id uuid REFERENCES deims.commercial_invoice_lines(id),
    packing_list_line_id uuid REFERENCES deims.packing_list_lines(id),
    item_id uuid REFERENCES deims.items(id),
    trade_container_id uuid REFERENCES deims.trade_containers(id),
    goods_description varchar(1000) NOT NULL,
    package_count numeric(20,4),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bill_of_lading_id)
        REFERENCES deims.bills_of_lading(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_bl_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_bl_line_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (quantity IS NULL OR quantity > 0) AND
        (gross_weight IS NULL OR gross_weight >= 0) AND (measurement_value IS NULL OR measurement_value >= 0)
    )
);

CREATE TABLE IF NOT EXISTS deims.air_waybills (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    trade_document_id uuid NOT NULL,
    ...
    parent_master_airwaybill_id uuid REFERENCES deims.air_waybills(id),
    ...
    forwarder_partner_id uuid REFERENCES deims.business_partners(id),
    flight_id uuid REFERENCES deims.flights(id),
    airport_of_departure_code varchar(10) REFERENCES deims.ports(port_code),
    airport_of_destination_code varchar(10) REFERENCES deims.ports(port_code),
    issued_on date,
    package_count numeric(20,4),
    gross_weight numeric(24,8),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_awb_type CHECK (airwaybill_type IN ('MASTER','HOUSE','DIRECT')),
    CONSTRAINT ck_deims_awb_parent CHECK (
        parent_master_airwaybill_id IS NULL OR (parent_master_airwaybill_id <> id AND airwaybill_type = 'HOUSE')
    ),
    CONSTRAINT ck_deims_awb_qty CHECK (
    ...
    CONSTRAINT ck_deims_awb_status CHECK (status IN ('DRAFT','ISSUED','ACCEPTED','RELEASED','AMENDED','CANCELLED'))
);

CREATE TABLE IF NOT EXISTS deims.air_waybill_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, air_waybill_id)
        REFERENCES deims.air_waybills(tenant_id, owner_partner_id, id) ON DELETE CASCADE,
    CONSTRAINT ck_deims_awb_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_awb_line_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (quantity IS NULL OR quantity > 0) AND
        (gross_weight IS NULL OR gross_weight >= 0) AND (chargeable_weight IS NULL OR chargeable_weight >= 0)
    )
);

CREATE TABLE IF NOT EXISTS deims.receipt_trade_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    ...
    commercial_invoice_line_id uuid REFERENCES deims.commercial_invoice_lines(id),
    packing_list_line_id uuid REFERENCES deims.packing_list_lines(id),
    bill_of_lading_line_id uuid REFERENCES deims.bill_of_lading_lines(id),
    air_waybill_line_id uuid REFERENCES deims.air_waybill_lines(id),
    trade_container_id uuid REFERENCES deims.trade_containers(id),
    allocated_quantity numeric(24,8) NOT NULL,
    uom_code varchar(20) NOT NULL REFERENCES deims.units_of_measure(uom_code),
    allocated_base_quantity numeric(24,8) NOT NULL,
    base_uom_code varchar(20) NOT NULL REFERENCES deims.units_of_measure(uom_code),
    created_by uuid REFERENCES deims.users(id),
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (receipt_line_id, trade_shipment_id, commercial_invoice_line_id, packing_list_line_id),
    FOREIGN KEY (tenant_id, receipt_id) REFERENCES deims.receipts(tenant_id, id),
    FOREIGN KEY (tenant_id, receipt_line_id) REFERENCES deims.receipt_lines(tenant_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_receipt_trade_allocation_qty CHECK (
        allocated_quantity > 0 AND allocated_base_quantity > 0
    )
);

CREATE TABLE IF NOT EXISTS deims.shipment_trade_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    shipment_line_id uuid NOT NULL REFERENCES deims.shipment_lines(id),
    trade_shipment_id uuid NOT NULL,
    commercial_invoice_line_id uuid REFERENCES deims.commercial_invoice_lines(id),
    packing_list_line_id uuid REFERENCES deims.packing_list_lines(id),
    bill_of_lading_line_id uuid REFERENCES deims.bill_of_lading_lines(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, trade_shipment_id)
        REFERENCES deims.trade_shipments(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_shipment_trade_allocation_qty CHECK (
        allocated_quantity > 0 AND allocated_base_quantity > 0
    )
);

CREATE TABLE IF NOT EXISTS deims.customs_message_schemas (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    jurisdiction_country_code char(2) NOT NULL DEFAULT 'KR' REFERENCES deims.countries(country_code),
    authority_code varchar(50) NOT NULL DEFAULT 'KCS',
    ...
    CONSTRAINT ck_deims_customs_schema_direction CHECK (direction IN ('IN','OUT')),
    CONSTRAINT ck_deims_customs_schema_format CHECK (schema_format IN ('XML_XSD','JSON_SCHEMA','FIXED_WIDTH','CSV','OTHER')),
    CONSTRAINT ck_deims_customs_schema_hash CHECK (schema_sha256 ~ '^[0-9A-Fa-f]{64}$'),
    CONSTRAINT ck_deims_customs_schema_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)
);

CREATE TABLE IF NOT EXISTS deims.unipass_connection_profiles (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    CONSTRAINT ck_deims_unipass_profile_environment CHECK (environment IN ('SANDBOX','TEST','PRODUCTION')),
    CONSTRAINT ck_deims_unipass_profile_status CHECK (status IN ('ACTIVE','INACTIVE','SUSPENDED','EXPIRED')),
    CONSTRAINT ck_deims_unipass_profile_dates CHECK (valid_to IS NULL OR valid_to > valid_from),
    CONSTRAINT ck_deims_unipass_profile_secret_ref CHECK (
        credential_secret_ref IS NOT NULL OR certificate_secret_ref IS NOT NULL
    ),
    CONSTRAINT ck_deims_unipass_profile_no_inline_secrets CHECK (
        NOT deims.jsonb_contains_forbidden_secret_key(config_non_secret)
    )
);

COMMENT ON TABLE deims.unipass_connection_profiles IS
    'UNI-PASS connectivity metadata. Credentials, private keys, and certificates are external secret references only.';

CREATE TABLE IF NOT EXISTS deims.customs_declarations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    warehouse_id uuid REFERENCES deims.warehouses(id),
    ...
    UNIQUE (tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_customs_declaration_kind CHECK (declaration_kind IN (
        'IMPORT','EXPORT','RETURN_EXPORT','RETURN_IMPORT','TRANSIT','BONDED','OTHER'
    )),
    CONSTRAINT ck_deims_customs_declaration_status CHECK (workflow_status IN (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_customs_declaration_no
    ON deims.customs_declarations (tenant_id, customs_office_code, declaration_no_normalized)
    WHERE declaration_no_normalized IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_deims_customs_declaration_owner_status
    ON deims.customs_declarations (tenant_id, owner_partner_id, workflow_status, created_at DESC);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_versions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    supersedes_version_id uuid REFERENCES deims.customs_declaration_versions(id),
    submission_no varchar(200),
    ...
    declarant_partner_id uuid REFERENCES deims.business_partners(id),
    customs_broker_partner_id uuid REFERENCES deims.business_partners(id),
    importer_exporter_partner_id uuid REFERENCES deims.business_partners(id),
    ...
    incoterm_code char(3) REFERENCES deims.incoterms(incoterm_code),
    invoice_currency_code char(3) REFERENCES deims.currencies(currency_code),
    exchange_rate numeric(24,12),
    total_invoice_amount numeric(24,4),
    customs_value_currency_code char(3) REFERENCES deims.currencies(currency_code),
    ...
    prepared_by uuid REFERENCES deims.users(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_id)
        REFERENCES deims.customs_declarations(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_version_no CHECK (version_no > 0),
    CONSTRAINT ck_deims_customs_version_action CHECK (filing_action IN (
        'ORIGINAL','CORRECTION','WITHDRAWAL','CANCELLATION','OFFICIAL_CORRECTION','RESUBMISSION'
    )),
    CONSTRAINT ck_deims_customs_version_supersedes CHECK (supersedes_version_id IS NULL OR supersedes_version_id <> id),
    CONSTRAINT ck_deims_customs_version_exchange CHECK (exchange_rate IS NULL OR exchange_rate > 0),
    CONSTRAINT ck_deims_customs_version_amounts CHECK (
    ...
    CONSTRAINT ck_deims_customs_version_quantities CHECK (
        (package_count IS NULL OR package_count >= 0) AND
        (gross_weight IS NULL OR gross_weight >= 0) AND
        (net_weight IS NULL OR net_weight >= 0) AND
        (gross_weight IS NULL OR net_weight IS NULL OR gross_weight >= net_weight) AND
    ...
    CONSTRAINT ck_deims_customs_version_seal CHECK (
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'deims.customs_declarations'::regclass
          AND conname = 'fk_deims_customs_current_version'
    ) THEN
        ALTER TABLE deims.customs_declarations
            ADD CONSTRAINT fk_deims_customs_current_version
            FOREIGN KEY (id, current_version_id)
            REFERENCES deims.customs_declaration_versions(declaration_id, id)
...
CREATE OR REPLACE FUNCTION deims.require_shipment_customs_release(
    p_tenant_id uuid,
...
BEGIN
    ...
    FOR trade_row IN
        ...
        FROM deims.shipment_trade_allocations allocation
    ...
    LOOP
        ...
        FOR official_evidence_row IN
            SELECT evidence.id, evidence.official_release_at
              FROM deims.customs_release_evidence evidence
              JOIN deims.customs_declarations declaration
                ON declaration.id = evidence.declaration_id
              JOIN deims.unipass_messages official_message
             ...
             WHERE evidence.tenant_id = p_tenant_id
               ...
               AND deims.customs_release_evidence_is_current(evidence.id)
               ...
               AND EXISTS (
                   SELECT 1
                     FROM deims.customs_release_line_coverages coverage
                     JOIN deims.shipment_trade_allocations allocation
    ...
    FOR line_row IN
        SELECT line.id, line.base_quantity
          FROM deims.shipment_lines line
    ...
    IF allocation_count > 0 THEN
        ...
        LOOP
            SELECT COALESCE(sum(coverage.covered_base_quantity), 0)
              INTO covered_base_quantity
              FROM deims.customs_release_line_coverages coverage
              JOIN deims.customs_release_evidence evidence
                ON evidence.id = coverage.customs_release_evidence_id
              JOIN deims.customs_declaration_lines declaration_line
                ON declaration_line.id = coverage.declaration_line_id
              JOIN deims.shipment_lines shipment_line
    ...
    IF allocation_count = 0 THEN
        ...
        FOR official_evidence_row IN
            SELECT evidence.id, evidence.official_release_at
             ...
             WHERE evidence.tenant_id = p_tenant_id
               ...
               AND official_message.customs_system_recorded_at IS NOT NULL
               AND EXISTS (
                   SELECT 1 FROM deims.customs_release_line_coverages coverage
                    WHERE coverage.customs_release_evidence_id = evidence.id
        ...
        LOOP
            SELECT COALESCE(sum(coverage.covered_base_quantity), 0)
              ...
              JOIN deims.customs_declaration_lines declaration_line
                ON declaration_line.id = coverage.declaration_line_id
              JOIN deims.shipment_lines current_shipment_line
...
REVOKE ALL ON FUNCTION deims.require_shipment_customs_release(uuid,uuid,uuid,varchar,timestamptz)
    FROM PUBLIC;

CREATE OR REPLACE FUNCTION deims.enforce_shipment_customs_release()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
       ...
       AND (NEW.status IN ('DISPATCHED','IN_TRANSIT','DELIVERED','PARTIALLY_DELIVERED','CLOSED')
        ...
        IF NEW.actual_ship_at IS NULL THEN
            RAISE EXCEPTION 'Customs-controlled shipment % requires actual_ship_at before dispatch', NEW.id
                USING ERRCODE = '23514';
        END IF;
        NEW.customs_released_at := deims.require_shipment_customs_release(
...
CREATE OR REPLACE FUNCTION deims.enforce_shipping_confirmation_customs_release()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    shipment_row deims.shipments%ROWTYPE;
    release_is_required boolean;
BEGIN
    SELECT * INTO shipment_row
      FROM deims.shipments
    ...
    IF release_is_required THEN
        PERFORM deims.require_shipment_customs_release(
...
CREATE OR REPLACE FUNCTION deims.enforce_load_customs_release()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
       ...
       AND (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status) THEN
        ...
        FOR shipment_row IN
            SELECT shipment.tenant_id, shipment.owner_partner_id, shipment.id, shipment.shipment_type
              FROM deims.load_shipments allocation
              JOIN deims.shipments shipment ON shipment.id = allocation.shipment_id
             WHERE allocation.load_id = NEW.id
               AND (
                   ...
                   OR EXISTS (
                       SELECT 1
                         FROM deims.shipment_trade_allocations trade_allocation
...
REVOKE ALL ON FUNCTION deims.enforce_shipment_customs_release() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.enforce_shipping_confirmation_customs_release() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.enforce_load_customs_release() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_enforce_shipment_customs_release ON deims.shipments;
CREATE TRIGGER trg_enforce_shipment_customs_release
    ...
    ON deims.shipments
    FOR EACH ROW EXECUTE PROCEDURE deims.enforce_shipment_customs_release();

DROP TRIGGER IF EXISTS trg_enforce_shipping_confirmation_customs_release
    ON deims.shipping_confirmations;
CREATE TRIGGER trg_enforce_shipping_confirmation_customs_release
    BEFORE INSERT OR UPDATE OF shipment_id, confirmed_at
    ON deims.shipping_confirmations
    FOR EACH ROW EXECUTE PROCEDURE deims.enforce_shipping_confirmation_customs_release();

DROP TRIGGER IF EXISTS trg_enforce_load_customs_release ON deims.loads;
CREATE TRIGGER trg_enforce_load_customs_release
    BEFORE INSERT OR UPDATE OF status, actual_departure_at
    ON deims.loads
    FOR EACH ROW EXECUTE PROCEDURE deims.enforce_load_customs_release();

CREATE OR REPLACE FUNCTION deims.guard_shipment_trade_allocation_lifecycle()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    LOOP
        ...
        IF shipment_row.status IN ('DISPATCHED','IN_TRANSIT','DELIVERED','PARTIALLY_DELIVERED','EXCEPTION','CLOSED')
           OR EXISTS (
               SELECT 1 FROM deims.shipping_confirmations confirmation
                WHERE confirmation.shipment_id = target_shipment_id
           )
           OR EXISTS (
               SELECT 1
                 FROM deims.load_shipments load_allocation
                 JOIN deims.loads load ON load.id = load_allocation.load_id
...
CREATE OR REPLACE FUNCTION deims.guard_load_shipment_lifecycle()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    LOOP
        SELECT id, tenant_id, status, actual_departure_at
          INTO load_row FROM deims.loads WHERE id = target_load_id FOR UPDATE;
        IF NOT FOUND THEN
    ...
    LOOP
        SELECT id, tenant_id, status
          INTO shipment_row FROM deims.shipments WHERE id = target_shipment_id FOR UPDATE;
        IF NOT FOUND THEN
    ...
    IF TG_OP <> 'DELETE' THEN
        SELECT id, tenant_id, status, actual_departure_at
          INTO load_row FROM deims.loads WHERE id = NEW.load_id;
        SELECT id, tenant_id, status
          INTO shipment_row FROM deims.shipments WHERE id = NEW.shipment_id;
...
REVOKE ALL ON FUNCTION deims.guard_shipment_trade_allocation_lifecycle() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_load_shipment_lifecycle() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_guard_shipment_trade_allocation_lifecycle
    ON deims.shipment_trade_allocations;
CREATE TRIGGER trg_guard_shipment_trade_allocation_lifecycle
    BEFORE INSERT OR UPDATE OR DELETE ON deims.shipment_trade_allocations
    FOR EACH ROW EXECUTE PROCEDURE deims.guard_shipment_trade_allocation_lifecycle();

DROP TRIGGER IF EXISTS trg_guard_load_shipment_lifecycle ON deims.load_shipments;
CREATE TRIGGER trg_guard_load_shipment_lifecycle
    BEFORE INSERT OR UPDATE OR DELETE ON deims.load_shipments
    FOR EACH ROW EXECUTE PROCEDURE deims.guard_load_shipment_lifecycle();

COMMENT ON FUNCTION deims.require_shipment_customs_release(uuid,uuid,uuid,varchar,timestamptz) IS
    'Locks and verifies official RELEASED/CLEARED declaration evidence before a customs-controlled physical shipment event.';

CREATE TABLE IF NOT EXISTS deims.customs_declaration_parties (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_party_hash CHECK (
        personal_identifier_hash IS NULL OR personal_identifier_hash ~ '^[0-9A-Fa-f]{64}$'
    )
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_references (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_reference_dates CHECK (expires_on IS NULL OR issued_on IS NULL OR expires_on >= issued_on)
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    ...
    uom_1_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    quantity_2 numeric(24,8),
    uom_2_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    declared_base_quantity numeric(24,8),
    base_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    package_count numeric(20,4),
    package_type_code varchar(50),
    net_weight numeric(24,8),
    gross_weight numeric(24,8),
    ...
    FOREIGN KEY (hs_country_code, hs_nomenclature_version, hs_code)
        REFERENCES deims.hs_codes(country_code, nomenclature_version, hs_code),
    CONSTRAINT ck_deims_customs_line_no CHECK (line_no > 0),
    CONSTRAINT ck_deims_customs_line_quantities CHECK (
    ...
    CONSTRAINT ck_deims_customs_line_base_quantity CHECK (
    ...
    CONSTRAINT ck_deims_customs_line_amounts CHECK (
...
CREATE TABLE IF NOT EXISTS deims.customs_declaration_line_details (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (declaration_version_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(declaration_version_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_line_detail_value CHECK (
        num_nonnulls(detail_value_text, detail_value_number, detail_value_date, detail_value_json) = 1
    ),
    CONSTRAINT ck_deims_customs_line_detail_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_requirements (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    declaration_line_id uuid REFERENCES deims.customs_declaration_lines(id),
    ...
    FOREIGN KEY (declaration_version_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(declaration_version_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_requirement_status CHECK (required_status IN (
        'REQUIRED','SUBMITTED','ACCEPTED','REJECTED','WAIVED','NOT_APPLICABLE'
    )),
    CONSTRAINT ck_deims_customs_requirement_dates CHECK (expires_on IS NULL OR issued_on IS NULL OR expires_on >= issued_on)
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_taxes (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    ...
    FOREIGN KEY (declaration_version_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(declaration_version_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_tax_seq CHECK (tax_sequence > 0),
    CONSTRAINT ck_deims_customs_tax_amount CHECK (
...
CREATE TABLE IF NOT EXISTS deims.customs_declaration_containers (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    ...
    FOREIGN KEY (declaration_version_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(declaration_version_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_container_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (gross_weight IS NULL OR gross_weight >= 0)
    )
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_inspections (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_id uuid NOT NULL,
    ...
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_inspection_status CHECK (status IN (
        'SELECTED','SCHEDULED','IN_PROGRESS','COMPLETED','WAIVED','CANCELLED'
    )),
    CONSTRAINT ck_deims_customs_inspection_dates CHECK (
        (started_at IS NULL OR scheduled_at IS NULL OR started_at >= scheduled_at) AND
        (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
    )
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_relations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    source_declaration_id uuid NOT NULL REFERENCES deims.customs_declarations(id),
    target_declaration_id uuid NOT NULL REFERENCES deims.customs_declarations(id),
    relation_type varchar(50) NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, target_declaration_id)
        REFERENCES deims.customs_declarations(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_relation_self CHECK (source_declaration_id <> target_declaration_id),
    CONSTRAINT ck_deims_customs_relation_type CHECK (relation_type IN (
        'CORRECTS','WITHDRAWS','REPLACES','RETURNS','SPLITS','MERGES','PRECEDES','RELATED'
    ))
);

CREATE TABLE IF NOT EXISTS deims.customs_declaration_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_id uuid NOT NULL,
    ...
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_event_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.customs_documents (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_id uuid NOT NULL,
    ...
    issued_on date,
    expires_on date,
    trade_document_id uuid REFERENCES deims.trade_documents(id),
    file_id uuid REFERENCES deims.files(id),
    object_uri text,
    object_sha256 char(64),
    byte_size bigint,
    ...
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_document_revision CHECK (revision_no > 0),
    CONSTRAINT ck_deims_customs_document_source CHECK (
        trade_document_id IS NOT NULL OR file_id IS NOT NULL OR object_uri IS NOT NULL
    ),
    CONSTRAINT ck_deims_customs_document_hash CHECK (object_sha256 IS NULL OR object_sha256 ~ '^[0-9A-Fa-f]{64}$'),
    CONSTRAINT ck_deims_customs_document_size CHECK (byte_size IS NULL OR byte_size >= 0),
    CONSTRAINT ck_deims_customs_document_dates CHECK (expires_on IS NULL OR issued_on IS NULL OR expires_on >= issued_on),
    CONSTRAINT ck_deims_customs_document_status CHECK (submission_status IN (
        'PREPARED','SUBMITTED','ACCEPTED','REJECTED','SUPERSEDED','WITHDRAWN'
    ))
);

CREATE TABLE IF NOT EXISTS deims.customs_document_links (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    customs_document_id uuid NOT NULL REFERENCES deims.customs_documents(id) ON DELETE RESTRICT,
    declaration_version_id uuid,
    declaration_line_id uuid REFERENCES deims.customs_declaration_lines(id),
    requirement_id uuid REFERENCES deims.customs_declaration_requirements(id),
    relation_type varchar(50) NOT NULL DEFAULT 'SUPPORTS',
    created_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_deims_customs_document_link_target CHECK (
        num_nonnulls(declaration_version_id, declaration_line_id, requirement_id) = 1
    )
);

CREATE TABLE IF NOT EXISTS deims.customs_duty_payments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_id uuid NOT NULL,
    ...
    assessment_reference varchar(300),
    payer_partner_id uuid REFERENCES deims.business_partners(id),
    payment_method_code varchar(100),
    ...
    receipt_document_id uuid REFERENCES deims.customs_documents(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS deims.customs_duty_payment_allocations (
    ...
    UNIQUE (tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_id)
        REFERENCES deims.customs_declarations(tenant_id, owner_partner_id, id),
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, customs_duty_payment_id)
        REFERENCES deims.customs_duty_payments(tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_tax_id)
        REFERENCES deims.customs_declaration_taxes(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_customs_payment_allocation_amount CHECK (allocated_amount > 0)
);

CREATE TABLE IF NOT EXISTS deims.customs_refund_claims (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    declaration_id uuid NOT NULL,
    ...
    claimant_partner_id uuid REFERENCES deims.business_partners(id),
    currency_code char(3) NOT NULL REFERENCES deims.currencies(currency_code),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_id)
        REFERENCES deims.customs_declarations(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_customs_refund_amount CHECK (
    ...
    CONSTRAINT ck_deims_customs_refund_status CHECK (status IN (
        'DRAFT','FILED','UNDER_REVIEW','APPROVED','PARTIALLY_APPROVED','REJECTED',
        'PARTIALLY_PAID','PAID','CANCELLED'
    )),
    CONSTRAINT ck_deims_customs_refund_dates CHECK (
...
CREATE TABLE IF NOT EXISTS deims.customs_refund_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    declaration_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, refund_claim_id)
        REFERENCES deims.customs_refund_claims(tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, customs_duty_payment_id)
        REFERENCES deims.customs_duty_payments(tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_tax_id)
        REFERENCES deims.customs_declaration_taxes(tenant_id, owner_partner_id, id),
    CONSTRAINT ck_deims_customs_refund_allocation_target CHECK (
    ...
    CONSTRAINT ck_deims_customs_refund_allocation_amount CHECK (allocated_amount > 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_customs_refund_claim_payment
    ON deims.customs_refund_allocations (refund_claim_id, customs_duty_payment_id)
    WHERE customs_duty_payment_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_customs_refund_claim_tax
    ON deims.customs_refund_allocations (refund_claim_id, declaration_tax_id)
    WHERE declaration_tax_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_deims_customs_payment_allocation_tax
    ON deims.customs_duty_payment_allocations (declaration_tax_id, customs_duty_payment_id);

CREATE INDEX IF NOT EXISTS ix_deims_customs_refund_allocation_target
    ON deims.customs_refund_allocations (customs_duty_payment_id, declaration_tax_id, refund_claim_id);

CREATE OR REPLACE FUNCTION deims.validate_customs_duty_payment_allocation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    payment_row deims.customs_duty_payments%ROWTYPE;
    tax_row deims.customs_declaration_taxes%ROWTYPE;
...
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('deims-customs-finance:' || NEW.declaration_id::text));
    SELECT * INTO payment_row
      FROM deims.customs_duty_payments
    ...
    END IF;
    SELECT * INTO tax_row
      FROM deims.customs_declaration_taxes
    ...
    SELECT version.declaration_id, declaration.current_version_id,
      ...
      FROM deims.customs_declaration_versions version
      JOIN deims.customs_declarations declaration ON declaration.id = version.declaration_id
    ...
    SELECT COALESCE(sum(allocated_amount), 0)
      ...
      FROM deims.customs_duty_payment_allocations
...
CREATE OR REPLACE FUNCTION deims.validate_customs_refund_allocation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    claim_row deims.customs_refund_claims%ROWTYPE;
    payment_row deims.customs_duty_payments%ROWTYPE;
    tax_row deims.customs_declaration_taxes%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO claim_row
      FROM deims.customs_refund_claims
    ...
    IF NEW.customs_duty_payment_id IS NOT NULL THEN
        ...
        SELECT COALESCE(sum(allocated_amount), 0)
          ...
          FROM deims.customs_refund_allocations
         WHERE customs_duty_payment_id = NEW.customs_duty_payment_id;
...
CREATE OR REPLACE FUNCTION deims.guard_customs_financial_header_projection()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    IF TG_TABLE_NAME = 'customs_duty_payments' THEN
        SELECT COALESCE(sum(allocation.allocated_amount), 0)
          ...
          FROM deims.customs_duty_payment_allocations allocation
        ...
        SELECT COALESCE(sum(allocation.allocated_amount), 0)
          INTO allocated_amount
          FROM deims.customs_refund_allocations allocation
         WHERE allocation.customs_duty_payment_id = OLD.id;
        ...
        IF (
            EXISTS (
                SELECT 1 FROM deims.customs_duty_payment_allocations allocation
                 WHERE allocation.customs_duty_payment_id = OLD.id
            ) OR EXISTS (
                SELECT 1 FROM deims.customs_refund_allocations allocation
                 WHERE allocation.customs_duty_payment_id = OLD.id
            )
        ) AND (
...
REVOKE ALL ON FUNCTION deims.validate_customs_duty_payment_allocation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.validate_customs_refund_allocation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_customs_financial_header_projection() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_validate_customs_duty_payment_allocation
    ON deims.customs_duty_payment_allocations;
CREATE TRIGGER trg_validate_customs_duty_payment_allocation
    BEFORE INSERT ON deims.customs_duty_payment_allocations
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_customs_duty_payment_allocation();

DROP TRIGGER IF EXISTS trg_validate_customs_refund_allocation
    ON deims.customs_refund_allocations;
CREATE TRIGGER trg_validate_customs_refund_allocation
    BEFORE INSERT ON deims.customs_refund_allocations
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_customs_refund_allocation();

DROP TRIGGER IF EXISTS trg_guard_customs_duty_payment_projection
    ON deims.customs_duty_payments;
CREATE TRIGGER trg_guard_customs_duty_payment_projection
    ...
    ON deims.customs_duty_payments
    FOR EACH ROW EXECUTE PROCEDURE deims.guard_customs_financial_header_projection();

DROP TRIGGER IF EXISTS trg_guard_customs_refund_claim_projection
    ON deims.customs_refund_claims;
...
CREATE TABLE IF NOT EXISTS deims.unipass_messages (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid REFERENCES deims.business_partners(id),
    connection_profile_id uuid NOT NULL REFERENCES deims.unipass_connection_profiles(id),
    message_schema_id uuid REFERENCES deims.customs_message_schemas(id),
    declaration_id uuid REFERENCES deims.customs_declarations(id),
    declaration_version_id uuid REFERENCES deims.customs_declaration_versions(id),
    direction varchar(10) NOT NULL,
    message_type varchar(120) NOT NULL,
    ...
    causation_message_id uuid REFERENCES deims.unipass_messages(id),
    ...
    retention_until date,
    legal_hold boolean NOT NULL DEFAULT false,
    CONSTRAINT ck_deims_unipass_message_direction CHECK (direction IN ('IN','OUT')),
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_deims_unipass_message_declaration_version CHECK (
        declaration_version_id IS NULL OR declaration_id IS NOT NULL
    ),
    CONSTRAINT ck_deims_unipass_message_purpose CHECK (message_purpose IN (
    ...
    CONSTRAINT ck_deims_unipass_message_attempt CHECK (attempt_no > 0),
    CONSTRAINT ck_deims_unipass_message_hash CHECK (payload_sha256 ~ '^[0-9A-Fa-f]{64}$'),
    CONSTRAINT ck_deims_unipass_message_size CHECK (payload_byte_size >= 0),
    CONSTRAINT ck_deims_unipass_message_status CHECK (recorded_status IN (
        'CREATED','QUEUED','SENT','RECEIVED','ACKNOWLEDGED','ACCEPTED','REJECTED','ERROR'
    )),
    CONSTRAINT ck_deims_unipass_message_causation CHECK (causation_message_id IS NULL OR causation_message_id <> id),
    CONSTRAINT ck_deims_unipass_message_no_inline_secrets CHECK (
        NOT deims.jsonb_contains_forbidden_secret_key(payload_summary_masked)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_unipass_message_attempt
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_unipass_external_message
    ON deims.unipass_messages (connection_profile_id, direction, external_message_id)
    WHERE external_message_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_deims_unipass_correlation
    ON deims.unipass_messages (tenant_id, correlation_id, created_at DESC)
    WHERE correlation_id IS NOT NULL;

COMMENT ON TABLE deims.unipass_messages IS
...
CREATE OR REPLACE FUNCTION deims.validate_unipass_message_lifecycle()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
DROP TRIGGER IF EXISTS trg_validate_unipass_message_lifecycle ON deims.unipass_messages;
CREATE TRIGGER trg_validate_unipass_message_lifecycle
    ...
    ON deims.unipass_messages
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_unipass_message_lifecycle();

CREATE TABLE IF NOT EXISTS deims.unipass_message_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    unipass_message_id uuid NOT NULL REFERENCES deims.unipass_messages(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_unipass_message_event_seq CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS deims.unipass_message_errors (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    unipass_message_id uuid NOT NULL REFERENCES deims.unipass_messages(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_unipass_error_seq CHECK (error_sequence > 0),
    CONSTRAINT ck_deims_unipass_error_line CHECK (line_no IS NULL OR line_no > 0),
    CONSTRAINT ck_deims_unipass_error_severity CHECK (severity IN ('INFO','WARNING','ERROR','FATAL'))
);

CREATE TABLE IF NOT EXISTS deims.unipass_message_attachments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    unipass_message_id uuid NOT NULL REFERENCES deims.unipass_messages(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_deims_unipass_attachment_seq CHECK (attachment_sequence > 0),
    CONSTRAINT ck_deims_unipass_attachment_source CHECK (file_id IS NOT NULL OR object_uri IS NOT NULL),
    CONSTRAINT ck_deims_unipass_attachment_hash CHECK (object_sha256 IS NULL OR object_sha256 ~ '^[0-9A-Fa-f]{64}$'),
    CONSTRAINT ck_deims_unipass_attachment_size CHECK (byte_size IS NULL OR byte_size >= 0)
);
...
CREATE TABLE IF NOT EXISTS deims.customs_release_evidence (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    shipment_id uuid NOT NULL REFERENCES deims.shipments(id),
    ...
    declaration_id uuid NOT NULL,
    declaration_version_id uuid NOT NULL,
    unipass_message_id uuid NOT NULL REFERENCES deims.unipass_messages(id),
    ...
    FOREIGN KEY (declaration_id, declaration_version_id)
        REFERENCES deims.customs_declaration_versions(declaration_id, id),
    CONSTRAINT ck_deims_customs_release_evidence_type CHECK (evidence_type IN (
        'IMPORT_RELEASE','EXPORT_CLEARANCE','BONDED_RELEASE','TRANSIT_RELEASE','RETURN_RELEASE'
    )),
    CONSTRAINT ck_deims_customs_release_decision CHECK (official_decision_code IN (
        'IMPORT_RELEASED','EXPORT_CLEARED','BONDED_RELEASED','TRANSIT_RELEASED',
        'RETURN_IMPORT_RELEASED','RETURN_EXPORT_CLEARED'
    )),
    CONSTRAINT ck_deims_customs_release_verification CHECK (
        verification_method = 'UNIPASS_INBOUND' AND btrim(external_release_reference) <> ''
    ),
    CONSTRAINT ck_deims_customs_release_times CHECK (
        official_release_at <= verified_at AND created_at >= official_release_at
    )
);

CREATE OR REPLACE FUNCTION deims.customs_release_evidence_is_current(p_evidence_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
    SELECT EXISTS (
        SELECT 1
          ...
          JOIN deims.unipass_messages official_message
            ON official_message.id = evidence.unipass_message_id
          JOIN deims.unipass_connection_profiles connection_profile
            ON connection_profile.id = official_message.connection_profile_id
          JOIN deims.external_systems integration_system
            ON integration_system.id = connection_profile.external_system_id
          JOIN deims.customs_message_schemas message_schema
...
REVOKE ALL ON FUNCTION deims.customs_release_evidence_is_current(uuid) FROM PUBLIC;

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_customs_release_evidence_scope
    ON deims.customs_release_evidence (
...
CREATE INDEX IF NOT EXISTS ix_deims_customs_release_evidence_lookup
    ON deims.customs_release_evidence (
        tenant_id, owner_partner_id, shipment_id, trade_shipment_id, official_release_at DESC
    );

CREATE OR REPLACE FUNCTION deims.validate_customs_release_evidence()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
DECLARE
    shipment_row deims.shipments%ROWTYPE;
    declaration_row deims.customs_declarations%ROWTYPE;
    message_row deims.unipass_messages%ROWTYPE;
    connection_profile_row deims.unipass_connection_profiles%ROWTYPE;
    integration_system_row deims.external_systems%ROWTYPE;
    message_schema_row deims.customs_message_schemas%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO declaration_row
      FROM deims.customs_declarations
    ...
    SELECT * INTO connection_profile_row
      FROM deims.unipass_connection_profiles
    ...
    SELECT * INTO message_schema_row
      FROM deims.customs_message_schemas
    ...
    IF NEW.trade_shipment_id IS NULL THEN
        ...
        END IF;
        IF EXISTS (
            SELECT 1 FROM deims.shipment_trade_allocations allocation
...
REVOKE ALL ON FUNCTION deims.validate_customs_release_evidence() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_validate_customs_release_evidence ON deims.customs_release_evidence;
CREATE TRIGGER trg_validate_customs_release_evidence
    BEFORE INSERT ON deims.customs_release_evidence
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_customs_release_evidence();

CREATE OR REPLACE FUNCTION deims.guard_customs_release_legal_header()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
REVOKE ALL ON FUNCTION deims.guard_customs_release_legal_header() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_guard_customs_release_legal_header ON deims.customs_declarations;
CREATE TRIGGER trg_guard_customs_release_legal_header
    BEFORE UPDATE OR DELETE ON deims.customs_declarations
    FOR EACH ROW EXECUTE PROCEDURE deims.guard_customs_release_legal_header();

DROP TRIGGER IF EXISTS trg_block_immutable_change ON deims.customs_release_evidence;
CREATE TRIGGER trg_block_immutable_change
    BEFORE UPDATE OR DELETE ON deims.customs_release_evidence
    FOR EACH ROW EXECUTE PROCEDURE deims.block_immutable_change();

COMMENT ON TABLE deims.customs_release_evidence IS
...
CREATE TABLE IF NOT EXISTS deims.customs_release_line_coverages (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    customs_release_evidence_id uuid NOT NULL REFERENCES deims.customs_release_evidence(id),
    shipment_line_id uuid NOT NULL REFERENCES deims.shipment_lines(id),
    shipment_trade_allocation_id uuid REFERENCES deims.shipment_trade_allocations(id),
    declaration_line_id uuid NOT NULL,
    item_id_snapshot uuid NOT NULL REFERENCES deims.items(id),
    country_of_origin_snapshot char(2) NOT NULL REFERENCES deims.countries(country_code),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, customs_release_evidence_id)
        REFERENCES deims.customs_release_evidence(tenant_id, owner_partner_id, id),
    FOREIGN KEY (tenant_id, owner_partner_id, declaration_line_id)
        REFERENCES deims.customs_declaration_lines(tenant_id, owner_partner_id, id),
    FOREIGN KEY (hs_country_code_snapshot, hs_nomenclature_version_snapshot, hs_code_snapshot)
        REFERENCES deims.hs_codes(country_code, nomenclature_version, hs_code),
    CONSTRAINT ck_deims_customs_release_line_coverage_qty CHECK (covered_base_quantity > 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_deims_customs_release_line_coverage
    ON deims.customs_release_line_coverages (
...
CREATE INDEX IF NOT EXISTS ix_deims_customs_release_coverage_allocation
    ON deims.customs_release_line_coverages (
        shipment_trade_allocation_id, shipment_line_id, customs_release_evidence_id
    );

CREATE INDEX IF NOT EXISTS ix_deims_customs_release_coverage_declaration_line
    ON deims.customs_release_line_coverages (declaration_line_id, created_at, id);

CREATE OR REPLACE FUNCTION deims.validate_customs_release_line_coverage()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    evidence_row deims.customs_release_evidence%ROWTYPE;
    shipment_line_row deims.shipment_lines%ROWTYPE;
    allocation_row deims.shipment_trade_allocations%ROWTYPE;
    declaration_line_row deims.customs_declaration_lines%ROWTYPE;
    version_sealed_at timestamptz;
...
BEGIN
    SELECT * INTO evidence_row
      FROM deims.customs_release_evidence
    ...
    SELECT * INTO declaration_line_row
      FROM deims.customs_declaration_lines
    ...
    ELSE
        ...
        SELECT * INTO allocation_row
          FROM deims.shipment_trade_allocations
    ...
    SELECT COALESCE(sum(covered_base_quantity), 0)
      ...
      FROM deims.customs_release_line_coverages
...
REVOKE ALL ON FUNCTION deims.validate_customs_release_line_coverage() FROM PUBLIC;
...
CREATE TRIGGER trg_validate_customs_release_line_coverage
    BEFORE INSERT ON deims.customs_release_line_coverages
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_customs_release_line_coverage();

DROP TRIGGER IF EXISTS trg_block_immutable_change ON deims.customs_release_line_coverages;
CREATE TRIGGER trg_block_immutable_change
    BEFORE UPDATE OR DELETE ON deims.customs_release_line_coverages
    FOR EACH ROW EXECUTE PROCEDURE deims.block_immutable_change();

CREATE OR REPLACE FUNCTION deims.guard_customs_covered_shipment_line()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
REVOKE ALL ON FUNCTION deims.guard_customs_covered_shipment_line() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_guard_customs_covered_shipment_line ON deims.shipment_lines;
CREATE TRIGGER trg_guard_customs_covered_shipment_line
    BEFORE UPDATE OR DELETE ON deims.shipment_lines
    FOR EACH ROW EXECUTE PROCEDURE deims.guard_customs_covered_shipment_line();

COMMENT ON TABLE deims.customs_release_line_coverages IS
...
CREATE TABLE IF NOT EXISTS deims.unipass_sync_runs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    connection_profile_id uuid NOT NULL REFERENCES deims.unipass_connection_profiles(id),
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_deims_unipass_sync_status CHECK (status IN ('QUEUED','RUNNING','COMPLETED','PARTIAL','FAILED','CANCELLED')),
    CONSTRAINT ck_deims_unipass_sync_counts CHECK (
        item_count >= 0 AND success_count >= 0 AND error_count >= 0 AND success_count + error_count <= item_count
    ),
    CONSTRAINT ck_deims_unipass_sync_window CHECK (window_to IS NULL OR window_from IS NULL OR window_to >= window_from),
    CONSTRAINT ck_deims_unipass_sync_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS deims.unipass_sync_items (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    sync_run_id uuid NOT NULL REFERENCES deims.unipass_sync_runs(id) ON DELETE CASCADE,
    item_sequence integer NOT NULL,
    entity_type varchar(80),
    entity_id uuid,
    external_reference varchar(300),
    unipass_message_id uuid REFERENCES deims.unipass_messages(id),
    ...
    CONSTRAINT ck_deims_unipass_sync_item_seq CHECK (item_sequence > 0),
    CONSTRAINT ck_deims_unipass_sync_item_status CHECK (status IN ('PENDING','PROCESSED','SKIPPED','ERROR'))
);
...
CREATE OR REPLACE FUNCTION deims.validate_customs_version_seal()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
DROP TRIGGER IF EXISTS trg_10_validate_customs_version_seal ON deims.customs_declaration_versions;
CREATE TRIGGER trg_10_validate_customs_version_seal
    BEFORE INSERT OR UPDATE OF sealed_at ON deims.customs_declaration_versions
    FOR EACH ROW EXECUTE PROCEDURE deims.validate_customs_version_seal();

DROP TRIGGER IF EXISTS trg_20_protect_customs_version ON deims.customs_declaration_versions;
CREATE TRIGGER trg_20_protect_customs_version
    BEFORE UPDATE OR DELETE ON deims.customs_declaration_versions
    FOR EACH ROW EXECUTE PROCEDURE deims.protect_customs_declaration_version();
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_sealed_customs_version ON deims.%I', v_table);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_sealed_customs_version BEFORE INSERT OR UPDATE OR DELETE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.protect_customs_version_child()', v_table
        );
    END LOOP;
END;
$block$;
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_validate_tenant_owner_scope ON deims.%I', v_table);
        EXECUTE format(
            'CREATE TRIGGER trg_validate_tenant_owner_scope BEFORE INSERT OR UPDATE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.validate_tenant_owner_scope()', v_table
        );
    END LOOP;
END;
$block$;
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_block_immutable_change ON deims.%I', v_table);
        EXECUTE format(
            'CREATE TRIGGER trg_block_immutable_change BEFORE UPDATE OR DELETE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.block_immutable_change()', v_table
        );
    END LOOP;
END;
$block$;
...
DROP TRIGGER IF EXISTS trg_block_immutable_change ON deims.unipass_messages;

CREATE INDEX IF NOT EXISTS ix_deims_trade_shipment_schedule
    ON deims.trade_shipments (tenant_id, owner_partner_id, planned_arrival_at)
    WHERE status NOT IN ('CANCELLED','CLOSED');
CREATE INDEX IF NOT EXISTS ix_deims_trade_leg_schedule
    ON deims.trade_shipment_legs (tenant_id, planned_departure_at, planned_arrival_at);
CREATE INDEX IF NOT EXISTS ix_deims_customs_version_declaration
    ON deims.customs_declaration_versions (declaration_id, version_no DESC);
CREATE INDEX IF NOT EXISTS ix_deims_customs_line_hs
    ON deims.customs_declaration_lines (hs_country_code, hs_nomenclature_version, hs_code);
CREATE INDEX IF NOT EXISTS ix_deims_customs_event_timeline
    ON deims.customs_declaration_events (declaration_id, occurred_at, sequence_no);
CREATE INDEX IF NOT EXISTS ix_deims_unipass_message_declaration
    ON deims.unipass_messages (declaration_id, created_at DESC)
    WHERE declaration_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_deims_unipass_sync_status
    ON deims.unipass_sync_runs (tenant_id, status, created_at DESC);

CREATE INDEX IF NOT EXISTS brin_deims_trade_shipment_events_occurred
    ON deims.trade_shipment_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS brin_deims_customs_events_occurred
    ON deims.customs_declaration_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS brin_deims_unipass_messages_created
    ON deims.unipass_messages USING brin (created_at);

COMMENT ON TABLE deims.customs_declarations IS
...
COMMENT ON COLUMN deims.customs_declaration_parties.personal_identifier_encrypted IS
    'Encrypted personal identifier only; plaintext resident/customs identifiers are prohibited.';
COMMENT ON TABLE deims.customs_code_sets IS
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'deims.stock_buckets'::regclass
          AND conname = 'fk_deims_stock_bucket_customs_line'
    ) THEN
        ALTER TABLE deims.stock_buckets
            ADD CONSTRAINT fk_deims_stock_bucket_customs_line
            FOREIGN KEY (tenant_id, customs_declaration_line_id)
            REFERENCES deims.customs_declaration_lines(tenant_id, id) ON DELETE RESTRICT;
    END IF;
END;
$block$;
