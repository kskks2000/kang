-- DEIMS workflow, notifications, EDI/webhooks/jobs, audit extensions, and analytics.

CREATE TABLE IF NOT EXISTS deims.status_definitions (
...
CREATE TABLE IF NOT EXISTS deims.entity_tags (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    tag_id uuid NOT NULL REFERENCES deims.tags(id) ON DELETE CASCADE,
    assigned_by uuid REFERENCES deims.users(id),
    assigned_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, entity_type, entity_id, tag_id)
);

CREATE TABLE IF NOT EXISTS deims.notification_templates (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.notifications (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    event_type varchar(100) NOT NULL,
    template_id uuid REFERENCES deims.notification_templates(id),
    ...
    CONSTRAINT ck_notification_no_secret_variables CHECK (
        NOT deims.jsonb_contains_forbidden_secret_key(variables_masked)
    )
);

CREATE TABLE IF NOT EXISTS deims.notification_deliveries (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    notification_id uuid NOT NULL REFERENCES deims.notifications(id) ON DELETE CASCADE,
    user_id uuid REFERENCES deims.users(id),
    contact_id uuid REFERENCES deims.contacts(id),
    channel varchar(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.edi_documents (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.edi_validation_errors (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    edi_document_id uuid NOT NULL REFERENCES deims.edi_documents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.webhook_subscriptions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
COMMENT ON TABLE deims.webhook_subscriptions IS
...
CREATE TABLE IF NOT EXISTS deims.webhook_deliveries (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    subscription_id uuid NOT NULL REFERENCES deims.webhook_subscriptions(id) ON DELETE CASCADE,
    outbox_event_id uuid REFERENCES deims.outbox_events(id),
...
CREATE TABLE IF NOT EXISTS deims.inbox_deduplication (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    source_system_id uuid NOT NULL REFERENCES deims.external_systems(id),
...
CREATE TABLE IF NOT EXISTS deims.import_jobs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    job_no varchar(100) NOT NULL,
    import_type varchar(80) NOT NULL,
    source_file_id uuid NOT NULL REFERENCES deims.files(id),
    ...
    result_file_id uuid REFERENCES deims.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS deims.import_job_rows (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    import_job_id uuid NOT NULL REFERENCES deims.import_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.export_jobs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    job_no varchar(100) NOT NULL,
    ...
    output_file_id uuid REFERENCES deims.files(id),
    requested_by uuid REFERENCES deims.users(id),
    requested_at timestamptz NOT NULL DEFAULT now(),
    completed_at timestamptz,
    expires_at timestamptz,
...
CREATE TABLE IF NOT EXISTS deims.export_job_rows (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    export_job_id uuid NOT NULL REFERENCES deims.export_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.scheduled_jobs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_scheduled_job_no_inline_secrets CHECK (
        NOT deims.jsonb_contains_forbidden_secret_key(parameters)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_scheduled_jobs_scope
    ON deims.scheduled_jobs (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), job_code
    );

CREATE TABLE IF NOT EXISTS deims.job_runs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
    scheduled_job_id uuid NOT NULL REFERENCES deims.scheduled_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.dead_letter_messages (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.impersonation_sessions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    administrator_user_id uuid NOT NULL REFERENCES deims.users(id),
    impersonated_user_id uuid NOT NULL REFERENCES deims.users(id),
...
CREATE TABLE IF NOT EXISTS deims.impersonation_events (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    impersonation_session_id uuid NOT NULL REFERENCES deims.impersonation_sessions(id) ON DELETE RESTRICT,
    sequence_no integer NOT NULL,
    event_type varchar(20) NOT NULL,
    actor_user_id uuid NOT NULL REFERENCES deims.users(id),
    reason text,
...
CREATE TABLE IF NOT EXISTS deims.entity_change_logs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.data_access_logs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id),
    actor_user_id uuid REFERENCES deims.users(id),
    actor_api_client_id uuid REFERENCES deims.api_clients(id),
...
CREATE TABLE IF NOT EXISTS deims.reconciliation_runs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.reconciliation_details (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    reconciliation_run_id uuid NOT NULL REFERENCES deims.reconciliation_runs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.workflow_definitions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.workflow_versions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    workflow_definition_id uuid NOT NULL REFERENCES deims.workflow_definitions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.workflow_steps (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES deims.workflow_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.workflow_instances (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES deims.workflow_versions(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    status varchar(20) NOT NULL DEFAULT 'RUNNING',
    current_step_id uuid REFERENCES deims.workflow_steps(id),
...
CREATE TABLE IF NOT EXISTS deims.workflow_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    workflow_instance_id uuid NOT NULL REFERENCES deims.workflow_instances(id) ON DELETE CASCADE,
    workflow_step_id uuid NOT NULL REFERENCES deims.workflow_steps(id),
    task_no integer NOT NULL,
    assignee_user_id uuid REFERENCES deims.users(id),
    assignee_group_id uuid REFERENCES deims.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    completed_at timestamptz,
...
CREATE TABLE IF NOT EXISTS deims.approval_policies (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.approval_requests (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    request_no varchar(100) NOT NULL,
    policy_id uuid REFERENCES deims.approval_policies(id),
    workflow_instance_id uuid REFERENCES deims.workflow_instances(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    amount numeric(20,4),
    currency_code char(3) REFERENCES deims.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS deims.approval_steps (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    approval_request_id uuid NOT NULL REFERENCES deims.approval_requests(id) ON DELETE CASCADE,
    level_no integer NOT NULL,
    approver_user_id uuid REFERENCES deims.users(id),
    approver_group_id uuid REFERENCES deims.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    ...
    decision varchar(20),
    comments text,
    delegated_from_user_id uuid REFERENCES deims.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_user
    ON deims.approval_steps (approval_request_id, level_no, approver_user_id)
    WHERE approver_user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_group
    ON deims.approval_steps (approval_request_id, level_no, approver_group_id)
    WHERE approver_group_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS deims.approval_actions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    approval_step_id uuid NOT NULL REFERENCES deims.approval_steps(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.sod_rules (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS deims.sod_rule_conflicts (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
    sod_rule_id uuid NOT NULL REFERENCES deims.sod_rules(id) ON DELETE CASCADE,
    left_permission_id uuid NOT NULL REFERENCES deims.permissions(id),
    right_permission_id uuid NOT NULL REFERENCES deims.permissions(id),
...
CREATE TABLE IF NOT EXISTS deims.sod_violations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    sod_rule_id uuid NOT NULL REFERENCES deims.sod_rules(id),
    user_id uuid NOT NULL REFERENCES deims.users(id),
    left_role_assignment_id uuid REFERENCES deims.role_assignments(id),
    right_role_assignment_id uuid REFERENCES deims.role_assignments(id),
...
COMMENT ON COLUMN deims.workflow_versions.trigger_conditions IS
    'Declarative, allow-listed workflow DSL only; never execute this JSON as SQL, PL/pgSQL, shell, or template code.';
COMMENT ON COLUMN deims.workflow_steps.assignee_rule IS
...
CREATE TABLE IF NOT EXISTS deims.kpi_definitions (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
    ...
    owner_org_unit_id uuid REFERENCES deims.organization_units(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_definitions_scope
    ON deims.kpi_definitions (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        kpi_code, calculation_version
    );

COMMENT ON COLUMN deims.kpi_definitions.calculation_expression IS
    'Declarative, versioned KPI DSL only; never execute as SQL or application code.';

CREATE TABLE IF NOT EXISTS deims.kpi_targets (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES deims.kpi_definitions(id),
...
CREATE TABLE IF NOT EXISTS deims.kpi_values (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES deims.kpi_definitions(id),
    scope_type varchar(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.warehouse_daily_facts (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    fact_date date NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    warehouse_client_id uuid REFERENCES deims.warehouse_clients(id),
    owner_partner_id uuid REFERENCES deims.business_partners(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_inventory_daily_facts_grain
    ON deims.inventory_daily_facts (
        tenant_id, fact_date, warehouse_id,
        COALESCE(warehouse_client_id, '00000000-0000-0000-0000-000000000000'::uuid),
        owner_partner_id, item_id, inventory_status_id
    );

CREATE TABLE IF NOT EXISTS deims.operation_daily_facts (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    fact_date date NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_operation_daily_facts_grain
    ON deims.operation_daily_facts (
        tenant_id, fact_date, warehouse_id,
        COALESCE(warehouse_client_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(owner_partner_id, '00000000-0000-0000-0000-000000000000'::uuid),
        operation_type
...
CREATE TABLE IF NOT EXISTS deims.data_quality_rules (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid REFERENCES deims.tenants(id) ON DELETE CASCADE,
    rule_code varchar(100) NOT NULL,
    rule_name varchar(250) NOT NULL,
    ...
    is_active boolean NOT NULL DEFAULT true,
    owner_user_id uuid REFERENCES deims.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_data_quality_rules_scope
    ON deims.data_quality_rules (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        rule_code
    );

COMMENT ON COLUMN deims.data_quality_rules.rule_expression IS
    'Declarative, allow-listed validation DSL only; never execute this JSON as SQL or code.';

CREATE TABLE IF NOT EXISTS deims.data_quality_results (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    evaluation_id uuid NOT NULL,
    rule_id uuid NOT NULL REFERENCES deims.data_quality_rules(id),
...
CREATE INDEX IF NOT EXISTS ix_data_quality_results_evaluation
    ON deims.data_quality_results (tenant_id, evaluation_id, rule_id, status);

CREATE TABLE IF NOT EXISTS deims.etl_watermarks (
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
