-- DEIMS enterprise WMS schema - lot/serial/LPN, stock dimensions, immutable inventory ledger,
...
CREATE TABLE IF NOT EXISTS deims.inventory_lots (
    ...
    potency_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    ...
    FOREIGN KEY (tenant_id, source_receipt_line_id)
        REFERENCES deims.receipt_lines(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, source_lot_observation_id)
        REFERENCES deims.receipt_lot_observations(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_lot_attributes (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    inventory_lot_id uuid NOT NULL,
    attribute_definition_id uuid REFERENCES deims.item_lot_attribute_definitions(id),
    ...
    FOREIGN KEY (tenant_id, inventory_lot_id)
        REFERENCES deims.inventory_lots(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_lot_genealogy (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.serial_numbers (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    item_id uuid NOT NULL REFERENCES deims.items(id),
    ...
    FOREIGN KEY (tenant_id, source_serial_observation_id)
        REFERENCES deims.receipt_serial_observations(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.serial_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, serial_number_id)
        REFERENCES deims.serial_numbers(tenant_id, id) ON DELETE RESTRICT,
    UNIQUE (serial_number_id, event_sequence),
    CONSTRAINT ck_serial_history_sequence CHECK (event_sequence > 0)
);

CREATE TABLE IF NOT EXISTS deims.lpn_types (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS deims.lpns (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    lpn_no varchar(200) NOT NULL,
    sscc varchar(30),
    lpn_type_id uuid NOT NULL REFERENCES deims.lpn_types(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    owner_partner_id uuid REFERENCES deims.business_partners(id),
    parent_lpn_id uuid,
    current_location_id uuid REFERENCES deims.warehouse_locations(id),
    ...
    FOREIGN KEY (tenant_id, parent_lpn_id)
        REFERENCES deims.lpns(tenant_id, id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_lpns_sscc
    ON deims.lpns (tenant_id, sscc)
    WHERE sscc IS NOT NULL AND deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS deims.lpn_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.lpn_location_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    lpn_id uuid NOT NULL,
    event_sequence integer NOT NULL,
    from_location_id uuid REFERENCES deims.warehouse_locations(id),
    to_location_id uuid REFERENCES deims.warehouse_locations(id),
...
CREATE TABLE IF NOT EXISTS deims.lpn_relationship_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.stock_buckets (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_transaction_headers (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_inventory_tx_reversal
    ON deims.inventory_transaction_headers (reversal_of_transaction_id)
    WHERE reversal_of_transaction_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS deims.inventory_transaction_entries (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    entered_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
    catch_weight numeric(24,8),
    catch_weight_uom_code varchar(20) REFERENCES deims.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS deims.inventory_balances (
...
CREATE OR REPLACE VIEW deims.inventory_available_balances AS
SELECT
    b.tenant_id,
    b.stock_bucket_id,
    sb.owner_partner_id,
...
FROM deims.inventory_balances b
JOIN deims.stock_buckets sb ON sb.id = b.stock_bucket_id
WHERE sb.account_type = 'PHYSICAL';

CREATE OR REPLACE VIEW deims.lpn_current_contents AS
SELECT
    sb.tenant_id,
    sb.lpn_id,
    sb.stock_bucket_id,
    ...
    sb.serial_number_id,
    b.on_hand_quantity
FROM deims.inventory_available_balances sb
JOIN deims.inventory_balances b ON b.stock_bucket_id = sb.stock_bucket_id
WHERE sb.lpn_id IS NOT NULL AND b.on_hand_quantity <> 0;

CREATE TABLE IF NOT EXISTS deims.inventory_reservations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_reservation_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inventory_reservation_id)
        REFERENCES deims.inventory_reservations(tenant_id, id) ON DELETE RESTRICT,
    UNIQUE (inventory_reservation_id, event_sequence),
    CONSTRAINT ck_inventory_reservation_hist_seq CHECK (event_sequence > 0)
);

CREATE TABLE IF NOT EXISTS deims.inventory_holds (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    placed_by uuid REFERENCES deims.users(id),
    ...
    FOREIGN KEY (tenant_id, quality_hold_id)
        REFERENCES deims.quality_holds(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_hold_releases (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inventory_hold_id)
        REFERENCES deims.inventory_holds(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.receipt_inventory_postings (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, receipt_id)
        REFERENCES deims.receipts(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inventory_transaction_id)
        REFERENCES deims.inventory_transaction_headers(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, reversed_by_posting_id)
        REFERENCES deims.receipt_inventory_postings(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.receipt_inventory_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
BEGIN
    SELECT constraint_row.conname INTO legacy_constraint
     ...
     WHERE constraint_row.conrelid = 'deims.receipt_inventory_allocations'::regclass
       AND constraint_row.contype = 'u'
       AND constraint_row.conkey = ARRAY[
           (SELECT attnum FROM pg_attribute
             WHERE attrelid = 'deims.receipt_inventory_allocations'::regclass
    ...
    IF legacy_constraint IS NOT NULL THEN
        EXECUTE format('ALTER TABLE deims.receipt_inventory_allocations DROP CONSTRAINT %I',
                       legacy_constraint);
    END IF;
    IF NOT EXISTS (
        SELECT 1
        ...
        ALTER TABLE deims.receipt_inventory_allocations
            ADD CONSTRAINT uq_deims_receipt_inventory_allocation_split
            UNIQUE (receipt_inventory_posting_id, receipt_line_id, inventory_transaction_entry_id);
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS deims.supplier_return_inventory_allocations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, supplier_return_order_line_id)
        REFERENCES deims.supplier_return_order_lines(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.stock_transfer_orders (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    transfer_order_no varchar(100) NOT NULL,
    transfer_type varchar(30) NOT NULL DEFAULT 'INTERNAL',
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    source_warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    destination_warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    ...
    in_transit_location_id uuid REFERENCES deims.warehouse_locations(id),
    source_system_id uuid REFERENCES deims.external_systems(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_stock_transfer_external
    ON deims.stock_transfer_orders (tenant_id, source_system_id, source_document_id)
    WHERE source_system_id IS NOT NULL AND source_document_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS deims.stock_transfer_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    requested_inventory_status_id uuid REFERENCES deims.inventory_statuses(id),
    destination_inventory_status_id uuid REFERENCES deims.inventory_statuses(id),
    ...
    FOREIGN KEY (tenant_id, stock_transfer_order_id)
        REFERENCES deims.stock_transfer_orders(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.stock_transfer_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    stock_transfer_order_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.movement_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    destination_location_id uuid REFERENCES deims.warehouse_locations(id),
    destination_inventory_status_id uuid REFERENCES deims.inventory_statuses(id),
    ...
    assigned_equipment_id uuid REFERENCES deims.material_handling_equipment(id),
...
CREATE TABLE IF NOT EXISTS deims.movement_confirmations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    base_uom_code varchar(20) NOT NULL REFERENCES deims.units_of_measure(uom_code),
    inventory_transaction_id uuid NOT NULL,
    confirmed_by uuid REFERENCES deims.users(id),
    ...
    FOREIGN KEY (tenant_id, movement_task_id)
        REFERENCES deims.movement_tasks(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.movement_task_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    movement_task_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.putaway_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    putaway_request_id uuid NOT NULL,
    movement_task_id uuid NOT NULL,
    strategy_id uuid REFERENCES deims.putaway_strategies(id),
    suggested_location_id uuid REFERENCES deims.warehouse_locations(id),
    suggestion_rank integer,
    capacity_snapshot jsonb NOT NULL DEFAULT '{}'::jsonb,
    rule_trace jsonb NOT NULL DEFAULT '[]'::jsonb,
    override_reason_code_id uuid REFERENCES deims.reason_codes(id),
    override_by uuid REFERENCES deims.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    FOREIGN KEY (tenant_id, putaway_request_id)
        REFERENCES deims.putaway_requests(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, movement_task_id)
        REFERENCES deims.movement_tasks(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.putaway_confirmations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    putaway_task_id uuid NOT NULL,
    movement_confirmation_id uuid NOT NULL,
    destination_location_id uuid NOT NULL REFERENCES deims.warehouse_locations(id),
    confirmed_quantity numeric(24,8) NOT NULL,
    base_uom_code varchar(20) NOT NULL REFERENCES deims.units_of_measure(uom_code),
    confirmed_by uuid REFERENCES deims.users(id),
    confirmed_at timestamptz NOT NULL DEFAULT now(),
    FOREIGN KEY (tenant_id, putaway_task_id)
        REFERENCES deims.putaway_tasks(tenant_id, id) ON DELETE RESTRICT,
    UNIQUE (movement_confirmation_id),
    CONSTRAINT ck_putaway_confirmation_qty CHECK (confirmed_quantity > 0)
);

CREATE TABLE IF NOT EXISTS deims.replenishment_requests (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    owner_partner_id uuid NOT NULL REFERENCES deims.business_partners(id),
    item_id uuid NOT NULL REFERENCES deims.items(id),
    replenishment_policy_id uuid REFERENCES deims.replenishment_policies(id),
...
CREATE TABLE IF NOT EXISTS deims.replenishment_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, replenishment_request_id)
        REFERENCES deims.replenishment_requests(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, movement_task_id)
        REFERENCES deims.movement_tasks(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, source_stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_adjustments (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    reason_code_id uuid NOT NULL REFERENCES deims.reason_codes(id),
    reason_text text,
...
CREATE TABLE IF NOT EXISTS deims.inventory_adjustment_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    currency_code char(3) REFERENCES deims.currencies(currency_code),
    target_inventory_status_id uuid REFERENCES deims.inventory_statuses(id),
    target_owner_partner_id uuid REFERENCES deims.business_partners(id),
    inventory_transaction_entry_id uuid,
    note text,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    ...
    FOREIGN KEY (tenant_id, inventory_adjustment_id)
        REFERENCES deims.inventory_adjustments(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inventory_transaction_entry_id)
        REFERENCES deims.inventory_transaction_entries(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_adjustment_approvals (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    inventory_adjustment_id uuid NOT NULL,
    approval_sequence integer NOT NULL,
    approver_user_id uuid NOT NULL REFERENCES deims.users(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_adjustment_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    inventory_adjustment_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_plans (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_scopes (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    cycle_count_plan_id uuid NOT NULL,
    scope_sequence integer NOT NULL,
    owner_partner_id uuid REFERENCES deims.business_partners(id),
    zone_id uuid REFERENCES deims.warehouse_zones(id),
    location_id uuid REFERENCES deims.warehouse_locations(id),
    item_id uuid REFERENCES deims.items(id),
    inventory_lot_id uuid,
    lpn_id uuid,
    inventory_status_id uuid REFERENCES deims.inventory_statuses(id),
    ...
    FOREIGN KEY (tenant_id, cycle_count_plan_id)
        REFERENCES deims.cycle_count_plans(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inventory_lot_id)
        REFERENCES deims.inventory_lots(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, lpn_id)
        REFERENCES deims.lpns(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_tasks (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    warehouse_id uuid NOT NULL REFERENCES deims.warehouses(id),
    location_id uuid NOT NULL REFERENCES deims.warehouse_locations(id),
    owner_partner_id uuid REFERENCES deims.business_partners(id),
    ...
    FOREIGN KEY (tenant_id, cycle_count_scope_id)
        REFERENCES deims.cycle_count_scopes(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_observations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    cycle_count_task_id uuid NOT NULL,
    observation_sequence integer NOT NULL,
    stock_bucket_id uuid,
    scanned_item_id uuid NOT NULL REFERENCES deims.items(id),
    ...
    FOREIGN KEY (tenant_id, cycle_count_task_id)
        REFERENCES deims.cycle_count_tasks(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_results (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    cycle_count_task_id uuid NOT NULL,
    stock_bucket_id uuid,
    ...
    FOREIGN KEY (tenant_id, inventory_adjustment_line_id)
        REFERENCES deims.inventory_adjustment_lines(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.cycle_count_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    cycle_count_plan_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.inventory_freezes (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    status varchar(20) NOT NULL DEFAULT 'ACTIVE',
    frozen_by uuid REFERENCES deims.users(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_snapshots (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_snapshot_lines (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inventory_snapshot_id)
        REFERENCES deims.inventory_snapshots(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, last_transaction_id)
        REFERENCES deims.inventory_transaction_headers(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_valuation_layers (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_valuation_movements (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, valuation_layer_id)
        REFERENCES deims.inventory_valuation_layers(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inventory_transaction_entry_id)
        REFERENCES deims.inventory_transaction_entries(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_transformations (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_transformation_inputs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inventory_transformation_id)
        REFERENCES deims.inventory_transformations(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, source_stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inventory_transaction_entry_id)
        REFERENCES deims.inventory_transaction_entries(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_transformation_outputs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    inventory_transformation_id uuid NOT NULL,
    line_no integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.inventory_transformation_status_history (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    inventory_transformation_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS deims.inventory_reconciliation_runs (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
...
CREATE TABLE IF NOT EXISTS deims.inventory_reconciliation_details (
    id uuid PRIMARY KEY DEFAULT deims.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES deims.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inventory_reconciliation_run_id)
        REFERENCES deims.inventory_reconciliation_runs(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, stock_bucket_id)
        REFERENCES deims.stock_buckets(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, serial_number_id)
        REFERENCES deims.serial_numbers(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS deims.inventory_write_authorizations (
...
REVOKE ALL ON deims.inventory_write_authorizations FROM PUBLIC;
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON deims.inventory_balances FROM PUBLIC;
REVOKE UPDATE (posting_status, posted_at, posted_by) ON deims.inventory_transaction_headers FROM PUBLIC;

ALTER TABLE deims.serial_status_history
...
ALTER TABLE deims.lpn_status_history
...
ALTER TABLE deims.lpn_location_history
...
CREATE OR REPLACE FUNCTION deims.open_inventory_write_authorization(p_scope varchar)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
DECLARE
    new_token uuid := deims.generate_uuid();
BEGIN
    ...
    END IF;
    DELETE FROM deims.inventory_write_authorizations
     WHERE expires_at <= now()
        OR (backend_pid = pg_backend_pid()
            AND transaction_id = txid_current()
            AND authorization_scope = p_scope);
    INSERT INTO deims.inventory_write_authorizations (
    ...
    PERFORM set_config('deims.inventory_write_token', new_token::text, true);
    RETURN new_token;
END;
$function$;

CREATE OR REPLACE FUNCTION deims.close_inventory_write_authorization(p_token uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    DELETE FROM deims.inventory_write_authorizations
     WHERE authorization_token = p_token
       ...
       AND transaction_id = txid_current();
    PERFORM set_config('deims.inventory_write_token', '', true);
END;
$function$;

CREATE OR REPLACE FUNCTION deims.assert_inventory_write_authorized(p_scope varchar DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
DECLARE
    ...
    supplied_token uuid;
BEGIN
    token_text := NULLIF(current_setting('deims.inventory_write_token', true), '');
    ...
    IF NOT EXISTS (
        SELECT 1
          FROM deims.inventory_write_authorizations a
...
REVOKE ALL ON FUNCTION deims.open_inventory_write_authorization(varchar) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.close_inventory_write_authorization(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_inventory_write_authorized(varchar) FROM PUBLIC;

CREATE OR REPLACE FUNCTION deims.validate_lpn_parent()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION deims.validate_stock_bucket()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    item_row deims.items%ROWTYPE;
    lot_row deims.inventory_lots%ROWTYPE;
    serial_row deims.serial_numbers%ROWTYPE;
    lpn_row deims.lpns%ROWTYPE;
    location_warehouse_id uuid;
    client_row deims.warehouse_clients%ROWTYPE;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM deims.warehouses
    ...
    IF NOT EXISTS (
        SELECT 1 FROM deims.inventory_statuses
    ...
    IF NEW.lpn_id IS NOT NULL THEN
        ...
        IF NOT client_row.allow_mixed_owner_lpn AND EXISTS (
            SELECT 1 FROM deims.stock_buckets b
...
CREATE OR REPLACE FUNCTION deims.protect_stock_bucket_dimensions()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    IF ROW(
        ...
        EXISTS (SELECT 1 FROM deims.inventory_transaction_entries WHERE stock_bucket_id = OLD.id) OR
        EXISTS (
            SELECT 1 FROM deims.inventory_balances
...
CREATE OR REPLACE FUNCTION deims.validate_inventory_entry()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    SELECT posting_status INTO tx_status FROM deims.inventory_transaction_headers WHERE id = tx_id FOR UPDATE;
...
CREATE OR REPLACE FUNCTION deims.guard_inventory_transaction_header()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    IF NEW.posting_status = 'POSTED' THEN
        PERFORM deims.assert_inventory_write_authorized('POSTING');
    END IF;
...
CREATE OR REPLACE FUNCTION deims.guard_inventory_balance_write()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
BEGIN
    PERFORM deims.assert_inventory_write_authorized(NULL);
    IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
    RETURN NEW;
END;
$function$;

CREATE OR REPLACE FUNCTION deims.validate_inventory_balance()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    SELECT sb.account_type, sb.serial_number_id, COALESCE(wc.allow_negative_inventory, false)
      ...
      FROM deims.stock_buckets sb
      LEFT JOIN deims.warehouse_clients wc
...
CREATE OR REPLACE FUNCTION deims.project_inventory_reservation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    END IF;
    PERFORM deims.close_inventory_write_authorization(authorization_token);
    IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
    RETURN NEW;
END;
$function$;

CREATE OR REPLACE FUNCTION deims.project_inventory_hold()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    IF TG_OP IN ('INSERT','UPDATE') AND NEW.released_quantity <> COALESCE((
        SELECT sum(r.released_quantity)
          FROM deims.inventory_hold_releases r
    ...
    END IF;
    authorization_token := deims.open_inventory_write_authorization('HOLD');
...
CREATE OR REPLACE FUNCTION deims.apply_inventory_hold_release()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    hold_row deims.inventory_holds%ROWTYPE;
    total_released numeric(24,8);
BEGIN
    SELECT * INTO hold_row
      FROM deims.inventory_holds
    ...
    SELECT sum(released_quantity) INTO total_released
      FROM deims.inventory_hold_releases
...
CREATE OR REPLACE FUNCTION deims.validate_receipt_inventory_allocation()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, deims
AS $function$
DECLARE
    posting_row deims.receipt_inventory_postings%ROWTYPE;
    receipt_row deims.receipts%ROWTYPE;
    receipt_line_row deims.receipt_lines%ROWTYPE;
    transaction_row deims.inventory_transaction_headers%ROWTYPE;
    entry_row deims.inventory_transaction_entries%ROWTYPE;
    bucket_row deims.stock_buckets%ROWTYPE;
    lot_row deims.inventory_lots%ROWTYPE;
    serial_row deims.serial_numbers%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO posting_row
      FROM deims.receipt_inventory_postings
    ...
    IF NEW.receipt_lot_observation_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM deims.receipt_lot_observations
    ...
    IF NEW.receipt_serial_observation_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM deims.receipt_serial_observations
    ...
    SELECT COALESCE(sum(allocated_quantity), 0) INTO existing_line_quantity
      FROM deims.receipt_inventory_allocations allocation
      JOIN deims.receipt_inventory_postings other_posting
    ...
    SELECT COALESCE(sum(allocated_quantity), 0) INTO existing_entry_quantity
      FROM deims.receipt_inventory_allocations
...
CREATE OR REPLACE FUNCTION deims.assert_receipt_inventory_posting(
...
BEGIN
    ...
    IF posting_row.posting_type = 'REVERSAL' THEN
        ...
        END IF;
        IF EXISTS (
            SELECT 1 FROM deims.receipt_inventory_allocations allocation
        ...
        END IF;
        PERFORM 1
          FROM deims.inventory_transaction_headers original_transaction
          JOIN deims.receipt_inventory_postings original_posting
    ...
    IF posting_row.status = 'REVERSED' THEN
        ...
        END IF;
        PERFORM 1
          FROM deims.receipt_inventory_postings reversal_posting
          JOIN deims.inventory_transaction_headers reversal_transaction
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.receipt_inventory_allocations allocation
          JOIN deims.receipt_lines receipt_line ON receipt_line.id = allocation.receipt_line_id
          JOIN deims.inventory_transaction_entries entry
            ON entry.id = allocation.inventory_transaction_entry_id
          JOIN deims.stock_buckets bucket ON bucket.id = allocation.destination_stock_bucket_id
         WHERE allocation.receipt_inventory_posting_id = posting_row.id
           AND (
               ...
               OR (allocation.inventory_lot_id IS NOT NULL AND NOT EXISTS (
                   SELECT 1 FROM deims.inventory_lots inventory_lot
               ...
               OR (allocation.serial_number_id IS NOT NULL AND NOT EXISTS (
                   SELECT 1 FROM deims.serial_numbers serial_number
...
CREATE OR REPLACE FUNCTION deims.assert_receipt_inventory_transaction(
...
BEGIN
    ...
    IF transaction_row.transaction_type = 'RECEIPT' THEN
        SELECT count(*), min(posting.id::text)::uuid
          ...
          FROM deims.receipt_inventory_postings posting
        ...
        IF posting_count <> 1 THEN
            RAISE EXCEPTION 'POSTED RECEIPT transaction % requires exactly one final receipt posting',
                transaction_row.id USING ERRCODE = '23514';
        END IF;
        PERFORM deims.assert_receipt_inventory_posting(posting_id);
...
CREATE OR REPLACE FUNCTION deims.assert_receipt_document_inventory(
    p_receipt_id uuid
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
    ...
    SELECT * INTO receipt_row FROM deims.receipts WHERE id = p_receipt_id FOR SHARE;
    ...
    LOOP
        SELECT COALESCE(sum(allocation.allocated_quantity), 0)
          ...
          FROM deims.receipt_inventory_allocations allocation
          JOIN deims.receipt_inventory_postings posting
    ...
    END LOOP;
    FOR posting_row IN
        SELECT id FROM deims.receipt_inventory_postings
         WHERE receipt_id = receipt_row.id AND status IN ('POSTED','REVERSED')
         ORDER BY id
    LOOP
        PERFORM deims.assert_receipt_inventory_posting(posting_row.id);
    END LOOP;
END;
$function$;

CREATE OR REPLACE FUNCTION deims.guard_receipt_inventory_posting_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
CREATE OR REPLACE FUNCTION deims.guard_receipt_inventory_allocation_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    LOOP
        SELECT posting.status, transaction.posting_status
          ...
          FROM deims.receipt_inventory_postings posting
          JOIN deims.inventory_transaction_headers transaction
...
CREATE OR REPLACE FUNCTION deims.deferred_check_receipt_inventory_integrity()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deims
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'receipt_inventory_allocations' THEN
        ...
        SELECT inventory_transaction_id, receipt_id
          ...
          FROM deims.receipt_inventory_postings WHERE id = old_posting_id;
        SELECT inventory_transaction_id, receipt_id
          INTO new_transaction_id, new_receipt_id
          FROM deims.receipt_inventory_postings WHERE id = new_posting_id;
    ...
    ELSIF TG_TABLE_NAME IN ('inventory_lots','serial_numbers') THEN
        ...
        SELECT receipt_id INTO old_receipt_id
          FROM deims.receipt_lines WHERE id = old_receipt_line_id;
        SELECT receipt_id INTO new_receipt_id
          FROM deims.receipt_lines WHERE id = new_receipt_line_id;
    END IF;

    PERFORM deims.assert_receipt_inventory_posting(old_posting_id);
    IF new_posting_id IS DISTINCT FROM old_posting_id THEN
        PERFORM deims.assert_receipt_inventory_posting(new_posting_id);
    END IF;
    PERFORM deims.assert_receipt_inventory_transaction(old_transaction_id);
    IF new_transaction_id IS DISTINCT FROM old_transaction_id THEN
        PERFORM deims.assert_receipt_inventory_transaction(new_transaction_id);
    END IF;
    PERFORM deims.assert_receipt_document_inventory(old_receipt_id);
    IF new_receipt_id IS DISTINCT FROM old_receipt_id THEN
        PERFORM deims.assert_receipt_document_inventory(new_receipt_id);
    END IF;
    RETURN NULL;
END;
$function$;

REVOKE ALL ON FUNCTION deims.assert_receipt_inventory_posting(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_receipt_inventory_transaction(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.assert_receipt_document_inventory(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_receipt_inventory_posting_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.guard_receipt_inventory_allocation_mutation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.deferred_check_receipt_inventory_integrity() FROM PUBLIC;

CREATE OR REPLACE FUNCTION deims.validate_supplier_return_inventory_allocation()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, deims
AS $function$
...
BEGIN
    ...
    END IF;
    IF EXISTS (
        SELECT 1 FROM deims.inventory_transaction_headers h
...
CREATE OR REPLACE FUNCTION deims.post_inventory_transaction(
    p_inventory_transaction_id uuid,
    p_posted_by uuid
)
RETURNS void
...
AS $function$
DECLARE
    tx_row deims.inventory_transaction_headers%ROWTYPE;
    entry_count integer;
...
BEGIN
    ...
    IF p_posted_by IS NULL OR NOT EXISTS (
        SELECT 1 FROM deims.users WHERE id = p_posted_by AND status = 'ACTIVE'
    ) THEN
    ...
    IF tx_row.transaction_type = 'REVERSAL' THEN
        IF NOT EXISTS (
            SELECT 1 FROM deims.inventory_transaction_headers original
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.inventory_transaction_entries e
          JOIN deims.stock_buckets b ON b.id = e.stock_bucket_id
          JOIN deims.lpns l ON l.id = b.lpn_id AND l.sealed
         WHERE e.inventory_transaction_id = tx_row.id
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.inventory_transaction_entries e
          JOIN deims.stock_buckets b ON b.id = e.stock_bucket_id
          JOIN deims.inventory_freezes f
    ...
    authorization_token := deims.open_inventory_write_authorization('POSTING');
    INSERT INTO deims.inventory_balances (stock_bucket_id, tenant_id)
    ...
    PERFORM 1
      FROM deims.inventory_balances b
      JOIN deims.inventory_transaction_entries e ON e.stock_bucket_id = b.stock_bucket_id
     WHERE e.inventory_transaction_id = tx_row.id
    ...
    IF EXISTS (
        SELECT 1
          FROM deims.stock_buckets b
          JOIN deims.inventory_balances bal ON bal.stock_bucket_id = b.id
         WHERE b.tenant_id = tx_row.tenant_id
           AND b.account_type = 'PHYSICAL'
           AND b.serial_number_id IN (
               SELECT b2.serial_number_id
                 FROM deims.inventory_transaction_entries e2
                 JOIN deims.stock_buckets b2 ON b2.id = e2.stock_bucket_id
    ...
    UPDATE deims.inventory_transaction_headers
...
REVOKE ALL ON FUNCTION deims.post_inventory_transaction(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.project_inventory_reservation() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.project_inventory_hold() FROM PUBLIC;
REVOKE ALL ON FUNCTION deims.apply_inventory_hold_release() FROM PUBLIC;

-- Operational and reconciliation indexes.
CREATE INDEX IF NOT EXISTS ix_inventory_lots_lookup
    ON deims.inventory_lots (tenant_id, owner_partner_id, item_id, status, expires_on, id);
CREATE INDEX IF NOT EXISTS ix_inventory_lots_supplier_lot
    ON deims.inventory_lots (tenant_id, supplier_lot_no, item_id)
    WHERE supplier_lot_no IS NOT NULL AND deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_serial_numbers_lookup
    ON deims.serial_numbers (tenant_id, owner_partner_id, serial_no, status);
CREATE INDEX IF NOT EXISTS ix_lpns_worklist
    ON deims.lpns (tenant_id, warehouse_id, status, current_location_id, id);
CREATE INDEX IF NOT EXISTS ix_lpns_parent
    ON deims.lpns (tenant_id, parent_lpn_id) WHERE parent_lpn_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_stock_buckets_item_location
    ON deims.stock_buckets (tenant_id, owner_partner_id, warehouse_id, item_id, location_id, inventory_status_id);
CREATE INDEX IF NOT EXISTS ix_stock_buckets_lot
    ON deims.stock_buckets (tenant_id, inventory_lot_id, warehouse_id, location_id)
    WHERE inventory_lot_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_stock_buckets_serial
    ON deims.stock_buckets (tenant_id, serial_number_id)
    WHERE serial_number_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_stock_buckets_lpn
    ON deims.stock_buckets (tenant_id, lpn_id, location_id)
    WHERE lpn_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_stock_buckets_bonded
    ON deims.stock_buckets (tenant_id, warehouse_id, customs_legal_status_code, bonded_cargo_reference)
...
CREATE INDEX IF NOT EXISTS ix_inventory_balances_nonzero
    ON deims.inventory_balances (tenant_id, stock_bucket_id)
...
CREATE INDEX IF NOT EXISTS ix_inventory_transactions_worklist
    ON deims.inventory_transaction_headers (tenant_id, warehouse_id, owner_partner_id, posting_status, occurred_at DESC, id);
CREATE INDEX IF NOT EXISTS ix_inventory_transactions_source
    ON deims.inventory_transaction_headers (tenant_id, source_document_type, source_document_id, source_document_line_id);
CREATE INDEX IF NOT EXISTS ix_inventory_entries_bucket_time
    ON deims.inventory_transaction_entries (tenant_id, stock_bucket_id, inventory_transaction_id);
CREATE INDEX IF NOT EXISTS ix_inventory_entries_item
    ON deims.inventory_transaction_entries (tenant_id, item_id, inventory_transaction_id);
CREATE INDEX IF NOT EXISTS ix_inventory_reservations_active
    ON deims.inventory_reservations (tenant_id, stock_bucket_id, expires_at, priority, id)
    WHERE status IN ('ACTIVE','PARTIALLY_CONSUMED');
CREATE INDEX IF NOT EXISTS ix_inventory_reservations_demand
    ON deims.inventory_reservations (tenant_id, demand_document_type, demand_document_id, demand_line_id);
CREATE INDEX IF NOT EXISTS ix_inventory_holds_active
    ON deims.inventory_holds (tenant_id, stock_bucket_id, review_due_at, id)
    WHERE status IN ('ACTIVE','PARTIALLY_RELEASED');
CREATE INDEX IF NOT EXISTS ix_movement_tasks_queue
    ON deims.movement_tasks (tenant_id, warehouse_id, status, priority, planned_start_at, id);
CREATE INDEX IF NOT EXISTS ix_movement_tasks_source
    ON deims.movement_tasks (tenant_id, source_stock_bucket_id, status);
CREATE INDEX IF NOT EXISTS ix_putaway_tasks_request
    ON deims.putaway_tasks (tenant_id, putaway_request_id, movement_task_id);
CREATE INDEX IF NOT EXISTS ix_replenishment_requests_queue
    ON deims.replenishment_requests (tenant_id, warehouse_id, status, priority, required_at, id);
CREATE INDEX IF NOT EXISTS ix_adjustments_worklist
    ON deims.inventory_adjustments (tenant_id, warehouse_id, owner_partner_id, status, requested_at DESC);
CREATE INDEX IF NOT EXISTS ix_cycle_count_tasks_queue
    ON deims.cycle_count_tasks (tenant_id, warehouse_id, status, priority, assigned_at, id);
CREATE INDEX IF NOT EXISTS ix_inventory_freezes_active
    ON deims.inventory_freezes (tenant_id, warehouse_id, status, expires_at)
    WHERE status = 'ACTIVE';
CREATE INDEX IF NOT EXISTS ix_inventory_snapshots_asof
    ON deims.inventory_snapshots (tenant_id, warehouse_id, owner_partner_id, as_of_at DESC);
CREATE INDEX IF NOT EXISTS ix_valuation_layers_open
    ON deims.inventory_valuation_layers (tenant_id, owner_partner_id, warehouse_id, item_id, received_at, id)
    WHERE status = 'OPEN' AND remaining_quantity > 0;
CREATE INDEX IF NOT EXISTS ix_reconciliation_details_open
    ON deims.inventory_reconciliation_details (tenant_id, inventory_reconciliation_run_id, severity, exception_code)
    WHERE status IN ('OPEN','ACKNOWLEDGED');

DROP TRIGGER IF EXISTS trg_validate_lpn_parent ON deims.lpns;
CREATE TRIGGER trg_validate_lpn_parent
BEFORE INSERT OR UPDATE OF tenant_id, parent_lpn_id, warehouse_id
ON deims.lpns FOR EACH ROW EXECUTE PROCEDURE deims.validate_lpn_parent();

DROP TRIGGER IF EXISTS trg_protect_stock_bucket_dimensions ON deims.stock_buckets;
CREATE TRIGGER trg_protect_stock_bucket_dimensions
BEFORE UPDATE ON deims.stock_buckets
FOR EACH ROW EXECUTE PROCEDURE deims.protect_stock_bucket_dimensions();

DROP TRIGGER IF EXISTS trg_validate_stock_bucket ON deims.stock_buckets;
CREATE TRIGGER trg_validate_stock_bucket
BEFORE INSERT OR UPDATE ON deims.stock_buckets
FOR EACH ROW EXECUTE PROCEDURE deims.validate_stock_bucket();

DROP TRIGGER IF EXISTS trg_validate_inventory_entry ON deims.inventory_transaction_entries;
CREATE TRIGGER trg_validate_inventory_entry
BEFORE INSERT OR UPDATE ON deims.inventory_transaction_entries
FOR EACH ROW EXECUTE PROCEDURE deims.validate_inventory_entry();

DROP TRIGGER IF EXISTS trg_guard_inventory_entry_mutation ON deims.inventory_transaction_entries;
CREATE TRIGGER trg_guard_inventory_entry_mutation
BEFORE UPDATE OR DELETE ON deims.inventory_transaction_entries
FOR EACH ROW EXECUTE PROCEDURE deims.guard_inventory_entry_mutation();

DROP TRIGGER IF EXISTS trg_guard_inventory_tx_header ON deims.inventory_transaction_headers;
CREATE TRIGGER trg_guard_inventory_tx_header
BEFORE UPDATE OR DELETE ON deims.inventory_transaction_headers
FOR EACH ROW EXECUTE PROCEDURE deims.guard_inventory_transaction_header();

DROP TRIGGER IF EXISTS trg_guard_inventory_balance_write ON deims.inventory_balances;
CREATE TRIGGER trg_guard_inventory_balance_write
BEFORE INSERT OR UPDATE OR DELETE ON deims.inventory_balances
FOR EACH ROW EXECUTE PROCEDURE deims.guard_inventory_balance_write();

DROP TRIGGER IF EXISTS trg_validate_inventory_balance ON deims.inventory_balances;
CREATE TRIGGER trg_validate_inventory_balance
BEFORE INSERT OR UPDATE ON deims.inventory_balances
FOR EACH ROW EXECUTE PROCEDURE deims.validate_inventory_balance();

DROP TRIGGER IF EXISTS trg_project_inventory_reservation ON deims.inventory_reservations;
CREATE TRIGGER trg_project_inventory_reservation
AFTER INSERT OR UPDATE OR DELETE ON deims.inventory_reservations
FOR EACH ROW EXECUTE PROCEDURE deims.project_inventory_reservation();

DROP TRIGGER IF EXISTS trg_project_inventory_hold ON deims.inventory_holds;
CREATE TRIGGER trg_project_inventory_hold
AFTER INSERT OR UPDATE OR DELETE ON deims.inventory_holds
FOR EACH ROW EXECUTE PROCEDURE deims.project_inventory_hold();

DROP TRIGGER IF EXISTS trg_apply_inventory_hold_release ON deims.inventory_hold_releases;
CREATE TRIGGER trg_apply_inventory_hold_release
AFTER INSERT ON deims.inventory_hold_releases
FOR EACH ROW EXECUTE PROCEDURE deims.apply_inventory_hold_release();

DROP TRIGGER IF EXISTS trg_validate_receipt_inventory_alloc
ON deims.receipt_inventory_allocations;
CREATE TRIGGER trg_validate_receipt_inventory_alloc
BEFORE INSERT OR UPDATE ON deims.receipt_inventory_allocations
FOR EACH ROW EXECUTE PROCEDURE deims.validate_receipt_inventory_allocation();

DROP TRIGGER IF EXISTS trg_guard_receipt_inventory_posting
ON deims.receipt_inventory_postings;
CREATE TRIGGER trg_guard_receipt_inventory_posting
BEFORE UPDATE OR DELETE ON deims.receipt_inventory_postings
FOR EACH ROW EXECUTE PROCEDURE deims.guard_receipt_inventory_posting_mutation();

DROP TRIGGER IF EXISTS trg_guard_receipt_inventory_allocation
ON deims.receipt_inventory_allocations;
CREATE TRIGGER trg_guard_receipt_inventory_allocation
BEFORE UPDATE OR DELETE ON deims.receipt_inventory_allocations
FOR EACH ROW EXECUTE PROCEDURE deims.guard_receipt_inventory_allocation_mutation();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.receipt_inventory_postings
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.receipt_inventory_allocations
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.inventory_transaction_headers
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.inventory_transaction_entries
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();
...
AFTER INSERT OR UPDATE OR DELETE ON deims.receipts
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();

DROP TRIGGER IF EXISTS trg_receipt_line_inventory_integrity
ON deims.receipt_lines;
CREATE CONSTRAINT TRIGGER trg_receipt_line_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.receipt_lines
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();

DROP TRIGGER IF EXISTS trg_receipt_lot_inventory_integrity ON deims.inventory_lots;
CREATE CONSTRAINT TRIGGER trg_receipt_lot_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.inventory_lots
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();

DROP TRIGGER IF EXISTS trg_receipt_serial_inventory_integrity ON deims.serial_numbers;
CREATE CONSTRAINT TRIGGER trg_receipt_serial_inventory_integrity
AFTER INSERT OR UPDATE OR DELETE ON deims.serial_numbers
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE deims.deferred_check_receipt_inventory_integrity();

DROP TRIGGER IF EXISTS trg_prevent_core_truncate ON deims.receipt_inventory_allocations;
CREATE TRIGGER trg_prevent_core_truncate
BEFORE TRUNCATE ON deims.receipt_inventory_allocations
FOR EACH STATEMENT EXECUTE PROCEDURE deims.prevent_core_document_truncate();

COMMENT ON TABLE deims.receipt_inventory_allocations IS
...
ON deims.supplier_return_inventory_allocations;
CREATE TRIGGER trg_validate_supplier_return_alloc
BEFORE INSERT OR UPDATE ON deims.supplier_return_inventory_allocations
FOR EACH ROW EXECUTE PROCEDURE deims.validate_supplier_return_inventory_allocation();
...
BEGIN
    FOREACH table_name IN ARRAY ARRAY[
        ...
        EXECUTE format('DROP TRIGGER IF EXISTS trg_touch_row ON deims.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_touch_row BEFORE UPDATE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.touch_row()', table_name
        );
    END LOOP;
    ...
    FOREACH table_name IN ARRAY ARRAY[
        ...
        EXECUTE format('DROP TRIGGER IF EXISTS trg_append_only ON deims.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_append_only BEFORE UPDATE OR DELETE ON deims.%I '
            'FOR EACH ROW EXECUTE PROCEDURE deims.reject_append_only_change()', table_name
        );
    END LOOP;
END;
$block$;

COMMENT ON FUNCTION deims.post_inventory_transaction(uuid, uuid) IS
...
COMMENT ON TABLE deims.inventory_freezes IS
...
COMMENT ON TABLE deims.inventory_reconciliation_runs IS
