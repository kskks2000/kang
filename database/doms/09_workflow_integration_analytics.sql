-- DOMS workflow, notifications, EDI/webhooks/jobs, audit extensions, and analytics.

CREATE TABLE IF NOT EXISTS doms.status_definitions (
...
CREATE TABLE IF NOT EXISTS doms.entity_tags (
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    tag_id uuid NOT NULL REFERENCES doms.tags(id) ON DELETE CASCADE,
    assigned_by uuid REFERENCES doms.users(id),
    assigned_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, entity_type, entity_id, tag_id)
);

CREATE TABLE IF NOT EXISTS doms.notification_templates (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.notifications (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    event_type varchar(100) NOT NULL,
    template_id uuid REFERENCES doms.notification_templates(id),
    ...
    CONSTRAINT ck_notification_no_secret_variables CHECK (
        NOT doms.jsonb_contains_forbidden_secret_key(variables_masked)
    )
);

CREATE TABLE IF NOT EXISTS doms.notification_deliveries (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    notification_id uuid NOT NULL REFERENCES doms.notifications(id) ON DELETE CASCADE,
    user_id uuid REFERENCES doms.users(id),
    contact_id uuid REFERENCES doms.contacts(id),
    channel varchar(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS doms.edi_documents (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
...
CREATE TABLE IF NOT EXISTS doms.edi_validation_errors (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    edi_document_id uuid NOT NULL REFERENCES doms.edi_documents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.webhook_subscriptions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
...
COMMENT ON TABLE doms.webhook_subscriptions IS
...
CREATE TABLE IF NOT EXISTS doms.webhook_deliveries (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    subscription_id uuid NOT NULL REFERENCES doms.webhook_subscriptions(id) ON DELETE CASCADE,
    outbox_event_id uuid REFERENCES doms.outbox_events(id),
...
CREATE TABLE IF NOT EXISTS doms.inbox_deduplication (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    source_system_id uuid NOT NULL REFERENCES doms.external_systems(id),
...
CREATE TABLE IF NOT EXISTS doms.import_jobs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    job_no varchar(100) NOT NULL,
    import_type varchar(80) NOT NULL,
    source_file_id uuid NOT NULL REFERENCES doms.files(id),
    ...
    result_file_id uuid REFERENCES doms.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS doms.import_job_rows (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    import_job_id uuid NOT NULL REFERENCES doms.import_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.export_jobs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    job_no varchar(100) NOT NULL,
    ...
    output_file_id uuid REFERENCES doms.files(id),
    requested_by uuid REFERENCES doms.users(id),
    requested_at timestamptz NOT NULL DEFAULT now(),
    completed_at timestamptz,
    expires_at timestamptz,
...
CREATE TABLE IF NOT EXISTS doms.export_job_rows (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    export_job_id uuid NOT NULL REFERENCES doms.export_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.scheduled_jobs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_scheduled_job_no_inline_secrets CHECK (
        NOT doms.jsonb_contains_forbidden_secret_key(parameters)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_scheduled_jobs_scope
    ON doms.scheduled_jobs (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), job_code
    );

CREATE TABLE IF NOT EXISTS doms.job_runs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    scheduled_job_id uuid NOT NULL REFERENCES doms.scheduled_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.dead_letter_messages (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    ...
    resolved_by uuid REFERENCES doms.users(id),
    resolution text,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS doms.impersonation_sessions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    administrator_user_id uuid NOT NULL REFERENCES doms.users(id),
    impersonated_user_id uuid NOT NULL REFERENCES doms.users(id),
...
CREATE TABLE IF NOT EXISTS doms.impersonation_events (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    impersonation_session_id uuid NOT NULL REFERENCES doms.impersonation_sessions(id) ON DELETE RESTRICT,
    sequence_no integer NOT NULL,
    event_type varchar(20) NOT NULL,
    actor_user_id uuid NOT NULL REFERENCES doms.users(id),
    reason text,
...
CREATE TABLE IF NOT EXISTS doms.entity_change_logs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS doms.data_access_logs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id),
    actor_user_id uuid REFERENCES doms.users(id),
    actor_api_client_id uuid REFERENCES doms.api_clients(id),
...
-- Order, inventory promise, payment, fulfillment, return, and interface reconciliation.

CREATE TABLE IF NOT EXISTS doms.reconciliation_runs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    reconciliation_no varchar(120) NOT NULL,
    reconciliation_type varchar(40) NOT NULL,
    fulfillment_node_id uuid REFERENCES doms.fulfillment_nodes(id),
...
CREATE TABLE IF NOT EXISTS doms.reconciliation_details (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    reconciliation_run_id uuid NOT NULL REFERENCES doms.reconciliation_runs(id) ON DELETE CASCADE,
    ...
    assigned_to uuid REFERENCES doms.users(id),
    resolved_by uuid REFERENCES doms.users(id),
    resolved_at timestamptz,
    resolution text,
...
CREATE TABLE IF NOT EXISTS doms.workflow_definitions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
...
CREATE TABLE IF NOT EXISTS doms.workflow_versions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    workflow_definition_id uuid NOT NULL REFERENCES doms.workflow_definitions(id) ON DELETE CASCADE,
    version_no integer NOT NULL,
    effective_from timestamptz NOT NULL,
    effective_to timestamptz,
    trigger_conditions jsonb NOT NULL DEFAULT '{}'::jsonb,
    status varchar(20) NOT NULL DEFAULT 'DRAFT',
    published_by uuid REFERENCES doms.users(id),
...
CREATE TABLE IF NOT EXISTS doms.workflow_steps (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES doms.workflow_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.workflow_instances (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    workflow_version_id uuid NOT NULL REFERENCES doms.workflow_versions(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    status varchar(20) NOT NULL DEFAULT 'RUNNING',
    current_step_id uuid REFERENCES doms.workflow_steps(id),
...
CREATE TABLE IF NOT EXISTS doms.workflow_tasks (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    workflow_instance_id uuid NOT NULL REFERENCES doms.workflow_instances(id) ON DELETE CASCADE,
    workflow_step_id uuid NOT NULL REFERENCES doms.workflow_steps(id),
    task_no integer NOT NULL,
    assignee_user_id uuid REFERENCES doms.users(id),
    assignee_group_id uuid REFERENCES doms.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    completed_at timestamptz,
...
CREATE TABLE IF NOT EXISTS doms.approval_policies (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
...
CREATE TABLE IF NOT EXISTS doms.approval_requests (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    request_no varchar(100) NOT NULL,
    policy_id uuid REFERENCES doms.approval_policies(id),
    workflow_instance_id uuid REFERENCES doms.workflow_instances(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    amount numeric(20,4),
    currency_code char(3) REFERENCES doms.currencies(currency_code),
    reason text,
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    requested_by uuid NOT NULL REFERENCES doms.users(id),
    requested_at timestamptz NOT NULL DEFAULT now(),
    completed_at timestamptz,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS doms.approval_steps (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    approval_request_id uuid NOT NULL REFERENCES doms.approval_requests(id) ON DELETE CASCADE,
    level_no integer NOT NULL,
    approver_user_id uuid REFERENCES doms.users(id),
    approver_group_id uuid REFERENCES doms.user_groups(id),
    status varchar(20) NOT NULL DEFAULT 'PENDING',
    assigned_at timestamptz NOT NULL DEFAULT now(),
    due_at timestamptz,
    ...
    decision varchar(20),
    comments text,
    delegated_from_user_id uuid REFERENCES doms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_user
    ON doms.approval_steps (approval_request_id, level_no, approver_user_id)
    WHERE approver_user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_approval_steps_group
    ON doms.approval_steps (approval_request_id, level_no, approver_group_id)
    WHERE approver_group_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS doms.approval_actions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    approval_step_id uuid NOT NULL REFERENCES doms.approval_steps(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS doms.sod_rules (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_sod_rule_scope CHECK (
        scope_type IN ('TENANT','ORGANIZATION','FULFILLMENT_NODE','OWNER','ENTITY')
    )
);
...
CREATE TABLE IF NOT EXISTS doms.sod_rule_conflicts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    sod_rule_id uuid NOT NULL REFERENCES doms.sod_rules(id) ON DELETE CASCADE,
    left_permission_id uuid NOT NULL REFERENCES doms.permissions(id),
    right_permission_id uuid NOT NULL REFERENCES doms.permissions(id),
...
CREATE TABLE IF NOT EXISTS doms.sod_violations (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    sod_rule_id uuid NOT NULL REFERENCES doms.sod_rules(id),
    user_id uuid NOT NULL REFERENCES doms.users(id),
    left_role_assignment_id uuid REFERENCES doms.role_assignments(id),
    right_role_assignment_id uuid REFERENCES doms.role_assignments(id),
    ...
    waived_by uuid REFERENCES doms.users(id),
...
COMMENT ON COLUMN doms.workflow_versions.trigger_conditions IS
    'Declarative, allow-listed workflow DSL only; never execute this JSON as SQL, PL/pgSQL, shell, or template code.';
COMMENT ON COLUMN doms.workflow_steps.assignee_rule IS
    'Declarative, allow-listed assignment DSL only; server-side code must validate operators and fields.';

-- KPI catalog, targets, calculated values, fulfillment-node facts, and data quality.

CREATE TABLE IF NOT EXISTS doms.kpi_definitions (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    ...
    owner_org_unit_id uuid REFERENCES doms.organization_units(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_definitions_scope
    ON doms.kpi_definitions (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        kpi_code, calculation_version
    );

COMMENT ON COLUMN doms.kpi_definitions.calculation_expression IS
    'Declarative, versioned KPI DSL only; never execute as SQL or application code.';

CREATE TABLE IF NOT EXISTS doms.kpi_targets (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES doms.kpi_definitions(id),
    ...
    CONSTRAINT ck_kpi_target_scope CHECK (
        scope_type IN ('TENANT','ORGANIZATION','FULFILLMENT_NODE','OWNER','CLIENT','ITEM','USER','TEAM')
    ),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_targets_scope_period
    ON doms.kpi_targets (
        ...
        COALESCE(fulfillment_node_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(owner_partner_id, '00000000-0000-0000-0000-000000000000'::uuid),
        period_start, period_end
    );

CREATE TABLE IF NOT EXISTS doms.kpi_values (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES doms.kpi_definitions(id),
    scope_type varchar(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS doms.fulfillment_node_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    fulfillment_node_id uuid NOT NULL REFERENCES doms.fulfillment_nodes(id),
    channel_account_id uuid REFERENCES doms.channel_accounts(id),
    owner_partner_id uuid REFERENCES doms.business_partners(id),
    supplier_order_count bigint NOT NULL DEFAULT 0,
    supplier_receipt_count bigint NOT NULL DEFAULT 0,
    customer_order_count bigint NOT NULL DEFAULT 0,
    shipment_count bigint NOT NULL DEFAULT 0,
    return_order_count bigint NOT NULL DEFAULT 0,
    supply_received_quantity numeric(30,8) NOT NULL DEFAULT 0,
    ...
    active_sku_count bigint NOT NULL DEFAULT 0,
    available_sku_count bigint NOT NULL DEFAULT 0,
    exception_count bigint NOT NULL DEFAULT 0,
    on_time_supplier_receipt_count bigint NOT NULL DEFAULT 0,
    on_time_shipment_count bigint NOT NULL DEFAULT 0,
    labor_hours numeric(20,4) NOT NULL DEFAULT 0,
    service_fulfillment_count bigint NOT NULL DEFAULT 0,
    loaded_at timestamptz NOT NULL DEFAULT now(),
    source_watermark timestamptz,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_fulfillment_node_daily_counts CHECK (
        supplier_order_count >= 0 AND supplier_receipt_count >= 0 AND customer_order_count >= 0 AND
        shipment_count >= 0 AND return_order_count >= 0 AND active_sku_count >= 0 AND
        available_sku_count >= 0 AND exception_count >= 0 AND
        on_time_supplier_receipt_count >= 0 AND on_time_shipment_count >= 0 AND
        service_fulfillment_count >= 0
    ),
    CONSTRAINT ck_fulfillment_node_daily_quantities CHECK (
        supply_received_quantity >= 0 AND shipped_unit_quantity >= 0 AND
        returned_unit_quantity >= 0 AND inventory_unit_quantity >= 0 AND labor_hours >= 0
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_fulfillment_node_daily_facts_grain
    ON doms.fulfillment_node_daily_facts (
        tenant_id, fact_date, fulfillment_node_id,
        COALESCE(channel_account_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(owner_partner_id, '00000000-0000-0000-0000-000000000000'::uuid)
    );

CREATE TABLE IF NOT EXISTS doms.inventory_promise_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    fulfillment_node_id uuid NOT NULL REFERENCES doms.fulfillment_nodes(id),
    ...
    inventory_status_id uuid NOT NULL REFERENCES doms.inventory_statuses(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_inventory_promise_daily_facts_grain
    ON doms.inventory_promise_daily_facts (
        tenant_id, fact_date, fulfillment_node_id,
        COALESCE(channel_account_id, '00000000-0000-0000-0000-000000000000'::uuid),
        owner_partner_id, item_id, inventory_status_id
    );

CREATE TABLE IF NOT EXISTS doms.order_operation_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    fulfillment_node_id uuid NOT NULL REFERENCES doms.fulfillment_nodes(id),
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_order_operation_daily_type CHECK (operation_type IN (
        'ORDER_CAPTURE','VALIDATION','RISK_REVIEW','PAYMENT','SOURCING',
        'RESERVATION','ALLOCATION','FULFILLMENT','SHIPMENT','DELIVERY',
        'CANCELLATION','RETURN','REFUND','EXCHANGE','CUSTOMER_SERVICE'
    )),
    CONSTRAINT ck_order_operation_daily_counts CHECK (
    ...
    CONSTRAINT ck_order_operation_daily_measures CHECK (
        processed_unit_quantity >= 0 AND standard_minutes >= 0 AND actual_minutes >= 0 AND labor_hours >= 0
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_order_operation_daily_facts_grain
    ON doms.order_operation_daily_facts (
        tenant_id, fact_date, fulfillment_node_id,
        COALESCE(channel_account_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(owner_partner_id, '00000000-0000-0000-0000-000000000000'::uuid),
        operation_type
...
CREATE TABLE IF NOT EXISTS doms.order_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    sales_channel_id uuid REFERENCES doms.sales_channels(id),
    selling_organization_id uuid REFERENCES doms.selling_organizations(id),
    ...
    ordered_quantity numeric(30,8) NOT NULL DEFAULT 0,
    gross_amount numeric(30,6) NOT NULL DEFAULT 0,
    discount_amount numeric(30,6) NOT NULL DEFAULT 0,
    charge_amount numeric(30,6) NOT NULL DEFAULT 0,
    tax_amount numeric(30,6) NOT NULL DEFAULT 0,
    grand_total_amount numeric(30,6) NOT NULL DEFAULT 0,
    average_confirmation_minutes numeric(20,4),
    loaded_at timestamptz NOT NULL DEFAULT now(),
    source_watermark timestamptz,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_order_daily_facts_counts CHECK (
        order_count >= 0 AND confirmed_order_count >= 0 AND cancelled_order_count >= 0 AND
        order_line_count >= 0 AND ordered_quantity >= 0 AND
        confirmed_order_count <= order_count AND cancelled_order_count <= order_count
    ),
    CONSTRAINT ck_order_daily_facts_amounts CHECK (
        gross_amount >= 0 AND discount_amount >= 0 AND charge_amount >= 0 AND
        tax_amount >= 0 AND grand_total_amount >= 0 AND
        (average_confirmation_minutes IS NULL OR average_confirmation_minutes >= 0)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_order_daily_facts_grain
    ON doms.order_daily_facts (
        tenant_id, fact_date,
        COALESCE(sales_channel_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(selling_organization_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(customer_group_id, '00000000-0000-0000-0000-000000000000'::uuid),
        currency_code
    );

CREATE TABLE IF NOT EXISTS doms.payment_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    sales_channel_id uuid REFERENCES doms.sales_channels(id),
    ...
    authorized_amount numeric(30,6) NOT NULL DEFAULT 0,
    captured_amount numeric(30,6) NOT NULL DEFAULT 0,
    refunded_amount numeric(30,6) NOT NULL DEFAULT 0,
    chargeback_amount numeric(30,6) NOT NULL DEFAULT 0,
    average_authorization_seconds numeric(20,4),
    loaded_at timestamptz NOT NULL DEFAULT now(),
    source_watermark timestamptz,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_payment_daily_facts_counts CHECK (
        authorization_count >= 0 AND capture_count >= 0 AND failed_count >= 0 AND
        refund_count >= 0 AND dispute_count >= 0
    ),
    CONSTRAINT ck_payment_daily_facts_amounts CHECK (
        authorized_amount >= 0 AND captured_amount >= 0 AND refunded_amount >= 0 AND
        chargeback_amount >= 0 AND
        (average_authorization_seconds IS NULL OR average_authorization_seconds >= 0)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_payment_daily_facts_grain
    ON doms.payment_daily_facts (
        tenant_id, fact_date,
        COALESCE(sales_channel_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(provider_code, ''), COALESCE(payment_method_type, ''), currency_code
    );

CREATE TABLE IF NOT EXISTS doms.return_daily_facts (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    fact_date date NOT NULL,
    sales_channel_id uuid REFERENCES doms.sales_channels(id),
    ...
    exchanged_count bigint NOT NULL DEFAULT 0,
    returned_quantity numeric(30,8) NOT NULL DEFAULT 0,
    refund_amount numeric(30,6) NOT NULL DEFAULT 0,
    restock_quantity numeric(30,8) NOT NULL DEFAULT 0,
    scrap_quantity numeric(30,8) NOT NULL DEFAULT 0,
    average_cycle_hours numeric(20,4),
    loaded_at timestamptz NOT NULL DEFAULT now(),
    source_watermark timestamptz,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_return_daily_facts_counts CHECK (
        request_count >= 0 AND authorized_count >= 0 AND received_count >= 0 AND
        rejected_count >= 0 AND exchanged_count >= 0
    ),
    CONSTRAINT ck_return_daily_facts_measures CHECK (
        returned_quantity >= 0 AND refund_amount >= 0 AND restock_quantity >= 0 AND
        scrap_quantity >= 0 AND
        (average_cycle_hours IS NULL OR average_cycle_hours >= 0)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_return_daily_facts_grain
    ON doms.return_daily_facts (
        tenant_id, fact_date,
        COALESCE(sales_channel_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(fulfillment_node_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(reason_code, ''), currency_code
    );

CREATE TABLE IF NOT EXISTS doms.data_quality_rules (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid REFERENCES doms.tenants(id) ON DELETE CASCADE,
    rule_code varchar(100) NOT NULL,
    rule_name varchar(250) NOT NULL,
    ...
    is_active boolean NOT NULL DEFAULT true,
    owner_user_id uuid REFERENCES doms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_data_quality_rules_scope
    ON doms.data_quality_rules (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        rule_code
    );

COMMENT ON COLUMN doms.data_quality_rules.rule_expression IS
    'Declarative, allow-listed validation DSL only; never execute this JSON as SQL or code.';

CREATE TABLE IF NOT EXISTS doms.data_quality_results (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    evaluation_id uuid NOT NULL,
    rule_id uuid NOT NULL REFERENCES doms.data_quality_rules(id),
...
CREATE INDEX IF NOT EXISTS ix_data_quality_results_evaluation
    ON doms.data_quality_results (tenant_id, evaluation_id, rule_id, status);
CREATE UNIQUE INDEX IF NOT EXISTS ux_data_quality_results_entity_evaluation
    ON doms.data_quality_results (
        tenant_id, evaluation_id, rule_id,
        COALESCE(entity_id, '00000000-0000-0000-0000-000000000000'::uuid)
    );

CREATE OR REPLACE FUNCTION doms.validate_data_quality_result_lifecycle()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    IF TG_OP = 'INSERT' AND NEW.status NOT IN ('PASS','WARNING','FAIL','CRITICAL') THEN
        RAISE EXCEPTION 'A data-quality result must start in an evaluated state'
            USING ERRCODE = '23514';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status AND NOT (
        OLD.status IN ('WARNING','FAIL','CRITICAL')
        AND NEW.status IN ('WAIVED','RESOLVED')
    ) THEN
        RAISE EXCEPTION 'Invalid data-quality result transition: % -> %',
            OLD.status, NEW.status
            USING ERRCODE = '23514';
    END IF;
    IF NEW.status IN ('WAIVED','RESOLVED') THEN
        IF NEW.resolved_at IS NULL OR NEW.resolved_by IS NULL
           OR NULLIF(btrim(NEW.resolution), '') IS NULL
           OR NEW.resolved_at < NEW.evaluated_at THEN
            RAISE EXCEPTION 'Resolved or waived data-quality results require valid resolution evidence'
                USING ERRCODE = '23514';
        END IF;
    ELSIF NEW.resolved_at IS NOT NULL OR NEW.resolved_by IS NOT NULL
       OR NEW.resolution IS NOT NULL THEN
        RAISE EXCEPTION 'Open data-quality results cannot carry resolution evidence'
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_validate_data_quality_result_lifecycle
    ON doms.data_quality_results;
CREATE TRIGGER trg_validate_data_quality_result_lifecycle
BEFORE INSERT OR UPDATE OF status, resolved_at, resolved_by, resolution
ON doms.data_quality_results
FOR EACH ROW EXECUTE PROCEDURE doms.validate_data_quality_result_lifecycle();

CREATE TABLE IF NOT EXISTS doms.etl_watermarks (
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
...
CREATE TABLE IF NOT EXISTS doms.data_subject_requests (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    request_no varchar(100) NOT NULL,
    customer_id uuid REFERENCES doms.customers(id) ON DELETE RESTRICT,
    request_type varchar(30) NOT NULL,
    request_channel varchar(30) NOT NULL,
    requester_identity_hash char(64),
    identity_verification_status varchar(20) NOT NULL DEFAULT 'PENDING',
    identity_evidence_file_id uuid REFERENCES doms.files(id) ON DELETE RESTRICT,
    ...
    completed_at timestamptz,
    response_object_uri text,
    response_object_sha256 char(64),
    response_object_size_bytes bigint,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, request_no),
    CONSTRAINT ck_data_subject_requests_type CHECK (request_type IN (
        'ACCESS','ERASURE','PORTABILITY','CORRECTION','RESTRICTION','OBJECTION','CONSENT_WITHDRAWAL'
    )),
    CONSTRAINT ck_data_subject_requests_channel CHECK (request_channel IN ('WEB','EMAIL','PHONE','MAIL','AGENT','REGULATOR','OTHER')),
    CONSTRAINT ck_data_subject_requests_verification CHECK (identity_verification_status IN ('PENDING','VERIFIED','FAILED','WAIVED')),
    CONSTRAINT ck_data_subject_requests_status CHECK (status IN ('RECEIVED','IDENTITY_CHECK','IN_REVIEW','BLOCKED_LEGAL_HOLD','APPROVED','PARTIALLY_APPROVED','DENIED','PROCESSING','COMPLETED','CANCELLED')),
    CONSTRAINT ck_data_subject_requests_dates CHECK (
        due_at >= received_at AND (completed_at IS NULL OR completed_at >= received_at)
    ),
    CONSTRAINT ck_data_subject_requests_result_object CHECK (
        (response_object_uri IS NULL AND response_object_sha256 IS NULL AND response_object_size_bytes IS NULL) OR
        (response_object_uri IS NOT NULL AND response_object_sha256 ~ '^[0-9A-Fa-f]{64}$' AND response_object_size_bytes >= 0)
    ),
    CONSTRAINT ck_data_subject_requests_completion CHECK (status <> 'COMPLETED' OR completed_at IS NOT NULL)
);

CREATE TABLE IF NOT EXISTS doms.data_subject_request_items (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    data_subject_request_id uuid NOT NULL REFERENCES doms.data_subject_requests(id) ON DELETE RESTRICT,
    entity_type varchar(80) NOT NULL,
    entity_id uuid,
    ...
    record_count bigint,
    processed_at timestamptz,
    result_detail_masked jsonb NOT NULL DEFAULT '{}'::jsonb,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_data_subject_request_items_action CHECK (requested_action IN ('DISCLOSE','EXPORT','CORRECT','DELETE','ANONYMIZE','RESTRICT','RETAIN')),
    CONSTRAINT ck_data_subject_request_items_disposition CHECK (disposition IS NULL OR disposition IN ('APPROVED','PARTIAL','DENIED','RETAIN_LEGAL','NOT_FOUND','DUPLICATE')),
    CONSTRAINT ck_data_subject_request_items_status CHECK (status IN ('PENDING','PROCESSING','COMPLETED','FAILED','SKIPPED')),
    CONSTRAINT ck_data_subject_request_items_count CHECK (record_count IS NULL OR record_count >= 0),
    CONSTRAINT ck_data_subject_request_items_result_secrets CHECK (NOT doms.jsonb_contains_forbidden_secret_key(result_detail_masked))
);

CREATE TABLE IF NOT EXISTS doms.data_disposal_runs (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    run_no varchar(100) NOT NULL,
    data_retention_policy_id uuid NOT NULL REFERENCES doms.data_retention_policies(id) ON DELETE RESTRICT,
    entity_type varchar(80) NOT NULL,
    ...
    CONSTRAINT ck_data_disposal_runs_status CHECK (status IN ('PLANNED','PENDING_APPROVAL','APPROVED','RUNNING','COMPLETED','PARTIAL','FAILED','CANCELLED')),
    CONSTRAINT ck_data_disposal_runs_counts CHECK (
        candidate_count >= 0 AND processed_count >= 0 AND skipped_legal_hold_count >= 0 AND failed_count >= 0 AND
        processed_count + skipped_legal_hold_count + failed_count <= candidate_count
    ),
    CONSTRAINT ck_data_disposal_runs_approval CHECK (
        dry_run OR status NOT IN ('APPROVED','RUNNING','COMPLETED','PARTIAL') OR
        (approved_by IS NOT NULL AND approved_by IS DISTINCT FROM requested_by)
    ),
    CONSTRAINT ck_data_disposal_runs_dates CHECK (
        completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at
    )
);

CREATE TABLE IF NOT EXISTS doms.data_disposal_results (
    id uuid PRIMARY KEY DEFAULT doms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES doms.tenants(id),
    data_disposal_run_id uuid NOT NULL REFERENCES doms.data_disposal_runs(id) ON DELETE RESTRICT,
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    action_taken varchar(20) NOT NULL,
    status varchar(20) NOT NULL,
    record_count bigint NOT NULL DEFAULT 0,
    legal_hold_id uuid REFERENCES doms.legal_holds(id) ON DELETE RESTRICT,
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, data_disposal_run_id, entity_type, entity_id),
    CONSTRAINT ck_data_disposal_results_action CHECK (action_taken IN ('NONE','DELETE','ANONYMIZE','ARCHIVE','RESTRICT')),
    CONSTRAINT ck_data_disposal_results_status CHECK (status IN ('COMPLETED','SKIPPED_LEGAL_HOLD','FAILED','NOT_FOUND')),
    CONSTRAINT ck_data_disposal_results_count CHECK (record_count >= 0),
    CONSTRAINT ck_data_disposal_results_hold CHECK (
        (status = 'SKIPPED_LEGAL_HOLD' AND legal_hold_id IS NOT NULL AND action_taken = 'NONE') OR
        status <> 'SKIPPED_LEGAL_HOLD'
    ),
    CONSTRAINT ck_data_disposal_results_evidence CHECK (
        (evidence_object_uri IS NULL AND evidence_sha256 IS NULL) OR
        (evidence_object_uri IS NOT NULL AND evidence_sha256 ~ '^[0-9A-Fa-f]{64}$')
    ),
    CONSTRAINT ck_data_disposal_results_error CHECK (status <> 'FAILED' OR error_code IS NOT NULL)
);

CREATE OR REPLACE FUNCTION doms.prevent_disposal_under_legal_hold()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    IF NEW.action_taken IN ('DELETE','ANONYMIZE','ARCHIVE') AND EXISTS (
        SELECT 1
        FROM doms.legal_hold_entities held
        JOIN doms.legal_holds hold_row ON hold_row.id = held.legal_hold_id
        WHERE held.tenant_id = NEW.tenant_id
          AND held.entity_type = NEW.entity_type
          AND held.entity_id = NEW.entity_id
          AND held.released_at IS NULL
          AND hold_row.status = 'ACTIVE'
    ) THEN
        RAISE EXCEPTION 'Data disposal is prohibited while an active legal hold applies'
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_prevent_disposal_under_legal_hold ON doms.data_disposal_results;
CREATE TRIGGER trg_prevent_disposal_under_legal_hold
    BEFORE INSERT OR UPDATE ON doms.data_disposal_results
    FOR EACH ROW EXECUTE PROCEDURE doms.prevent_disposal_under_legal_hold();
