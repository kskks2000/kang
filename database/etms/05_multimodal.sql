-- ETMS scheduled and multimodal transport, bookings, containers, trade/customs documents.

CREATE TABLE IF NOT EXISTS etms.transport_schedules (
    ...
    origin_location_id uuid NOT NULL REFERENCES etms.locations(id),
    destination_location_id uuid NOT NULL REFERENCES etms.locations(id),
...
CREATE TABLE IF NOT EXISTS etms.transport_schedule_calls (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    schedule_id uuid NOT NULL REFERENCES etms.transport_schedules(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS etms.carrier_bookings (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    booking_no varchar(120) NOT NULL,
    carrier_booking_reference varchar(200),
    carrier_id uuid NOT NULL REFERENCES etms.business_partners(id),
    schedule_id uuid REFERENCES etms.transport_schedules(id),
...
CREATE TABLE IF NOT EXISTS etms.carrier_booking_items (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    booking_id uuid NOT NULL REFERENCES etms.carrier_bookings(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS etms.container_seals (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    ...
    applied_location_id uuid REFERENCES etms.locations(id),
    applied_by varchar(200),
    removed_at timestamptz,
    removed_location_id uuid REFERENCES etms.locations(id),
    removal_reason text,
...
CREATE TABLE IF NOT EXISTS etms.container_events (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    container_id uuid NOT NULL REFERENCES etms.containers(id),
    shipment_id uuid REFERENCES etms.shipments(id),
    booking_id uuid REFERENCES etms.carrier_bookings(id),
...
CREATE TABLE IF NOT EXISTS etms.container_load_items (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    container_id uuid NOT NULL REFERENCES etms.containers(id),
    ...
    load_shipment_stop_id uuid REFERENCES etms.shipment_stops(id),
    unload_shipment_stop_id uuid REFERENCES etms.shipment_stops(id),
...
CREATE TABLE IF NOT EXISTS etms.container_leg_assignments (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    container_id uuid NOT NULL REFERENCES etms.containers(id),
    shipment_leg_id uuid NOT NULL REFERENCES etms.shipment_legs(id),
    run_leg_id uuid REFERENCES etms.run_legs(id),
    booking_id uuid REFERENCES etms.carrier_bookings(id),
...
CREATE TABLE IF NOT EXISTS etms.transport_documents (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    ...
    shipper_snapshot_id uuid REFERENCES etms.party_snapshots(id),
    consignee_snapshot_id uuid REFERENCES etms.party_snapshots(id),
    notify_party_snapshot_id uuid REFERENCES etms.party_snapshots(id),
    ...
    file_id uuid REFERENCES etms.files(id),
    supersedes_document_id uuid REFERENCES etms.transport_documents(id),
    metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS etms.transport_document_lines (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    transport_document_id uuid NOT NULL REFERENCES etms.transport_documents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS etms.customs_declarations (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    declaration_no varchar(150) NOT NULL,
    declaration_type varchar(30) NOT NULL,
    shipment_id uuid NOT NULL REFERENCES etms.shipments(id),
    customs_broker_id uuid REFERENCES etms.business_partners(id),
    declarant_partner_id uuid REFERENCES etms.business_partners(id),
    export_country_code char(2) REFERENCES etms.countries(country_code),
    import_country_code char(2) REFERENCES etms.countries(country_code),
...
CREATE TABLE IF NOT EXISTS etms.customs_declaration_items (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    customs_declaration_id uuid NOT NULL REFERENCES etms.customs_declarations(id) ON DELETE CASCADE,
    line_no integer NOT NULL,
    order_line_id uuid REFERENCES etms.transport_order_lines(id),
...
CREATE TABLE IF NOT EXISTS etms.terminal_cutoffs (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    schedule_call_id uuid NOT NULL REFERENCES etms.transport_schedule_calls(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS etms.verified_gross_masses (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
    container_id uuid NOT NULL REFERENCES etms.containers(id),
    booking_id uuid REFERENCES etms.carrier_bookings(id),
    ...
    responsible_party_id uuid REFERENCES etms.business_partners(id),
...
CREATE TABLE IF NOT EXISTS etms.transport_permits (
    id uuid PRIMARY KEY DEFAULT etms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES etms.tenants(id),
