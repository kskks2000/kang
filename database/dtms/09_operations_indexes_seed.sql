-- DTMS notifications, EDI/webhooks/jobs, audit extensions, integrity triggers, indexes, and reference seeds.

CREATE TABLE IF NOT EXISTS dtms.status_definitions (
...
CREATE TABLE IF NOT EXISTS dtms.entity_tags (
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    tag_id uuid NOT NULL REFERENCES dtms.tags(id) ON DELETE CASCADE,
    assigned_by uuid REFERENCES dtms.users(id),
    assigned_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (entity_type, entity_id, tag_id)
);

CREATE TABLE IF NOT EXISTS dtms.notification_templates (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.notifications (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    event_type varchar(100) NOT NULL,
    template_id uuid REFERENCES dtms.notification_templates(id),
...
CREATE TABLE IF NOT EXISTS dtms.notification_deliveries (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    notification_id uuid NOT NULL REFERENCES dtms.notifications(id) ON DELETE CASCADE,
    user_id uuid REFERENCES dtms.users(id),
    partner_contact_id uuid REFERENCES dtms.contacts(id),
    channel varchar(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dtms.edi_documents (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.edi_validation_errors (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    edi_document_id uuid NOT NULL REFERENCES dtms.edi_documents(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.webhook_subscriptions (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.webhook_deliveries (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    subscription_id uuid NOT NULL REFERENCES dtms.webhook_subscriptions(id) ON DELETE CASCADE,
    outbox_event_id uuid REFERENCES dtms.outbox_events(id),
...
CREATE TABLE IF NOT EXISTS dtms.inbox_deduplication (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    source_system_id uuid NOT NULL REFERENCES dtms.external_systems(id),
...
CREATE TABLE IF NOT EXISTS dtms.import_jobs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    job_no varchar(100) NOT NULL,
    import_type varchar(80) NOT NULL,
    source_file_id uuid NOT NULL REFERENCES dtms.files(id),
    ...
    result_file_id uuid REFERENCES dtms.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS dtms.import_job_rows (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    import_job_id uuid NOT NULL REFERENCES dtms.import_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.export_jobs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    job_no varchar(100) NOT NULL,
    ...
    output_file_id uuid REFERENCES dtms.files(id),
    requested_by uuid REFERENCES dtms.users(id),
    requested_at timestamptz NOT NULL DEFAULT now(),
    completed_at timestamptz,
    expires_at timestamptz,
...
CREATE TABLE IF NOT EXISTS dtms.scheduled_jobs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_scheduled_jobs_scope
    ON dtms.scheduled_jobs (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), job_code
    );

CREATE TABLE IF NOT EXISTS dtms.job_runs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    scheduled_job_id uuid NOT NULL REFERENCES dtms.scheduled_jobs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.dead_letter_messages (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    resolved_by uuid REFERENCES dtms.users(id),
    resolution text,
    CONSTRAINT ck_dead_letter_retry CHECK (retry_count >= 0)
);

CREATE TABLE IF NOT EXISTS dtms.impersonation_events (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    administrator_user_id uuid NOT NULL REFERENCES dtms.users(id),
    impersonated_user_id uuid NOT NULL REFERENCES dtms.users(id),
...
CREATE TABLE IF NOT EXISTS dtms.entity_change_logs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    ...
    actor_user_id uuid REFERENCES dtms.users(id),
    correlation_id varchar(100),
    occurred_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_entity_change_operation CHECK (operation IN ('INSERT','UPDATE','DELETE','RESTORE'))
);

CREATE TABLE IF NOT EXISTS dtms.data_access_logs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id),
    actor_user_id uuid REFERENCES dtms.users(id),
    actor_api_client_id uuid REFERENCES dtms.api_clients(id),
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER TABLE dtms.%I ADD COLUMN IF NOT EXISTS tenant_id uuid', table_name
        );
        ...
        IF NOT EXISTS (
            ...
            WHERE c.conrelid = format('dtms.%I', table_name)::regclass
              AND c.conname = constraint_name
        ) THEN
            EXECUTE format(
                'ALTER TABLE dtms.%I ADD CONSTRAINT %I FOREIGN KEY (tenant_id) '
                'REFERENCES dtms.tenants(id)',
        ...
        IF NOT (table_name = ANY (tenant_nullable_tables)) THEN
            EXECUTE format(
                'ALTER TABLE dtms.%I ALTER COLUMN tenant_id SET NOT NULL', table_name
            );
        END IF;
    END LOOP;
END;
...
BEGIN
    ...
    FOR r IN
        ...
        WHERE con.contype = 'f'
          ...
          AND child_ns.nspname = 'dtms'
          AND parent_ns.nspname = 'dtms'
...
CREATE OR REPLACE FUNCTION dtms.prevent_tenant_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION dtms.enforce_same_tenant_fk()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION dtms.validate_order_line_stops()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.pickup_stop_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM dtms.transport_order_stops s
...
CREATE OR REPLACE FUNCTION dtms.validate_order_stop_line()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
DECLARE
    ...
    line_order_id uuid;
BEGIN
    SELECT order_id INTO stop_order_id FROM dtms.transport_order_stops WHERE id = NEW.order_stop_id;
    SELECT order_id INTO line_order_id FROM dtms.transport_order_lines WHERE id = NEW.order_line_id;
...
CREATE OR REPLACE FUNCTION dtms.validate_shipment_leg_stops()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM dtms.shipment_stops f, dtms.shipment_stops t
...
CREATE OR REPLACE FUNCTION dtms.validate_shipment_stop_order()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.shipment_stops WHERE id = NEW.shipment_stop_id;

    IF NEW.order_stop_id IS NOT NULL THEN
        SELECT order_id INTO parent_order_id
        FROM dtms.transport_order_stops WHERE id = NEW.order_stop_id;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM dtms.shipment_orders so
...
CREATE OR REPLACE FUNCTION dtms.validate_handling_unit_content()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
DECLARE
    ...
    line_order_id uuid;
BEGIN
    SELECT order_id INTO handling_order_id FROM dtms.handling_units WHERE id = NEW.handling_unit_id;
    SELECT order_id INTO line_order_id FROM dtms.transport_order_lines WHERE id = NEW.order_line_id;
...
CREATE OR REPLACE FUNCTION dtms.validate_leg_allocation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    SELECT shipment_id INTO parent_shipment_id
    FROM dtms.shipment_legs WHERE id = NEW.shipment_leg_id;
    SELECT order_id INTO parent_order_id
    FROM dtms.transport_order_lines WHERE id = NEW.order_line_id;
    ...
    IF NEW.handling_unit_id IS NOT NULL THEN
        ...
        FROM dtms.handling_units WHERE id = NEW.handling_unit_id;
...
CREATE OR REPLACE FUNCTION dtms.validate_pod_order()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    IF NEW.run_stop_id IS NOT NULL THEN
        SELECT run_id INTO pod_run_id FROM dtms.run_stops WHERE id = NEW.run_stop_id;
        IF NOT EXISTS (
            SELECT 1
            FROM dtms.run_legs rl
            LEFT JOIN dtms.shipment_legs direct_leg ON direct_leg.id = rl.shipment_leg_id
            LEFT JOIN dtms.run_leg_allocations rla ON rla.run_leg_id = rl.id
            LEFT JOIN dtms.shipment_leg_allocations sla ON sla.id = rla.shipment_leg_allocation_id
            LEFT JOIN dtms.shipment_legs allocated_leg ON allocated_leg.id = sla.shipment_leg_id
...
CREATE OR REPLACE FUNCTION dtms.validate_pod_item_parentage()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.proof_of_deliveries WHERE id = NEW.pod_id;
...
DROP TRIGGER IF EXISTS trg_validate_order_line_stops ON dtms.transport_order_lines;
CREATE TRIGGER trg_validate_order_line_stops
BEFORE INSERT OR UPDATE OF order_id, pickup_stop_id, delivery_stop_id
ON dtms.transport_order_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_order_line_stops();

DROP TRIGGER IF EXISTS trg_validate_order_stop_line ON dtms.order_stop_lines;
CREATE TRIGGER trg_validate_order_stop_line
BEFORE INSERT OR UPDATE OF order_stop_id, order_line_id
ON dtms.order_stop_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_order_stop_line();

DROP TRIGGER IF EXISTS trg_validate_shipment_leg_stops ON dtms.shipment_legs;
CREATE TRIGGER trg_validate_shipment_leg_stops
BEFORE INSERT OR UPDATE OF shipment_id, from_stop_id, to_stop_id
ON dtms.shipment_legs
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_shipment_leg_stops();

DROP TRIGGER IF EXISTS trg_validate_shipment_stop_order ON dtms.shipment_stop_orders;
CREATE TRIGGER trg_validate_shipment_stop_order
BEFORE INSERT OR UPDATE OF shipment_stop_id, order_id, order_stop_id
ON dtms.shipment_stop_orders
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_shipment_stop_order();

DROP TRIGGER IF EXISTS trg_validate_handling_unit_content ON dtms.handling_unit_contents;
CREATE TRIGGER trg_validate_handling_unit_content
BEFORE INSERT OR UPDATE OF handling_unit_id, order_line_id
ON dtms.handling_unit_contents
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_handling_unit_content();

DROP TRIGGER IF EXISTS trg_validate_leg_allocation ON dtms.shipment_leg_allocations;
CREATE TRIGGER trg_validate_leg_allocation
BEFORE INSERT OR UPDATE OF shipment_leg_id, order_line_id, handling_unit_id
ON dtms.shipment_leg_allocations
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_leg_allocation();

DROP TRIGGER IF EXISTS trg_validate_pod_order ON dtms.proof_of_deliveries;
CREATE TRIGGER trg_validate_pod_order
BEFORE INSERT OR UPDATE OF shipment_id, order_id
ON dtms.proof_of_deliveries
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_pod_order();

DROP TRIGGER IF EXISTS trg_validate_pod_item_parentage ON dtms.pod_items;
CREATE TRIGGER trg_validate_pod_item_parentage
BEFORE INSERT OR UPDATE OF pod_id, order_line_id, handling_unit_id
ON dtms.pod_items
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_pod_item_parentage();

CREATE OR REPLACE FUNCTION dtms.validate_unit_conversion()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    SELECT dimension INTO from_dimension FROM dtms.units_of_measure WHERE uom_code = NEW.from_uom_code;
    SELECT dimension INTO to_dimension FROM dtms.units_of_measure WHERE uom_code = NEW.to_uom_code;
    ...
    END IF;
    IF EXISTS (
        SELECT 1 FROM dtms.unit_conversions c
...
CREATE OR REPLACE FUNCTION dtms.validate_rate_basis_uom()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    IF expected_dimension IS NOT NULL THEN
        ...
        FROM dtms.units_of_measure WHERE uom_code = NEW.basis_uom_code;
...
CREATE OR REPLACE FUNCTION dtms.prevent_driver_assignment_overlap()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    IF NEW.status NOT IN ('CANCELLED','REJECTED','RELEASED') AND EXISTS (
        SELECT 1 FROM dtms.run_driver_assignments a
...
CREATE OR REPLACE FUNCTION dtms.prevent_equipment_assignment_overlap()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    IF NEW.status NOT IN ('CANCELLED','REJECTED','RELEASED') AND EXISTS (
        SELECT 1 FROM dtms.run_equipment_assignments a
...
CREATE OR REPLACE FUNCTION dtms.validate_polymorphic_asset_reference()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF kind_value = 'VEHICLE' THEN
        SELECT tenant_id INTO asset_tenant FROM dtms.vehicles WHERE id = asset_value;
    ELSIF kind_value = 'TRAILER' THEN
        SELECT tenant_id INTO asset_tenant FROM dtms.trailers WHERE id = asset_value;
    ELSIF kind_value = 'CONTAINER' THEN
        SELECT tenant_id INTO asset_tenant FROM dtms.containers WHERE id = asset_value;
    ELSIF kind_value = 'HANDLING_UNIT' THEN
        SELECT tenant_id INTO asset_tenant FROM dtms.handling_units WHERE id = asset_value;
    ELSIF kind_value = 'SHIPMENT' THEN
        SELECT tenant_id INTO asset_tenant FROM dtms.shipments WHERE id = asset_value;
    ELSE
...
DROP TRIGGER IF EXISTS trg_validate_unit_conversion ON dtms.unit_conversions;
CREATE TRIGGER trg_validate_unit_conversion
BEFORE INSERT OR UPDATE ON dtms.unit_conversions
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_unit_conversion();

DROP TRIGGER IF EXISTS trg_validate_rate_basis_uom ON dtms.rate_rules;
CREATE TRIGGER trg_validate_rate_basis_uom
BEFORE INSERT OR UPDATE OF basis_type, basis_uom_code ON dtms.rate_rules
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_rate_basis_uom();

DROP TRIGGER IF EXISTS trg_prevent_driver_assignment_overlap ON dtms.run_driver_assignments;
CREATE TRIGGER trg_prevent_driver_assignment_overlap
BEFORE INSERT OR UPDATE OF driver_id, valid_from, valid_to, status ON dtms.run_driver_assignments
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_driver_assignment_overlap();

DROP TRIGGER IF EXISTS trg_prevent_equipment_assignment_overlap ON dtms.run_equipment_assignments;
...
ON dtms.run_equipment_assignments
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_equipment_assignment_overlap();

DROP TRIGGER IF EXISTS trg_validate_run_equipment_asset ON dtms.run_equipment_assignments;
CREATE TRIGGER trg_validate_run_equipment_asset
BEFORE INSERT OR UPDATE OF equipment_kind, equipment_id, tenant_id
ON dtms.run_equipment_assignments
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_polymorphic_asset_reference('equipment_kind','equipment_id');

DROP TRIGGER IF EXISTS trg_validate_equipment_document_asset ON dtms.equipment_documents;
CREATE TRIGGER trg_validate_equipment_document_asset
BEFORE INSERT OR UPDATE OF equipment_kind, equipment_id, tenant_id
ON dtms.equipment_documents
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_polymorphic_asset_reference('equipment_kind','equipment_id');

DROP TRIGGER IF EXISTS trg_validate_equipment_availability_asset ON dtms.equipment_availability;
CREATE TRIGGER trg_validate_equipment_availability_asset
BEFORE INSERT OR UPDATE OF equipment_kind, equipment_id, tenant_id
ON dtms.equipment_availability
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_polymorphic_asset_reference('equipment_kind','equipment_id');

DROP TRIGGER IF EXISTS trg_validate_device_assignment_asset ON dtms.device_assignments;
CREATE TRIGGER trg_validate_device_assignment_asset
BEFORE INSERT OR UPDATE OF asset_type, asset_id, tenant_id
ON dtms.device_assignments
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_polymorphic_asset_reference('asset_type','asset_id');

DROP TRIGGER IF EXISTS trg_validate_sensor_assignment_asset ON dtms.sensor_assignments;
CREATE TRIGGER trg_validate_sensor_assignment_asset
BEFORE INSERT OR UPDATE OF asset_type, asset_id, tenant_id
ON dtms.sensor_assignments
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_polymorphic_asset_reference('asset_type','asset_id');

-- Financial documents are editable in DRAFT, but totals must reconcile before posting/finalization.
CREATE OR REPLACE FUNCTION dtms.validate_invoice_totals()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.status NOT IN ('DRAFT','RECEIVED','VALIDATING','REJECTED','CANCELLED') THEN
        ...
        FROM dtms.invoice_lines WHERE invoice_id = NEW.id;
        ...
        FROM dtms.invoice_taxes WHERE invoice_id = NEW.id;
...
CREATE OR REPLACE FUNCTION dtms.validate_tax_invoice_totals()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.nts_status NOT IN ('DRAFT','VALIDATING','REJECTED','CANCELLED') THEN
        ...
        FROM dtms.tax_invoice_lines WHERE tax_invoice_id = NEW.id;
...
CREATE OR REPLACE FUNCTION dtms.validate_settlement_totals()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.status NOT IN ('DRAFT','OPEN','CALCULATING','REJECTED','CANCELLED') THEN
        ...
        FROM dtms.settlement_lines WHERE settlement_id = NEW.id;
...
CREATE OR REPLACE FUNCTION dtms.validate_payment_application()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.payments WHERE id = NEW.payment_id FOR UPDATE;
    IF NEW.invoice_id IS NOT NULL THEN
        SELECT currency_code INTO target_currency
        FROM dtms.invoices WHERE id = NEW.invoice_id FOR UPDATE;
    ELSIF NEW.settlement_id IS NOT NULL THEN
        SELECT currency_code INTO target_currency
        FROM dtms.settlements WHERE id = NEW.settlement_id FOR UPDATE;
    ELSE
        SELECT currency_code INTO target_currency
        FROM dtms.claim_settlements WHERE id = NEW.claim_settlement_id FOR UPDATE;
    END IF;
...
CREATE OR REPLACE FUNCTION dtms.validate_payment_application_sum()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.payments WHERE id = target_payment_id FOR UPDATE;
    SELECT COALESCE(sum(applied_amount), 0) INTO application_total
    FROM dtms.payment_applications WHERE payment_id = target_payment_id;
...
CREATE OR REPLACE FUNCTION dtms.assert_journal_entry_balanced(target_entry_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.journal_entries e
    JOIN dtms.journal_batches b ON b.id = e.journal_batch_id
    ...
    IF entry_status = 'POSTED' OR batch_status = 'POSTED' THEN
        ...
        FROM dtms.journal_lines WHERE journal_entry_id = target_entry_id;
...
CREATE OR REPLACE FUNCTION dtms.validate_journal_line_balance()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        PERFORM dtms.assert_journal_entry_balanced(OLD.journal_entry_id);
    ELSIF TG_OP = 'UPDATE' THEN
        PERFORM dtms.assert_journal_entry_balanced(OLD.journal_entry_id);
        IF NEW.journal_entry_id IS DISTINCT FROM OLD.journal_entry_id THEN
            PERFORM dtms.assert_journal_entry_balanced(NEW.journal_entry_id);
        END IF;
    ELSE
        PERFORM dtms.assert_journal_entry_balanced(NEW.journal_entry_id);
    END IF;
...
CREATE OR REPLACE FUNCTION dtms.validate_journal_entry_status()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.status = 'POSTED' THEN
        PERFORM dtms.assert_journal_entry_balanced(NEW.id);
    END IF;
    RETURN NEW;
END;
$function$;

CREATE OR REPLACE FUNCTION dtms.validate_journal_batch_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    IF NEW.status = 'POSTED' THEN
        ...
        FROM dtms.journal_entries e
        JOIN dtms.journal_lines l ON l.journal_entry_id = e.id
...
DROP TRIGGER IF EXISTS trg_validate_invoice_totals ON dtms.invoices;
CREATE TRIGGER trg_validate_invoice_totals
BEFORE INSERT OR UPDATE OF status, net_amount, tax_amount, gross_amount
ON dtms.invoices FOR EACH ROW EXECUTE PROCEDURE dtms.validate_invoice_totals();

DROP TRIGGER IF EXISTS trg_validate_tax_invoice_totals ON dtms.tax_invoices;
...
ON dtms.tax_invoices FOR EACH ROW EXECUTE PROCEDURE dtms.validate_tax_invoice_totals();

DROP TRIGGER IF EXISTS trg_validate_settlement_totals ON dtms.settlements;
...
ON dtms.settlements FOR EACH ROW EXECUTE PROCEDURE dtms.validate_settlement_totals();

DROP TRIGGER IF EXISTS trg_validate_payment_application ON dtms.payment_applications;
...
ON dtms.payment_applications FOR EACH ROW EXECUTE PROCEDURE dtms.validate_payment_application();

DROP TRIGGER IF EXISTS trg_validate_payment_application_sum ON dtms.payment_applications;
CREATE CONSTRAINT TRIGGER trg_validate_payment_application_sum
AFTER INSERT OR UPDATE OR DELETE ON dtms.payment_applications
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_payment_application_sum();

DROP TRIGGER IF EXISTS trg_validate_journal_line_balance ON dtms.journal_lines;
CREATE CONSTRAINT TRIGGER trg_validate_journal_line_balance
AFTER INSERT OR UPDATE OR DELETE ON dtms.journal_lines
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_journal_line_balance();

DROP TRIGGER IF EXISTS trg_validate_journal_entry_status ON dtms.journal_entries;
CREATE CONSTRAINT TRIGGER trg_validate_journal_entry_status
AFTER INSERT OR UPDATE OF status ON dtms.journal_entries
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_journal_entry_status();

DROP TRIGGER IF EXISTS trg_validate_journal_batch_posting ON dtms.journal_batches;
CREATE TRIGGER trg_validate_journal_batch_posting
BEFORE INSERT OR UPDATE OF status, total_debit, total_credit
ON dtms.journal_batches FOR EACH ROW EXECUTE PROCEDURE dtms.validate_journal_batch_posting();

CREATE OR REPLACE FUNCTION dtms.prevent_finalized_child_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FOREACH parent_reference IN ARRAY parent_references LOOP
        IF parent_reference IS NOT NULL THEN
            EXECUTE format(
                'SELECT %I::text FROM dtms.%I WHERE id = $1', TG_ARGV[2], TG_ARGV[1]
...
CREATE OR REPLACE FUNCTION dtms.protect_finalized_header()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
CREATE OR REPLACE FUNCTION dtms.validate_finance_status_transition()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION dtms.prevent_finalized_invoice_line_child_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FOREACH target_invoice_line_id IN ARRAY target_invoice_line_ids LOOP
        ...
        FROM dtms.invoice_lines l JOIN dtms.invoices i ON i.id = l.invoice_id
...
CREATE OR REPLACE FUNCTION dtms.prevent_posted_journal_line_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FOREACH target_entry_id IN ARRAY target_entry_ids LOOP
        SELECT e.status, b.status INTO entry_status, batch_status
        FROM dtms.journal_entries e JOIN dtms.journal_batches b ON b.id = e.journal_batch_id
        WHERE e.id = target_entry_id;
        IF entry_status = 'POSTED' OR batch_status = 'POSTED' THEN
...
CREATE OR REPLACE FUNCTION dtms.prevent_posted_journal_entry_mutation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FOREACH target_batch_id IN ARRAY target_batch_ids LOOP
        SELECT status INTO batch_status FROM dtms.journal_batches WHERE id = target_batch_id;
...
CREATE OR REPLACE FUNCTION dtms.validate_payment_target_limit()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    IF NEW.invoice_id IS NOT NULL THEN
        ...
        SELECT gross_amount INTO target_amount FROM dtms.invoices WHERE id = target_id;
        SELECT COALESCE(sum(applied_amount + discount_taken + writeoff_amount), 0)
          INTO applied_total FROM dtms.payment_applications WHERE invoice_id = target_id;
    ELSIF NEW.settlement_id IS NOT NULL THEN
        target_type := 'SETTLEMENT'; target_id := NEW.settlement_id;
        SELECT net_payable_amount INTO target_amount FROM dtms.settlements WHERE id = target_id;
        SELECT COALESCE(sum(applied_amount + discount_taken + writeoff_amount), 0)
          INTO applied_total FROM dtms.payment_applications WHERE settlement_id = target_id;
    ELSE
        target_type := 'CLAIM_SETTLEMENT'; target_id := NEW.claim_settlement_id;
        SELECT settlement_amount INTO target_amount FROM dtms.claim_settlements WHERE id = target_id;
        SELECT COALESCE(sum(applied_amount + discount_taken + writeoff_amount), 0)
          INTO applied_total FROM dtms.payment_applications WHERE claim_settlement_id = target_id;
    END IF;
...
CREATE OR REPLACE FUNCTION dtms.validate_payment_header_after_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, dtms
...
BEGIN
    ...
    FROM dtms.payment_applications WHERE payment_id = NEW.id;
    ...
    FROM dtms.payment_applications a
    LEFT JOIN dtms.invoices i ON i.id = a.invoice_id
    LEFT JOIN dtms.settlements s ON s.id = a.settlement_id
    LEFT JOIN dtms.claim_settlements c ON c.id = a.claim_settlement_id
...
DROP TRIGGER IF EXISTS trg_lock_final_invoice_lines ON dtms.invoice_lines;
CREATE TRIGGER trg_lock_final_invoice_lines
BEFORE INSERT OR UPDATE OR DELETE ON dtms.invoice_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_finalized_child_mutation(
    'invoice_id','invoices','status','DRAFT,RECEIVED,VALIDATING,REJECTED'
);

DROP TRIGGER IF EXISTS trg_lock_final_invoice_taxes ON dtms.invoice_taxes;
CREATE TRIGGER trg_lock_final_invoice_taxes
BEFORE INSERT OR UPDATE OR DELETE ON dtms.invoice_taxes
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_finalized_child_mutation(
    'invoice_id','invoices','status','DRAFT,RECEIVED,VALIDATING,REJECTED'
);

DROP TRIGGER IF EXISTS trg_lock_final_invoice_allocations ON dtms.invoice_line_charges;
CREATE TRIGGER trg_lock_final_invoice_allocations
BEFORE INSERT OR UPDATE OR DELETE ON dtms.invoice_line_charges
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_finalized_invoice_line_child_mutation();

DROP TRIGGER IF EXISTS trg_lock_final_tax_invoice_lines ON dtms.tax_invoice_lines;
CREATE TRIGGER trg_lock_final_tax_invoice_lines
BEFORE INSERT OR UPDATE OR DELETE ON dtms.tax_invoice_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_finalized_child_mutation(
    'tax_invoice_id','tax_invoices','nts_status','DRAFT,VALIDATING,REJECTED'
);

DROP TRIGGER IF EXISTS trg_lock_final_settlement_lines ON dtms.settlement_lines;
CREATE TRIGGER trg_lock_final_settlement_lines
BEFORE INSERT OR UPDATE OR DELETE ON dtms.settlement_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_finalized_child_mutation(
    'settlement_id','settlements','status','DRAFT,OPEN,CALCULATING,REJECTED'
);

DROP TRIGGER IF EXISTS trg_protect_final_invoice_header ON dtms.invoices;
CREATE TRIGGER trg_protect_final_invoice_header
BEFORE UPDATE OR DELETE ON dtms.invoices
FOR EACH ROW EXECUTE PROCEDURE dtms.protect_finalized_header(
...
DROP TRIGGER IF EXISTS trg_protect_final_tax_invoice_header ON dtms.tax_invoices;
CREATE TRIGGER trg_protect_final_tax_invoice_header
BEFORE UPDATE OR DELETE ON dtms.tax_invoices
FOR EACH ROW EXECUTE PROCEDURE dtms.protect_finalized_header(
...
DROP TRIGGER IF EXISTS trg_protect_final_settlement_header ON dtms.settlements;
CREATE TRIGGER trg_protect_final_settlement_header
BEFORE UPDATE OR DELETE ON dtms.settlements
FOR EACH ROW EXECUTE PROCEDURE dtms.protect_finalized_header(
...
DROP TRIGGER IF EXISTS trg_protect_posted_journal_batch ON dtms.journal_batches;
CREATE TRIGGER trg_protect_posted_journal_batch
BEFORE UPDATE OR DELETE ON dtms.journal_batches
FOR EACH ROW EXECUTE PROCEDURE dtms.protect_finalized_header(
...
DROP TRIGGER IF EXISTS trg_invoice_status_forward ON dtms.invoices;
CREATE TRIGGER trg_invoice_status_forward BEFORE UPDATE OF status ON dtms.invoices
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_finance_status_transition('INVOICE');

DROP TRIGGER IF EXISTS trg_tax_invoice_status_forward ON dtms.tax_invoices;
CREATE TRIGGER trg_tax_invoice_status_forward BEFORE UPDATE OF nts_status ON dtms.tax_invoices
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_finance_status_transition('TAX_INVOICE');

DROP TRIGGER IF EXISTS trg_settlement_status_forward ON dtms.settlements;
CREATE TRIGGER trg_settlement_status_forward BEFORE UPDATE OF status ON dtms.settlements
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_finance_status_transition('SETTLEMENT');

DROP TRIGGER IF EXISTS trg_journal_batch_status_forward ON dtms.journal_batches;
CREATE TRIGGER trg_journal_batch_status_forward BEFORE UPDATE OF status ON dtms.journal_batches
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_finance_status_transition('JOURNAL_BATCH');

DROP TRIGGER IF EXISTS trg_lock_posted_journal_lines ON dtms.journal_lines;
CREATE TRIGGER trg_lock_posted_journal_lines
BEFORE INSERT OR UPDATE OR DELETE ON dtms.journal_lines
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_posted_journal_line_mutation();

DROP TRIGGER IF EXISTS trg_lock_posted_journal_entries ON dtms.journal_entries;
CREATE TRIGGER trg_lock_posted_journal_entries
BEFORE INSERT OR UPDATE OR DELETE ON dtms.journal_entries
FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_posted_journal_entry_mutation();

DROP TRIGGER IF EXISTS trg_validate_payment_target_limit ON dtms.payment_applications;
CREATE CONSTRAINT TRIGGER trg_validate_payment_target_limit
AFTER INSERT OR UPDATE ON dtms.payment_applications
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_payment_target_limit();

DROP TRIGGER IF EXISTS trg_validate_payment_header_update ON dtms.payments;
CREATE TRIGGER trg_validate_payment_header_update
BEFORE UPDATE OF payment_amount, currency_code ON dtms.payments
FOR EACH ROW EXECUTE PROCEDURE dtms.validate_payment_header_after_update();

CREATE OR REPLACE FUNCTION dtms.prevent_ledger_update()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_append_only ON dtms.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_append_only BEFORE UPDATE ON dtms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_ledger_update()', table_name
        );
    END LOOP;
END;
$block$;
    ...
    LOOP
        IF NOT EXISTS (
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE dtms.touch_row()',
...
BEGIN
    FOR r IN
        ...
        WHERE table_schema = 'dtms' AND column_name = 'tenant_id'
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM pg_trigger t
            WHERE t.tgrelid = format('%I.%I', r.table_schema, r.table_name)::regclass
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE dtms.prevent_tenant_change()',
                r.table_schema, r.table_name
            );
        END IF;
    END LOOP;
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE dtms.enforce_same_tenant_fk(%L,%L,%L)',
...
-- and set `SET LOCAL dtms.tenant_id = '<tenant uuid>'` for every transaction.
CREATE OR REPLACE FUNCTION dtms.current_tenant_id()
RETURNS uuid
LANGUAGE sql
STABLE
AS $function$
    SELECT NULLIF(current_setting('dtms.tenant_id', true), '')::uuid;
$function$;
...
BEGIN
    ...
    LOOP
        ...
        IF r.table_name = ANY (global_reference_tables) THEN
            select_expression := 'tenant_id IS NULL OR tenant_id = dtms.current_tenant_id()';
        ELSE
            select_expression := 'tenant_id = dtms.current_tenant_id()';
        END IF;
        ...
        EXECUTE format(
            ...
            'USING (tenant_id = dtms.current_tenant_id()) '
            'WITH CHECK (tenant_id = dtms.current_tenant_id())',
            r.table_schema, r.table_name
        );
    END LOOP;
END;
...
BEGIN
    FOR r IN
        ...
        WHERE con.contype = 'f' AND ns.nspname = 'dtms'
    LOOP
...
CREATE INDEX IF NOT EXISTS ix_orders_tenant_status_pickup
    ON dtms.transport_orders (tenant_id, status, requested_pickup_from);
CREATE INDEX IF NOT EXISTS ix_orders_tenant_customer_created
    ON dtms.transport_orders (tenant_id, customer_id, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_orders_external_reference
    ON dtms.transport_orders (tenant_id, customer_reference)
    WHERE customer_reference IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_order_status_history_order_time
    ON dtms.order_status_history (order_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_shipments_tenant_status_pickup
    ON dtms.shipments (tenant_id, status, planned_pickup_at);
CREATE INDEX IF NOT EXISTS ix_shipments_tenant_customer_created
    ON dtms.shipments (tenant_id, customer_id, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_shipment_status_history_time
    ON dtms.shipment_status_history (shipment_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_runs_tenant_service_status
    ON dtms.transport_runs (tenant_id, service_date, status);
CREATE INDEX IF NOT EXISTS ix_runs_tenant_assignment
    ON dtms.transport_runs (tenant_id, assignment_status, planned_start_at);
CREATE INDEX IF NOT EXISTS ix_run_status_history_time
    ON dtms.run_status_history (run_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_transport_events_entity_time
    ON dtms.transport_events (tenant_id, shipment_id, run_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_transport_events_received
    ON dtms.transport_events (tenant_id, received_at DESC);
CREATE INDEX IF NOT EXISTS ix_transport_events_occurred_brin
    ON dtms.transport_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS ix_transport_events_payload_gin
    ON dtms.transport_events USING gin (payload jsonb_path_ops);
CREATE INDEX IF NOT EXISTS ix_gps_positions_vehicle_time
    ON dtms.gps_positions (tenant_id, vehicle_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_gps_positions_run_time
    ON dtms.gps_positions (tenant_id, run_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_gps_positions_occurred_brin
    ON dtms.gps_positions USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS ix_sensor_readings_sensor_time
    ON dtms.sensor_readings (sensor_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_sensor_readings_occurred_brin
    ON dtms.sensor_readings USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS ix_exceptions_open
    ON dtms.execution_exceptions (tenant_id, severity, due_at)
    WHERE status NOT IN ('RESOLVED','CLOSED','CANCELLED');
CREATE INDEX IF NOT EXISTS ix_charges_entity_stage
    ON dtms.charges (tenant_id, shipment_id, run_id, direction, stage);
CREATE INDEX IF NOT EXISTS ix_charges_parties_service_date
    ON dtms.charges (tenant_id, payer_partner_id, payee_partner_id, service_date);
CREATE INDEX IF NOT EXISTS ix_invoices_partner_status_due
    ON dtms.invoices (tenant_id, recipient_partner_id, status, due_date);
CREATE INDEX IF NOT EXISTS ix_claims_tenant_status_date
    ON dtms.claims (tenant_id, status, claim_date DESC);
CREATE INDEX IF NOT EXISTS ix_login_events_user_time
    ON dtms.login_events (user_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_login_events_occurred_brin
    ON dtms.login_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS ix_audit_events_entity_time
    ON dtms.audit_events (tenant_id, entity_type, entity_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_audit_events_occurred_brin
    ON dtms.audit_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS ix_outbox_unpublished
    ON dtms.outbox_events (available_at, occurred_at)
    WHERE published_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_integration_messages_pending
    ON dtms.integration_messages (tenant_id, status, received_at)
    WHERE processed_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_entity_files_lookup
    ON dtms.entity_files (tenant_id, entity_type, entity_id, document_type);
CREATE INDEX IF NOT EXISTS ix_custom_fields_entity
    ON dtms.custom_field_values (tenant_id, entity_id);

-- Global ISO/reference seeds intentionally contain only defaults needed to bootstrap DTMS.
INSERT INTO dtms.currencies (currency_code, currency_name, numeric_code, fraction_digits, rounding_increment)
VALUES
...
INSERT INTO dtms.countries (country_code, country_code3, numeric_code, country_name_en, country_name_local, default_currency_code)
VALUES
...
INSERT INTO dtms.units_of_measure (uom_code, uom_name, dimension, si_factor, decimal_places)
VALUES
...
INSERT INTO dtms.transport_modes (mode_code, mode_name, mode_group, is_scheduled)
VALUES
...
INSERT INTO dtms.permissions (permission_code, permission_name, module_code, action_code, is_sensitive)
VALUES
...
INSERT INTO dtms.roles (tenant_id, role_code, role_name, description, is_system_role)
...
WHERE NOT EXISTS (
    SELECT 1 FROM dtms.roles r WHERE r.tenant_id IS NULL AND r.role_code = v.role_code
);

-- System administrator gets all permissions. Other role grants are deliberately least-privilege.
INSERT INTO dtms.role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM dtms.roles r CROSS JOIN dtms.permissions p
...
SELECT r.id, p.id
FROM dtms.roles r
JOIN dtms.permissions p ON p.permission_code = ANY (
    CASE r.role_code
...
COMMENT ON SCHEMA dtms IS 'Enterprise transportation management system: IAM, master, order, planning, dispatch, execution, performance, settlement, accounting, analytics, and integration.';
COMMENT ON TABLE dtms.users IS 'Global application principals mapped one-to-one to Firebase UID. Tenant access is controlled by tenant_memberships.';
COMMENT ON COLUMN dtms.users.firebase_uid IS 'Canonical Firebase token subject (uid); do not identify users by mutable email.';
COMMENT ON TABLE dtms.transport_orders IS 'Customer transportation demand. Orders can split across shipments and multiple orders can consolidate into one shipment.';
COMMENT ON TABLE dtms.shipments IS 'Commercial/planned movement grouping one or more orders, potentially across several modal legs.';
COMMENT ON TABLE dtms.transport_runs IS 'Physical operating trip/vehicle rotation. Shipment legs are allocated to runs independently of commercial shipment structure.';
COMMENT ON TABLE dtms.transport_events IS 'Append-oriented execution event ledger. occurred_at and received_at are separate to preserve delayed/out-of-order events.';
COMMENT ON TABLE dtms.charges IS 'Canonical BUY/SELL charge ledger across estimate, accrual, actual, invoice, and settlement stages; corrections use adjustments or reversals.';
COMMENT ON TABLE dtms.files IS 'Object-storage metadata only. Binary POD photos, signatures, and documents are not stored in PostgreSQL.';
