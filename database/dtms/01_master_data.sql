-- DTMS reference data, organization, partner, location, cargo, and fleet masters.

CREATE TABLE IF NOT EXISTS dtms.countries (
...
CREATE TABLE IF NOT EXISTS dtms.currencies (
...
CREATE TABLE IF NOT EXISTS dtms.units_of_measure (
...
CREATE TABLE IF NOT EXISTS dtms.unit_conversions (
    from_uom_code varchar(20) NOT NULL REFERENCES dtms.units_of_measure(uom_code),
    to_uom_code varchar(20) NOT NULL REFERENCES dtms.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS dtms.transport_modes (
...
CREATE TABLE IF NOT EXISTS dtms.service_levels (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_service_levels_scope
    ON dtms.service_levels (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        service_level_code
    );

CREATE TABLE IF NOT EXISTS dtms.equipment_types (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_equipment_types_scope
    ON dtms.equipment_types (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        equipment_type_code
    );

CREATE TABLE IF NOT EXISTS dtms.vehicle_types (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_vehicle_types_scope
    ON dtms.vehicle_types (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        vehicle_type_code
    );

CREATE TABLE IF NOT EXISTS dtms.payment_terms (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_payment_terms_scope
    ON dtms.payment_terms (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        payment_term_code
    );

CREATE TABLE IF NOT EXISTS dtms.charge_codes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_charge_codes_scope
    ON dtms.charge_codes (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        charge_code
    );

CREATE TABLE IF NOT EXISTS dtms.tax_codes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
    tax_code varchar(50) NOT NULL,
    tax_name varchar(150) NOT NULL,
    country_code char(2) REFERENCES dtms.countries(country_code),
...
CREATE TABLE IF NOT EXISTS dtms.tax_rates (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_tax_rates_scope_date
    ON dtms.tax_rates (tax_code_id, COALESCE(jurisdiction_code, ''), valid_from);

CREATE TABLE IF NOT EXISTS dtms.exchange_rates (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
    rate_date date NOT NULL,
    from_currency_code char(3) NOT NULL REFERENCES dtms.currencies(currency_code),
    to_currency_code char(3) NOT NULL REFERENCES dtms.currencies(currency_code),
...
ALTER TABLE dtms.countries
    ...
    REFERENCES dtms.currencies(currency_code);

ALTER TABLE dtms.tenants
...
CREATE TABLE IF NOT EXISTS dtms.cost_centers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS dtms.business_partners (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    default_currency_code char(3) REFERENCES dtms.currencies(currency_code),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_partners_registration_active
    ON dtms.business_partners (tenant_id, country_code, business_registration_no)
    WHERE business_registration_no IS NOT NULL AND deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS dtms.partner_roles (
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.partner_identifiers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.partner_relationships (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    parent_partner_id uuid NOT NULL REFERENCES dtms.business_partners(id),
    child_partner_id uuid NOT NULL REFERENCES dtms.business_partners(id),
...
CREATE TABLE IF NOT EXISTS dtms.addresses (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    country_code char(2) NOT NULL REFERENCES dtms.countries(country_code),
...
CREATE TABLE IF NOT EXISTS dtms.partner_addresses (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
    address_id uuid NOT NULL REFERENCES dtms.addresses(id),
...
CREATE TABLE IF NOT EXISTS dtms.contacts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.partner_contacts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
    contact_id uuid NOT NULL REFERENCES dtms.contacts(id),
...
CREATE TABLE IF NOT EXISTS dtms.partner_bank_accounts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
    bank_country_code char(2) REFERENCES dtms.countries(country_code),
...
CREATE TABLE IF NOT EXISTS dtms.partner_tax_profiles (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
    country_code char(2) NOT NULL REFERENCES dtms.countries(country_code),
    tax_registration_no varchar(80) NOT NULL,
    tax_classification varchar(50),
    default_tax_code_id uuid REFERENCES dtms.tax_codes(id),
...
CREATE TABLE IF NOT EXISTS dtms.partner_certifications (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.carrier_profiles (
    partner_id uuid PRIMARY KEY REFERENCES dtms.business_partners(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.carrier_insurance_policies (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    carrier_id uuid NOT NULL REFERENCES dtms.carrier_profiles(partner_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.locations (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    calendar_id uuid REFERENCES dtms.business_calendars(id),
...
CREATE TABLE IF NOT EXISTS dtms.location_contacts (
    location_id uuid NOT NULL REFERENCES dtms.locations(id) ON DELETE CASCADE,
    contact_id uuid NOT NULL REFERENCES dtms.contacts(id),
    contact_role varchar(50) NOT NULL,
    is_primary boolean NOT NULL DEFAULT false,
...
CREATE TABLE IF NOT EXISTS dtms.location_operating_hours (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    location_id uuid NOT NULL REFERENCES dtms.locations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.location_docks (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    location_id uuid NOT NULL REFERENCES dtms.locations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.geofences (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    location_id uuid REFERENCES dtms.locations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.location_blackouts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    location_id uuid NOT NULL REFERENCES dtms.locations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.commodity_classes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.items (
    ...
    base_uom_code varchar(20) NOT NULL REFERENCES dtms.units_of_measure(uom_code),
    ...
    value_currency_code char(3) REFERENCES dtms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dtms.item_packagings (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    item_id uuid NOT NULL REFERENCES dtms.items(id) ON DELETE CASCADE,
    packaging_code varchar(50) NOT NULL,
    packaging_level varchar(30) NOT NULL,
    units_per_package numeric(20,6) NOT NULL,
    package_uom_code varchar(20) NOT NULL REFERENCES dtms.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS dtms.dangerous_goods_profiles (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    item_id uuid NOT NULL REFERENCES dtms.items(id) ON DELETE CASCADE,
    ...
    sds_file_id uuid REFERENCES dtms.files(id),
    valid_from date NOT NULL DEFAULT CURRENT_DATE,
    valid_to date,
    UNIQUE (item_id, un_number, valid_from)
);

CREATE TABLE IF NOT EXISTS dtms.transport_requirement_profiles (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    item_id uuid NOT NULL REFERENCES dtms.items(id) ON DELETE CASCADE,
    min_temperature_c numeric(8,3),
    max_temperature_c numeric(8,3),
...
CREATE TABLE IF NOT EXISTS dtms.drivers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    home_location_id uuid REFERENCES dtms.locations(id),
    preferred_language varchar(20) DEFAULT 'ko',
    status varchar(20) NOT NULL DEFAULT 'ACTIVE',
    firebase_user_id uuid REFERENCES dtms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    deleted_at timestamptz,
...
CREATE TABLE IF NOT EXISTS dtms.driver_licenses (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    driver_id uuid NOT NULL REFERENCES dtms.drivers(id) ON DELETE CASCADE,
    license_type varchar(50) NOT NULL,
    license_no_encrypted text NOT NULL,
    license_no_search_hash char(64) NOT NULL,
    issuing_country_code char(2) REFERENCES dtms.countries(country_code),
    issued_at date,
...
CREATE TABLE IF NOT EXISTS dtms.driver_certifications (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    driver_id uuid NOT NULL REFERENCES dtms.drivers(id) ON DELETE CASCADE,
    certification_type varchar(80) NOT NULL,
    certificate_no varchar(150),
...
CREATE TABLE IF NOT EXISTS dtms.driver_availability (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    driver_id uuid NOT NULL REFERENCES dtms.drivers(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.vehicles (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    owner_partner_id uuid REFERENCES dtms.business_partners(id),
    operating_carrier_id uuid REFERENCES dtms.business_partners(id),
    vehicle_code varchar(80) NOT NULL,
    registration_no varchar(50) NOT NULL,
    vin_encrypted text,
    vin_search_hash char(64),
    vehicle_type_id uuid NOT NULL REFERENCES dtms.vehicle_types(id),
...
CREATE TABLE IF NOT EXISTS dtms.trailers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    owner_partner_id uuid REFERENCES dtms.business_partners(id),
    trailer_code varchar(80) NOT NULL,
    registration_no varchar(50),
    equipment_type_id uuid NOT NULL REFERENCES dtms.equipment_types(id),
    max_payload_kg numeric(20,6),
    max_volume_m3 numeric(20,6),
    home_location_id uuid REFERENCES dtms.locations(id),
    status varchar(20) NOT NULL DEFAULT 'AVAILABLE',
...
CREATE TABLE IF NOT EXISTS dtms.containers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    owner_partner_id uuid REFERENCES dtms.business_partners(id),
...
CREATE TABLE IF NOT EXISTS dtms.equipment_documents (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.equipment_availability (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    equipment_kind varchar(20) NOT NULL,
    equipment_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS dtms.odometer_readings (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    vehicle_id uuid NOT NULL REFERENCES dtms.vehicles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.maintenance_work_orders (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    work_order_no varchar(80) NOT NULL,
    vehicle_id uuid REFERENCES dtms.vehicles(id),
    ...
    odometer_km numeric(20,3),
    vendor_partner_id uuid REFERENCES dtms.business_partners(id),
...
CREATE TABLE IF NOT EXISTS dtms.telematics_devices (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    provider_partner_id uuid REFERENCES dtms.business_partners(id),
    status varchar(20) NOT NULL DEFAULT 'ACTIVE',
...
CREATE TABLE IF NOT EXISTS dtms.device_assignments (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    device_id uuid NOT NULL REFERENCES dtms.telematics_devices(id),
...
CREATE TABLE IF NOT EXISTS dtms.service_regions (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.carrier_service_regions (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    carrier_id uuid NOT NULL REFERENCES dtms.carrier_profiles(partner_id) ON DELETE CASCADE,
    service_region_id uuid NOT NULL REFERENCES dtms.service_regions(id),
    ...
    capacity_uom_code varchar(20) REFERENCES dtms.units_of_measure(uom_code)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_carrier_service_regions_scope
    ON dtms.carrier_service_regions (
        carrier_id, service_region_id, mode_code,
        COALESCE(service_level_id, '00000000-0000-0000-0000-000000000000'::uuid),
        valid_from
    );

CREATE TABLE IF NOT EXISTS dtms.lanes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    lane_code varchar(80) NOT NULL,
    lane_name varchar(200) NOT NULL,
    origin_location_id uuid REFERENCES dtms.locations(id),
    origin_region_id uuid REFERENCES dtms.service_regions(id),
    destination_location_id uuid REFERENCES dtms.locations(id),
    destination_region_id uuid REFERENCES dtms.service_regions(id),
    mode_code varchar(20) NOT NULL REFERENCES dtms.transport_modes(mode_code),
    standard_distance_km numeric(16,3),
    standard_transit_minutes integer,
    toll_estimate numeric(20,4),
    toll_currency_code char(3) REFERENCES dtms.currencies(currency_code),
    is_active boolean NOT NULL DEFAULT true,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS dtms.lane_points (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    lane_id uuid NOT NULL REFERENCES dtms.lanes(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.routes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.route_legs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    route_id uuid NOT NULL REFERENCES dtms.routes(id) ON DELETE CASCADE,
    sequence_no integer NOT NULL,
    from_location_id uuid NOT NULL REFERENCES dtms.locations(id),
    to_location_id uuid NOT NULL REFERENCES dtms.locations(id),
    lane_id uuid REFERENCES dtms.lanes(id),
...
CREATE TABLE IF NOT EXISTS dtms.location_distance_matrix (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    from_location_id uuid NOT NULL REFERENCES dtms.locations(id),
    to_location_id uuid NOT NULL REFERENCES dtms.locations(id),
