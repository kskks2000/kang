-- DTMS commercial contracts, rates, quotes, fuel indexes, SLA, and capacity commitments.

CREATE TABLE IF NOT EXISTS dtms.rate_contracts (
...
CREATE TABLE IF NOT EXISTS dtms.rate_contract_versions (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    contract_id uuid NOT NULL REFERENCES dtms.rate_contracts(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.contract_parties (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    contract_id uuid NOT NULL REFERENCES dtms.rate_contracts(id) ON DELETE CASCADE,
    partner_id uuid NOT NULL REFERENCES dtms.business_partners(id),
    party_role varchar(30) NOT NULL,
    is_primary boolean NOT NULL DEFAULT false,
    billing_currency_code char(3) REFERENCES dtms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dtms.contract_service_scopes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    contract_version_id uuid NOT NULL REFERENCES dtms.rate_contract_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.rate_lanes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    contract_version_id uuid NOT NULL REFERENCES dtms.rate_contract_versions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.rate_rules (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    contract_version_id uuid NOT NULL REFERENCES dtms.rate_contract_versions(id) ON DELETE CASCADE,
    rate_lane_id uuid REFERENCES dtms.rate_lanes(id) ON DELETE CASCADE,
    ...
    basis_uom_code varchar(20) REFERENCES dtms.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS dtms.rate_rule_tiers (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    rate_rule_id uuid NOT NULL REFERENCES dtms.rate_rules(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.accessorial_rules (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    contract_version_id uuid NOT NULL REFERENCES dtms.rate_contract_versions(id) ON DELETE CASCADE,
    charge_code_id uuid NOT NULL REFERENCES dtms.charge_codes(id),
    ...
    grace_uom_code varchar(20) REFERENCES dtms.units_of_measure(uom_code),
    unit_rate numeric(24,8),
    flat_amount numeric(20,4),
    maximum_amount numeric(20,4),
    currency_code char(3) NOT NULL REFERENCES dtms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dtms.fuel_indices (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    ...
    currency_code char(3) NOT NULL REFERENCES dtms.currencies(currency_code),
    price_uom_code varchar(20) NOT NULL REFERENCES dtms.units_of_measure(uom_code),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_fuel_indices_scope
    ON dtms.fuel_indices (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), index_code
    );

CREATE TABLE IF NOT EXISTS dtms.fuel_index_values (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    fuel_index_id uuid NOT NULL REFERENCES dtms.fuel_indices(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.fuel_surcharge_rules (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    contract_version_id uuid NOT NULL REFERENCES dtms.rate_contract_versions(id) ON DELETE CASCADE,
    fuel_index_id uuid NOT NULL REFERENCES dtms.fuel_indices(id),
    charge_code_id uuid NOT NULL REFERENCES dtms.charge_codes(id),
...
CREATE TABLE IF NOT EXISTS dtms.rate_quotes (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    CONSTRAINT ck_rate_quotes_amounts CHECK (net_amount >= 0 AND tax_amount >= 0 AND gross_amount >= 0)
);

CREATE TABLE IF NOT EXISTS dtms.rate_quote_lines (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    quote_id uuid NOT NULL REFERENCES dtms.rate_quotes(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.rating_runs (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    run_type varchar(20) NOT NULL,
    entity_type varchar(30) NOT NULL,
    entity_id uuid NOT NULL,
    requested_currency_code char(3) REFERENCES dtms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dtms.rating_run_details (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    rating_run_id uuid NOT NULL REFERENCES dtms.rating_runs(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.sla_policies (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.sla_policy_milestones (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    sla_policy_id uuid NOT NULL REFERENCES dtms.sla_policies(id) ON DELETE CASCADE,
    ...
    business_calendar_id uuid REFERENCES dtms.business_calendars(id),
...
CREATE TABLE IF NOT EXISTS dtms.carrier_capacity_commitments (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    contract_version_id uuid REFERENCES dtms.rate_contract_versions(id),
    carrier_id uuid NOT NULL REFERENCES dtms.business_partners(id),
    lane_id uuid REFERENCES dtms.lanes(id),
    service_region_id uuid REFERENCES dtms.service_regions(id),
    equipment_type_id uuid REFERENCES dtms.equipment_types(id),
