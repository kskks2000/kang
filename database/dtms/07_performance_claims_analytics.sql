-- DTMS transport results, SLA/KPI, carrier performance, emissions, claims, and reporting facts.

CREATE TABLE IF NOT EXISTS dtms.shipment_results (
    ...
    buy_currency_code char(3) REFERENCES dtms.currencies(currency_code),
    sell_currency_code char(3) REFERENCES dtms.currencies(currency_code),
...
CREATE TABLE IF NOT EXISTS dtms.run_results (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    ...
    energy_uom_code varchar(20) REFERENCES dtms.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS dtms.stop_results (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.run_leg_results (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    run_leg_execution_id uuid NOT NULL REFERENCES dtms.run_leg_executions(id),
    result_version integer NOT NULL DEFAULT 1,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_run_leg_results_final
    ON dtms.run_leg_results (run_leg_execution_id)
    WHERE is_final;

CREATE TABLE IF NOT EXISTS dtms.service_level_results (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    sla_policy_id uuid NOT NULL REFERENCES dtms.sla_policies(id),
    sla_milestone_id uuid REFERENCES dtms.sla_policy_milestones(id),
...
CREATE TABLE IF NOT EXISTS dtms.carrier_scorecards (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_carrier_scorecards_scope
    ON dtms.carrier_scorecards (
        tenant_id, carrier_id, period_type, period_start,
        COALESCE(mode_code, ''),
        COALESCE(lane_id, '00000000-0000-0000-0000-000000000000'::uuid)
    );

CREATE TABLE IF NOT EXISTS dtms.carrier_scorecard_metrics (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    scorecard_id uuid NOT NULL REFERENCES dtms.carrier_scorecards(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.emission_factors (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    ...
    mode_code varchar(20) REFERENCES dtms.transport_modes(mode_code),
    vehicle_type_id uuid REFERENCES dtms.vehicle_types(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_emission_factors_scope
    ON dtms.emission_factors (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), factor_code, valid_from
    );

CREATE TABLE IF NOT EXISTS dtms.emission_calculations (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    entity_type varchar(20) NOT NULL,
    entity_id uuid NOT NULL,
    emission_scope varchar(30) NOT NULL,
    factor_id uuid REFERENCES dtms.emission_factors(id),
    methodology varchar(100) NOT NULL,
    methodology_version varchar(80) NOT NULL,
    ...
    fuel_uom_code varchar(20) REFERENCES dtms.units_of_measure(uom_code),
...
CREATE TABLE IF NOT EXISTS dtms.claims (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    claim_no varchar(100) NOT NULL,
    claim_type varchar(30) NOT NULL,
    claimant_partner_id uuid NOT NULL REFERENCES dtms.business_partners(id),
    respondent_partner_id uuid NOT NULL REFERENCES dtms.business_partners(id),
    liable_carrier_id uuid REFERENCES dtms.business_partners(id),
    order_id uuid REFERENCES dtms.transport_orders(id),
    shipment_id uuid REFERENCES dtms.shipments(id),
    run_id uuid REFERENCES dtms.transport_runs(id),
    incident_id uuid REFERENCES dtms.incidents(id),
    damage_report_id uuid REFERENCES dtms.damage_reports(id),
...
CREATE TABLE IF NOT EXISTS dtms.claim_items (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    claim_id uuid NOT NULL REFERENCES dtms.claims(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.claim_events (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    claim_id uuid NOT NULL REFERENCES dtms.claims(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.claim_reserves (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    claim_id uuid NOT NULL REFERENCES dtms.claims(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.claim_settlements (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    claim_id uuid NOT NULL REFERENCES dtms.claims(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dtms.customer_feedback (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.kpi_definitions (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
    ...
    owner_org_unit_id uuid REFERENCES dtms.organization_units(id),
    valid_from date NOT NULL,
    valid_to date,
    is_active boolean NOT NULL DEFAULT true,
    row_version bigint NOT NULL DEFAULT 1,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_definitions_scope
    ON dtms.kpi_definitions (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), kpi_code, calculation_version
    );

CREATE TABLE IF NOT EXISTS dtms.kpi_targets (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES dtms.kpi_definitions(id),
...
CREATE TABLE IF NOT EXISTS dtms.kpi_values (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    kpi_definition_id uuid NOT NULL REFERENCES dtms.kpi_definitions(id),
    scope_type varchar(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dtms.transport_daily_facts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
...
CREATE TABLE IF NOT EXISTS dtms.lane_performance_facts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    fact_month date NOT NULL,
    lane_id uuid NOT NULL REFERENCES dtms.lanes(id),
    carrier_id uuid REFERENCES dtms.business_partners(id),
    shipment_count bigint NOT NULL DEFAULT 0,
    total_weight_kg numeric(30,6) NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_lane_performance_facts_grain
    ON dtms.lane_performance_facts (
        tenant_id, fact_month, lane_id,
        COALESCE(carrier_id, '00000000-0000-0000-0000-000000000000'::uuid)
    );

CREATE TABLE IF NOT EXISTS dtms.carrier_performance_facts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    fact_month date NOT NULL,
    carrier_id uuid NOT NULL REFERENCES dtms.business_partners(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_carrier_performance_facts_grain
    ON dtms.carrier_performance_facts (
        tenant_id, fact_month, carrier_id, COALESCE(mode_code, '')
    );

CREATE TABLE IF NOT EXISTS dtms.financial_performance_facts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    fact_month date NOT NULL,
    organization_id uuid NOT NULL REFERENCES dtms.organizations(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_financial_performance_facts_grain
    ON dtms.financial_performance_facts (
...
CREATE TABLE IF NOT EXISTS dtms.fleet_utilization_facts (
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    fact_date date NOT NULL,
...
CREATE TABLE IF NOT EXISTS dtms.emission_facts (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    fact_month date NOT NULL,
    organization_id uuid NOT NULL REFERENCES dtms.organizations(id),
...
CREATE TABLE IF NOT EXISTS dtms.data_quality_rules (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid REFERENCES dtms.tenants(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_data_quality_rules_scope
    ON dtms.data_quality_rules (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), rule_code
    );

CREATE TABLE IF NOT EXISTS dtms.data_quality_results (
    id uuid PRIMARY KEY DEFAULT dtms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
    rule_id uuid NOT NULL REFERENCES dtms.data_quality_rules(id),
...
CREATE TABLE IF NOT EXISTS dtms.etl_watermarks (
    tenant_id uuid NOT NULL REFERENCES dtms.tenants(id),
