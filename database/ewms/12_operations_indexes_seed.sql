-- EWMS enterprise WMS schema - operational integrity, RLS, indexes, and baseline seeds.
-- PostgreSQL 11 compatible. Applied last, after all 00-10 modules, in one transaction.

CREATE OR REPLACE FUNCTION ewms.prevent_tenant_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.enforce_same_tenant_fk()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION ewms.prevent_append_only_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.protect_message_core()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION ewms.protect_finalized_row()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'settlements' AND TG_OP = 'UPDATE' THEN
        IF ewms.finance_projection_authorized('PAYMENT_APPLICATION') THEN
            RETURN NEW;
        END IF;
...
CREATE OR REPLACE FUNCTION ewms.protect_committed_row()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'settlements' AND TG_OP = 'UPDATE'
       AND ewms.finance_projection_authorized('PAYMENT_APPLICATION') THEN
...
CREATE OR REPLACE FUNCTION ewms.protect_versioned_filing_aggregate()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
    ...
    IF entering_protected_state
       ...
       OR new_version_no IS DISTINCT FROM OLD.current_version_no THEN
        EXECUTE format(
            ...
            'FROM ewms.%I WHERE id = $1 FOR SHARE',
            TG_ARGV[3], TG_ARGV[2]
...
CREATE OR REPLACE FUNCTION ewms.protect_child_when_parent_final()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF old_parent_id IS NOT NULL THEN
        EXECUTE format('SELECT %I::text FROM ewms.%I WHERE id = $1 FOR SHARE', TG_ARGV[2], TG_ARGV[1])
           INTO parent_status
           USING old_parent_id;
...
CREATE OR REPLACE FUNCTION ewms.validate_inventory_transaction_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
CREATE OR REPLACE FUNCTION ewms.protect_journal_line_when_posted()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF old_entry_id IS NOT NULL THEN
        SELECT b.status INTO batch_status
          FROM ewms.journal_entries e
          JOIN ewms.journal_batches b ON b.id = e.journal_batch_id
...
CREATE OR REPLACE FUNCTION ewms.protect_invoice_line_child_when_posted()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
CREATE OR REPLACE FUNCTION ewms.validate_journal_batch_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
          FROM ewms.journal_entries entry
          JOIN ewms.journal_lines line ON line.journal_entry_id = entry.id
          JOIN ewms.gl_accounts account ON account.id = line.gl_account_id
    ...
    IF EXISTS (
        SELECT 1
          FROM ewms.journal_entries entry
          LEFT JOIN ewms.journal_lines line ON line.journal_entry_id = entry.id
         WHERE entry.journal_batch_id = NEW.id
         GROUP BY entry.id
...
CREATE OR REPLACE FUNCTION ewms.validate_invoice_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE line.invoice_id = NEW.id
           AND EXISTS (
               SELECT 1
                 FROM ewms.invoice_line_charges allocation_history
    ...
    IF EXISTS (
        SELECT 1
          ...
          JOIN ewms.invoice_lines line ON line.id = allocation.invoice_line_id
          JOIN ewms.charges charge ON charge.id = allocation.charge_id
    ...
    IF EXISTS (
        SELECT 1
          FROM ewms.invoice_line_charges allocation
          JOIN ewms.settlement_lines settlement_line
            ON settlement_line.charge_id = allocation.charge_id
          JOIN ewms.settlements settlement
...
CREATE OR REPLACE FUNCTION ewms.protect_posted_invoice_core()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        IF OLD.status IN ('POSTED','PARTIALLY_PAID','PAID','CANCELLED','REVERSED')
           OR EXISTS (SELECT 1 FROM ewms.payment_applications WHERE invoice_id = OLD.id) THEN
...
CREATE OR REPLACE FUNCTION ewms.protect_sealed_version()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.protect_child_when_parent_sealed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF old_parent_id IS NOT NULL THEN
        EXECUTE format('SELECT sealed_at FROM ewms.%I WHERE id = $1 FOR SHARE', TG_ARGV[1])
           INTO parent_sealed_at
           USING old_parent_id;
        IF parent_sealed_at IS NOT NULL THEN
            RAISE EXCEPTION 'Child cannot move from or change under sealed ewms.% version %',
    ...
    IF new_parent_id IS NOT NULL AND new_parent_id IS DISTINCT FROM old_parent_id THEN
        EXECUTE format('SELECT sealed_at FROM ewms.%I WHERE id = $1 FOR SHARE', TG_ARGV[1])
           ...
           USING new_parent_id;
        IF parent_sealed_at IS NOT NULL THEN
            RAISE EXCEPTION 'Child cannot be added or moved under sealed ewms.% version %',
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_movement_posting()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM ewms.bonded_inventory_movements
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('ewms.%I', target.table_name)) IS NULL THEN
            CONTINUE;
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_row ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_row BEFORE UPDATE OR DELETE ON ewms.%I '
        ...
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_truncate ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_truncate BEFORE TRUNCATE ON ewms.%I '
...
DROP TRIGGER IF EXISTS trg_protect_customs_filing_aggregate ON ewms.customs_declarations;
CREATE TRIGGER trg_protect_customs_filing_aggregate
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.customs_declarations
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_versioned_filing_aggregate(
        'workflow_status',
...
DROP TRIGGER IF EXISTS trg_protect_bonded_filing_aggregate ON ewms.bonded_inout_reports;
CREATE TRIGGER trg_protect_bonded_filing_aggregate
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.bonded_inout_reports
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_versioned_filing_aggregate(
        'report_status',
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_committed_row ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_committed_row BEFORE INSERT OR UPDATE OR DELETE ON ewms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE ewms.protect_committed_row(%L,%L,%L)',
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('ewms.%I', target.table_name)) IS NULL THEN
            CONTINUE;
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_parent ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_parent BEFORE INSERT OR UPDATE OR DELETE ON ewms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE ewms.protect_child_when_parent_final(%L,%L,%L,%L)',
            target.table_name, target.parent_fk, target.parent_table,
            target.parent_status_column, target.terminal_statuses
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_finalized_child_truncate ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_finalized_child_truncate BEFORE TRUNCATE ON ewms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE ewms.prevent_append_only_change()',
            target.table_name
        );
    END LOOP;
...
CREATE TRIGGER trg_guard_inventory_entry_mutation
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.inventory_transaction_entries
...
CREATE TRIGGER trg_validate_inventory_transaction_posting
    BEFORE INSERT OR UPDATE OF posting_status ON ewms.inventory_transaction_headers
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_inventory_transaction_posting();

DROP TRIGGER IF EXISTS trg_validate_bonded_movement_posting ON ewms.bonded_movement_groups;
CREATE TRIGGER trg_validate_bonded_movement_posting
    BEFORE INSERT OR UPDATE OF status ON ewms.bonded_movement_groups
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_movement_posting();

DROP TRIGGER IF EXISTS trg_protect_bonded_report_version
    ON ewms.bonded_inout_report_versions;
CREATE TRIGGER trg_protect_bonded_report_version
    BEFORE UPDATE OR DELETE ON ewms.bonded_inout_report_versions
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_sealed_version();

DROP TRIGGER IF EXISTS trg_protect_bonded_report_line
    ON ewms.bonded_inout_report_lines;
CREATE TRIGGER trg_protect_bonded_report_line
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.bonded_inout_report_lines
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_child_when_parent_sealed(
        'bonded_inout_report_version_id','bonded_inout_report_versions'
    );

DROP TRIGGER IF EXISTS trg_protect_posted_journal_line ON ewms.journal_lines;
CREATE TRIGGER trg_protect_posted_journal_line
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.journal_lines
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_journal_line_when_posted();

DROP TRIGGER IF EXISTS trg_validate_journal_batch_posting ON ewms.journal_batches;
CREATE TRIGGER trg_validate_journal_batch_posting
    BEFORE INSERT OR UPDATE OF status ON ewms.journal_batches
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_journal_batch_posting();

DROP TRIGGER IF EXISTS trg_validate_invoice_posting ON ewms.invoices;
CREATE TRIGGER trg_validate_invoice_posting
    BEFORE INSERT OR UPDATE OF status ON ewms.invoices
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_invoice_posting();

DROP TRIGGER IF EXISTS trg_protect_posted_invoice_core ON ewms.invoices;
CREATE TRIGGER trg_protect_posted_invoice_core
    BEFORE UPDATE OR DELETE ON ewms.invoices
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_posted_invoice_core();

DROP TRIGGER IF EXISTS trg_protect_invoice_line_charge ON ewms.invoice_line_charges;
CREATE TRIGGER trg_protect_invoice_line_charge
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.invoice_line_charges
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_invoice_line_child_when_posted();
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_integrity_truncate ON ewms.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_integrity_truncate BEFORE TRUNCATE ON ewms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE ewms.prevent_append_only_change()',
            table_name
        );
    END LOOP;
...
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            SELECT 1
            FROM pg_trigger t
            WHERE t.tgrelid = format('ewms.%I', table_name)::regclass
              AND t.tgname IN ('trg_append_only','trg_block_immutable_change','trg_ewms_append_only')
              AND NOT t.tgisinternal
        ) THEN
            EXECUTE format(
                'CREATE TRIGGER trg_ewms_append_only BEFORE UPDATE OR DELETE ON ewms.%I '
                'FOR EACH ROW EXECUTE PROCEDURE ewms.prevent_append_only_change()',
                table_name
            );
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_ewms_append_only_truncate ON ewms.%I', table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_ewms_append_only_truncate BEFORE TRUNCATE ON ewms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE ewms.prevent_append_only_change()',
            table_name
        );
    END LOOP;
...
BEGIN
    ...
    LOOP
        ...
        IF target.table_name = 'unipass_messages' THEN
            EXECUTE 'DROP TRIGGER IF EXISTS trg_block_immutable_change ON ewms.unipass_messages';
            EXECUTE 'DROP TRIGGER IF EXISTS trg_ewms_append_only ON ewms.unipass_messages';
            EXECUTE 'DROP TRIGGER IF EXISTS trg_append_only ON ewms.unipass_messages';
        END IF;
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_message_core ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_message_core BEFORE UPDATE OR DELETE ON ewms.%I '
            'FOR EACH ROW EXECUTE PROCEDURE ewms.protect_message_core(%s)',
            target.table_name, target.trigger_arguments
        );
        EXECUTE format('DROP TRIGGER IF EXISTS trg_protect_message_truncate ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_protect_message_truncate BEFORE TRUNCATE ON ewms.%I '
            'FOR EACH STATEMENT EXECUTE PROCEDURE ewms.prevent_append_only_change()',
            target.table_name
        );
    END LOOP;
...
STABLE
AS $function$
    SELECT NULLIF(current_setting('ewms.tenant_id', true), '')::uuid;
$function$;
...
CREATE OR REPLACE FUNCTION ewms.post_inventory_transaction_for_tenant(
...
BEGIN
    context_tenant_id := ewms.current_tenant_id();
    ...
    SELECT header.tenant_id
      ...
      FROM ewms.inventory_transaction_headers header
    ...
    IF NOT EXISTS (
        SELECT 1
          ...
          JOIN ewms.users principal
            ON principal.id = membership.user_id
           AND principal.status = 'ACTIVE'
          JOIN ewms.role_assignments assignment
           ...
           AND (
               ...
               OR EXISTS (
                   SELECT 1
                     FROM ewms.user_group_members group_member
          ...
          JOIN ewms.role_permissions role_permission
          ...
          JOIN ewms.permissions permission
    ...
    PERFORM ewms.post_inventory_transaction(p_inventory_transaction_id, p_posted_by);
END;
$function$;

REVOKE ALL ON FUNCTION ewms.post_inventory_transaction_for_tenant(uuid, uuid) FROM PUBLIC;

COMMENT ON FUNCTION ewms.post_inventory_transaction_for_tenant(uuid, uuid) IS
...
BEGIN
    FOR relation IN
        ...
        WHERE con.contype = 'f' AND ns.nspname = 'ewms'
    ...
    LOOP
        IF NOT EXISTS (
            ...
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE ewms.prevent_tenant_change()',
                target.table_schema, target.table_name
            );
        END IF;
    END LOOP;
...
BEGIN
    FOR relation IN
        ...
        WHERE con.contype = 'f'
          AND child_ns.nspname = 'ewms'
          AND parent_ns.nspname = 'ewms'
...
CREATE INDEX IF NOT EXISTS ix_ops_inventory_tx_occurred_brin
    ON ewms.inventory_transaction_headers USING brin (occurred_at);
...
CREATE INDEX IF NOT EXISTS ix_ops_inventory_balance_last_tx
    ON ewms.inventory_balances (tenant_id, last_transaction_at DESC, stock_bucket_id);

CREATE INDEX IF NOT EXISTS ix_ops_receipts_work_queue
    ON ewms.receipts (tenant_id, warehouse_id, owner_partner_id, status, arrived_at, id)
...
CREATE INDEX IF NOT EXISTS ix_ops_pick_task_queue
    ON ewms.pick_tasks (tenant_id, warehouse_id, status, priority, scheduled_at, id)
    WHERE status NOT IN ('COMPLETED','CANCELLED');
CREATE INDEX IF NOT EXISTS ix_ops_shipments_status_schedule
    ON ewms.shipments (tenant_id, warehouse_id, status, planned_ship_at, id);

CREATE INDEX IF NOT EXISTS ix_ops_customs_declaration_queue
    ON ewms.customs_declarations (
...
INSERT INTO ewms.units_of_measure (
...
INSERT INTO ewms.transport_modes (mode_code, mode_name, mode_group, is_active) VALUES
    ('ROAD','Road','LAND',true),
    ('SEA','Sea','OCEAN',true),
    ('AIR','Air','AIR',true),
    ('RAIL','Rail','LAND',true),
...
INSERT INTO ewms.incoterms (incoterm_code, incoterm_name, version_year, is_active) VALUES
...
INSERT INTO ewms.inventory_statuses (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM ewms.inventory_statuses existing
...
INSERT INTO ewms.location_types (
...
WITH seed(role_code, role_name, description) AS (
    VALUES
        ('EWMS_VIEWER','EWMS Viewer','Read-only operational visibility; excludes privileged audit/data-access views.'),
        ('EWMS_OPERATOR','EWMS Operator','Inbound/outbound execution without ledger posting or customs/finance approval.'),
        ('EWMS_SUPERVISOR','EWMS Supervisor','Operational supervision and assigned workflow approvals.'),
        ('EWMS_INVENTORY_CONTROLLER','EWMS Inventory Controller','Inventory posting, adjustment and hold authority.'),
        ('EWMS_CUSTOMS_SPECIALIST','EWMS Customs Specialist','Customs and bonded filing authority.'),
        ('EWMS_BILLING_SPECIALIST','EWMS Billing Specialist','Billing and settlement approval authority.'),
        ('EWMS_AUDITOR','EWMS Auditor','Read-only audit, regulated access and control evidence.'),
        ('EWMS_TENANT_ADMIN','EWMS Tenant Administrator','IAM/configuration administration without inventory, customs or finance posting authority.')
)
INSERT INTO ewms.roles (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM ewms.roles existing
    WHERE existing.tenant_id IS NULL AND existing.role_code = s.role_code
);

WITH grants(role_code, permission_code) AS (
    VALUES
        ('EWMS_VIEWER','master.read'),
        ('EWMS_VIEWER','inbound.read'),
        ('EWMS_VIEWER','inventory.read'),
        ('EWMS_VIEWER','outbound.read'),
        ('EWMS_OPERATOR','master.read'),
        ('EWMS_OPERATOR','inbound.read'),
        ('EWMS_OPERATOR','inbound.execute'),
        ('EWMS_OPERATOR','inventory.read'),
        ('EWMS_OPERATOR','outbound.read'),
        ('EWMS_OPERATOR','outbound.execute'),
        ('EWMS_SUPERVISOR','master.read'),
        ('EWMS_SUPERVISOR','inbound.read'),
        ('EWMS_SUPERVISOR','inbound.execute'),
        ('EWMS_SUPERVISOR','inventory.read'),
        ('EWMS_SUPERVISOR','outbound.read'),
        ('EWMS_SUPERVISOR','outbound.execute'),
        ('EWMS_SUPERVISOR','workflow.approve'),
        ('EWMS_INVENTORY_CONTROLLER','master.read'),
        ('EWMS_INVENTORY_CONTROLLER','inbound.read'),
        ('EWMS_INVENTORY_CONTROLLER','inventory.read'),
        ('EWMS_INVENTORY_CONTROLLER','inventory.post'),
        ('EWMS_INVENTORY_CONTROLLER','inventory.adjust'),
        ('EWMS_INVENTORY_CONTROLLER','inventory.hold'),
        ('EWMS_INVENTORY_CONTROLLER','workflow.approve'),
        ('EWMS_CUSTOMS_SPECIALIST','master.read'),
        ('EWMS_CUSTOMS_SPECIALIST','inventory.read'),
        ('EWMS_CUSTOMS_SPECIALIST','customs.read'),
        ('EWMS_CUSTOMS_SPECIALIST','customs.submit'),
        ('EWMS_CUSTOMS_SPECIALIST','bonded.read'),
        ('EWMS_CUSTOMS_SPECIALIST','bonded.report'),
        ('EWMS_CUSTOMS_SPECIALIST','workflow.approve'),
        ('EWMS_BILLING_SPECIALIST','master.read'),
        ('EWMS_BILLING_SPECIALIST','billing.read'),
        ('EWMS_BILLING_SPECIALIST','billing.approve'),
        ('EWMS_BILLING_SPECIALIST','workflow.approve'),
        ('EWMS_AUDITOR','master.read'),
        ('EWMS_AUDITOR','inbound.read'),
        ('EWMS_AUDITOR','inventory.read'),
        ('EWMS_AUDITOR','outbound.read'),
        ('EWMS_AUDITOR','customs.read'),
        ('EWMS_AUDITOR','bonded.read'),
        ('EWMS_AUDITOR','billing.read'),
        ('EWMS_AUDITOR','audit.read'),
        ('EWMS_AUDITOR','data_access.read'),
        ('EWMS_AUDITOR','integration.monitor'),
        ('EWMS_TENANT_ADMIN','iam.user.read'),
        ('EWMS_TENANT_ADMIN','iam.role.manage'),
        ('EWMS_TENANT_ADMIN','config.manage'),
        ('EWMS_TENANT_ADMIN','master.read'),
        ('EWMS_TENANT_ADMIN','master.manage'),
        ('EWMS_TENANT_ADMIN','integration.monitor')
)
INSERT INTO ewms.role_permissions (
    tenant_id, role_id, permission_id, granted_at,
    row_version, created_at, updated_at
)
SELECT NULL, role_row.id, permission_row.id, now(), 1, now(), now()
FROM grants g
JOIN ewms.roles role_row
  ON role_row.tenant_id IS NULL AND role_row.role_code = g.role_code
JOIN ewms.permissions permission_row
  ON permission_row.permission_code = g.permission_code
WHERE NOT EXISTS (
    SELECT 1 FROM ewms.role_permissions existing
...
INSERT INTO ewms.data_retention_policies (
...
FROM seed s
WHERE NOT EXISTS (
    SELECT 1 FROM ewms.data_retention_policies existing
