-- EWMS enterprise WMS schema - Korea bonded facilities, cargo control, transport, and compliance.
...
CREATE TABLE IF NOT EXISTS ewms.bonded_facilities (
    ...
    operator_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    ...
    supervising_customs_office_code varchar(20) REFERENCES ewms.customs_offices(customs_office_code),
    ...
    UNIQUE (tenant_id, id),
    CONSTRAINT ck_ewms_bonded_facility_capacity CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_facility_status CHECK (status IN ('PLANNED','ACTIVE','SUSPENDED','EXPIRED','CLOSED')),
    CONSTRAINT ck_ewms_bonded_facility_dates CHECK (valid_to IS NULL OR valid_from IS NULL OR valid_to >= valid_from)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_facility_customs_code
    ON ewms.bonded_facilities (tenant_id, customs_facility_code)
    WHERE customs_facility_code IS NOT NULL AND deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS ewms.bonded_facility_licenses (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    ...
    issuing_customs_office_code varchar(20) REFERENCES ewms.customs_offices(customs_office_code),
    issued_on date NOT NULL,
    effective_from date NOT NULL,
    effective_to date,
    renewal_of_license_id uuid REFERENCES ewms.bonded_facility_licenses(id),
    suspension_from date,
    suspension_to date,
    revocation_on date,
    status varchar(20) NOT NULL DEFAULT 'ACTIVE',
    license_document_id uuid REFERENCES ewms.files(id),
    ...
    FOREIGN KEY (tenant_id, bonded_facility_id)
        REFERENCES ewms.bonded_facilities(tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_license_dates CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_license_renewal CHECK (renewal_of_license_id IS NULL OR renewal_of_license_id <> id),
    CONSTRAINT ck_ewms_bonded_license_status CHECK (status IN ('PENDING','ACTIVE','SUSPENDED','EXPIRED','REVOKED','SUPERSEDED'))
);

CREATE TABLE IF NOT EXISTS ewms.bonded_zone_controls (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    bonded_facility_id uuid NOT NULL,
    facility_license_id uuid REFERENCES ewms.bonded_facility_licenses(id),
    warehouse_zone_id uuid REFERENCES ewms.warehouse_zones(id),
    warehouse_location_id uuid REFERENCES ewms.warehouse_locations(id),
    ...
    FOREIGN KEY (tenant_id, bonded_facility_id)
        REFERENCES ewms.bonded_facilities(tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_zone_scope CHECK (num_nonnulls(warehouse_zone_id, warehouse_location_id) >= 1),
    CONSTRAINT ck_ewms_bonded_zone_type CHECK (control_type IN ('RECEIVING','STORAGE','INSPECTION','HANDLING','RELEASE','DISPOSAL','TRANSIT','OTHER')),
    CONSTRAINT ck_ewms_bonded_zone_status CHECK (status IN ('ACTIVE','BLOCKED','SUSPENDED','INACTIVE')),
    CONSTRAINT ck_ewms_bonded_zone_dates CHECK (active_to IS NULL OR active_to > active_from)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_staff_assignments (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    bonded_facility_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, bonded_facility_id)
        REFERENCES ewms.bonded_facilities(tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_staff_dates CHECK (valid_to IS NULL OR valid_to >= valid_from),
    CONSTRAINT ck_ewms_bonded_staff_status CHECK (status IN ('PENDING','ACTIVE','SUSPENDED','EXPIRED','REVOKED'))
);

CREATE TABLE IF NOT EXISTS ewms.bonded_storage_rules (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    bonded_facility_id uuid REFERENCES ewms.bonded_facilities(id),
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ck_ewms_bonded_storage_rule_period CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_storage_rule_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_storage_rule_scope
    ON ewms.bonded_storage_rules (
        COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
        COALESCE(bonded_facility_id, '00000000-0000-0000-0000-000000000000'::uuid),
        rule_code, effective_from
    );

CREATE TABLE IF NOT EXISTS ewms.bonded_cargo (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    shipper_partner_id uuid REFERENCES ewms.business_partners(id),
    ...
    FOREIGN KEY (tenant_id, bonded_facility_id)
        REFERENCES ewms.bonded_facilities(tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_cargo_physical CHECK (physical_status IN (
        'EXPECTED','ARRIVED','RECEIVING','STORED','HANDLING','STAGED_OUT','PARTIALLY_RELEASED',
        'RELEASED','IN_TRANSIT','MISSING','DESTROYED','CLOSED'
    )),
    CONSTRAINT ck_ewms_bonded_cargo_legal CHECK (legal_status IN (
        'FOREIGN','DOMESTIC','EXPORT','CONDITIONAL_RELEASE','BONDED_TRANSIT','SEIZED','ABANDONED','DISPOSAL_PENDING','DISPOSED'
    )),
    CONSTRAINT ck_ewms_bonded_cargo_hold CHECK (customs_hold_status IN ('CLEAR','HOLD','INSPECTION','SEIZED','RELEASED')),
    CONSTRAINT ck_ewms_bonded_cargo_qty CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_cargo_dates CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_cargo_management_no
    ON ewms.bonded_cargo (tenant_id, owner_partner_id, cargo_management_no_normalized)
    WHERE cargo_management_no_normalized IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_ewms_bonded_cargo_due
    ON ewms.bonded_cargo (tenant_id, bonded_facility_id, current_storage_due_on)
    WHERE physical_status NOT IN ('RELEASED','DESTROYED','CLOSED');

CREATE TABLE IF NOT EXISTS ewms.bonded_cargo_status_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_cargo_event_seq CHECK (sequence_no > 0),
    CONSTRAINT ck_ewms_bonded_cargo_event_change CHECK (
        to_physical_status IS NOT NULL OR to_legal_status IS NOT NULL OR event_type IS NOT NULL
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_cargo_items (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_cargo_id uuid NOT NULL,
    ...
    customs_declaration_line_id uuid REFERENCES ewms.customs_declaration_lines(id),
    commercial_invoice_line_id uuid REFERENCES ewms.commercial_invoice_lines(id),
    inventory_lot_id uuid REFERENCES ewms.inventory_lots(id),
    serial_number_id uuid REFERENCES ewms.serial_numbers(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_cargo_item_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_cargo_item_qty CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_cargo_item_weight CHECK (
        (package_count IS NULL OR package_count >= 0) AND (gross_weight IS NULL OR gross_weight >= 0) AND
        (net_weight IS NULL OR net_weight >= 0) AND
        (gross_weight IS NULL OR net_weight IS NULL OR gross_weight >= net_weight)
    ),
    CONSTRAINT ck_ewms_bonded_cargo_item_status CHECK (status IN (
        'EXPECTED','RECEIVED','STORED','PARTIALLY_RELEASED','RELEASED','DAMAGED','MISSING','DESTROYED','CLOSED'
    ))
);

CREATE TABLE IF NOT EXISTS ewms.bonded_cargo_relations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    parent_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    child_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    ...
    CONSTRAINT ck_ewms_bonded_cargo_relation_self CHECK (parent_cargo_id <> child_cargo_id),
    CONSTRAINT ck_ewms_bonded_cargo_relation_type CHECK (relation_type IN ('SPLIT','MERGE','REPACK','SUCCESSOR','RETURN','RELATED')),
    CONSTRAINT ck_ewms_bonded_cargo_relation_qty CHECK (
        (parent_quantity IS NULL OR parent_quantity > 0) AND (child_quantity IS NULL OR child_quantity > 0)
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_cargo_containers (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_cargo_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_cargo_container_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (gross_weight IS NULL OR gross_weight >= 0)
    ),
    CONSTRAINT ck_ewms_bonded_cargo_container_dates CHECK (unstuffed_at IS NULL OR stuffed_at IS NULL OR unstuffed_at >= stuffed_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_inout_reports (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    bonded_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    ...
    accepted_unipass_message_id uuid REFERENCES ewms.unipass_messages(id),
    retention_until date,
    legal_hold boolean NOT NULL DEFAULT false,
    row_version bigint NOT NULL DEFAULT 1,
    created_by uuid REFERENCES ewms.users(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_report_type CHECK (report_type IN ('INBOUND','OUTBOUND','DISCREPANCY','CORRECTION','CANCELLATION')),
    CONSTRAINT ck_ewms_bonded_report_status CHECK (report_status IN (
        'DRAFT','PREPARED','SUBMITTED','ACCEPTED','REJECTED','CORRECTION_REQUIRED','WITHDRAWN','CANCELLED'
    )),
    CONSTRAINT ck_ewms_bonded_report_acceptance CHECK (acceptance_status IN (
        'NOT_SUBMITTED','PENDING','ACCEPTED','REJECTED','ERROR','WITHDRAWN'
    )),
    CONSTRAINT ck_ewms_bonded_report_version CHECK (
        (current_version_id IS NULL AND current_version_no IS NULL) OR
        (current_version_id IS NOT NULL AND current_version_no > 0)
    ),
    CONSTRAINT ck_ewms_bonded_report_acceptance_fields CHECK (
        (acceptance_status <> 'ACCEPTED') OR (accepted_at IS NOT NULL AND accepted_unipass_message_id IS NOT NULL)
    ),
    CONSTRAINT ck_ewms_bonded_report_dates CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_customs_report_no
    ON ewms.bonded_inout_reports (tenant_id, customs_report_no_normalized)
    WHERE customs_report_no_normalized IS NOT NULL;

CREATE TABLE IF NOT EXISTS ewms.bonded_inout_report_versions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    supersedes_version_id uuid REFERENCES ewms.bonded_inout_report_versions(id),
    ...
    declared_base_uom_code varchar(20) REFERENCES ewms.units_of_measure(uom_code),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_inout_report_id)
        REFERENCES ewms.bonded_inout_reports(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_report_version_no CHECK (version_no > 0),
    CONSTRAINT ck_ewms_bonded_report_version_action CHECK (filing_action IN (
        'ORIGINAL','CORRECTION','CANCELLATION','RESUBMISSION','OFFICIAL_CORRECTION'
    )),
    CONSTRAINT ck_ewms_bonded_report_version_supersedes CHECK (supersedes_version_id IS NULL OR supersedes_version_id <> id),
    CONSTRAINT ck_ewms_bonded_report_version_qty CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_report_version_seal CHECK (
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'ewms.bonded_inout_reports'::regclass
          AND conname = 'fk_ewms_bonded_report_current_version'
    ) THEN
        ALTER TABLE ewms.bonded_inout_reports
            ADD CONSTRAINT fk_ewms_bonded_report_current_version
            FOREIGN KEY (id, current_version_id)
            REFERENCES ewms.bonded_inout_report_versions(bonded_inout_report_id, id)
            DEFERRABLE INITIALLY DEFERRED;
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS ewms.bonded_inout_report_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_inout_report_version_id uuid NOT NULL,
    line_no integer NOT NULL,
    bonded_cargo_item_id uuid NOT NULL REFERENCES ewms.bonded_cargo_items(id),
    warehouse_location_id uuid REFERENCES ewms.warehouse_locations(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_inout_report_version_id)
        REFERENCES ewms.bonded_inout_report_versions(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_report_line_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_report_line_qty CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.unipass_message_results (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    ...
    subject_owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    ...
    UNIQUE (tenant_id, owner_partner_id, id),
    CONSTRAINT ck_ewms_unipass_result_sequence CHECK (result_sequence > 0),
    CONSTRAINT ck_ewms_unipass_result_purpose CHECK (
        result_purpose IN ('BONDED_INOUT_ACCEPTANCE')
    ),
    CONSTRAINT ck_ewms_unipass_result_reference CHECK (
        legal_reference_type = 'BONDED_INOUT_REPORT' AND
        btrim(legal_reference_no_normalized) <> '' AND legal_version_no > 0
    ),
    CONSTRAINT ck_ewms_unipass_result_decision CHECK (
        decision_code IN ('BONDED_REPORT_ACCEPTED')
    ),
    CONSTRAINT ck_ewms_unipass_result_hash CHECK (
        payload_sha256_snapshot ~ '^[0-9A-Fa-f]{64}$'
    ),
    CONSTRAINT ck_ewms_unipass_result_times CHECK (
        transport_authenticated_at <= schema_validated_at AND
        schema_validated_at <= parsed_at AND official_decision_at <= parsed_at
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_discrepancies (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_cargo_id uuid NOT NULL,
    bonded_cargo_item_id uuid REFERENCES ewms.bonded_cargo_items(id),
    ...
    reason_code_id uuid REFERENCES ewms.reason_codes(id),
    description text,
    evidence_file_id uuid REFERENCES ewms.files(id),
    customs_report_id uuid REFERENCES ewms.bonded_inout_reports(id),
    ...
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_discrepancy_type CHECK (discrepancy_type IN (
        'SHORTAGE','OVERAGE','DAMAGE','MISDESCRIPTION','WRONG_ITEM','SEAL_BROKEN','MISSING','OTHER'
    )),
    CONSTRAINT ck_ewms_bonded_discrepancy_qty CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_movement_groups (
    ...
    reversal_of_group_id uuid REFERENCES ewms.bonded_movement_groups(id),
    reason_code_id uuid REFERENCES ewms.reason_codes(id),
    reason_text text,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_group_type CHECK (group_type IN (
        'INBOUND','OUTBOUND','TRANSFER','LEGAL_STATUS_CHANGE','ADJUSTMENT','SPLIT','MERGE',
        'HANDLING','SAMPLE','DISPOSAL','COUNT','REVERSAL','OTHER'
    )),
    CONSTRAINT ck_ewms_bonded_group_conservation CHECK (
        group_type NOT IN ('TRANSFER','LEGAL_STATUS_CHANGE','SPLIT','MERGE') OR requires_zero_sum
    ),
    CONSTRAINT ck_ewms_bonded_group_status CHECK (status IN ('DRAFT','VALIDATED','POSTED','REVERSED','CANCELLED')),
    CONSTRAINT ck_ewms_bonded_group_posted CHECK (
        (status IN ('POSTED','REVERSED') AND posted_at IS NOT NULL AND posted_by IS NOT NULL) OR
        status NOT IN ('POSTED','REVERSED')
    ),
    CONSTRAINT ck_ewms_bonded_group_reversal CHECK (
        (group_type = 'REVERSAL' AND reversal_of_group_id IS NOT NULL) OR
        (group_type <> 'REVERSAL' AND reversal_of_group_id IS NULL)
    ),
    CONSTRAINT ck_ewms_bonded_group_reversal_self CHECK (reversal_of_group_id IS NULL OR reversal_of_group_id <> id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_group_reversal
    ON ewms.bonded_movement_groups (reversal_of_group_id)
    WHERE reversal_of_group_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS ewms.bonded_exception_authorizations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_facility_id uuid NOT NULL,
    ...
    approved_by uuid REFERENCES ewms.users(id),
    approved_at timestamptz,
    rejected_by uuid REFERENCES ewms.users(id),
    rejected_at timestamptz,
    decision_reason text,
    cancelled_by uuid REFERENCES ewms.users(id),
    cancelled_at timestamptz,
    cancellation_reason text,
    evidence_file_id uuid REFERENCES ewms.files(id),
    consumed_at timestamptz,
    consumed_by_movement_id uuid,
    revoked_by uuid REFERENCES ewms.users(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_inout_report_id)
        REFERENCES ewms.bonded_inout_reports(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_exception_scope CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_exception_movement CHECK (
        authorized_movement_type IN ('OUTBOUND','TRANSFER_OUT','SAMPLE_OUT','DISPOSAL_OUT')
    ),
    CONSTRAINT ck_ewms_bonded_exception_quantity CHECK (authorized_quantity > 0),
    CONSTRAINT ck_ewms_bonded_exception_window CHECK (valid_until > valid_from),
    CONSTRAINT ck_ewms_bonded_exception_status CHECK (
        status IN ('DRAFT','PENDING','APPROVED','CONSUMED','REJECTED','REVOKED','CANCELLED')
    ),
    CONSTRAINT ck_ewms_bonded_exception_approval CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_exception_rejection CHECK (
        (status = 'REJECTED' AND
    ...
    CONSTRAINT ck_ewms_bonded_exception_cancellation CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_exception_consumption CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_exception_revocation CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_exception_dates CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_inventory_movements (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_movement_group_id uuid NOT NULL REFERENCES ewms.bonded_movement_groups(id),
    bonded_cargo_id uuid NOT NULL,
    bonded_cargo_item_id uuid NOT NULL,
    movement_sequence integer NOT NULL,
    movement_type varchar(40) NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES ewms.warehouses(id),
    warehouse_location_id uuid NOT NULL REFERENCES ewms.warehouse_locations(id),
    ...
    inventory_lot_id uuid REFERENCES ewms.inventory_lots(id),
    serial_number_id uuid REFERENCES ewms.serial_numbers(id),
    lpn_id uuid REFERENCES ewms.lpns(id),
    quantity_delta numeric(24,8) NOT NULL,
    base_uom_code varchar(20) NOT NULL REFERENCES ewms.units_of_measure(uom_code),
    package_delta numeric(20,4),
    weight_delta numeric(24,8),
    weight_uom_code varchar(20) REFERENCES ewms.units_of_measure(uom_code),
    bonded_inout_report_line_id uuid REFERENCES ewms.bonded_inout_report_lines(id),
    inventory_transaction_id uuid NOT NULL,
    inventory_transaction_entry_id uuid NOT NULL,
    discrepancy_id uuid REFERENCES ewms.bonded_discrepancies(id),
    source_document_type varchar(80),
    source_document_id uuid,
    exception_authorization_id uuid REFERENCES ewms.bonded_exception_authorizations(id),
    exception_authorization_reference varchar(300),
    reversal_of_movement_id uuid REFERENCES ewms.bonded_inventory_movements(id),
    reason_code_id uuid REFERENCES ewms.reason_codes(id),
    reason_text text,
    occurred_at timestamptz NOT NULL,
    recorded_at timestamptz NOT NULL DEFAULT now(),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_movement_group_id)
        REFERENCES ewms.bonded_movement_groups(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, owner_partner_id, exception_authorization_id)
        REFERENCES ewms.bonded_exception_authorizations(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_ewms_bonded_movement_seq CHECK (movement_sequence > 0),
    CONSTRAINT ck_ewms_bonded_movement_type CHECK (movement_type IN (
    ...
    CONSTRAINT ck_ewms_bonded_movement_qty CHECK (
        CASE
    ...
    CONSTRAINT ck_ewms_bonded_movement_exception CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_movement_reversal CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_movement_reversal_self CHECK (reversal_of_movement_id IS NULL OR reversal_of_movement_id <> id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_movement_reversal
    ON ewms.bonded_inventory_movements (reversal_of_movement_id)
    WHERE reversal_of_movement_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_ewms_bonded_exception_single_use
    ON ewms.bonded_inventory_movements (exception_authorization_id)
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'ewms.bonded_exception_authorizations'::regclass
          AND conname = 'fk_ewms_bonded_exception_consumed_movement'
    ) THEN
        ALTER TABLE ewms.bonded_exception_authorizations
            ADD CONSTRAINT fk_ewms_bonded_exception_consumed_movement
            FOREIGN KEY (consumed_by_movement_id)
            REFERENCES ewms.bonded_inventory_movements(id) ON DELETE RESTRICT;
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS ewms.bonded_exception_authorization_usages (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    ...
    used_by uuid REFERENCES ewms.users(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_inventory_movement_id)
        REFERENCES ewms.bonded_inventory_movements(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_exception_usage_qty CHECK (used_quantity > 0)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_inventory_balances (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    ...
    base_uom_code varchar(20) NOT NULL REFERENCES ewms.units_of_measure(uom_code),
    last_movement_id uuid NOT NULL REFERENCES ewms.bonded_inventory_movements(id),
    last_movement_at timestamptz NOT NULL,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_ewms_bonded_balance_dimensions UNIQUE (
        tenant_id, owner_partner_id, bonded_cargo_item_id, warehouse_location_id, legal_status, inventory_status_id
    ),
    CONSTRAINT ck_ewms_bonded_balance_nonnegative CHECK (on_hand_quantity >= 0)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_transport_orders (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    transport_order_no varchar(120) NOT NULL,
    transport_type_code varchar(100) NOT NULL,
    source_bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    destination_bonded_facility_id uuid REFERENCES ewms.bonded_facilities(id),
    destination_customs_office_code varchar(20) REFERENCES ewms.customs_offices(customs_office_code),
    destination_address_id uuid REFERENCES ewms.addresses(id),
...
CREATE TABLE IF NOT EXISTS ewms.bonded_transport_cargo (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_transport_order_id)
        REFERENCES ewms.bonded_transport_orders(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_transport_cargo_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_transport_cargo_qty CHECK (
        (package_count IS NULL OR package_count >= 0) AND (quantity IS NULL OR quantity > 0) AND
        (gross_weight IS NULL OR gross_weight >= 0)
    ),
    CONSTRAINT ck_ewms_bonded_transport_cargo_dates CHECK (received_at IS NULL OR loaded_at IS NULL OR received_at >= loaded_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_transport_vehicles (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_transport_order_id uuid NOT NULL REFERENCES ewms.bonded_transport_orders(id),
    ...
    CONSTRAINT ck_ewms_bonded_transport_vehicle_no CHECK (assignment_no > 0),
    CONSTRAINT ck_ewms_bonded_transport_vehicle_dates CHECK (assigned_to IS NULL OR assigned_to >= assigned_from)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_transport_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_transport_order_id uuid NOT NULL REFERENCES ewms.bonded_transport_orders(id) ON DELETE RESTRICT,
    bonded_transport_vehicle_id uuid REFERENCES ewms.bonded_transport_vehicles(id),
    ...
    recipient_partner_id uuid REFERENCES ewms.business_partners(id),
    exception_code varchar(100),
    detail_masked jsonb NOT NULL DEFAULT '{}'::jsonb,
    retention_until date,
    legal_hold boolean NOT NULL DEFAULT false,
    UNIQUE (bonded_transport_order_id, sequence_no),
    CONSTRAINT ck_ewms_bonded_transport_event_no CHECK (sequence_no > 0),
    CONSTRAINT ck_ewms_bonded_transport_event_type CHECK (event_type IN (
    ...
    CONSTRAINT ck_ewms_bonded_transport_arrival_evidence CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_seals (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_cargo_id uuid REFERENCES ewms.bonded_cargo(id),
    trade_container_id uuid REFERENCES ewms.trade_containers(id),
    bonded_transport_order_id uuid REFERENCES ewms.bonded_transport_orders(id),
    seal_no varchar(150) NOT NULL,
    seal_type_code varchar(100) NOT NULL,
    issuer_code varchar(150),
    applied_at timestamptz NOT NULL,
    applied_location varchar(300),
    applied_by uuid REFERENCES ewms.users(id),
    removed_at timestamptz,
    removal_reason_code varchar(100),
    replacement_seal_id uuid REFERENCES ewms.bonded_seals(id),
    status varchar(20) NOT NULL DEFAULT 'APPLIED',
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, seal_no, applied_at),
    CONSTRAINT ck_ewms_bonded_seal_target CHECK (
        num_nonnulls(bonded_cargo_id, trade_container_id, bonded_transport_order_id) >= 1
    ),
    CONSTRAINT ck_ewms_bonded_seal_status CHECK (status IN ('APPLIED','VERIFIED','BROKEN','REMOVED','REPLACED','VOID')),
    CONSTRAINT ck_ewms_bonded_seal_dates CHECK (removed_at IS NULL OR removed_at >= applied_at),
    CONSTRAINT ck_ewms_bonded_seal_replacement CHECK (replacement_seal_id IS NULL OR replacement_seal_id <> id)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_seal_checks (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_seal_id uuid NOT NULL REFERENCES ewms.bonded_seals(id) ON DELETE RESTRICT,
    ...
    checked_by uuid REFERENCES ewms.users(id),
    evidence_file_id uuid REFERENCES ewms.files(id),
    discrepancy_id uuid REFERENCES ewms.bonded_discrepancies(id),
    note text,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (bonded_seal_id, check_sequence),
    CONSTRAINT ck_ewms_bonded_seal_check_no CHECK (check_sequence > 0),
    CONSTRAINT ck_ewms_bonded_seal_check_result CHECK (result IN ('MATCH','MISMATCH','BROKEN','MISSING','REPLACED','NOT_CHECKED'))
);

CREATE TABLE IF NOT EXISTS ewms.bonded_storage_periods (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL,
    owner_partner_id uuid NOT NULL,
    bonded_cargo_id uuid NOT NULL,
    storage_rule_id uuid NOT NULL REFERENCES ewms.bonded_storage_rules(id),
    ...
    FOREIGN KEY (tenant_id, owner_partner_id, bonded_cargo_id)
        REFERENCES ewms.bonded_cargo(tenant_id, owner_partner_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_storage_period_no CHECK (period_sequence > 0),
    CONSTRAINT ck_ewms_bonded_storage_period_dates CHECK (
        original_due_on >= period_start_on AND current_due_on >= original_due_on AND
        (closed_on IS NULL OR closed_on >= period_start_on)
    ),
    CONSTRAINT ck_ewms_bonded_storage_period_extension CHECK (extension_days_granted >= 0 AND extension_count >= 0),
    CONSTRAINT ck_ewms_bonded_storage_period_status CHECK (status IN (
        'ACTIVE','EXTENSION_PENDING','EXTENDED','DUE_SOON','OVERDUE','RELEASED','DISPOSED','CLOSED','CANCELLED'
    ))
);

CREATE TABLE IF NOT EXISTS ewms.bonded_storage_extension_requests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_storage_period_id uuid NOT NULL REFERENCES ewms.bonded_storage_periods(id),
    ...
    supporting_document_id uuid REFERENCES ewms.files(id),
    ...
    CONSTRAINT ck_ewms_bonded_extension_days CHECK (requested_days > 0),
    CONSTRAINT ck_ewms_bonded_extension_status CHECK (decision_status IN (
        'DRAFT','SUBMITTED','UNDER_REVIEW','APPROVED','PARTIALLY_APPROVED','REJECTED','WITHDRAWN','CANCELLED'
    )),
    CONSTRAINT ck_ewms_bonded_extension_decision CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_release_notices (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_storage_period_id uuid NOT NULL REFERENCES ewms.bonded_storage_periods(id),
    ...
    notice_document_id uuid REFERENCES ewms.files(id),
    sent_at timestamptz,
    ...
    CONSTRAINT ck_ewms_bonded_notice_status CHECK (delivery_status IN ('PENDING','SENT','DELIVERED','FAILED','ACKNOWLEDGED','WAIVED','CANCELLED')),
    CONSTRAINT ck_ewms_bonded_notice_dates CHECK (acknowledged_at IS NULL OR sent_at IS NULL OR acknowledged_at >= sent_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_overdue_cases (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_storage_period_id uuid NOT NULL REFERENCES ewms.bonded_storage_periods(id),
    ...
    CONSTRAINT ck_ewms_bonded_overdue_status CHECK (status IN (
        'OPEN','NOTICE_SENT','UNDER_REVIEW','EXTENSION_PENDING','DISPOSITION_PENDING','RESOLVED','CANCELLED'
    )),
    CONSTRAINT ck_ewms_bonded_overdue_dates CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_overdue_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_overdue_case_id uuid NOT NULL REFERENCES ewms.bonded_overdue_cases(id) ON DELETE RESTRICT,
    sequence_no integer NOT NULL,
    event_type varchar(80) NOT NULL,
    ...
    CONSTRAINT ck_ewms_bonded_overdue_event_no CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_handling_operations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    ...
    correction_declaration_id uuid REFERENCES ewms.customs_declarations(id),
    status varchar(30) NOT NULL DEFAULT 'DRAFT',
    inventory_transaction_id uuid REFERENCES ewms.inventory_transaction_headers(id),
    movement_group_id uuid REFERENCES ewms.bonded_movement_groups(id),
    instructions text,
    result_summary text,
    row_version bigint NOT NULL DEFAULT 1,
    created_by uuid REFERENCES ewms.users(id),
    ...
    CONSTRAINT ck_ewms_bonded_handling_correction CHECK (
        NOT requires_declaration_correction OR correction_declaration_id IS NOT NULL
    ),
    CONSTRAINT ck_ewms_bonded_handling_plan_dates CHECK (planned_end_at IS NULL OR planned_start_at IS NULL OR planned_end_at >= planned_start_at),
    CONSTRAINT ck_ewms_bonded_handling_actual_dates CHECK (actual_end_at IS NULL OR actual_start_at IS NULL OR actual_end_at >= actual_start_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_handling_inputs (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_handling_operation_id uuid NOT NULL REFERENCES ewms.bonded_handling_operations(id) ON DELETE RESTRICT,
    line_no integer NOT NULL,
    bonded_cargo_item_id uuid NOT NULL REFERENCES ewms.bonded_cargo_items(id),
    inventory_lot_id uuid REFERENCES ewms.inventory_lots(id),
    input_quantity numeric(24,8) NOT NULL,
    base_uom_code varchar(20) NOT NULL REFERENCES ewms.units_of_measure(uom_code),
    bonded_inventory_movement_id uuid REFERENCES ewms.bonded_inventory_movements(id),
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (bonded_handling_operation_id, line_no),
    CONSTRAINT ck_ewms_bonded_handling_input_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_handling_input_qty CHECK (input_quantity > 0)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_handling_outputs (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_handling_operation_id uuid NOT NULL REFERENCES ewms.bonded_handling_operations(id) ON DELETE RESTRICT,
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (bonded_handling_operation_id, line_no),
    CONSTRAINT ck_ewms_bonded_handling_output_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_handling_output_type CHECK (output_type IN ('PRODUCT','BYPRODUCT','WASTE','LOSS','SAMPLE')),
    CONSTRAINT ck_ewms_bonded_handling_output_qty CHECK (output_quantity > 0)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_sample_movements (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    ...
    destination_partner_id uuid REFERENCES ewms.business_partners(id),
    destination_address_id uuid REFERENCES ewms.addresses(id),
    outbound_movement_id uuid REFERENCES ewms.bonded_inventory_movements(id),
    return_movement_id uuid REFERENCES ewms.bonded_inventory_movements(id),
    status varchar(30) NOT NULL DEFAULT 'OUTSTANDING',
    result_document_id uuid REFERENCES ewms.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    ...
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_sample_qty CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_disposal_cases (
    ...
    contractor_partner_id uuid REFERENCES ewms.business_partners(id),
    method_code varchar(100),
    location_text varchar(300),
    status varchar(30) NOT NULL DEFAULT 'DRAFT',
    movement_group_id uuid REFERENCES ewms.bonded_movement_groups(id),
    inventory_transaction_id uuid REFERENCES ewms.inventory_transaction_headers(id),
    certificate_document_id uuid REFERENCES ewms.files(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_by uuid REFERENCES ewms.users(id),
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, disposal_case_no),
    CONSTRAINT ck_ewms_bonded_disposal_type CHECK (disposal_type IN ('DESTROY','ABANDON','SALE','RETURN','TRANSFER','OTHER')),
    CONSTRAINT ck_ewms_bonded_disposal_status CHECK (status IN (
        'DRAFT','AUTHORIZATION_PENDING','AUTHORIZED','SCHEDULED','IN_PROGRESS','COMPLETED','REJECTED','CANCELLED','REVERSED'
    )),
    CONSTRAINT ck_ewms_bonded_disposal_dates CHECK (executed_at IS NULL OR planned_at IS NULL OR executed_at >= planned_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_disposal_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_disposal_case_id uuid NOT NULL REFERENCES ewms.bonded_disposal_cases(id) ON DELETE RESTRICT,
    line_no integer NOT NULL,
    bonded_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    bonded_cargo_item_id uuid NOT NULL REFERENCES ewms.bonded_cargo_items(id),
    ...
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_disposal_line_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_disposal_line_qty CHECK (
        approved_quantity > 0 AND (actual_quantity IS NULL OR actual_quantity >= 0) AND
        (residue_quantity IS NULL OR residue_quantity >= 0)
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_inventory_counts (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    ...
    supervised_by uuid REFERENCES ewms.users(id),
    ...
    CONSTRAINT ck_ewms_bonded_count_type CHECK (count_type IN ('QUARTERLY','CYCLE','ANNUAL','AD_HOC','CUSTOMS_DIRECTED')),
    CONSTRAINT ck_ewms_bonded_count_year CHECK (calendar_year BETWEEN 2000 AND 2200),
    CONSTRAINT ck_ewms_bonded_count_quarter CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_count_period CHECK (period_to >= period_from),
    CONSTRAINT ck_ewms_bonded_count_status CHECK (status IN (
        'PLANNED','FROZEN','COUNTING','RECONCILING','APPROVAL_PENDING','APPROVED','POSTED','CANCELLED'
    )),
    CONSTRAINT ck_ewms_bonded_count_dates CHECK (completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_inventory_count_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_inventory_count_id uuid NOT NULL REFERENCES ewms.bonded_inventory_counts(id) ON DELETE RESTRICT,
    line_no integer NOT NULL,
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_cargo_id uuid NOT NULL REFERENCES ewms.bonded_cargo(id),
    bonded_cargo_item_id uuid NOT NULL REFERENCES ewms.bonded_cargo_items(id),
    ...
    base_uom_code varchar(20) NOT NULL REFERENCES ewms.units_of_measure(uom_code),
    discrepancy_id uuid REFERENCES ewms.bonded_discrepancies(id),
    adjustment_movement_id uuid REFERENCES ewms.bonded_inventory_movements(id),
    count_status varchar(30) NOT NULL DEFAULT 'NOT_COUNTED',
    counted_by uuid REFERENCES ewms.users(id),
    counted_at timestamptz,
    recounted_by uuid REFERENCES ewms.users(id),
    recounted_at timestamptz,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (bonded_inventory_count_id, line_no),
    FOREIGN KEY (bonded_cargo_id, bonded_cargo_item_id)
        REFERENCES ewms.bonded_cargo_items(bonded_cargo_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_ewms_bonded_count_line_no CHECK (line_no > 0),
    CONSTRAINT ck_ewms_bonded_count_line_qty CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_count_line_status CHECK (count_status IN (
        'NOT_COUNTED','COUNTED','RECOUNT_REQUIRED','RECOUNTED','MATCHED','VARIANCE','APPROVED','POSTED'
    )),
    CONSTRAINT ck_ewms_bonded_count_line_dates CHECK (recounted_at IS NULL OR counted_at IS NULL OR recounted_at >= counted_at)
);

CREATE TABLE IF NOT EXISTS ewms.bonded_facility_incidents (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    ...
    CONSTRAINT ck_ewms_bonded_incident_type CHECK (incident_type IN (
    ...
    CONSTRAINT ck_ewms_bonded_incident_severity CHECK (severity IN ('LOW','MEDIUM','HIGH','CRITICAL')),
    CONSTRAINT ck_ewms_bonded_incident_status CHECK (status IN ('OPEN','REPORTED','INVESTIGATING','CORRECTIVE_ACTION','RESOLVED','CLOSED','CANCELLED')),
    CONSTRAINT ck_ewms_bonded_incident_dates CHECK (
    ...
    CONSTRAINT ck_ewms_bonded_incident_notice CHECK (
        NOT customs_notification_required OR status = 'OPEN' OR customs_notified_at IS NOT NULL
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_compliance_reports (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_facility_id uuid NOT NULL REFERENCES ewms.bonded_facilities(id),
    ...
    inventory_count_id uuid REFERENCES ewms.bonded_inventory_counts(id),
    generated_at timestamptz,
    generated_by uuid REFERENCES ewms.users(id),
    report_file_id uuid REFERENCES ewms.files(id),
    ...
    CONSTRAINT ck_ewms_bonded_compliance_type CHECK (report_type IN (
        'QUARTERLY_RECONCILIATION','ANNUAL_INVENTORY','INOUT_LEDGER','TRANSPORT_LEDGER',
        'OVERDUE_CARGO','INCIDENT','CUSTOMS_DIRECTED','OTHER'
    )),
    CONSTRAINT ck_ewms_bonded_compliance_period CHECK (reporting_period_to >= reporting_period_from),
    CONSTRAINT ck_ewms_bonded_compliance_status CHECK (submission_status IN (
        'DRAFT','GENERATED','APPROVED','SUBMITTED','ACCEPTED','REJECTED','CORRECTION_REQUIRED','SUPERSEDED'
    )),
    CONSTRAINT ck_ewms_bonded_compliance_dates CHECK (
...
CREATE TABLE IF NOT EXISTS ewms.bonded_compliance_findings (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    bonded_compliance_report_id uuid NOT NULL REFERENCES ewms.bonded_compliance_reports(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_ewms_bonded_finding_no CHECK (finding_no > 0),
    CONSTRAINT ck_ewms_bonded_finding_severity CHECK (severity IN ('INFO','LOW','MEDIUM','HIGH','CRITICAL')),
    CONSTRAINT ck_ewms_bonded_finding_action_status CHECK (corrective_action_status IN (
        'NOT_REQUIRED','OPEN','IN_PROGRESS','COMPLETED','VERIFIED','OVERDUE','WAIVED'
    )),
    CONSTRAINT ck_ewms_bonded_finding_action CHECK (
        NOT corrective_action_required OR corrective_action_due_on IS NOT NULL
    )
);

CREATE TABLE IF NOT EXISTS ewms.bonded_inout_report_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    owner_partner_id uuid NOT NULL REFERENCES ewms.business_partners(id),
    bonded_inout_report_id uuid NOT NULL REFERENCES ewms.bonded_inout_reports(id) ON DELETE RESTRICT,
    bonded_inout_report_version_id uuid REFERENCES ewms.bonded_inout_report_versions(id),
    sequence_no integer NOT NULL,
    event_type varchar(80) NOT NULL,
    from_status varchar(30),
    to_status varchar(30),
    ...
    CONSTRAINT ck_ewms_bonded_report_event_no CHECK (sequence_no > 0)
);

CREATE OR REPLACE FUNCTION ewms.validate_unipass_message_result()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    SELECT message.tenant_id AS message_tenant_id,
      ...
      INTO source_row
      FROM ewms.unipass_messages message
      JOIN ewms.unipass_connection_profiles profile
...
REVOKE ALL ON FUNCTION ewms.validate_unipass_message_result() FROM PUBLIC;

CREATE OR REPLACE FUNCTION ewms.validate_bonded_report_official_acceptance()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    IF NEW.report_status = 'ACCEPTED' OR NEW.acceptance_status = 'ACCEPTED' THEN
        ...
        SELECT version_row.sealed_at
          ...
          FROM ewms.bonded_inout_report_versions version_row
        ...
        SELECT result.id, result.official_decision_at
          INTO result_row
          FROM ewms.unipass_message_results result
...
REVOKE ALL ON FUNCTION ewms.validate_bonded_report_official_acceptance() FROM PUBLIC;

CREATE OR REPLACE FUNCTION ewms.protect_bonded_report_version()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_facility_scope()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    SELECT tenant_id INTO v_warehouse_tenant_id FROM ewms.warehouses WHERE id = NEW.warehouse_id;
    SELECT tenant_id INTO v_operator_tenant_id FROM ewms.business_partners WHERE id = NEW.operator_partner_id;
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_cargo_facility_scope()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_report_version_seal()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    IF NEW.sealed_at IS NOT NULL AND OLD.sealed_at IS NULL THEN
        SELECT count(*), COALESCE(sum(reported_quantity), 0)
          ...
          FROM ewms.bonded_inout_report_lines
         WHERE bonded_inout_report_version_id = NEW.id;
        SELECT count(*)
          INTO v_scope_mismatch_count
          FROM ewms.bonded_inout_report_lines l
          JOIN ewms.bonded_cargo_items i ON i.id = l.bonded_cargo_item_id
          JOIN ewms.bonded_inout_report_versions v ON v.id = l.bonded_inout_report_version_id
          JOIN ewms.bonded_inout_reports r ON r.id = v.bonded_inout_report_id
...
CREATE OR REPLACE FUNCTION ewms.protect_bonded_report_child()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    SELECT sealed_at INTO v_sealed_at
      FROM ewms.bonded_inout_report_versions
...
CREATE OR REPLACE FUNCTION ewms.prevent_bonded_cargo_relation_cycle()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    WITH RECURSIVE descendants(cargo_id) AS (
        SELECT r.child_cargo_id
          FROM ewms.bonded_cargo_relations r
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_exception_authorization()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF NEW.bonded_cargo_item_id IS NOT NULL THEN
        SELECT item.bonded_cargo_id
          ...
          FROM ewms.bonded_cargo_items item
    ...
    IF NEW.bonded_inout_report_id IS NOT NULL THEN
        SELECT report.bonded_facility_id, report.bonded_cargo_id, report.report_type
          INTO v_report
          FROM ewms.bonded_inout_reports report
    ...
    IF TG_OP = 'UPDATE' AND OLD.status = 'PENDING' AND NEW.status = 'APPROVED' THEN
        ...
        PERFORM 1
          FROM ewms.bonded_staff_assignments staff
          JOIN ewms.users approver
...
CREATE OR REPLACE FUNCTION ewms.protect_bonded_exception_authorization()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    ELSIF OLD.status = 'APPROVED' THEN
        ...
        IF NEW.status = 'CONSUMED' AND NOT EXISTS (
            SELECT 1
              FROM ewms.bonded_inventory_movements movement
...
CREATE OR REPLACE FUNCTION ewms.force_bonded_group_policy()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF NEW.group_type = 'REVERSAL' THEN
        SELECT original.tenant_id, original.owner_partner_id, original.bonded_cargo_id,
          ...
          INTO original_group
          FROM ewms.bonded_movement_groups original
    ...
    IF TG_OP = 'UPDATE' AND EXISTS (
        SELECT 1 FROM ewms.bonded_inventory_movements movement
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_inventory_movement()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    v_original ewms.bonded_inventory_movements%ROWTYPE;
    v_entry record;
    v_group record;
    v_report record;
...
BEGIN
    SELECT movement_group.tenant_id, movement_group.owner_partner_id,
      ...
      INTO v_group
      FROM ewms.bonded_movement_groups movement_group
    ...
    SELECT cargo.bonded_facility_id, facility.warehouse_id
      ...
      FROM ewms.bonded_cargo cargo
      JOIN ewms.bonded_facilities facility
    ...
    IF NEW.warehouse_id IS DISTINCT FROM v_cargo_warehouse_id THEN
        ...
        SELECT transport.tenant_id, transport.owner_partner_id,
               ...
               transport.status
          INTO v_transport
          FROM ewms.bonded_transport_orders transport
          JOIN ewms.bonded_transport_cargo transport_cargo
            ON transport_cargo.bonded_transport_order_id = transport.id
           AND transport_cargo.bonded_cargo_id = NEW.bonded_cargo_id
          JOIN ewms.bonded_facilities destination
    ...
    PERFORM 1
      FROM ewms.bonded_zone_controls zone_control
    ...
    IF v_required_report_type IS NOT NULL AND NEW.bonded_inout_report_line_id IS NOT NULL THEN
        SELECT report.id AS report_id, report.report_type, report.report_status,
               ...
               message_schema.is_active AS schema_is_active
          INTO v_report
          FROM ewms.bonded_inout_report_lines report_line
          JOIN ewms.bonded_inout_report_versions version_row
            ON version_row.id = report_line.bonded_inout_report_version_id
          JOIN ewms.bonded_inout_reports report
            ON report.id = version_row.bonded_inout_report_id
          JOIN ewms.unipass_messages message
            ON message.id = report.accepted_unipass_message_id
          JOIN ewms.unipass_message_results acceptance_result
    ...
    ELSIF v_required_report_type = 'OUTBOUND' AND NOT v_has_official_report THEN
        SELECT auth.*
          ...
          FROM ewms.bonded_exception_authorizations auth
    ...
    IF v_group.group_type = 'ADJUSTMENT' THEN
        SELECT discrepancy.tenant_id, discrepancy.owner_partner_id, discrepancy.bonded_cargo_id,
          ...
          INTO v_source
          FROM ewms.bonded_discrepancies discrepancy
    ...
    IF v_group.group_type = 'COUNT' THEN
        ...
        SELECT count_header.tenant_id, count_header.bonded_facility_id,
          ...
          INTO v_source
          FROM ewms.bonded_inventory_count_lines count_line
          JOIN ewms.bonded_inventory_counts count_header
    ...
    IF v_group.group_type = 'HANDLING' THEN
        IF NEW.movement_type = 'HANDLING_INPUT' THEN
            ...
            SELECT operation.tenant_id, operation.owner_partner_id, operation.bonded_cargo_id,
              ...
              INTO v_source
              FROM ewms.bonded_handling_inputs input_line
              JOIN ewms.bonded_handling_operations operation
        ...
        ELSE
            ...
            SELECT operation.tenant_id, operation.owner_partner_id, operation.bonded_cargo_id,
              ...
              INTO v_source
              FROM ewms.bonded_handling_outputs output_line
              JOIN ewms.bonded_handling_operations operation
    ...
    IF v_group.group_type = 'SAMPLE' THEN
        ...
        SELECT sample.tenant_id, sample.owner_partner_id, sample.bonded_cargo_id,
          ...
          INTO v_source
          FROM ewms.bonded_sample_movements sample
    ...
    IF NEW.movement_type = 'REVERSAL' THEN
        SELECT * INTO v_original
          FROM ewms.bonded_inventory_movements
        ...
        IF NOT FOUND OR NEW.quantity_delta <> -v_original.quantity_delta
           ...
           OR NOT EXISTS (
               SELECT 1 FROM ewms.bonded_inventory_movements original_group_movement
...
CREATE OR REPLACE FUNCTION ewms.consume_bonded_exception_authorization()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    UPDATE ewms.bonded_exception_authorizations auth
       SET status = 'CONSUMED',
           consumed_at = now(),
    ...
    INSERT INTO ewms.bonded_exception_authorization_usages (
...
CREATE OR REPLACE FUNCTION ewms.apply_bonded_inventory_balance()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    IF NEW.quantity_delta < 0 THEN
        UPDATE ewms.bonded_inventory_balances
        ...
        END IF;
    ELSE
        INSERT INTO ewms.bonded_inventory_balances (
        ...
        ON CONFLICT ON CONSTRAINT uq_ewms_bonded_balance_dimensions
        DO UPDATE SET
            on_hand_quantity = ewms.bonded_inventory_balances.on_hand_quantity + EXCLUDED.on_hand_quantity,
            last_movement_id = EXCLUDED.last_movement_id,
            last_movement_at = EXCLUDED.last_movement_at,
            row_version = ewms.bonded_inventory_balances.row_version + 1,
            updated_at = now()
        WHERE ewms.bonded_inventory_balances.base_uom_code = EXCLUDED.base_uom_code;
        GET DIAGNOSTICS v_rows = ROW_COUNT;
        IF v_rows <> 1 THEN
...
CREATE OR REPLACE FUNCTION ewms.validate_bonded_group_posting()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    IF NEW.status = 'POSTED' THEN
        ...
        IF NEW.group_type = 'REVERSAL' AND EXISTS (
            SELECT 1
              FROM ewms.bonded_inventory_movements original
              LEFT JOIN ewms.bonded_inventory_movements reversal
...
CREATE OR REPLACE FUNCTION ewms.protect_posted_bonded_group()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.ensure_bonded_movement_group_posted()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    SELECT movement.tenant_id, movement.owner_partner_id, movement.bonded_cargo_id,
      ...
      INTO link_row
      FROM ewms.bonded_inventory_movements movement
      JOIN ewms.bonded_movement_groups movement_group
...
REVOKE ALL ON FUNCTION ewms.ensure_bonded_movement_group_posted() FROM PUBLIC;

CREATE OR REPLACE FUNCTION ewms.ensure_bonded_inventory_transaction_linked()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, ewms
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
          FROM ewms.inventory_transaction_entries entry
          JOIN ewms.stock_buckets bucket ON bucket.id = entry.stock_bucket_id
          LEFT JOIN ewms.bonded_inventory_movements movement
            ON movement.inventory_transaction_entry_id = entry.id
          LEFT JOIN ewms.bonded_movement_groups movement_group
            ON movement_group.id = movement.bonded_movement_group_id
          LEFT JOIN ewms.bonded_cargo_items cargo_item
            ON cargo_item.id = movement.bonded_cargo_item_id
         WHERE entry.inventory_transaction_id = NEW.id
           AND bucket.account_type = 'PHYSICAL'
           AND (
...
REVOKE ALL ON FUNCTION ewms.ensure_bonded_inventory_transaction_linked() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_validate_bonded_facility_scope ON ewms.bonded_facilities;
CREATE TRIGGER trg_validate_bonded_facility_scope
    BEFORE INSERT OR UPDATE ON ewms.bonded_facilities
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_facility_scope();

DROP TRIGGER IF EXISTS trg_validate_bonded_cargo_facility_scope ON ewms.bonded_cargo;
CREATE TRIGGER trg_validate_bonded_cargo_facility_scope
    BEFORE INSERT OR UPDATE ON ewms.bonded_cargo
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_cargo_facility_scope();
...
DROP TRIGGER IF EXISTS trg_10_validate_bonded_report_seal ON ewms.bonded_inout_report_versions;
CREATE TRIGGER trg_10_validate_bonded_report_seal
    BEFORE UPDATE OF sealed_at ON ewms.bonded_inout_report_versions
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_report_version_seal();

DROP TRIGGER IF EXISTS trg_20_protect_bonded_report_version ON ewms.bonded_inout_report_versions;
CREATE TRIGGER trg_20_protect_bonded_report_version
    BEFORE UPDATE OR DELETE ON ewms.bonded_inout_report_versions
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_bonded_report_version();

DROP TRIGGER IF EXISTS trg_validate_unipass_message_result ON ewms.unipass_message_results;
CREATE TRIGGER trg_validate_unipass_message_result
    BEFORE INSERT ON ewms.unipass_message_results
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_unipass_message_result();

DROP TRIGGER IF EXISTS trg_validate_bonded_report_official_acceptance
    ON ewms.bonded_inout_reports;
CREATE TRIGGER trg_validate_bonded_report_official_acceptance
    ...
    ON ewms.bonded_inout_reports
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_report_official_acceptance();

DROP TRIGGER IF EXISTS trg_protect_bonded_report_line ON ewms.bonded_inout_report_lines;
CREATE TRIGGER trg_protect_bonded_report_line
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.bonded_inout_report_lines
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_bonded_report_child();

DROP TRIGGER IF EXISTS trg_prevent_bonded_cargo_relation_cycle ON ewms.bonded_cargo_relations;
CREATE TRIGGER trg_prevent_bonded_cargo_relation_cycle
    BEFORE INSERT OR UPDATE ON ewms.bonded_cargo_relations
    FOR EACH ROW EXECUTE PROCEDURE ewms.prevent_bonded_cargo_relation_cycle();

DROP TRIGGER IF EXISTS trg_10_validate_bonded_exception
    ON ewms.bonded_exception_authorizations;
CREATE TRIGGER trg_10_validate_bonded_exception
    BEFORE INSERT OR UPDATE ON ewms.bonded_exception_authorizations
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_exception_authorization();

DROP TRIGGER IF EXISTS trg_20_protect_bonded_exception
    ON ewms.bonded_exception_authorizations;
CREATE TRIGGER trg_20_protect_bonded_exception
    BEFORE INSERT OR UPDATE OR DELETE ON ewms.bonded_exception_authorizations
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_bonded_exception_authorization();

DROP TRIGGER IF EXISTS trg_block_bonded_exception_truncate
    ON ewms.bonded_exception_authorizations;
CREATE TRIGGER trg_block_bonded_exception_truncate
    BEFORE TRUNCATE ON ewms.bonded_exception_authorizations
    FOR EACH STATEMENT EXECUTE PROCEDURE ewms.block_immutable_change();

DROP TRIGGER IF EXISTS trg_05_force_bonded_group_policy ON ewms.bonded_movement_groups;
CREATE TRIGGER trg_05_force_bonded_group_policy
    ...
    ON ewms.bonded_movement_groups
    FOR EACH ROW EXECUTE PROCEDURE ewms.force_bonded_group_policy();

DROP TRIGGER IF EXISTS trg_10_validate_bonded_movement ON ewms.bonded_inventory_movements;
CREATE TRIGGER trg_10_validate_bonded_movement
    BEFORE INSERT ON ewms.bonded_inventory_movements
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_inventory_movement();

DROP TRIGGER IF EXISTS trg_20_consume_bonded_exception ON ewms.bonded_inventory_movements;
CREATE TRIGGER trg_20_consume_bonded_exception
    AFTER INSERT ON ewms.bonded_inventory_movements
    FOR EACH ROW EXECUTE PROCEDURE ewms.consume_bonded_exception_authorization();

DROP TRIGGER IF EXISTS trg_90_apply_bonded_balance ON ewms.bonded_inventory_movements;
CREATE TRIGGER trg_90_apply_bonded_balance
    AFTER INSERT ON ewms.bonded_inventory_movements
    FOR EACH ROW EXECUTE PROCEDURE ewms.apply_bonded_inventory_balance();

DROP TRIGGER IF EXISTS trg_95_require_posted_bonded_group ON ewms.bonded_inventory_movements;
CREATE CONSTRAINT TRIGGER trg_95_require_posted_bonded_group
    AFTER INSERT ON ewms.bonded_inventory_movements
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE ewms.ensure_bonded_movement_group_posted();
...
CREATE CONSTRAINT TRIGGER trg_95_require_bonded_inventory_links
    ...
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE PROCEDURE ewms.ensure_bonded_inventory_transaction_linked();

DROP TRIGGER IF EXISTS trg_10_validate_bonded_group_posting ON ewms.bonded_movement_groups;
CREATE TRIGGER trg_10_validate_bonded_group_posting
    BEFORE INSERT OR UPDATE OF status ON ewms.bonded_movement_groups
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_bonded_group_posting();

DROP TRIGGER IF EXISTS trg_20_protect_posted_bonded_group ON ewms.bonded_movement_groups;
CREATE TRIGGER trg_20_protect_posted_bonded_group
    BEFORE UPDATE OR DELETE ON ewms.bonded_movement_groups
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_posted_bonded_group();
...
DROP TRIGGER IF EXISTS trg_block_bonded_exception_usage_truncate
    ON ewms.bonded_exception_authorization_usages;
CREATE TRIGGER trg_block_bonded_exception_usage_truncate
    BEFORE TRUNCATE ON ewms.bonded_exception_authorization_usages
    FOR EACH STATEMENT EXECUTE PROCEDURE ewms.block_immutable_change();

CREATE INDEX IF NOT EXISTS ix_ewms_bonded_cargo_owner_status
    ON ewms.bonded_cargo (tenant_id, owner_partner_id, physical_status, legal_status);
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_cargo_item_lookup
    ON ewms.bonded_cargo_items (tenant_id, owner_partner_id, item_id, status);
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_report_acceptance
    ON ewms.bonded_inout_reports (tenant_id, acceptance_status, submitted_at)
    WHERE acceptance_status IN ('PENDING','REJECTED','ERROR');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_exception_approval_queue
    ON ewms.bonded_exception_authorizations (tenant_id, status, requested_at, id)
    WHERE status IN ('DRAFT','PENDING');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_exception_validity
    ON ewms.bonded_exception_authorizations (
        tenant_id, owner_partner_id, bonded_cargo_id, authorized_movement_type,
        valid_from, valid_until, id
    ) WHERE status = 'APPROVED';
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_movement_item_time
    ON ewms.bonded_inventory_movements (bonded_cargo_item_id, occurred_at, movement_sequence);
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_movement_report_usage
    ON ewms.bonded_inventory_movements (
        bonded_inout_report_line_id, movement_type, occurred_at, id
    ) WHERE bonded_inout_report_line_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_movement_source_usage
    ON ewms.bonded_inventory_movements (
        source_document_type, source_document_id, movement_type, id
    ) WHERE source_document_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_balance_location
    ON ewms.bonded_inventory_balances (
        tenant_id, owner_partner_id, warehouse_location_id, legal_status, inventory_status_id
    );
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_transport_due
    ON ewms.bonded_transport_orders (tenant_id, authorized_arrival_due_at, status)
    WHERE status IN ('AUTHORIZED','DISPATCHED','IN_TRANSIT','EXCEPTION');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_storage_period_due
    ON ewms.bonded_storage_periods (tenant_id, current_due_on, status)
    WHERE status IN ('ACTIVE','EXTENSION_PENDING','EXTENDED','DUE_SOON','OVERDUE');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_notice_due
    ON ewms.bonded_release_notices (tenant_id, notice_due_on, delivery_status)
    WHERE delivery_status IN ('PENDING','FAILED');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_overdue_open
    ON ewms.bonded_overdue_cases (tenant_id, overdue_from, status)
    WHERE status NOT IN ('RESOLVED','CANCELLED');
CREATE INDEX IF NOT EXISTS ix_ewms_bonded_count_period
    ON ewms.bonded_inventory_counts (tenant_id, bonded_facility_id, calendar_year, calendar_quarter);

CREATE INDEX IF NOT EXISTS brin_ewms_bonded_movements_occurred
    ON ewms.bonded_inventory_movements USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS brin_ewms_bonded_transport_events_occurred
    ON ewms.bonded_transport_events USING brin (occurred_at);
CREATE INDEX IF NOT EXISTS brin_ewms_bonded_cargo_events_occurred
    ON ewms.bonded_cargo_status_events USING brin (occurred_at);

COMMENT ON TABLE ewms.bonded_cargo IS
...
COMMENT ON TABLE ewms.bonded_inventory_movements IS
...
COMMENT ON TABLE ewms.bonded_exception_authorizations IS
...
COMMENT ON TABLE ewms.bonded_exception_authorization_usages IS
...
COMMENT ON TABLE ewms.bonded_inventory_counts IS
    'Physical-to-system reconciliation headers, including quarterly counts, with immutable movement-based variance posting.';
COMMENT ON SCHEMA ewms IS
    'EWMS database model. Customs/bonded structures are implementation design references and require validation by responsible customs/legal professionals.';
