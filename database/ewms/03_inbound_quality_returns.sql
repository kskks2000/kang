-- EWMS enterprise WMS schema - inbound orders, receiving, quality, putaway/cross-dock requests, and supplier returns.
...
CREATE TABLE IF NOT EXISTS ewms.inbound_orders (
    ...
    ship_from_partner_id uuid REFERENCES ewms.business_partners(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_inbound_order_external
    ON ewms.inbound_orders (tenant_id, source_system_id, source_document_id)
    WHERE source_system_id IS NOT NULL AND source_document_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS ewms.inbound_order_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    item_packaging_id uuid REFERENCES ewms.item_packagings(id),
    ...
    expected_country_of_origin char(2) REFERENCES ewms.countries(country_code),
    ...
    FOREIGN KEY (tenant_id, inbound_order_id)
        REFERENCES ewms.inbound_orders(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.inbound_order_parties (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    inbound_order_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.inbound_order_references (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    inbound_order_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.inbound_order_status_history (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    inbound_order_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.dock_appointments (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.dock_appointment_inbound_orders (
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    FOREIGN KEY (tenant_id, appointment_id)
        REFERENCES ewms.dock_appointments(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, inbound_order_id)
        REFERENCES ewms.inbound_orders(tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_dock_appt_order_sequence CHECK (sequence_no > 0)
);

CREATE TABLE IF NOT EXISTS ewms.dock_appointment_history (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    appointment_id uuid NOT NULL,
    event_sequence integer NOT NULL,
    ...
    previous_dock_door_id uuid REFERENCES ewms.dock_doors(id),
    new_dock_door_id uuid REFERENCES ewms.dock_doors(id),
    reason_code_id uuid REFERENCES ewms.reason_codes(id),
    reason_text text,
    actor_user_id uuid REFERENCES ewms.users(id) ON DELETE SET NULL,
    occurred_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS ewms.receipts (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    appointment_id uuid,
    receiving_location_id uuid REFERENCES ewms.warehouse_locations(id),
    receiving_inventory_status_id uuid REFERENCES ewms.inventory_statuses(id),
    supplier_partner_id uuid REFERENCES ewms.business_partners(id),
    carrier_partner_id uuid REFERENCES ewms.business_partners(id),
    vehicle_registration_no varchar(80),
    trailer_no varchar(100),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_receipt_external
    ON ewms.receipts (tenant_id, source_system_id, source_document_id)
    WHERE source_system_id IS NOT NULL AND source_document_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS ewms.receipt_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    FOREIGN KEY (tenant_id, inbound_order_line_id)
        REFERENCES ewms.inbound_order_lines(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.receipt_lot_observations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    observed_by uuid REFERENCES ewms.users(id),
...
CREATE TABLE IF NOT EXISTS ewms.receipt_serial_observations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    receipt_line_id uuid NOT NULL,
    ...
    accepted boolean,
    rejection_reason_code_id uuid REFERENCES ewms.reason_codes(id),
    observed_attributes jsonb NOT NULL DEFAULT '{}'::jsonb,
    observed_by uuid REFERENCES ewms.users(id),
    observed_at timestamptz NOT NULL DEFAULT now(),
    row_version bigint NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS ewms.receipt_discrepancies (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    receipt_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.receipt_status_history (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    receipt_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.unloading_tasks (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.unloading_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    FOREIGN KEY (tenant_id, unloading_task_id)
        REFERENCES ewms.unloading_tasks(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, receipt_line_id)
        REFERENCES ewms.receipt_lines(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.quality_specifications (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
ALTER TABLE ewms.inbound_order_lines
    DROP CONSTRAINT IF EXISTS fk_inbound_line_quality_spec;
ALTER TABLE ewms.inbound_order_lines
    ADD CONSTRAINT fk_inbound_line_quality_spec
    FOREIGN KEY (quality_specification_id) REFERENCES ewms.quality_specifications(id);

ALTER TABLE ewms.receipt_lines
    DROP CONSTRAINT IF EXISTS fk_receipt_line_quality_spec;
ALTER TABLE ewms.receipt_lines
    ADD CONSTRAINT fk_receipt_line_quality_spec
    FOREIGN KEY (quality_specification_id) REFERENCES ewms.quality_specifications(id);

CREATE TABLE IF NOT EXISTS ewms.quality_specification_tests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    FOREIGN KEY (tenant_id, quality_specification_id)
        REFERENCES ewms.quality_specifications(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.quality_inspections (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.quality_inspection_samples (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    sampled_by uuid REFERENCES ewms.users(id),
    sampled_at timestamptz NOT NULL DEFAULT now(),
    chain_of_custody_ref varchar(150),
    sample_location_id uuid REFERENCES ewms.warehouse_locations(id),
    ...
    FOREIGN KEY (tenant_id, quality_inspection_id)
        REFERENCES ewms.quality_inspections(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.quality_inspection_tests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    quality_inspection_id uuid NOT NULL,
    ...
    assigned_user_id uuid REFERENCES ewms.users(id),
    laboratory_partner_id uuid REFERENCES ewms.business_partners(id),
    started_at timestamptz,
    completed_at timestamptz,
    ...
    FOREIGN KEY (tenant_id, specification_test_id)
        REFERENCES ewms.quality_specification_tests(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, sample_id)
        REFERENCES ewms.quality_inspection_samples(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.quality_inspection_results (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    measured_by uuid REFERENCES ewms.users(id),
    ...
    FOREIGN KEY (tenant_id, inspection_test_id)
        REFERENCES ewms.quality_inspection_tests(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.quality_holds (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.quality_hold_status_history (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.quality_dispositions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    quality_inspection_id uuid,
    ...
    uom_code varchar(20) NOT NULL REFERENCES ewms.units_of_measure(uom_code),
    target_inventory_status_id uuid REFERENCES ewms.inventory_statuses(id),
    target_location_id uuid REFERENCES ewms.warehouse_locations(id),
    supplier_return_required boolean NOT NULL DEFAULT false,
    rework_instructions text,
    reason_code_id uuid REFERENCES ewms.reason_codes(id),
    approved_by uuid REFERENCES ewms.users(id),
    approved_at timestamptz,
    executed_by uuid REFERENCES ewms.users(id),
...
CREATE TABLE IF NOT EXISTS ewms.putaway_requests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    preferred_zone_id uuid REFERENCES ewms.warehouse_zones(id),
    preferred_location_id uuid REFERENCES ewms.warehouse_locations(id),
    target_inventory_status_id uuid REFERENCES ewms.inventory_statuses(id),
    putaway_strategy_id uuid REFERENCES ewms.putaway_strategies(id),
    priority integer NOT NULL DEFAULT 100,
    status varchar(20) NOT NULL DEFAULT 'OPEN',
...
CREATE TABLE IF NOT EXISTS ewms.crossdock_requests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    request_no varchar(80) NOT NULL,
    warehouse_id uuid NOT NULL REFERENCES ewms.warehouses(id),
...
CREATE TABLE IF NOT EXISTS ewms.supplier_return_orders (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    return_reason_code_id uuid REFERENCES ewms.reason_codes(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_supplier_return_external
    ON ewms.supplier_return_orders (tenant_id, source_system_id, source_document_id)
    WHERE source_system_id IS NOT NULL AND source_document_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS ewms.supplier_return_order_lines (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    FOREIGN KEY (tenant_id, supplier_return_order_id)
        REFERENCES ewms.supplier_return_orders(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, original_receipt_line_id)
        REFERENCES ewms.receipt_lines(tenant_id, id) ON DELETE RESTRICT,
    FOREIGN KEY (tenant_id, quality_disposition_id)
        REFERENCES ewms.quality_dispositions(tenant_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ewms.supplier_return_status_history (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    supplier_return_order_id uuid NOT NULL,
    event_sequence integer NOT NULL,
...
CREATE OR REPLACE FUNCTION ewms.guard_finalized_document_child()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    LOOP
        parent_status := NULL;
        EXECUTE format('SELECT %I::text FROM ewms.%I WHERE id = $1 FOR SHARE',
                       TG_ARGV[2], TG_ARGV[0])
           INTO parent_status
           USING parent_id;
        IF parent_status IS NULL THEN
            RAISE EXCEPTION 'Referenced parent ewms.% % is unavailable', TG_ARGV[0], parent_id
                USING ERRCODE = '23503';
        END IF;
        IF parent_status = ANY (string_to_array(TG_ARGV[3], ',')) THEN
            RAISE EXCEPTION 'Core child rows of finalized ewms.% parent % (status %) are immutable',
...
CREATE OR REPLACE FUNCTION ewms.prevent_core_document_truncate()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.protect_inbound_document_header()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
DROP TRIGGER IF EXISTS trg_protect_inbound_document_header ON ewms.inbound_orders;
CREATE TRIGGER trg_protect_inbound_document_header
    BEFORE UPDATE OR DELETE ON ewms.inbound_orders
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_inbound_document_header();

DROP TRIGGER IF EXISTS trg_protect_receipt_header ON ewms.receipts;
CREATE TRIGGER trg_protect_receipt_header
    BEFORE UPDATE OR DELETE ON ewms.receipts
    FOR EACH ROW EXECUTE PROCEDURE ewms.protect_inbound_document_header();
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS trg_guard_finalized_parent ON ewms.%I', target.table_name);
        EXECUTE format(
            'CREATE TRIGGER trg_guard_finalized_parent BEFORE INSERT OR UPDATE OR DELETE ON ewms.%I '
...
CREATE INDEX IF NOT EXISTS ix_inbound_orders_worklist
    ON ewms.inbound_orders (tenant_id, warehouse_id, owner_partner_id, status, expected_arrival_from, id);
CREATE INDEX IF NOT EXISTS ix_inbound_orders_supplier
    ON ewms.inbound_orders (tenant_id, supplier_partner_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_inbound_lines_item
    ON ewms.inbound_order_lines (tenant_id, owner_partner_id, item_id, status);
CREATE INDEX IF NOT EXISTS ix_inbound_parties_partner
    ON ewms.inbound_order_parties (tenant_id, partner_id, party_role);
CREATE INDEX IF NOT EXISTS ix_inbound_references_search
    ON ewms.inbound_order_references (tenant_id, reference_type, reference_value);
CREATE INDEX IF NOT EXISTS ix_inbound_status_history_order
    ON ewms.inbound_order_status_history (tenant_id, inbound_order_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_dock_appointments_schedule
    ON ewms.dock_appointments (tenant_id, warehouse_id, status, scheduled_start_at, scheduled_end_at);
CREATE INDEX IF NOT EXISTS ix_dock_appointments_door
    ON ewms.dock_appointments (tenant_id, dock_door_id, scheduled_start_at)
    WHERE status NOT IN ('COMPLETED','CHECKED_OUT','NO_SHOW','CANCELLED');
CREATE INDEX IF NOT EXISTS ix_receipts_worklist
    ON ewms.receipts (tenant_id, warehouse_id, owner_partner_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_receipts_inbound_order
    ON ewms.receipts (tenant_id, inbound_order_id, status);
CREATE INDEX IF NOT EXISTS ix_receipts_posting_pending
    ON ewms.receipts (tenant_id, warehouse_id, posting_status, receiving_completed_at)
    WHERE posting_status IN ('NOT_POSTED','PARTIAL','FAILED');
CREATE INDEX IF NOT EXISTS ix_receipt_lines_item
    ON ewms.receipt_lines (tenant_id, owner_partner_id, item_id, posting_status);
CREATE INDEX IF NOT EXISTS ix_receipt_lines_inbound_line
    ON ewms.receipt_lines (tenant_id, inbound_order_line_id);
CREATE INDEX IF NOT EXISTS ix_receipt_lot_observations_lot
    ON ewms.receipt_lot_observations (tenant_id, supplier_lot_no, expires_on);
CREATE INDEX IF NOT EXISTS ix_receipt_serial_observations_serial
    ON ewms.receipt_serial_observations (tenant_id, serial_no);
CREATE INDEX IF NOT EXISTS ix_receipt_discrepancies_open
    ON ewms.receipt_discrepancies (tenant_id, receipt_id, severity, reported_at)
    WHERE status IN ('OPEN','UNDER_REVIEW');
CREATE INDEX IF NOT EXISTS ix_receipt_status_history_receipt
    ON ewms.receipt_status_history (tenant_id, receipt_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_unloading_tasks_queue
    ON ewms.unloading_tasks (tenant_id, status, priority, planned_start_at, id);
CREATE INDEX IF NOT EXISTS ix_quality_specs_item
    ON ewms.quality_specifications (tenant_id, owner_partner_id, item_id, status, valid_from);
CREATE INDEX IF NOT EXISTS ix_quality_inspections_queue
    ON ewms.quality_inspections (tenant_id, warehouse_id, status, assigned_at, id);
CREATE INDEX IF NOT EXISTS ix_quality_inspections_receipt_line
    ON ewms.quality_inspections (tenant_id, receipt_line_id, status);
CREATE INDEX IF NOT EXISTS ix_quality_holds_active
    ON ewms.quality_holds (tenant_id, warehouse_id, owner_partner_id, review_due_at)
    WHERE status IN ('ACTIVE','UNDER_REVIEW','PARTIALLY_RELEASED');
CREATE INDEX IF NOT EXISTS ix_quality_dispositions_pending
    ON ewms.quality_dispositions (tenant_id, status, approved_at)
    WHERE status IN ('PROPOSED','APPROVED','IN_PROGRESS');
CREATE INDEX IF NOT EXISTS ix_putaway_requests_queue
    ON ewms.putaway_requests (tenant_id, warehouse_id, status, priority, requested_at, id);
CREATE INDEX IF NOT EXISTS ix_crossdock_requests_queue
    ON ewms.crossdock_requests (tenant_id, warehouse_id, status, priority, required_ship_at, id);
CREATE INDEX IF NOT EXISTS ix_supplier_returns_worklist
    ON ewms.supplier_return_orders (tenant_id, warehouse_id, owner_partner_id, status, requested_ship_date, id);
CREATE INDEX IF NOT EXISTS ix_supplier_return_lines_item
    ON ewms.supplier_return_order_lines (tenant_id, owner_partner_id, item_id, status);

CREATE OR REPLACE FUNCTION ewms.reject_append_only_change()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
COMMENT ON TABLE ewms.receipt_lot_observations IS
...
COMMENT ON TABLE ewms.quality_holds IS
