-- EWMS enterprise WMS schema - foundation, tenancy, Firebase IAM, and platform services.
-- PostgreSQL 11 compatible. Executed by scripts/apply_ewms_schema.py in one transaction.

CREATE SCHEMA IF NOT EXISTS ewms;

CREATE OR REPLACE FUNCTION ewms.generate_uuid()
RETURNS uuid
LANGUAGE sql
VOLATILE
AS $function$
...
CREATE OR REPLACE FUNCTION ewms.jsonb_contains_forbidden_secret_key(p_value jsonb)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
STRICT
    ...
    IF jsonb_typeof(p_value) = 'object' THEN
        ...
        LOOP
            ...
            IF jsonb_typeof(object_entry.value) IN ('object','array')
               AND ewms.jsonb_contains_forbidden_secret_key(object_entry.value) THEN
                RETURN true;
            END IF;
        END LOOP;
    ELSIF jsonb_typeof(p_value) = 'array' THEN
        ...
        LOOP
            IF jsonb_typeof(array_value) IN ('object','array')
               AND ewms.jsonb_contains_forbidden_secret_key(array_value) THEN
                RETURN true;
            END IF;
        END LOOP;
    END IF;
...
CREATE OR REPLACE FUNCTION ewms.touch_row()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE TABLE IF NOT EXISTS ewms.schema_migrations (
...
CREATE TABLE IF NOT EXISTS ewms.tenants (
...
CREATE TABLE IF NOT EXISTS ewms.tenant_settings (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_tenant_settings_no_inline_secrets CHECK (
        setting_value IS NULL OR NOT ewms.jsonb_contains_forbidden_secret_key(setting_value)
    )
);

COMMENT ON TABLE ewms.tenant_settings IS
    'Tenant configuration. Secret values stay in an external secret manager; this table stores only an opaque secret_reference.';

CREATE TABLE IF NOT EXISTS ewms.organizations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS ewms.organization_units (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    organization_id uuid NOT NULL REFERENCES ewms.organizations(id),
    parent_unit_id uuid REFERENCES ewms.organization_units(id),
...
CREATE TABLE IF NOT EXISTS ewms.users (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'ewms.users'::regclass
          AND conname = 'fk_ewms_users_platform_user'
    ) THEN
        ALTER TABLE ewms.users
            ADD CONSTRAINT fk_ewms_users_platform_user
...
CREATE INDEX IF NOT EXISTS ix_users_email_search
    ON ewms.users (lower(primary_email))
    WHERE primary_email IS NOT NULL AND deleted_at IS NULL;

COMMENT ON TABLE ewms.users IS
    'EWMS principals mapped to canonical kang.users and Firebase token subjects. Tenant access is granted only through tenant_memberships.';
COMMENT ON COLUMN ewms.users.firebase_uid IS
    'Canonical Firebase token subject. The unique identity key is firebase_project_id, COALESCE(firebase_tenant_id, ''''), firebase_uid.';
COMMENT ON COLUMN ewms.users.primary_email IS
...
COMMENT ON COLUMN ewms.users.platform_user_id IS
    'Required canonical platform principal in kang.users; EWMS must not maintain an independent password database.';

CREATE OR REPLACE FUNCTION ewms.validate_user_platform_subject()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'EWMS platform_user_id and Firebase uid must identify the same canonical kang.users row'
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
...
REVOKE ALL ON FUNCTION ewms.validate_user_platform_subject() FROM PUBLIC;

DROP TRIGGER IF EXISTS trg_validate_user_platform_subject ON ewms.users;
CREATE TRIGGER trg_validate_user_platform_subject
    BEFORE INSERT OR UPDATE OF platform_user_id, firebase_uid ON ewms.users
    FOR EACH ROW EXECUTE PROCEDURE ewms.validate_user_platform_subject();

CREATE OR REPLACE FUNCTION ewms.prevent_user_identity_rekey()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    IF OLD.firebase_project_id IS DISTINCT FROM NEW.firebase_project_id
       ...
       OR OLD.platform_user_id IS DISTINCT FROM NEW.platform_user_id THEN
        RAISE EXCEPTION
            'EWMS canonical user identity keys are immutable; use an explicit audited account-link/migration workflow';
    END IF;
    RETURN NEW;
END;
$function$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'ewms.users'::regclass
        ...
        ON ewms.users
        FOR EACH ROW EXECUTE PROCEDURE ewms.prevent_user_identity_rekey();
    END IF;
END;
$block$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'ewms.organization_units'::regclass
          AND conname = 'fk_org_units_manager'
    ) THEN
        ALTER TABLE ewms.organization_units
            ADD CONSTRAINT fk_org_units_manager FOREIGN KEY (manager_user_id)
            REFERENCES ewms.users(id) ON DELETE SET NULL;
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS ewms.user_profiles (
    user_id uuid PRIMARY KEY REFERENCES ewms.users(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN ewms.user_profiles.phone_encrypted IS
    'Encrypted personal data only; the encryption key must remain in an external KMS/secret manager.';

CREATE TABLE IF NOT EXISTS ewms.auth_identities (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    ...
    CONSTRAINT ck_auth_identities_no_secret_metadata CHECK (
        NOT ewms.jsonb_contains_forbidden_secret_key(metadata)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_auth_identities_provider_subject
    ON ewms.auth_identities (
        firebase_project_id,
        COALESCE(firebase_tenant_id, ''),
        provider_code,
        provider_subject
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_auth_identities_active_user_provider
    ON ewms.auth_identities (user_id, provider_code)
    WHERE disabled_at IS NULL;

COMMENT ON TABLE ewms.auth_identities IS
...
COMMENT ON COLUMN ewms.auth_identities.firebase_uid IS
    'Firebase account subject from the verified ID token; deliberately separate from the provider-specific subject.';
COMMENT ON COLUMN ewms.auth_identities.provider_subject IS
...
COMMENT ON COLUMN ewms.auth_identities.provider_email IS
...
COMMENT ON COLUMN ewms.auth_identities.metadata IS
    'Non-secret provider metadata only. Password material and raw Firebase/Google tokens are prohibited.';

CREATE OR REPLACE FUNCTION ewms.validate_auth_identity_owner()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    owner_row ewms.users%ROWTYPE;
BEGIN
    IF TG_OP = 'UPDATE' AND TG_TABLE_NAME = 'auth_identities' AND (
        ...
        RAISE EXCEPTION
            'EWMS authentication identity keys are immutable; disable the identity and complete a new explicit link request';
    END IF;
    ...
    IF TG_OP = 'UPDATE' AND TG_TABLE_NAME = 'account_link_requests' AND (
        ...
        RAISE EXCEPTION
            'EWMS account-link identity keys are immutable; cancel the request and create a new one';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'EWMS auth identity user does not exist';
    END IF;
    IF owner_row.firebase_project_id IS DISTINCT FROM NEW.firebase_project_id
       ...
       OR owner_row.firebase_uid IS DISTINCT FROM NEW.firebase_uid THEN
        RAISE EXCEPTION 'EWMS auth identity Firebase subject must match its user';
    END IF;
    RETURN NEW;
END;
$function$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'ewms.auth_identities'::regclass
        ...
        ON ewms.auth_identities
        FOR EACH ROW EXECUTE PROCEDURE ewms.validate_auth_identity_owner();
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS ewms.account_link_requests (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES ewms.users(id) ON DELETE CASCADE,
    ...
    completed_identity_id uuid REFERENCES ewms.auth_identities(id) ON DELETE SET NULL,
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS ix_account_link_requests_user_status
    ON ewms.account_link_requests (user_id, status, requested_at DESC);

COMMENT ON TABLE ewms.account_link_requests IS
...
COMMENT ON COLUMN ewms.account_link_requests.provider_email IS
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgrelid = 'ewms.account_link_requests'::regclass
          AND tgname = 'trg_validate_account_link_owner'
          AND NOT tgisinternal
    ) THEN
CREATE TRIGGER trg_validate_account_link_owner
        BEFORE INSERT OR UPDATE OF user_id, firebase_project_id, firebase_tenant_id, firebase_uid, provider_code, provider_subject
        ON ewms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE ewms.validate_auth_identity_owner();
    END IF;
END;
$block$;

CREATE OR REPLACE FUNCTION ewms.validate_account_link_completion()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
    identity_row ewms.auth_identities%ROWTYPE;
BEGIN
...
CREATE OR REPLACE FUNCTION ewms.validate_account_link_transition()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    IF NOT (
        ...
        RAISE EXCEPTION 'Invalid EWMS account-link status transition: % -> %',
...
BEGIN
    IF NOT EXISTS (
        ...
        ON ewms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE ewms.validate_account_link_completion();
    END IF;
END;
$block$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ON ewms.account_link_requests
        FOR EACH ROW EXECUTE PROCEDURE ewms.validate_account_link_transition();
    END IF;
END;
$block$;

CREATE TABLE IF NOT EXISTS ewms.account_link_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    request_id uuid NOT NULL REFERENCES ewms.account_link_requests(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT ck_account_link_event_no_secrets CHECK (
        NOT ewms.jsonb_contains_forbidden_secret_key(evidence_masked)
    )
);

COMMENT ON TABLE ewms.account_link_events IS
    'Append-oriented audit trail for explicit account-link decisions; raw credentials, tokens, and nonces are prohibited.';

CREATE TABLE IF NOT EXISTS ewms.user_invitations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    email varchar(320) NOT NULL,
    invitation_token_hash char(64) NOT NULL UNIQUE,
    invited_by uuid REFERENCES ewms.users(id),
    expires_at timestamptz NOT NULL,
    accepted_by uuid REFERENCES ewms.users(id),
...
COMMENT ON TABLE ewms.user_invitations IS
...
CREATE TABLE IF NOT EXISTS ewms.tenant_memberships (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    user_id uuid NOT NULL REFERENCES ewms.users(id) ON DELETE CASCADE,
    organization_id uuid REFERENCES ewms.organizations(id),
    default_org_unit_id uuid REFERENCES ewms.organization_units(id),
    employee_no varchar(50),
...
CREATE TABLE IF NOT EXISTS ewms.user_groups (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.user_group_members (
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    group_id uuid NOT NULL REFERENCES ewms.user_groups(id) ON DELETE CASCADE,
    membership_id uuid NOT NULL REFERENCES ewms.tenant_memberships(id) ON DELETE CASCADE,
    joined_by uuid REFERENCES ewms.users(id),
    joined_at timestamptz NOT NULL DEFAULT now(),
    removed_by uuid REFERENCES ewms.users(id),
...
CREATE TABLE IF NOT EXISTS ewms.permissions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS ewms.roles (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_roles_tenant_code
    ON ewms.roles (COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), role_code);

CREATE TABLE IF NOT EXISTS ewms.role_permissions (
    tenant_id uuid REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    role_id uuid NOT NULL REFERENCES ewms.roles(id) ON DELETE CASCADE,
    permission_id uuid NOT NULL REFERENCES ewms.permissions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ewms.role_assignments (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    membership_id uuid REFERENCES ewms.tenant_memberships(id) ON DELETE CASCADE,
    group_id uuid REFERENCES ewms.user_groups(id) ON DELETE CASCADE,
    role_id uuid NOT NULL REFERENCES ewms.roles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ewms.delegations (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    delegator_membership_id uuid NOT NULL REFERENCES ewms.tenant_memberships(id),
    delegate_membership_id uuid NOT NULL REFERENCES ewms.tenant_memberships(id),
...
CREATE TABLE IF NOT EXISTS ewms.user_sessions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES ewms.users(id) ON DELETE CASCADE,
    ...
    FOREIGN KEY (active_membership_id, user_id)
        REFERENCES ewms.tenant_memberships(id, user_id) ON DELETE CASCADE,
...
COMMENT ON TABLE ewms.login_events IS
...
CREATE TABLE IF NOT EXISTS ewms.user_devices (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES ewms.users(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN ewms.user_devices.push_token_encrypted IS
    'Push token ciphertext only; encryption keys must remain in the external secret manager.';

CREATE TABLE IF NOT EXISTS ewms.terms_versions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
...
CREATE TABLE IF NOT EXISTS ewms.user_terms_agreements (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    user_id uuid NOT NULL REFERENCES ewms.users(id) ON DELETE CASCADE,
    terms_version_id uuid NOT NULL REFERENCES ewms.terms_versions(id),
...
CREATE TABLE IF NOT EXISTS ewms.api_clients (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.api_client_permissions (
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    api_client_id uuid NOT NULL REFERENCES ewms.api_clients(id) ON DELETE CASCADE,
    permission_id uuid NOT NULL REFERENCES ewms.permissions(id) ON DELETE CASCADE,
    granted_by uuid REFERENCES ewms.users(id),
    granted_at timestamptz NOT NULL DEFAULT now(),
    revoked_by uuid REFERENCES ewms.users(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_code_sets_scope
    ON ewms.code_sets (COALESCE(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid), code_set);

CREATE TABLE IF NOT EXISTS ewms.code_values (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    code_set_id uuid NOT NULL REFERENCES ewms.code_sets(id) ON DELETE CASCADE,
    code varchar(80) NOT NULL,
    code_name varchar(200) NOT NULL,
    description text,
    sort_order integer NOT NULL DEFAULT 0,
    parent_code_value_id uuid REFERENCES ewms.code_values(id),
...
CREATE TABLE IF NOT EXISTS ewms.number_sequences (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ewms.business_calendars (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.calendar_dates (
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    calendar_id uuid NOT NULL REFERENCES ewms.business_calendars(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ewms.files (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    uploaded_by uuid REFERENCES ewms.users(id),
    row_version bigint NOT NULL DEFAULT 1,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    deleted_at timestamptz,
...
COMMENT ON TABLE ewms.files IS
    'Object-storage metadata only. File bytes, credentials, and encryption keys are never stored in PostgreSQL.';

CREATE TABLE IF NOT EXISTS ewms.entity_files (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
    file_id uuid NOT NULL REFERENCES ewms.files(id),
...
CREATE TABLE IF NOT EXISTS ewms.entity_notes (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.custom_field_definitions (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    entity_type varchar(80) NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.custom_field_values (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    definition_id uuid NOT NULL REFERENCES ewms.custom_field_definitions(id) ON DELETE CASCADE,
    entity_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.idempotency_keys (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
COMMENT ON COLUMN ewms.outbox_events.payload IS
    'Business event payload with secrets and authentication tokens removed.';

CREATE TABLE IF NOT EXISTS ewms.external_systems (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
    ...
    CONSTRAINT ck_external_systems_no_inline_secrets CHECK (
        NOT ewms.jsonb_contains_forbidden_secret_key(config)
    )
);

COMMENT ON COLUMN ewms.external_systems.credential_secret_ref IS
    'Opaque external secret-manager reference; never store a credential value here or in config.';

CREATE TABLE IF NOT EXISTS ewms.external_id_mappings (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.integration_messages (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
COMMENT ON TABLE ewms.integration_messages IS
...
CREATE TABLE IF NOT EXISTS ewms.audit_events (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid REFERENCES ewms.tenants(id),
    actor_user_id uuid REFERENCES ewms.users(id) ON DELETE SET NULL,
    actor_api_client_id uuid REFERENCES ewms.api_clients(id) ON DELETE SET NULL,
...
COMMENT ON TABLE ewms.audit_events IS
...
CREATE TABLE IF NOT EXISTS ewms.data_retention_policies (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    entity_type varchar(80) NOT NULL,
...
CREATE TABLE IF NOT EXISTS ewms.legal_holds (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id),
...
CREATE TABLE IF NOT EXISTS ewms.legal_hold_entities (
    id uuid PRIMARY KEY DEFAULT ewms.generate_uuid(),
    tenant_id uuid NOT NULL REFERENCES ewms.tenants(id) ON DELETE CASCADE,
    legal_hold_id uuid NOT NULL REFERENCES ewms.legal_holds(id) ON DELETE RESTRICT,
    entity_type varchar(80) NOT NULL,
    entity_id uuid NOT NULL,
...
COMMENT ON TABLE ewms.legal_hold_entities IS
