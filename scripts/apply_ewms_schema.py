"""Apply and verify the EWMS PostgreSQL 11 schema without exposing credentials."""
...
SQL_DIR = REPO_ROOT / "database" / "ewms"
FORWARD_MIGRATION_DIR = SQL_DIR / "migrations"
MIGRATION_VERSION = "20260713_01_enterprise_ewms"
MIGRATION_DESCRIPTION = (
    "Enterprise EWMS schema: authenticated tenant context, warehouse masters, inbound, "
    "inventory ledger, outbound, returns, import/export, Korea customs and UNI-PASS, "
    "bonded operations, settlement, automation, audit, and analytics"
)
REQUIRED_MODULE_NUMBERS = tuple(range(15))
FORWARD_MIGRATION_ALLOWLIST = (
    (
        "20260713_02_same_tenant_reference_hardening",
        "Enforce same-tenant integrity for high-risk EWMS reference relationships",
        "20260713_02_same_tenant_reference_hardening.sql",
    ),
    (
        "20260713_03_authenticated_membership_lock_policy",
        "Permit protected session functions to lock authenticated active memberships",
        "20260713_03_authenticated_membership_lock_policy.sql",
    ),
)
...
class ForwardMigration:
    """An ordered forward-only migration with an immutable SQL checksum."""
...
TABLE_PATTERN = re.compile(r"^CREATE TABLE IF NOT EXISTS ewms\.([a-z0-9_]+)", re.MULTILINE)
...
CRITICAL_AUTH_SECURITY_DEFINERS = (
    "protect_user_session_core",
    "has_active_session_issue_context",
    "has_active_session_bind_context",
    "revoke_sessions_after_subject_change",
    "issue_ewms_session",
    "bind_request_context",
    "current_tenant_id",
    "current_ewms_user_id",
    "cleanup_request_contexts",
)
...
def _assert_actual_target(
    ...
    if (
        ...
        raise RuntimeError(
            "Refusing EWMS DDL on the actual PostgreSQL target outside "
...
def _sql_files() -> list[Path]:
    ...
    if not files:
        raise RuntimeError(f"No EWMS SQL modules found under {SQL_DIR}")

    modules: dict[int, Path] = {}
    for path in files:
        match = re.fullmatch(r"(0[0-9]|1[0-4])_[a-z0-9_]+\.sql", path.name)
        if match is None:
            raise RuntimeError(
                f"Unexpected numbered EWMS SQL module {path.name}; expected only 00 through 14"
            )
        module_number = int(match.group(1))
        if module_number in modules:
            raise RuntimeError(
                f"Duplicate EWMS SQL module prefix {module_number:02d}: "
    ...
    if missing:
        raise RuntimeError(
            "EWMS SQL modules are incomplete; missing: "
...
def _forward_migration_files() -> list[Path]:
    ...
    if len(allowlisted_names) != len(set(allowlisted_names)):
        raise RuntimeError("Duplicate EWMS forward migration file in the allowlist")
    if allowlisted_names != tuple(sorted(allowlisted_names)):
        raise RuntimeError(
            "EWMS forward migration allowlist must be ordered by file name"
        )

    discovered_names = tuple(
        path.name
        for path in sorted(
            FORWARD_MIGRATION_DIR.glob("*.sql"), key=lambda item: item.name
        )
    )
    if discovered_names != allowlisted_names:
        ...
        raise RuntimeError(
            "EWMS forward migration file set is invalid: " + "; ".join(details)
        )
...
def _load_forward_migrations() -> tuple[ForwardMigration, ...]:
    ...
    versions: list[str] = []
    for (version, description, file_name), path in zip(
        FORWARD_MIGRATION_ALLOWLIST, files
    ):
        if path.name != file_name or path.stem != version:
            raise RuntimeError(
                f"EWMS forward migration {file_name} must use version {version}"
            )
    ...
    if len(versions) != len(set(versions)):
        raise RuntimeError("Duplicate EWMS forward migration version in the allowlist")
    if versions != sorted(versions):
        raise RuntimeError("EWMS forward migration versions must be strictly ordered")
    if versions and MIGRATION_VERSION >= versions[0]:
        raise RuntimeError("EWMS forward migration versions must follow the baseline")
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE constraint_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE index_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE table_namespace.nspname = 'ewms'
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE function_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE type_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE range_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE sequence_namespace.nspname = 'ewms'
                """
            ).fetchall(),
        )
    )
    return _stable_fingerprint("ewms-catalog-pg11-v1", sections)
...
BASELINE_SYSTEM_ROLE_CODES = (
    "EWMS_VIEWER", "EWMS_OPERATOR", "EWMS_SUPERVISOR", "EWMS_INVENTORY_CONTROLLER",
    "EWMS_CUSTOMS_SPECIALIST", "EWMS_BILLING_SPECIALIST", "EWMS_AUDITOR",
    "EWMS_TENANT_ADMIN",
)
...
def _baseline_seed_fingerprint(conn: psycopg.Connection) -> str:
    ...
    sections: list[
        ...
        (
            ...
            rows(
                ...
                FROM ewms.currencies WHERE currency_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                SELECT country_code::text, country_code3::text, numeric_code::text,
                       ...
                       is_active::text
                FROM ewms.countries WHERE country_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM ewms.units_of_measure WHERE uom_code = ANY(%s::text[])
                """,
                BASELINE_UOM_CODES,
            ),
        ),
        (
            ...
            rows(
                ...
                FROM ewms.transport_modes WHERE mode_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM ewms.incoterms WHERE incoterm_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM ewms.location_types location_type
                LEFT JOIN ewms.inventory_statuses default_status
        ...
        (
            ...
            rows(
                ...
                FROM ewms.permissions WHERE permission_code = ANY(%s::text[])
                """,
                BASELINE_PERMISSION_CODES,
            ),
        ),
        ...
        (
            ...
            rows(
                ...
                FROM ewms.role_permissions role_permission
                JOIN ewms.roles role_row ON role_row.id = role_permission.role_id
        ...
        (
            ...
            rows(
                ...
                FROM ewms.data_retention_policies
    ...
    return _stable_fingerprint("ewms-baseline-seeds-v1", sections)
...
def _configured_append_only_tables(files: list[Path]) -> set[str]:
    operations_sql = next(path for path in files if path.name.startswith("12_")).read_text(
        encoding="utf-8"
    )
    ...
    if start_marker not in operations_sql or end_marker not in operations_sql:
        raise RuntimeError("Unable to locate the module 12 append-only configuration")
    ...
    if array_match is None:
        raise RuntimeError("Unable to parse the module 12 append-only table list")
    ...
    for version, recorded_checksum, _, _ in rows:
        ...
        if not recorded_checksum or recorded_checksum.strip() != expected_checksum:
            if version == MIGRATION_VERSION:
                raise RuntimeError(
                    "Deployed EWMS baseline checksum does not match local modules 00..14. "
                    "Create a forward migration instead of rewriting deployed history."
                )
            raise RuntimeError(
                f"Deployed EWMS forward migration checksum for {version} does not "
...
def _assert_migration_history_prefix(
    ...
    if actual_versions != expected_versions[: len(actual_versions)]:
        raise RuntimeError(
            "The target EWMS migration history is ahead of, out of order, or "
...
def _assert_current_fingerprints(
    ...
    if (
        ...
        raise RuntimeError(
            f"Deployed EWMS catalog fingerprint does not match latest migration {version}"
        )
    ...
    if not recorded_seed_checksum or recorded_seed_checksum.strip() != actual_seed_checksum:
        raise RuntimeError(
            "Deployed EWMS baseline seed fingerprint does not match latest "
            f"migration {version}"
        )
...
def _assert_exact_migration_history(
    ...
    expected = _expected_migration_checksums(baseline_checksum, forward_migrations)
    ...
    if actual_versions != expected_versions:
        ...
        if prefix_matches and len(actual_versions) < len(expected_versions):
            ...
            raise RuntimeError(
                "EWMS migration history is incomplete. Expected exact ordered history ["
                + ", ".join(expected_versions)
                + "]; found ["
                + (", ".join(actual_versions) if actual_versions else "empty")
                + "]; missing ["
                + ", ".join(missing_versions)
                + "]. Run apply_ewms_schema.py without --verify-only."
            )
        raise RuntimeError(
            "EWMS migration history is incompatible. Expected exact ordered history ["
            + ", ".join(expected_versions)
            + "]; found ["
            + (", ".join(actual_versions) if actual_versions else "empty")
            + "]."
...
def _verify(
    ...
    if database_name != actual_database or database_user != actual_user:
        raise RuntimeError("PostgreSQL target identity changed during EWMS verification")
    if not 110000 <= server_version_num < 120000:
        raise RuntimeError(
            f"EWMS migration is pinned to PostgreSQL 11; server reports {server_version}"
        )
    ...
    if schema_owner is None:
        raise RuntimeError("ewms schema does not exist after migration")
    if schema_owner[0] != REQUIRED_POSTGRES_USER:
        raise RuntimeError(
            f"ewms schema owner must be {REQUIRED_POSTGRES_USER}, found {schema_owner[0]}"
        )

    migration_table_exists = conn.execute(
        "SELECT to_regclass('ewms.schema_migrations') IS NOT NULL"
    ).fetchone()[0]
    if not migration_table_exists:
        raise RuntimeError("ewms.schema_migrations is missing")
    ...
    if unexpected_owners:
        ...
        raise RuntimeError(f"EWMS objects have unexpected owners: {detail}")
    ...
    if (
        ...
        raise RuntimeError(
            "PUBLIC must have no CREATE on ewms and no EWMS relation/sequence/function privileges"
        )
    ...
    table_count = conn.execute(
        ...
        FROM pg_class c
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'ewms' AND c.relkind IN ('r', 'p')
        """
    ).fetchone()[0]
    fk_count = conn.execute(
        ...
        WHERE n.nspname = 'ewms' AND c.contype = 'f'
        """
    ).fetchone()[0]
    check_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'ewms' AND c.contype = 'c'
        """
    ).fetchone()[0]
    index_count = conn.execute(
        ...
        FROM pg_class c
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'ewms' AND c.relkind = 'i'
        """
    ).fetchone()[0]
    trigger_count = conn.execute(
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'ewms' AND NOT t.tgisinternal
        """
    ).fetchone()[0]
    ...
    if missing_tables:
        raise RuntimeError(f"Expected EWMS tables are missing: {', '.join(missing_tables)}")
    ...
    if missing_indexes:
        raise RuntimeError(f"Expected EWMS indexes are missing: {', '.join(missing_indexes)}")
    ...
    if missing_index_targets:
        ...
        raise RuntimeError(f"EWMS index target/uniqueness mismatch: {formatted}")
    ...
    if missing_triggers:
        ...
        raise RuntimeError(f"Expected EWMS triggers are missing: {formatted}")
    ...
    if missing_critical_triggers:
        ...
        raise RuntimeError(f"Critical EWMS integrity triggers are missing or miswired: {formatted}")
    ...
    invalid_critical_trigger_shapes = conn.execute(
        ...
        LEFT JOIN pg_namespace namespace_row ON namespace_row.nspname = 'ewms'
    ...
    bonded_link_constraints = {
        ...
        for row in conn.execute(
            ...
            WHERE constraint_row.conrelid = 'ewms.bonded_inventory_movements'::regclass
            ...
            WHERE NOT EXISTS (
                ...
                WHERE namespace_row.nspname = 'ewms'
                  ...
                  AND EXISTS (
                      ...
                      WHERE replace(setting, ' ', '') = 'search_path=pg_catalog,ewms'
                  )
            )
    ...
    invalid_auth_definers = [
        ...
        for value in conn.execute(
            ...
            ORDER BY expected.function_name
            """,
            (list(CRITICAL_AUTH_SECURITY_DEFINERS),),
        ).fetchall()
    ]
    if invalid_auth_definers:
        raise RuntimeError(
            "Critical authentication functions require SECURITY DEFINER and a fixed search_path: "
            + ", ".join(invalid_auth_definers)
        )

    tenant_context_definition_count = conn.execute(
        """
        SELECT count(*)
        FROM pg_proc function_row
        JOIN pg_namespace namespace_row
        ...
        WHERE namespace_row.nspname = 'ewms'
          ...
          AND position('request_contexts' IN lower(function_row.prosrc)) > 0
          AND position('current_setting' IN lower(function_row.prosrc)) = 0
        """
    ).fetchone()[0]
    if tenant_context_definition_count != 1:
        raise RuntimeError(
            "current_tenant_id() must derive tenant identity only from protected request_contexts"
        )
    ...
    invalid_fk_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'ewms' AND c.contype = 'f' AND NOT c.convalidated
        """
    ).fetchone()[0]
    if invalid_fk_count:
    ...
    invalid_check_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'ewms' AND c.contype = 'c' AND NOT c.convalidated
        """
    ).fetchone()[0]
    ...
    if missing_security_checks:
        raise RuntimeError(
            "Required EWMS authentication/secret checks are missing: "
            ...
            WHERE n.nspname = 'ewms'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE t.tgrelid = c.oid
                    AND t.tgfoid = to_regprocedure('ewms.touch_row()')
            ...
            WHERE n.nspname = 'ewms'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE trigger_row.tgrelid = c.oid
                    ...
                    AND trigger_row.tgfoid = to_regprocedure('ewms.prevent_tenant_change()')
                    AND NOT trigger_row.tgisinternal
                    AND trigger_row.tgenabled <> 'D'
    ...
    missing_append_only_guards = [
        ...
        for value in conn.execute(
            ...
            JOIN pg_namespace namespace_row
             ...
             AND namespace_row.nspname = 'ewms'
            WHERE NOT EXISTS (
                SELECT 1
                FROM pg_trigger trigger_row
                JOIN pg_proc function_row ON function_row.oid = trigger_row.tgfoid
            ...
            OR NOT EXISTS (
                ...
                FROM pg_trigger trigger_row
                WHERE trigger_row.tgrelid = table_row.oid
                  AND trigger_row.tgname = 'trg_ewms_append_only_truncate'
                  AND trigger_row.tgfoid = to_regprocedure('ewms.prevent_append_only_change()')
                  AND NOT trigger_row.tgisinternal
                  AND trigger_row.tgenabled <> 'D'
    ...
    platform_fk_count = conn.execute(
        ...
        SELECT count(*)
        FROM pg_constraint c
        WHERE c.conrelid = 'ewms.users'::regclass
          AND c.conname = 'fk_ewms_users_platform_user'
          ...
          AND c.conkey = ARRAY[(
              SELECT attnum
              FROM pg_attribute
              WHERE attrelid = 'ewms.users'::regclass
    ...
    if platform_fk_count != 1:
        raise RuntimeError("Canonical kang.users -> ewms.users platform FK is missing")

    platform_subject_trigger_count = conn.execute(
        """
        SELECT count(*)
        FROM pg_trigger t
        WHERE t.tgrelid = 'ewms.users'::regclass
          AND t.tgname = 'trg_validate_user_platform_subject'
          AND t.tgfoid = to_regprocedure('ewms.validate_user_platform_subject()')
          AND NOT t.tgisinternal
          AND t.tgenabled <> 'D'
          AND (t.tgtype & 1) = 1
          AND (t.tgtype & 2) = 2
    ...
    if platform_subject_mismatch_count:
        raise RuntimeError(
            f"Found {platform_subject_mismatch_count} EWMS users mapped to a different "
    ...
    session_membership_fk_count = conn.execute(
        ...
        SELECT count(*)
        FROM pg_constraint constraint_row
        WHERE constraint_row.conrelid = 'ewms.user_sessions'::regclass
          AND constraint_row.confrelid = 'ewms.tenant_memberships'::regclass
          ...
          AND constraint_row.confkey = ARRAY[
              (SELECT attnum FROM pg_attribute
                WHERE attrelid = 'ewms.tenant_memberships'::regclass
    ...
    if session_membership_fk_count != 1:
        raise RuntimeError(
            "user_sessions active membership must belong to the same EWMS user"
        )
    ...
    firebase_subject_index_count = conn.execute(
        ...
        WHERE n.nspname = 'ewms'
          ...
          AND x.indrelid = 'ewms.users'::regclass
          AND x.indisunique
          AND x.indisvalid
    ...
    if firebase_subject_index_count != 1:
        raise RuntimeError("Canonical EWMS Firebase subject unique index is missing or invalid")

    provider_constraint = conn.execute(
        """
        SELECT pg_get_constraintdef(c.oid)
        FROM pg_constraint c
        WHERE c.conrelid = 'ewms.auth_identities'::regclass
    ...
    if provider_constraint is None:
        raise RuntimeError("EWMS auth identity provider constraint is missing")
    ...
    if (
        ...
        raise RuntimeError(
            "EWMS auth identities must allow only password and google.com providers"
        )
...
def _record_migration(
    ...
    conn.execute(
        """
        INSERT INTO ewms.schema_migrations
            (
...
def _assert_empty_schema_for_new_baseline(conn: psycopg.Connection) -> None:
    ...
    if preexisting_object_count:
        raise RuntimeError(
            "Refusing to install the EWMS baseline without its migration row into a "
            f"non-empty ewms schema ({preexisting_object_count} user objects found)."
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="Read the deployed ewms catalog without running DDL.",
    )
    parser.add_argument(
        "--force-reapply",
        action="store_true",
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        if metadata is None or not 110000 <= metadata[0] < 120000:
            raise RuntimeError("EWMS migration requires PostgreSQL 11.x")

        if args.verify_only:
            result = _verify(conn, files, checksum, forward_migrations)
            print(
                "EWMS verified: "
        ...
        conn.execute("SELECT pg_advisory_xact_lock(hashtext('ewms_schema_migration'))")
        ...
        if migration_table_exists:
            migration_columns = {
                ...
                for row in conn.execute(
                    ...
                    SELECT column_name
                    FROM information_schema.columns
                    WHERE table_schema = 'ewms' AND table_name = 'schema_migrations'
                    """
                ).fetchall()
            }
            ...
            if missing_migration_columns:
                raise RuntimeError(
                    "Existing EWMS migration ledger is incompatible; missing columns: "
        ...
        should_execute_baseline = bool(args.force_reapply or not baseline_is_recorded)
        if should_execute_baseline:
            ...
            # error rolls this state back and the final hardening module restores FORCE RLS.
            conn.execute(
                """
                DO $block$
                DECLARE
        ...
        if recorded_rows and len(recorded_rows) < len(expected_migrations):
            # A new migration row must never bless pre-existing catalog or baseline-seed
            # drift. Verify the last deployed fingerprint before executing pending SQL.
        ...
        if args.dry_run:
            conn.rollback()
            print(
                "EWMS dry run verified and rolled back: "
                f"PostgreSQL {result['server_version']}, "
                f"tables={result['tables']}, foreign_keys={result['foreign_keys']}, "
                f"checks={result['checks']}, indexes={result['indexes']}, "
                f"triggers={result['triggers']}, rls_tables={result['rls_tables']}"
