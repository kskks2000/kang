"""Apply and verify the DEIMS PostgreSQL 11 schema without exposing credentials."""
...
SQL_DIR = REPO_ROOT / "database" / "deims"
MIGRATION_VERSION = "20260712_01_enterprise_deims"
MIGRATION_DESCRIPTION = (
    "Enterprise DEIMS schema: Firebase IAM, import/export cases, logistics, FTA/origin, "
    "trade compliance, customs/bonded operations, settlement, audit, and analytics"
)
REQUIRED_MODULE_NUMBERS = tuple(range(13))
TABLE_PATTERN = re.compile(r"^CREATE TABLE IF NOT EXISTS deims\.([a-z0-9_]+)", re.MULTILINE)
...
def _assert_actual_target(
    ...
    if (
        ...
        raise RuntimeError(
            "Refusing DEIMS DDL on the actual PostgreSQL target outside "
...
def _sql_files() -> list[Path]:
    ...
    if not files:
        raise RuntimeError(f"No DEIMS SQL modules found under {SQL_DIR}")

    modules: dict[int, Path] = {}
    for path in files:
        match = re.fullmatch(r"(0[0-9]|1[0-2])_[a-z0-9_]+\.sql", path.name)
        if match is None:
            raise RuntimeError(
                f"Unexpected numbered DEIMS SQL module {path.name}; expected only 00 through 12"
            )
        module_number = int(match.group(1))
        if module_number in modules:
            raise RuntimeError(
                f"Duplicate DEIMS SQL module prefix {module_number:02d}: "
    ...
    if missing:
        raise RuntimeError(
            "DEIMS SQL modules are incomplete; missing: "
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE namespace_row.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE constraint_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE index_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE table_namespace.nspname = 'deims'
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE function_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE type_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    ...
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE range_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    sections.append(
        (
            ...
            conn.execute(
                ...
                WHERE sequence_namespace.nspname = 'deims'
                """
            ).fetchall(),
        )
    )
    return _stable_fingerprint("deims-catalog-pg11-v1", sections)
...
BASELINE_SYSTEM_ROLE_CODES = (
    "DEIMS_VIEWER", "DEIMS_OPERATOR", "DEIMS_SUPERVISOR", "DEIMS_INVENTORY_CONTROLLER",
    "DEIMS_CUSTOMS_SPECIALIST", "DEIMS_BILLING_SPECIALIST", "DEIMS_AUDITOR",
    "DEIMS_TENANT_ADMIN",
)
...
def _baseline_seed_fingerprint(conn: psycopg.Connection) -> str:
    ...
    sections: list[
        ...
        (
            ...
            rows(
                ...
                FROM deims.currencies WHERE currency_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                SELECT country_code::text, country_code3::text, numeric_code::text,
                       ...
                       is_active::text
                FROM deims.countries WHERE country_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM deims.units_of_measure WHERE uom_code = ANY(%s::text[])
                """,
                BASELINE_UOM_CODES,
            ),
        ),
        (
            ...
            rows(
                ...
                FROM deims.transport_modes WHERE mode_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM deims.incoterms WHERE incoterm_code = ANY(%s::text[])
                """,
        ...
        (
            ...
            rows(
                ...
                FROM deims.location_types location_type
                LEFT JOIN deims.inventory_statuses default_status
        ...
        (
            ...
            rows(
                ...
                FROM deims.permissions WHERE permission_code = ANY(%s::text[])
                """,
                BASELINE_PERMISSION_CODES,
            ),
        ),
        ...
        (
            ...
            rows(
                ...
                FROM deims.role_permissions role_permission
                JOIN deims.roles role_row ON role_row.id = role_permission.role_id
        ...
        (
            ...
            rows(
                ...
                FROM deims.data_retention_policies
    ...
    return _stable_fingerprint("deims-baseline-seeds-v1", sections)
...
def _verify(
    ...
    if database_name != actual_database or database_user != actual_user:
        raise RuntimeError("PostgreSQL target identity changed during DEIMS verification")
    if not 110000 <= server_version_num < 120000:
        raise RuntimeError(
            f"DEIMS migration is pinned to PostgreSQL 11; server reports {server_version}"
        )
    ...
    if schema_owner is None:
        raise RuntimeError("deims schema does not exist after migration")
    if schema_owner[0] != REQUIRED_POSTGRES_USER:
        raise RuntimeError(
            f"deims schema owner must be {REQUIRED_POSTGRES_USER}, found {schema_owner[0]}"
        )

    migration_table_exists = conn.execute(
        "SELECT to_regclass('deims.schema_migrations') IS NOT NULL"
    ).fetchone()[0]
    if not migration_table_exists:
        raise RuntimeError("deims.schema_migrations is missing")
    recorded_migration_versions = {
        row[0]
        for row in conn.execute(
            "SELECT version FROM deims.schema_migrations ORDER BY version"
        ).fetchall()
    }
    if recorded_migration_versions != {MIGRATION_VERSION}:
        raise RuntimeError(
            "Verified DEIMS migration history must contain exactly the supported baseline "
    ...
    if unexpected_owners:
        ...
        raise RuntimeError(f"DEIMS objects have unexpected owners: {detail}")
    ...
    if (
        ...
        raise RuntimeError(
            "PUBLIC must have no CREATE on deims and no DEIMS relation/sequence/function privileges"
        )
    ...
    table_count = conn.execute(
        ...
        FROM pg_class c
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'deims' AND c.relkind IN ('r', 'p')
        """
    ).fetchone()[0]
    fk_count = conn.execute(
        ...
        WHERE n.nspname = 'deims' AND c.contype = 'f'
        """
    ).fetchone()[0]
    check_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'deims' AND c.contype = 'c'
        """
    ).fetchone()[0]
    index_count = conn.execute(
        ...
        FROM pg_class c
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'deims' AND c.relkind = 'i'
        """
    ).fetchone()[0]
    trigger_count = conn.execute(
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'deims' AND NOT t.tgisinternal
        """
    ).fetchone()[0]
    ...
    if missing_tables:
        raise RuntimeError(f"Expected DEIMS tables are missing: {', '.join(missing_tables)}")
    ...
    if missing_indexes:
        raise RuntimeError(f"Expected DEIMS indexes are missing: {', '.join(missing_indexes)}")
    ...
    if missing_index_targets:
        ...
        raise RuntimeError(f"DEIMS index target/uniqueness mismatch: {formatted}")
    ...
    if missing_triggers:
        ...
        raise RuntimeError(f"Expected DEIMS triggers are missing: {formatted}")
    ...
    missing_critical_triggers = conn.execute(
        ...
        WHERE NOT EXISTS (
            ...
            WHERE table_namespace.nspname = 'deims'
              ...
              AND function_namespace.nspname = 'deims'
    ...
    if missing_critical_triggers:
        ...
        raise RuntimeError(f"Critical DEIMS integrity triggers are missing or miswired: {formatted}")
    ...
    invalid_critical_trigger_shapes = conn.execute(
        ...
        LEFT JOIN pg_namespace namespace_row ON namespace_row.nspname = 'deims'
    ...
    bonded_link_constraints = {
        ...
        for row in conn.execute(
            ...
            WHERE constraint_row.conrelid = 'deims.bonded_inventory_movements'::regclass
            ...
            WHERE NOT EXISTS (
                ...
                WHERE namespace_row.nspname = 'deims'
                  ...
                  AND EXISTS (
                      ...
                      WHERE replace(setting, ' ', '') = 'search_path=pg_catalog,deims'
                  )
            )
    ...
    invalid_fk_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'deims' AND c.contype = 'f' AND NOT c.convalidated
        """
    ).fetchone()[0]
    if invalid_fk_count:
    ...
    invalid_check_count = conn.execute(
        ...
        FROM pg_constraint c
        JOIN pg_namespace n ON n.oid = c.connamespace
        WHERE n.nspname = 'deims' AND c.contype = 'c' AND NOT c.convalidated
        """
    ).fetchone()[0]
    ...
    if missing_security_checks:
        raise RuntimeError(
            "Required DEIMS authentication/secret checks are missing: "
            ...
            WHERE n.nspname = 'deims'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE t.tgrelid = c.oid
                    AND t.tgfoid = to_regprocedure('deims.touch_row()')
            ...
            WHERE n.nspname = 'deims'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE trigger_row.tgrelid = c.oid
                    ...
                    AND trigger_row.tgfoid = to_regprocedure('deims.prevent_tenant_change()')
                    AND NOT trigger_row.tgisinternal
                    AND trigger_row.tgenabled <> 'D'
    ...
    missing_append_only_guards = [
        ...
        for value in conn.execute(
            ...
            JOIN pg_namespace namespace_row
             ...
             AND namespace_row.nspname = 'deims'
            WHERE NOT EXISTS (
                SELECT 1
                FROM pg_trigger trigger_row
                JOIN pg_proc function_row ON function_row.oid = trigger_row.tgfoid
            ...
            OR NOT EXISTS (
                ...
                FROM pg_trigger trigger_row
                WHERE trigger_row.tgrelid = table_row.oid
                  AND trigger_row.tgname = 'trg_deims_append_only_truncate'
                  AND trigger_row.tgfoid = to_regprocedure('deims.prevent_append_only_change()')
                  AND NOT trigger_row.tgisinternal
                  AND trigger_row.tgenabled <> 'D'
    ...
    platform_fk_count = conn.execute(
        ...
        SELECT count(*)
        FROM pg_constraint c
        WHERE c.conrelid = 'deims.users'::regclass
          AND c.conname = 'fk_deims_users_platform_user'
          ...
          AND c.conkey = ARRAY[(
              SELECT attnum
              FROM pg_attribute
              WHERE attrelid = 'deims.users'::regclass
    ...
    if platform_fk_count != 1:
        raise RuntimeError("Canonical kang.users -> deims.users platform FK is missing")

    platform_subject_trigger_count = conn.execute(
        """
        SELECT count(*)
        FROM pg_trigger t
        WHERE t.tgrelid = 'deims.users'::regclass
          AND t.tgname = 'trg_validate_user_platform_subject'
          AND t.tgfoid = to_regprocedure('deims.validate_user_platform_subject()')
          AND NOT t.tgisinternal
          AND t.tgenabled <> 'D'
          AND (t.tgtype & 1) = 1
          AND (t.tgtype & 2) = 2
    ...
    platform_subject_mismatch_count = conn.execute(
        """
        SELECT count(*)
        FROM deims.users deims_user
        LEFT JOIN kang.users platform_user
          ON platform_user.id = deims_user.platform_user_id
         AND platform_user.firebase_uid = deims_user.firebase_uid
        WHERE platform_user.id IS NULL
        """
    ).fetchone()[0]
    if platform_subject_mismatch_count:
        raise RuntimeError(
            f"Found {platform_subject_mismatch_count} DEIMS users mapped to a different "
    ...
    session_membership_fk_count = conn.execute(
        ...
        SELECT count(*)
        FROM pg_constraint constraint_row
        WHERE constraint_row.conrelid = 'deims.user_sessions'::regclass
          AND constraint_row.confrelid = 'deims.tenant_memberships'::regclass
          AND constraint_row.contype = 'f'
          AND constraint_row.convalidated
          AND constraint_row.conkey = ARRAY[
              (SELECT attnum FROM pg_attribute
                WHERE attrelid = 'deims.user_sessions'::regclass
          ...
          AND constraint_row.confkey = ARRAY[
              (SELECT attnum FROM pg_attribute
                WHERE attrelid = 'deims.tenant_memberships'::regclass
    ...
    if session_membership_fk_count != 1:
        raise RuntimeError(
            "user_sessions active membership must belong to the same DEIMS user"
        )
    ...
    firebase_subject_index_count = conn.execute(
        ...
        WHERE n.nspname = 'deims'
          ...
          AND x.indrelid = 'deims.users'::regclass
          AND x.indisunique
          AND x.indisvalid
    ...
    if firebase_subject_index_count != 1:
        raise RuntimeError("Canonical DEIMS Firebase subject unique index is missing or invalid")

    provider_constraint = conn.execute(
        """
        SELECT pg_get_constraintdef(c.oid)
        FROM pg_constraint c
        WHERE c.conrelid = 'deims.auth_identities'::regclass
    ...
    if provider_constraint is None:
        raise RuntimeError("DEIMS auth identity provider constraint is missing")
    ...
    if (
        ...
        raise RuntimeError(
            "DEIMS auth identities must allow only password and google.com providers"
        )
    ...
    migration_row = conn.execute(
        ...
        FROM deims.schema_migrations
        WHERE version = %s
        """,
        (MIGRATION_VERSION,),
    ).fetchone()
    if not migration_row or not migration_row[0] or migration_row[0].strip() != expected_checksum:
        raise RuntimeError("Deployed DEIMS migration checksum does not match the local SQL")
    ...
    if (
        ...
        raise RuntimeError(
            "Deployed DEIMS catalog fingerprint does not match the immutable baseline"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Deployed DEIMS baseline seed fingerprint does not match the immutable baseline"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="Read the deployed deims catalog without running DDL.",
    )
    parser.add_argument(
        "--force-reapply",
        action="store_true",
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        if metadata is None or not 110000 <= metadata[0] < 120000:
            raise RuntimeError("DEIMS migration requires PostgreSQL 11.x")
        ...
        conn.execute("SELECT pg_advisory_xact_lock(hashtext('deims_schema_migration'))")
        ...
        if migration_table_exists:
            ...
            if recorded_migration_versions not in (set(), {MIGRATION_VERSION}):
                ...
                raise RuntimeError(
                    "The target DEIMS migration history is ahead of or incompatible with "
        ...
        if migration_row and (
            ...
            raise RuntimeError(
                "The recorded DEIMS migration checksum differs from the local SQL. "
        ...
        if migration_row is None:
            ...
            if preexisting_object_count:
                raise RuntimeError(
                    "Refusing to install the DEIMS baseline without its migration row into a "
                    f"non-empty deims schema ({preexisting_object_count} user objects found)."
                )
        ...
        if should_execute_baseline:
            ...
            conn.execute(
                """
                INSERT INTO deims.schema_migrations
                    (
        ...
        if args.dry_run:
            conn.rollback()
            print(
                "DEIMS dry run verified and rolled back: "
                f"PostgreSQL {result['server_version']}, "
                f"tables={result['tables']}, foreign_keys={result['foreign_keys']}, "
                f"checks={result['checks']}, indexes={result['indexes']}, "
                f"triggers={result['triggers']}, rls_tables={result['rls_tables']}"
